...
## 2026-09-08 — Classical opportunity expansion v1 completed

Confirmed fact. Independent branch exp/classical-opportunity-expansion-v1-20260908 completed frozen Batch A: 12 AQUALOC physical windows, raw [0,900), 72 B/C-all formal technical replays. Outcome counts: 3 PRACTICAL_GAIN (A04/A07/H02, all ROBUST), 2 PRACTICAL_LOSS (A01/A03, both severe), 6 SMALL_OR_UNCERTAIN, 0 FAIL, 1 NOT_EVALUABLE (A05, 27 common poses<30). Registered final decision ADDITIVE_OPPORTUNITY_NOT_GENERALIZED; EXPANSION_STOPPED_AFTER_BATCH_A because 2 severe exceed the allowed <=1. Frozen Batch B is NOT_ACTIVATED / Not evaluated. Local cross-sequence opportunity is observed, but acceptable-risk generalization fails.

C-all remains classical control/additive probe, not the AQUA-FE main method or innovation. Protected KLT/profile interpretation, old continuation NO_EXPANSION and additive-budget conclusions remain unchanged. No external backend modifications or new learned arms. Final identity/delivery/receipt/evaluation audit PASS (1656 artifact hashes, max actual eligible644<1000, all72 exact per-ID/runability checks, max evo difference4.9937014e-7m). Numerical runability is distinct from useful accuracy; A03, A04 baseline, A06 and H03 anomalies remain explicit.

Read papers/frontend_classical_opportunity_expansion_v1/report.md, case_registry.csv, case_interpretation.csv, case_mechanism_handoff.md and docs/CODEX_HANDOFF_CLASSICAL_EXPANSION.md. New cases are 12 sequence-held-out only relative to six developer windows, not globally unseen project data; broader checked history overlap7/12. Now outcome-known, they are development cases for any future mechanism designed with them, requiring separately frozen independent confirmation. The sole successor is observation-utility/risk mechanism research; stop additive-observation primary-hypothesis expansion and do not automatically tune C-all or search replacement windows.
...
## 2026-09-11 — SEA-RAFT real-image predictions complete; independent reference pending
Independent SEA-RAFT REAL CORRESPONDENCE CHECK completed 12 fixed real pairs and 92 source-grid queries, with the unchanged model/patch gate and no training or VINS. Source points were restored from the preceding saved B output using one original KLT/GFTT step; the grid queries were frozen before prediction. Only H02 t+10 shows C1/S1 acceptance differences (9/12 of 16, 4 S-only, 1 C-only); these do not establish correctness. Independent human pixel reference is absent, so the decision is REFERENCE_PENDING and all accuracy cells are blank. Six blind sheets and 92 reference rows are ready for review; entry: docs/CODEX_HANDOFF_SEARAFT_REAL_CORRESPONDENCE.md. Old CONTROLLED_GAIN_ONLY and EVIDENCE_GATE_NOT_SUPPORTED remain unchanged. Next: blind human annotation, then comparison of saved predictions; no automatic additional inference or integration.

## 2026-09-11 — SEA-RAFT system probe closed
The user prioritized a finite system experiment over completing92-point annotation. frontend_searaft_system_probe_v1 is COMPLETE / UNSAFE_OR_UNRESOLVED (H02/A02,B/C/R×3,18 new replays). H02 has local R/B improvement, but C has kilometer-scale anomaly; A02 R severely loses to B and has no practical increment over C. Baseline repeat instability further limits interpretation. Stop this combination; no automatic expansion, tuning, new annotation requirement or deployment. See docs/CODEX_HANDOFF_SEARAFT_SYSTEM_PROBE.md and the new report/results. Existing CONTROLLED_GAIN_ONLY, EVIDENCE_GATE_NOT_SUPPORTED, REFERENCE_PENDING and prior evidence remain unchanged.
...
## 2026-09-12 — Saved-output birth-anchor diagnostic
Post-hoc fixed birthzero calculation fromcd615f0, no updates/inference/VINS/download. TestT-anchor EPEp95=1.6237 versusB1.6136, full temporalp95=.8292 and lastblockp95 regression26.07%; original gates still fail. Nonbirthp95 local gain coexists with worse median/mean/temporal errors. Complete diagnostic DIAGNOSTIC_UNRESOLVED because original trainer did not persist validation per-observation predictions; test result remains usable negative evidence. Old NO_TEMPORAL_REFINEMENT_GAIN and MIMIR SUPERVISION_UNAVAILABLE unchanged. Stop this check; no automatic new inference/training/backend. Entry papers/frontend_temporal_observation_refinement_tartanair_v1/anchor_consistency_check/report.md.

## 2026-09-14 — New authorized scope: learned global loop association baseline

User explicitly starts an independent full-sequence global-association baseline, not a revival of local XFeat/SEA-RAFT/coordinate variants. Isolated branch exp/learned-loop-baseline-v1-20260914 based on8f5d916. Finite existing-data check confirms provider-described revisits and raw sensor/calibration/proxy availability for full AFRL Bus and Cemetery; both historical development sequences, now preregistered without new learned outcomes. Fixed ViT-S/14 final normalized patch tokens + matching provided K32 vocabulary; traditional and learned retrieval must share saved KLT local trajectories/keyframes/native geometry/4DoF graph. This is mature-component integration, not original algorithm promotion.

Current status PARTIAL_BLOCKED_RESOURCE: root1.44GiB, available runtime volumes1.72GiB/217MiB below conservative reserves. Local VIO0/6, graph0/12, arm outputs0/18; candidate helper8/8 synthetic tests and vocabulary identity check do not establish system gain. Real encoder, native archive/verification bridge and execution identity lock pending. Exhaustive revisit labels/vocabulary fitting image manifest Unknown. Only next task after capacity is restored: complete this same fixed small matrix, not alternate-model/window search. No unattended run exists. Entry docs/CODEX_HANDOFF_LEARNED_LOOP_BASELINE.md; complete scope and evidence papers/frontend_learned_loop_baseline_v1/. All earlier negative/stop conclusions remain unchanged.

### 2026-09-14 16:14+08 — Learned-loop interface compiled; execution still partial

Current isolated branch now has exact keyframe/3D archive and original-PnP/graph candidate interfaces,17/17 synthetic tests and a successful native adapter build; real geometry/model/local VIO still0. Root recovered to7.78GiB but remains below frozen runtime8GiB plus artifact allowance. Backend binary/canonical YAML identities match history; checked shadow exporter differs from old registered hash, so full KLT execution identity remains unresolved (behavioral impact Unknown). This is not a historical-result invalidation or scientific loop outcome. Handoff/decision updated; fixed2-sequence/18-arm-row denominator unchanged, old local-method stops preserved, no unattended experiment started.
...
### 2026-09-22 — Learned-loop baseline fixed matrix executed, outcome unresolved

The independent mature-component global-association baseline on complete AFRL Bus Outside and Cemetery has now completed its registered six valid shared-KLT local repeats and 12 C/L native graph runs, with all 18 B/C/L rows retained. C passed no native geometric loops; fixed L passed Bus 3/2/5 and Cemetery 0/1/1 (the latter two are one exact original image pair). All numerical common-support gates pass, but exact COLMAP proxy pose convention and independent loop truth remain unverified; fixed-scale/Sim3 APE/RPE, true recall and global WIN/TIE/LOSS are Not evaluated. An interrupted Cemetery local run caused seven actual starts against the original maximum-six budget and is recorded as a protocol deviation, not an independent repeat. The passive image archiver was corrected for duplicate header timestamps using the frozen published-image phase; KLT, VINS and loop geometry are unchanged. No further matrix runs or automatic method expansion are pending. Current entry: `docs/CODEX_HANDOFF_LEARNED_LOOP_BASELINE.md`; details: `papers/frontend_learned_loop_baseline_v1/report.md` and receipts. Old local-learning/temporal-refinement stop conclusions are unchanged.

### 2026-09-23 — Conditional loop accuracy now computed; formal claim remains uncertified

Exact reference files traced to the author's pinned aligned trajectories; allposes and Cemeterytimestampformat reproduced from the older references by a pureSE3/originalwriter. Scale remainsconflicted with provider documentation; exactper-filecamera/metric certification stillmissing. Allsixsavedblocks/18arms now have clearly separated conditionalproxyAPE/RPE andexplicitSim3,36evo crosschecksPASS. C/LmedianfixedAPE:Bus1.040787/0.910719(−12.497%),Cemetery0.854371/0.880116(+3.013%); unitsnotcertifiedmetres. Busshowsconditionalgain,Cemeterysmallregression. All8acceptedimagepairs reviewednonblind; formaltruth/trueRecallUnknown andformalaccuracyNot evaluated. No newVIO/graphs/inference/training; original7starts/6valid deviationretained. No automatic furtherexperimentpending; missingproviderreferenceprovenance/independentlabels are the remaining certificationgap. Latest taskhandoff unchanged in location, newCSV `papers/frontend_learned_loop_baseline_v1/proxy_diagnostic_accuracy.csv`; oldlocalmethodstops unchanged.
