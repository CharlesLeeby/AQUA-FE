...
## 2026-09-14 — Authorized global association baseline, distinct from local frontend gains

- Research question: can one pretrained retrieval component yield additional true geometrically usable loops over BRIEF/DBoW and improve the global trajectory when KLT local output, keyframes, geometric verifier and graph are identical?
- Hypothesis: learned appearance representations may recover revisits missed by traditional retrieval. This is Hypothesis / Inference, not a result or an original algorithm claim.
- Motivation / related baseline: all stopped local recovery/refinement routes remain stopped. B has no loops; C uses existing VINS retrieval; L changes retrieval only. L>B without L>C does not establish learned-specific value.
- Proposed idea / why it might work: reuse mature DINOv2 ViT-S/14 plus matching K32 vocabulary outside ROS, pass candidate IDs to the same native BRIEF/PnP. Decoupling retrieval from geometry prevents descriptor similarity from becoming an unsupported metric edge.
- Assumptions / potential failure cases: full repeated views, preserved local 3D observations, valid camera/body calibration and timestamp identities, compatible official weights/vocabulary. Appearance aliasing, missing overlap labels, poor local scale and invalid 3D can prevent any system gain; four-DoF loop optimization is not scale repair.
- Evidence: Confirmed fact: provider describes loops/repeated traversals for full Bus/Cemetery and their existing raw images/IMU/calibration/proxy were inspected. Fixed provided vocabulary has32x384 float centers and registered SHA. Candidate helper8/8 synthetic tests passes, but real encoder and geometric bridge unvalidated. A02 overlap row mapping unresolved, so it is not used to fabricate labels. No new learned/system outcomes observed.
...
### 2026-09-22 — Learned-loop fixed matrix: geometric increment is not yet system accuracy

- Research question/hypothesis: under exactly shared KLT local estimates, can fixed pretrained retrieval produce more *true* usable loops than native BRIEF and improve global error?
- Motivation/related baseline/proposed idea: full-sequence Bus/Cemetery B/C/L mature-component comparison, with the same native PnP and 4DoF graph and no learned local points. No new method was introduced.
- Why it might work/assumptions: an appearance descriptor may rank revisits through underwater changes; a valid 3D–2D constraint may reduce drift only if its association and scale are correct.
- Potential failure cases: false geometry on repeated structure, retrieval-score calibration rather than representation, bad local scale, ambiguous COLMAP proxy semantics; technical repeats may rediscover one physical pair.
- Confirmed fact: 6 valid local repeats and 12 graph runs completed. C passed geometry 0/0/0 on each sequence; L passed Bus 3/2/5 and Cemetery 0/1/1. Cemetery r2/r3 passes are the same exact image pair. All six numeric common-support gates pass; C=B bytes in 6/6, L changes five outputs. One aborted local attempt raises starts to seven, a protocol execution-cap deviation; no result selection.
- Hypothesis / Inference: learned retrieval may expose revisits missed by fixed C configuration, but Bus C's original score gate blocks all L-pass queries and independent truth labels are unavailable. Geometry acceptance alone cannot establish correct loops or error reduction.
- Current conclusion: fixed-matrix execution is complete; global APE/RPE and true Recall@4 remain Not evaluated because exact reference-pose convention and independent pair truth are unverified. No `PROMISING_GLOBAL_ASSOCIATION_BASELINE` or evaluated negative designation. This does not revive stopped local-front-end routes.
- Open questions/next experiment: only if a formal global-accuracy claim is required, verify the exact reference files' pose convention and accepted pairs independently, then evaluate the already saved trajectories. No automatic new VIO, threshold or model sweep.

## 2026-09-23 — Source provenance enables bounded diagnostics, not automatic reference certification

- Research question/hypothesis: do the extra native learned-loop constraints reduce disagreement with the existing proxy, and can its coordinate/scale meaning be traced without choosing a favorable interpretation?
- Motivation/related baseline/proposed idea: finish the shared-KLT B/C/L evidence with primary-source and saved-output analysis; the algorithm is unchanged. Trace exact reference bytes first, then use one source-supported interpretation in a separately labeled diagnostic.
- Why it might work/assumptions: author's COLMAP inversion/writer establishes camera-pose intent; an origin-only SE3 preserves relative poses. Assumes exact published references correspond to that documented camera pipeline, which remains incompletely certified without original reconstruction/camera-ID records. Disputed metric units prevent a certified metre-error claim.
- Confirmed facts/evidence: exactlocal/HFfiles equal author'salignedfiles. Their3388/2519rows are rigid transforms of oldup-to-scalefiles withscale1 andpositionresidual<6e-15; author'swriter reproduces malformedCemeterytimestamps. On frozen1scommon support, all18conditionalrows/36evo crosschecks complete. BusmedianfixedAPE drops12.497%,Cemeteryrises3.013%;Sim3direction agrees. Eightacceptedpairs reviewednonblind; no newVIO/model/graph execution.
- Hypothesis / Inference: visible shared Bus landmarks support actual revisits and explain conditional drift reduction; repetitive Cemetery rocks support plausible overlap but cannot certify the measured loop edge. A plausible revisit can still introduce a noisy/wrong relative transform, so passage through PnP is insufficient for no-harm.
- Potential failure cases: scale orcameraorigin error inproxy; falsegeometry; reviewbias; traditionaldefaultscoregate; two known developmentsequences and technicalrepeats. Exact old/newscaleequality does not prove no earliermetriccalibration.
- Current conclusion: useful conditional numerical evidence now exists, including a negative Cemetery result. Formalmetricaccuracy/independenttruth remainunresolved. Do not promote source-supported assumptions into Confirmed fact or the conditional Busgain into overallsuperiority.
- Open questions/next experiment: missing provider per-filecamera/scale receipt and independentaccepted-pairlabels. No furtherautomatic experiment is pending; retain allsavedoutputs for eventualcertifiedevaluation, withoutnewthreshold/model/sequence search.
