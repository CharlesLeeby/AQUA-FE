...
## LEARNED-LOOP-BASELINE-V1-READINESS-20260914

- Date / status / scientific role: 2026-09-14; PARTIAL_BLOCKED_RESOURCE; outcome-blind availability/interface readiness, not evaluated loop retrieval or system benchmark.
- Git commit / branch: base8f5d91681c28460a2e4579bc71995ebefbe3abe4; exp/learned-loop-baseline-v1-20260914 in its own worktree. No new model/VINS execution commit exists; publication commit is separate.
- Dataset / sequence: finite roster AQUALOC A02/H05, AFRL Bus/Cemetery/Cave Gennie, CIRS Cala Viuda; six physical sequences/seven existing full bags. Fixed selected full Bus584.999s and Cemetery433.997s, both development-exposed.
- Environment / exact configuration: existing Python3.8+NumPy+cv2 for small tests; no installation. Fixed target DINOv2 ViT-S/14 final normalized patch tokens,280x448 letterbox,K32x384 provided vocabulary; official pretrained runtime loading Not evaluated. B no loop, C existing BRIEF/DBoW, L learned candidate indices; same allowed history/top4/single verifier and shared raw KLT local outputs.
- Baseline / proposed modification: global retrieval only. No changes to local KLT observations, init, old export gates, backend algorithms or prior stopped experiments. Actual native integration still incomplete.
- Commands / artifacts: python3 -B -m unittest discover -s scripts/tests -p test_learned_loop_candidates_v1.py -v; read-only rosbag metadata and reference-time checks. Seven required outputs under papers/frontend_learned_loop_baseline_v1/; fixed entry docs/CODEX_HANDOFF_LEARNED_LOOP_BASELINE.md. No new run-scale artifacts.
- Metrics / results: adapter tests8/8; remote vocabulary identity49160bytes,(32,384) validated. Bus3388 monotone proxy rows; Cemetery2519 monotone repaired times,2514 inside bag record-time bounds. New training/model load/inference/local VIO/pose-graph counts all0. New local0/6, graph0/12, arm results0/18. APE/RPE/init/coverage/Recall@4/correct loop counts/cost Not evaluated.
- Validity checks / common support: metadata counts are not measured common support; pair-level exhaustive labels Unknown, COLMAP only proxy. All eighteen planned arm rows retained NOT_RUN_RESOURCE with blank numerical fields; no retrieval execution means candidate table header-only, not zero-loop evidence.
- Interpretation: real revisit descriptions and matching vocabulary support readiness, not learned improvement. Root1.44GiB/runtime candidates1.72GiB and217MiB fail conservative operational reserves; no unsafe long experiment started. Config/build lock, real encoder and archive-to-native bridge remain pending.
- Conclusion / follow-up: PARTIAL_BLOCKED_RESOURCE, scientific outcome Not evaluated. Restore safe runtime capacity and finish this fixed matrix only; no alternate model/dataset, local-refinement reopening, or unbounded search.

## LEARNED-LOOP-BASELINE-V1-NATIVE-INTERFACE-20260914

- Date / status / scientific role:2026-09-14; COMPILED_NOT_SYSTEM_VALIDATED; synthetic interface validation, not a SLAM benchmark. Overall task PARTIAL_BLOCKED_RESOURCE.
- Git commit / branch: parent dff87ec477e0ccf31449e3504656cf628c31f0c2 on exp/learned-loop-baseline-v1-20260914 plus new adapter patch identified in decision.json; publication SHA is separate from runtime source identity.
- Dataset / sequence: no new real sequence run; frozen full Bus/Cemetery denominator unchanged. Synthetic archive messages/assets only for tests; hash payload is explicitly not a decoded geometric image.
- Environment / exact configuration: existing GCC9.4, OpenCV4.2, Ceres1.14, Eigen3.3.7, ROS noetic, Python3+NumPy. Target350-point cap, native roster/pose/3D observations, same geometry/4DoF settings. No model instantiated or new dependency installed.
- Baseline / modification: original native loop source read-only; isolated candidate routing/history plus passive diagnostic/archive interfaces only. No local KLT/VINS observations, init or solver algorithm changed.
- Commands / artifacts: python3 -B scripts/build_native_loop_baseline_v1.py --build-dir experiments/learned_loop_baseline_v1/native_adapter_build_attempt2; cmake --build experiments/learned_loop_baseline_v1/native_adapter_build_attempt2/build --parallel 1; python3 -B -m unittest discover -s scripts/tests -p test_archive_loop_keyframes_v1.py -v; same command with test_learned_loop_candidates_v1.py. Snapshot and original-file identity at attempt2/snapshot_identity.json. Binary identity in task decision/report.
- Metrics / results:17/17 synthetic tests pass (9 archive,8 candidates); native adapter compilation succeeds; usage-only executable check exit64 expected. Native keyframe/PnP file byte-identical,4DoF/6DoF solver bodies unchanged except read-only completion notification. Actual model load/inference/retrieval/PnP/local VIO/graph0; init/coverage/APE/RPE/Recall@4/runtime performance Not evaluated.
...
### 2026-09-15 — frontend_learned_loop_baseline_v1 / Bus r1 paired global block

- Status/role: PARTIAL_SYSTEM_MATRIX_RUNNING; mature-component baseline, not algorithmic novelty. Branch exp/learned-loop-baseline-v1-20260914; local adapter commit7c08909, C/archiver4ee416e, L902a886; fixed original KLT3c50b742. Full Bus Outside and Cemetery, not short windows/held-out sequences.
- Environment/config/baseline: existing ROSnoetic/OpenCV4.2/Ceres1.14 and unchanged origin VINS binary4e91d8ac;350KLT observations maximum, no learned local points. Brawlocal/CnativeBRIEF/LfixedDINO-S14K32. Same archive/PnP/4DoF math; only retrieval differs.
- Commands/artifacts: exact commands and SHA receipts in papers/frontend_learned_loop_baseline_v1/runtime_receipts.json; runtime experiments/learned_loop_baseline_v1/bus_outside_r1. Tables generated by python3 -B scripts/summarize_loop_baseline_v1.py --runtime experiments/learned_loop_baseline_v1 --output papers/frontend_learned_loop_baseline_v1.
- Metrics/results: local3654poses,2430KF,init2.227895s; C0/443,L3/2198geometrypasses. Common497poses579s92.037%PASS. Cglobal=Bbytes; Lglobalchanged. Encoding2430images511.77s, native C21.02/L22.02s; pure optimizationUnknown. TrajectoryAPE/RPE/scale/trueRecallNot evaluated, exact reference convention unresolved.
...
### 2026-09-22 — frontend_learned_loop_baseline_v1 / full fixed Bus–Cemetery matrix

- Experiment ID/date/status/scientific role: `frontend_learned_loop_baseline_v1`, 2026-09-22, `FULL_MATRIX_EXECUTED_OUTCOME_UNRESOLVED`; mature-component global-association baseline, not a new algorithm or local-frontend result.
- Git/branch: task branch `exp/learned-loop-baseline-v1-20260914`; exact run source/build identities per `papers/frontend_learned_loop_baseline_v1/runtime_receipts.json` (this publication commit is not the running source).
- Dataset/sequence/config: complete AFRL Bus Outside and Cemetery, three technical repeats each; frozen KLT source `3c50b742`, origin VINS binary `4e91d8ac`, same canonical local YAML and feature bag within each repeat. B = no loop, C = native BRIEF retrieval, L = fixed DINOv2-S/14 K32 retrieval; C/L share native BRIEF/3D–2D PnP and 4DoF graph.
- Commands/artifacts: exact replay/archive/graph commands, hashes and physical paths under `/media/ma/Elements/AQUA-FE_learned_loop_baseline_v1_runtime/` in `runtime_receipts.json`; complete compact output in `system_results.csv`, `loop_candidates.csv`, and `report.md` in the same paper directory. Task-specific summary command: `python3 -B scripts/summarize_loop_baseline_v1.py --runtime /media/ma/Elements/AQUA-FE_learned_loop_baseline_v1_runtime/summary_view_complete --output papers/frontend_learned_loop_baseline_v1`.
- Metrics/results: 2 physical sequences, 6/6 valid shared-local runs, 12/12 C/L graph runs, 18/18 arm rows. Bus C geometry passes 0/0/0 versus L 3/2/5; Cemetery C 0/0/0 versus L 0/1/1, the two Cemetery passes being the same original image pair. Cemetery each 3806 local poses, 406 common poses over 420 s, 96.897% numerical proxy-grid coverage, first nonlinear output 11.155363228 s. All C global outputs byte-identical to B; L changed output in 5/6 repeats. APE, 1 s RPE, fixed-scale/Sim3 accuracy, true Recall@4, formal WIN/TIE/LOSS and tracking-loss count: Not evaluated or Unknown as specified in report.
- Validity/common support: all six numerical common-support gates PASS; exact proxy camera-pose convention and independent loop correctness labels remain Unknown, so numerical support does not authorize an accuracy claim. Repeats are not independent scenes. Accepted constraints cannot be equated to correct loops. The configured ranking plus default score gate, not learned representation in isolation, is the tested contrast.
- Failures/deviation: seven actual local VIO starts for six valid completions, exceeding the planned maximum of six because first Cemetery r1 was interrupted when an unrelated VINS started mid-play; partial output preserved. Initial Cemetery archive failed on duplicate raw image timestamps and was preserved; passive archive repair selects only the originally published image-index phase, without changing estimator, retrieval or geometry. No window/result was removed.
- Interpretation/conclusion/follow-up: full fixed matrix establishes additional legal native geometric constraints for L, but neither independent correct-loop gain nor global trajectory improvement. Do not tune, retrain or add sequences. If accuracy is essential, first independently certify exact proxy pose convention and pair truth, then evaluate saved outputs without rerunning VIO.

## 2026-09-23 — frontend_learned_loop_baseline_v1 / conditional saved-output evaluation

- Experiment ID/date/status/role: `frontend_learned_loop_baseline_v1_reference_completion`,2026-09-23,`CONDITIONAL_PROXY_EVALUATION_COMPLETE_FORMAL_CLAIM_UNRESOLVED`; bounded diagnostic completion, not a new trained method or a certified primary benchmark.
- Source/branch/environment: `c23a483` plus explicit diagnostic evaluator/provenance/publisher changes, exact scriptSHA in new compact receipts; branch `exp/learned-loop-baseline-v1-20260914`. Existing systemPython/scipy/numpy/evo, one BLAS thread, no package upgrade. Historical VIO source remains separately recorded.
- Dataset/config/baseline/modification: same full AFRLBus/Cemetery, six saved local blocks,18B/C/L outputs. C/B no accepted loop; L frozenDINO-S14K32. No input/estimator/graph/retrieval change. Source-supported camera interpretation is explicit; full independent certification staysfalse. FixedproperSE3 versus separateSim3,1sgrid,1sgap,30poses/10s/70% gates retained.
- Commands/artifacts: `scripts/audit_loop_reference_provenance_v1.py` with frozen dataset root/priorchecks; `scripts/evaluate_loop_system_v1.py --source-supported-proxy-diagnostic` for all six blocks into Elements `reference_completion_20260923/<sequence>_r<repeat>`; `scripts/summarize_loop_evaluation_completion_v1.py` publishes `papers/frontend_learned_loop_baseline_v1/{proxy_diagnostic_accuracy.csv,accepted_pair_review.csv,evaluation_completion_receipts.json}`. Source inputs and provenance in `reference_provenance_completion.json`; evaluation scope in completion addendum.
- Metrics/results: C/L medianfixedAPE Bus1.040787/0.910719, ratio−12.497%; Cemetery0.854371/0.880116,+3.013%. PairedchangesBus−9.508/−16.407/−18.677%,Cemetery0/+3.452/+2.437%. MedianSim3APE Bus0.689820/0.484339,Cemetery0.810968/0.846645. Fixed1sRPE mediansBus0.042158/0.042044,Cemetery0.058709/0.058709, with nonzero small pairedregressions retained. Translation values are uncertified suppliedproxyunits, notmetres. Initialization/coverage inherited unchanged. NewVIO/graphs/modelinference/training0.
- Validity/common support: all six numerical gatesPASS;36/36 independent/evo alignmentchecksPASS;29/29 testsPASS. Publicauthoralignedfiles equal localbytes; all3388/2519 poses reproduced withscale1 first-poseSE3 residual<6e-15. Author-source convention support is not complete per-file certification. Formal18rowoutcomesNOT_EVALUABLE_REFERENCE_AND_LABELS; no formalWIN/TIE/LOSS/trueRecall.
- Pairtruth/failures: all8uniqueacceptedpairs reviewed,Bus7visuallysupported,Cemetery1plausible/repetitive; reviewnonblinded andformaltruthUnknown. No newfailedrun; preserve original7starts/6valid budgetdeviation and interruptedartifact.
- Interpretation/conclusion/follow-up: repeatable conditional Busgain coexists with Cemeteryregression. Current evidence cannot establish whole-system or representation superiority. Existing local data computation finished; stop automatic experiment expansion. Provider camera/scaleprovenance and independentlabels are the remaining external evidence gap.
