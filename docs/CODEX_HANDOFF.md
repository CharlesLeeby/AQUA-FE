...
**AUDIT-20260910-A08-EXISTING-REPLAYS：只读阶段完成，新增回放/算法/窗口均0。**
[报告](../papers/frontend_a08_replay_identity_audit_v1/report.md)、
[8条身份记录](../papers/frontend_a08_replay_identity_audit_v1/identity.csv)、
[21对工程差异](../papers/frontend_a08_replay_identity_audit_v1/pairwise_direct_differences.csv)、
[逐事件证据](../papers/frontend_a08_replay_identity_audit_v1/first_fork_events.json)、
[范围/停止条件](../papers/frontend_a08_replay_identity_audit_v1/audit_scope.md)、
[决策](../papers/frontend_a08_replay_identity_audit_v1/decision.json)。

- 7次同冻结诊断后端B，另1次不同日志后端单列；97个receipt产物、1471锁条目核对通过。
  8条均收到450帧/157500个feature观测，接收时刻/ID/次数/顺序相同，440个位姿输出头相同。
- 异常运行与锚点前6个保存位姿相同；第7次优化迭代记录7→6，位置开始分叉；
  两边仍是40ms预算，记录的资格/残差集合相同。集合差异到201、预算分支差异到206才出现。
- **实际求解工作量不同已确认；时间预算/主机负载造成最终发散尚未因果隔离。**
  不是已修复算法，不把所有前端恶化归给环境。原始最大位置范数283235.52m不是APE。
- 下一项唯一建议：单独冻结3次同输入/后端/YAML、原预算的工程重复，补齐每次运行环境记录；
  **尚未启动**，不恢复旧27次矩阵、不调前端、不扩窗。现有guard仍仅安全回退，正例收益未保留。
...
- 主线仍是安全回退，收益保留修复未完成。其后已完成上方A08只读诊断；
  没有重开替换/调参或已停止的回放矩阵。
