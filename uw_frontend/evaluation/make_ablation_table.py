...
def parse_spec(raw: str) -> AblationSpec:
    parts = raw.split(":", 1)
    if len(parts) != 2:
        raise argparse.ArgumentTypeError("spec must be label:path.csv")
    return AblationSpec(parts[0], Path(parts[1]))


def main() -> int:
    parser = argparse.ArgumentParser(description="Create a compact ablation table from frontend CSV files.")
    parser.add_argument("--spec", action="append", type=parse_spec, required=True)
    parser.add_argument("--output-csv", required=True)
    parser.add_argument("--output-md", required=True)
    args = parser.parse_args()

    rows = [_summarize(spec) for spec in args.spec]
    table = pd.DataFrame(rows)
    if not table.empty:
        base_dropout = float(table.loc[0, "dropout_mean"])
        base_age = float(table.loc[0, "track_age_median"])
        table["dropout_reduction_vs_first_pct"] = (base_dropout - table["dropout_mean"]) / max(base_dropout, 1e-6) * 100.0
        table["track_age_gain_vs_first"] = table["track_age_median"] - base_age
...
def _summarize(spec: AblationSpec) -> dict:
    ...
    return {
        ...
        "fundamental_inlier_median": _median(df, "fundamental_inlier_ratio"),
        "epipolar_error_median": _median(df, "median_epipolar_error"),
        ...
        "orb_recovery_mean": _mean(df, "orb_recovery_tracks"),
