#!/usr/bin/python3
"""Prepare and run the open-loop HFNet arms of runtime-exclusive v2.
...
RUNTIME_EXCLUSIVITY_ADDENDUM = (
    ROOT / "papers/fair_stability_runtime_exclusivity_addendum_v2.md"
)
...
RUNTIME_RESOURCE_MONITOR = (
    ROOT / "scripts/fair_stability_runtime_resource_monitor_v2.py"
)
...
EXPECTED_CONTROL_DOCUMENTS = {
    "runtime_exclusivity_addendum": (
        RUNTIME_EXCLUSIVITY_ADDENDUM,
        3_526,
        "1e48b96a78b7b2c940a9b02984b39fdb6eb0e52e52c935c5f3c0f5a082ac6d37",
    ),
    "v1_runtime_contamination_report": (
        V1_RUNTIME_CONTAMINATION_REPORT,
        3_436,
    ...
    "control_supersession": (
        CONTROL_SUPERSESSION,
        2_409,
        "19db817008cd5df08094dd9c70033fee2121c7011d6a3d274ad8acc429418408",
    ),
    "shutdown_addendum": (
        SHUTDOWN_ADDENDUM,
        1_645,
...
def freeze_inputs() -> dict[str, object]:
    ...
    try:
        ...
        manifest = {
            "schema_version": "aqua-fe-fair-stability-hfnet-input-freeze-v2",
...
def load_experiment() -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != "aqua-fe-fair-stability-hfnet-input-freeze-v2":
...
def prepare_attempt(
    ...
    try:
        ...
        attempt = {
            "schema_version": "aqua-fe-fair-stability-hfnet-attempt-v2",
            "experiment_id": EXPERIMENT_ID,
...
def prepare_all() -> list[dict[str, object]]:
    ...
    for cell in schedule:
        ...
        if root.exists():
            continue
        prepared.append(
            prepare_attempt(str(cell["case_id"]), budget, int(cell["repeat"]))
        )
    return prepared
...
def verify_attempt(
    ...
    if attempt.get("schema_version") != "aqua-fe-fair-stability-hfnet-attempt-v2":
...
def process_start_receipt(process: subprocess.Popen[bytes], attempt: Mapping[str, Any]) -> dict[str, object]:
    ...
    return {
        "schema_version": "aqua-fe-fair-stability-hfnet-launch-receipt-v2",
        "experiment_id": EXPERIMENT_ID,
        "valid": not errors,
        "errors": errors,
        "pid": process.pid,
...
def run_attempt(case_id: str, budget: int, repeat: int, attempt_index: int = 1) -> dict[str, object]:
    ...
    with lock_path.open("r+b") as lock_stream:
        ...
        claim = {
            "schema_version": "aqua-fe-fair-stability-hfnet-start-claim-v2",
            "experiment_id": EXPERIMENT_ID,
        ...
        try:
            ...
            with stdout_path.open("xb") as stdout, stderr_path.open("xb") as stderr:
                ...
                returncode, reaped, timed_out = runtime_monitor.wait(
        ...
        result = {
            "schema_version": "aqua-fe-fair-stability-hfnet-result-v2",
            "experiment_id": EXPERIMENT_ID,
            "status": status,
...
def summary() -> dict[str, object]:
    ...
    report = {
        "schema_version": "aqua-fe-fair-stability-hfnet-summary-v2",
        "experiment_id": EXPERIMENT_ID,
