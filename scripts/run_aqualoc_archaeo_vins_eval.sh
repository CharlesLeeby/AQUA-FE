...
SEQ_ARG="${2:-5}"          # 1..10, with or without leading zero
START_INDEX="${3:-2840}"
END_INDEX="${4:-3200}"
METHOD="${5:-hybrid_superpoint_lightglue}"
EVERY_N="${6:-2}"

ROOT="${ROOT:-/home/ma/AQUA-FE_WS}"
VINS_WS="${VINS_WS:-/home/ma/SLAM/VINS-Fusion-origin}"
AQUALOC_ROOT="${AQUALOC_ROOT:-$ROOT/datasets/full_downloads/aqualoc/Archaeological_site_sequences}"

SEQ_INT=$((10#$SEQ_ARG))
SEQ_PAD="$(printf "%02d" "$SEQ_INT")"

RAW_TAR="${RAW_TAR:-$AQUALOC_ROOT/archaeo_sequence_${SEQ_INT}_raw_data.tar.gz}"
RAW_ROOT="${RAW_ROOT:-raw_data}"
GT_TXT="${GT_TXT:-$AQUALOC_ROOT/archaeo_groundtruth_files/new_archaeo_colmap_traj_sequence_${SEQ_PAD}.txt}"
...
EXPORT_MAX_FEATURES="${EXPORT_MAX_FEATURES:-}"
if [[ -z "$EXPORT_MAX_FEATURES" ]]; then
  if [[ "$FORMAL_THREE_LAYER_EXPORT" == "1" ]]; then
    EXPORT_MAX_FEATURES="350"
  else
    EXPORT_MAX_FEATURES="150"
  fi
fi
...
if [[ -z "$BACKEND_QUALITY_MODE" && "$FORMAL_THREE_LAYER_EXPORT" != "1" ]]; then
  BACKEND_QUALITY_MODE="source_aware"
fi
BACKEND_QUALITY_FLOOR="${BACKEND_QUALITY_FLOOR:-}"
if [[ -z "$BACKEND_QUALITY_FLOOR" && -n "$BACKEND_QUALITY_MODE" ]]; then
  BACKEND_QUALITY_FLOOR="0.80"
fi
BACKEND_QUALITY_ALPHA="${BACKEND_QUALITY_ALPHA:-}"
if [[ -z "$BACKEND_QUALITY_ALPHA" && -n "$BACKEND_QUALITY_MODE" ]]; then
...
BACKEND_STATE_ADAPTIVE_HOLD_FRAMES="${BACKEND_STATE_ADAPTIVE_HOLD_FRAMES:-0}"
...
VINS_TD="${VINS_TD:--0.053694112369382575}"
VINS_ESTIMATE_TD="${VINS_ESTIMATE_TD:-0}"
...
ROSBAG_PLAY_TOPICS="${ROSBAG_PLAY_TOPICS:-}"
...
TAG="${TAG:-a${SEQ_PAD}_${START_INDEX}_${END_INDEX}}"
RUN_DIR="$ROOT/logs/aqualoc_archaeo_vins/${MODE}_${METHOD}_every${EVERY_N}_${TAG}"
RAW_BAG="${RAW_BAG:-$ROOT/datasets/aqualoc/rosbags/archaeo${SEQ_PAD}_${START_INDEX}_${END_INDEX}.bag}"
...
EXPORT_START_OFFSET="${EXPORT_START_OFFSET:-}"
EXPORT_DURATION="${EXPORT_DURATION:-}"
CAMERA_CONFIG="$RUN_DIR/aqualoc_archaeo${SEQ_PAD}_pinhole.yaml"
VINS_CONFIG="$RUN_DIR/vins_aqualoc_archaeo_${MODE}.yaml"
VINS_OUTPUT="$RUN_DIR/vins_output"
VINS_LOG="$RUN_DIR/vins.log"
APE_REPORT="$RUN_DIR/ape.txt"

mkdir -p "$RUN_DIR" "$(dirname "$RAW_BAG")"
if [[ "$RUN_VINS" == "1" ]]; then
  mkdir -p "$VINS_OUTPUT"
fi
...
if [[ "${BACKEND_REPLAY_ONLY:-0}" == "1" ]]; then
  if [[ "${AQUAFE_P07_FROZEN_ROS_ENV:-0}" != "1" || \
    ...
    echo "formal AQUALOC archaeology runner lacks the frozen minimal ROS environment" >&2
    exit 65
  fi
  ...
  if [[ "$AQUAFE_P07_PYTHON_INTERPRETER" != /proc/self/fd/* || \
    ...
    echo "formal archaeology Python interpreter is not sealed" >&2
    exit 65
  fi
...
if [[ ! -f "$RAW_TAR" ]]; then
  echo "missing raw tar: $RAW_TAR" >&2
  exit 2
fi
if [[ ! -f "$GT_TXT" ]]; then
  echo "missing GT txt: $GT_TXT" >&2
  exit 2
fi
...
if [[ ! -f "$RAW_BAG" ]]; then
  if [[ "${BACKEND_REPLAY_ONLY:-0}" == "1" ]]; then
    echo "formal backend requires a pre-materialized AQUALOC archaeology bag" >&2
    exit 66
  fi
  "${PYTHON_COMMAND[@]}" -m uw_frontend.datasets.aqualoc_raw_to_rosbag \
    --input "$RAW_TAR" \
    --output-bag "$RAW_BAG" \
    --sequence-name "archaeo_sequence_${SEQ_INT}" \
    --raw-root "$RAW_ROOT" \
    --image-dir "images_sequence_${SEQ_INT}" \
    --image-csv "img_sequence_${SEQ_INT}.csv" \
    --imu-csv "imu_sequence_${SEQ_INT}.csv" \
    --gt-txt "$GT_TXT" \
...
---
model_type: PINHOLE
camera_name: aqualoc_archaeo
image_width: 968
image_height: 608
distortion_parameters:
   k1: -0.1255945656257394
   k2: 0.053221287232781606
   p1: 9.94070021080493e-05
   p2: 9.550660927242349e-05
...
if [[ "$MODE" == "external" && ! -f "$FEATURE_BAG" ]]; then
  ...
  if [[ -n "$EXPORT_START_OFFSET" ]]; then
    EXPORT_ARGS+=(--start-offset "$EXPORT_START_OFFSET")
  fi
  if [[ -n "$EXPORT_DURATION" ]]; then
    EXPORT_ARGS+=(--duration "$EXPORT_DURATION")
  fi
  ...
  EXPORT_ARGS+=(
    ...
    --backend-state-adaptive-hold-frames "$BACKEND_STATE_ADAPTIVE_HOLD_FRAMES"
  )
...
if [[ "$AQUALOC_BODY_T_CAM0_MODE" == "auto" ]]; then
  # Archaeological sequences do not show the same clear Harbor-style fix:
  # A10 remains scale-unstable with either direction, so keep the legacy
  # direction unless a probe explicitly overrides it.
...
case "$AQUALOC_BODY_T_CAM0_MODE" in
  ...
  cam_imu)
   ...
   cols: 4
   dt: d
   data: [ -0.999372214240, 0.009015607185, 0.034262169099, -0.016780437426,
            -0.034374888514, -0.012659747010, -0.999328823684, -0.029657550728,
            -0.008575805730, -0.999879217522, 0.012961709893, -0.174939498459,
            0.0, 0.0, 0.0, 1.0 ]'
    ;;
  *)
...
if [[ -n "${VINS_NODE_BIN:-}" ]]; then
  if [[ "$VINS_NODE_BIN" != /* || ! -f "$VINS_NODE_BIN" || ! -x "$VINS_NODE_BIN" ]]; then
    echo "invalid absolute VINS_NODE_BIN=$VINS_NODE_BIN" >&2
    exit 65
  fi
  "$VINS_NODE_BIN" "$VINS_CONFIG" > "$VINS_LOG" 2>&1 &
else
  # Compatibility path for older exploratory invocations. Formal frozen runs
  # set VINS_NODE_BIN and therefore never delegate executable selection to
  # rospack/rosrun.
...
if [[ -n "$ROSBAG_PLAY_TOPICS" ]]; then
  read -r -a PLAY_TOPIC_ARRAY <<< "$ROSBAG_PLAY_TOPICS"
  PLAY_ARGS+=(--topics "${PLAY_TOPIC_ARRAY[@]}")
fi
