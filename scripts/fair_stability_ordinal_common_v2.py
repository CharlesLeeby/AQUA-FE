#!/usr/bin/python3
"""Fail-closed schedule state for fair-stability runtime-exclusive v2."""
...
EXPERIMENT_ID = "fair-stability-positive-roster-openloop-runtimeexcl-v2"
...
def control_boundary(experiment_root: Path) -> dict[str, dict[str, object]]:
    ...
    controller = common.parent / "run_fair_stability_next_v2.py"
    freeze_path = (
        experiment_root / "vins_dev_nativeq_schedfix_runtimeexcl_v2" / "backend_freeze.json"
    )
    ...
    return {
        ...
        "attempt_matrix_freeze": identity(experiment_root / "attempt_matrix_freeze_v2.json"),
    }
...
def load_attempt_matrix(experiment_root: Path) -> dict[str, Any]:
    """Verify the immutable 120-attempt v2 matrix without freezing later retries."""
    path = experiment_root / "attempt_matrix_freeze_v2.json"
    require_no_symlink_components(path, experiment_root)
    if path.is_symlink() or not path.is_file():
    ...
    if (
        ...
        != "aqua-fe-fair-stability-attempt-matrix-freeze-v2"
        or value.get("experiment_id") != EXPERIMENT_ID
        or value.get("status") != "FROZEN_BEFORE_ANY_V2_ESTIMATOR_START"
    ):
...
def attempt_root(experiment_root: Path, cell: Mapping[str, Any], attempt_index: int) -> Path:
    ...
    if arm.startswith("hfnet_openloop_"):
        ...
        base = (
            experiment_root / "vins_dev_nativeq_schedfix_runtimeexcl_v2" / "attempts"
    ...
    while attempt_index <= 99:
        ...
        if result_path.is_file():
            ...
            if status in TERMINAL_STATUSES:
                ...
                if terminal_receipt.is_file():
                    ...
                    if (
                        ...
                        != "aqua-fe-fair-stability-ordinal-terminal-v2"
            ...
            if (
                ...
                != "aqua-fe-fair-stability-pipeline-invalid-adjudication-v2"
...
def verify_dispatch_claim(
    ...
    if claim.get("schema_version") != "aqua-fe-fair-stability-ordinal-dispatch-v2":
