#!/usr/bin/python3
"""Continuous resource-exclusivity evidence for fair-stability v6 runs.
...
def _classify_gpu_applications(
    ...
    for source in applications:
        ...
        # Deliberately no ToDesk or desktop-process exemption in v6.
        external.append(row)
    return {
        "external": external,
...
def resource_gate(
    ...
    """Run the strict v6 pre-start exclusivity gate.
        ...
        with self._lock:
            ...
            while changed:
                ...
                for identity, record in current.items():
                    ...
                    if (
                        parent_now is None
                        or child_now is None
                        or _process_identity(parent_now) != expected_parent
                        or _process_identity(child_now) != identity
                        or int(child_now["ppid"]) != parent_pid
                    ):
                        continue
                    alive.add(identity)
                    changed = True
    ...
    def watchdog_sigterm_exact_group(
        ...
        *,
        ...
        if initial_returncode is not None:
            return passive("BEFORE_FIRST_FINAL_GROUP_PROOF", initial_returncode)
