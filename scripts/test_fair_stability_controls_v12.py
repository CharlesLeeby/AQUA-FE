...
VINS = load("fair_vins_v12", SCRIPTS / "run_fair_stability_vins_replay_v12.py")
HFNET = load("fair_hfnet_v12", SCRIPTS / "run_fair_stability_hfnet_openloop_v12.py")
ORDINAL = load("fair_ordinal_v12", SCRIPTS / "fair_stability_ordinal_common_v12.py")
NEXT = load("fair_next_v12", SCRIPTS / "run_fair_stability_next_v12.py")
SUMMARY = load("fair_summary_v12", SCRIPTS / "summarize_fair_stability_v12.py")
RESOURCE = load(
    "fair_runtime_resource_monitor_v12",
    SCRIPTS / "fair_stability_runtime_resource_monitor_v12.py",
)
SYSTEMD = load(
    "fair_systemd_supervisor_v12",
    SCRIPTS / "run_fair_stability_systemd_supervisor_v12.py",
)
...
class LifecycleAndScheduleTests(unittest.TestCase):
    def test_vins_lifecycle_exact_identity(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            start_values = {
                "schema_version": "aqua-fe-fair-stability-vins-child-start-v12",
            ...
            lifecycle_values.update(
                {
                    "schema_version": "aqua-fe-fair-stability-vins-child-lifecycle-v12",
    ...
    def test_v12_schedule_and_pristine_attempt_matrix(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
                ...
                ORDINAL.canonical_json(
                    {
                        ...
                        "claims": {
                            **{
                                f"v{version}_results_imported": False
            ...
            backend = (
                root
                / "vins_dev_nativeq_schedfix_runtimeexcl_v12"
                / "backend_freeze.json"
            )
            ...
            for cell in schedule:
                ...
                attempt_manifest.write_bytes(
                    ORDINAL.canonical_json(
                        {
                            "schema_version": (
                                "aqua-fe-fair-stability-hfnet-attempt-v12"
                                if str(cell["arm"]).startswith("hfnet_openloop_")
                                else "aqua-fe-fair-stability-vins-attempt-v12"
                            ),
                            "experiment_id": ORDINAL.EXPERIMENT_ID,
            ...
            matrix_path = root / "attempt_matrix_freeze_v12.json"
            matrix_path.write_bytes(
                ORDINAL.canonical_json(
                    {
                        "schema_version": "aqua-fe-fair-stability-attempt-matrix-freeze-v12",
                        "experiment_id": ORDINAL.EXPERIMENT_ID,
                        "status": "FROZEN_BEFORE_ANY_V12_ESTIMATOR_START",
            ...
            self.assertIs(matrix["claims"]["v8_results_imported"], False)
            self.assertIs(matrix["claims"]["v9_results_imported"], False)
            self.assertIs(matrix["claims"]["v9_runtime_namespace_read"], False)
            self.assertIs(matrix["claims"]["v9_attempts_reused"], False)
            self.assertIs(matrix["claims"]["v9_cache_reused"], False)
            self.assertIs(matrix["claims"]["v10_results_imported"], False)
            self.assertIs(matrix["claims"]["v10_runtime_namespace_read"], False)
            self.assertIs(matrix["claims"]["v10_attempts_reused"], False)
            self.assertIs(matrix["claims"]["v10_cache_reused"], False)
            self.assertIs(matrix["claims"]["v11_results_imported"], False)
            self.assertIs(matrix["claims"]["v11_runtime_namespace_read"], False)
            self.assertIs(matrix["claims"]["v11_attempts_reused"], False)
            self.assertIs(matrix["claims"]["v11_cache_reused"], False)
            state = ORDINAL.next_state(root)
            self.assertEqual(state["state"], "READY")
            self.assertEqual(state["cell"]["ordinal"], 1)


class V12ControlRegressionTests(unittest.TestCase):
    def test_v12_control_document_identities_and_history_are_pinned(self) -> None:
        ...
        for label in (
            ...
            "v9_runtime_monitor_termination_race_abandonment_report",
            "v10_proc_tail_closure_abandonment_report",
            "v11_watchdog_pending_reap_abandonment_report",
        ):
        ...
        self.assertEqual(VINS.V8_ABANDONMENT_REPORT, HFNET.V8_ABANDONMENT_REPORT)
        self.assertEqual(VINS.V9_ABANDONMENT_REPORT, HFNET.V9_ABANDONMENT_REPORT)
        self.assertEqual(VINS.V10_ABANDONMENT_REPORT, HFNET.V10_ABANDONMENT_REPORT)
        self.assertEqual(VINS.V11_ABANDONMENT_REPORT, HFNET.V11_ABANDONMENT_REPORT)
        self.assertEqual(
            VINS.V8_ABANDONMENT_REPORT.name,
            "fair_stability_v8_historical_no_start_revalidation_abandonment_report_20260830_v9.md",
        )
        self.assertEqual(
            VINS.V9_ABANDONMENT_REPORT.name,
            "fair_stability_v9_runtime_monitor_termination_race_abandonment_report_20260830_v10.md",
        )
        self.assertEqual(
            VINS.V10_ABANDONMENT_REPORT.name,
            "fair_stability_v10_proc_tail_closure_abandonment_report_20260830_v11.md",
        )
        self.assertEqual(
            VINS.V11_ABANDONMENT_REPORT.name,
            "fair_stability_v11_watchdog_pending_reap_abandonment_report_20260830_v12.md",
        )
        self.assertIn(VINS.V7_ABANDONMENT_REPORT, VINS.CONTROL_PATHS)
        self.assertIn(VINS.V8_ABANDONMENT_REPORT, VINS.CONTROL_PATHS)
        self.assertIn(VINS.V9_ABANDONMENT_REPORT, VINS.CONTROL_PATHS)
        self.assertIn(VINS.V10_ABANDONMENT_REPORT, VINS.CONTROL_PATHS)
        self.assertIn(VINS.V11_ABANDONMENT_REPORT, VINS.CONTROL_PATHS)

    def test_v12_controls_never_read_predecessor_runtime_or_attempt_namespaces(self) -> None:
        controls = (
            "fair_stability_ordinal_common_v12.py",
            "fair_stability_runtime_resource_monitor_v12.py",
            "run_fair_stability_hfnet_openloop_v12.py",
            "run_fair_stability_vins_replay_v12.py",
            "run_fair_stability_next_v12.py",
            "run_fair_stability_systemd_supervisor_v12.py",
            "run_fair_stability_vins_replay_child_v12.sh",
            "summarize_fair_stability_v12.py",
        )
        forbidden = (
            "fair_stability_positive_roster_openloop_v8",
            "vins_dev_nativeq_schedfix_runtimeexcl_v8",
            "attempt_matrix_freeze_v8",
            "fair_stability_progress_summary_v8",
            "run_fair_stability_next_v8",
            "fair_stability_ordinal_common_v8",
            "fair_stability_positive_roster_openloop_v9",
            "vins_dev_nativeq_schedfix_runtimeexcl_v9",
            "attempt_matrix_freeze_v9",
            "fair_stability_progress_summary_v9",
            "run_fair_stability_next_v9",
            "fair_stability_ordinal_common_v9",
            "fair_stability_runtime_resource_monitor_v9",
            "run_fair_stability_hfnet_openloop_v9",
            "run_fair_stability_vins_replay_v9",
            "run_fair_stability_systemd_supervisor_v9",
            "run_fair_stability_vins_replay_child_v9",
            "summarize_fair_stability_v9",
            "fair_stability_positive_roster_openloop_v10",
            "vins_dev_nativeq_schedfix_runtimeexcl_v10",
            "attempt_matrix_freeze_v10",
            "fair_stability_progress_summary_v10",
            "run_fair_stability_next_v10",
            "fair_stability_ordinal_common_v10",
            "fair_stability_runtime_resource_monitor_v10",
            "run_fair_stability_hfnet_openloop_v10",
            "run_fair_stability_vins_replay_v10",
            "run_fair_stability_systemd_supervisor_v10",
            "run_fair_stability_vins_replay_child_v10",
            "summarize_fair_stability_v10",
            "test_fair_stability_controls_v10",
            "test_fair_stability_runtime_close_v10",
            "fair_stability_positive_roster_openloop_v11",
            "vins_dev_nativeq_schedfix_runtimeexcl_v11",
            "attempt_matrix_freeze_v11",
            "fair_stability_progress_summary_v11",
            "run_fair_stability_next_v11",
            "fair_stability_ordinal_common_v11",
            "fair_stability_runtime_resource_monitor_v11",
            "run_fair_stability_hfnet_openloop_v11",
            "run_fair_stability_vins_replay_v11",
            "run_fair_stability_systemd_supervisor_v11",
            "run_fair_stability_vins_replay_child_v11",
            "summarize_fair_stability_v11",
            "test_fair_stability_controls_v11",
            "test_fair_stability_runtime_close_v11",
        )
        allowed_v9_provenance = (
            "fair_stability_v8_historical_no_start_revalidation_abandonment_report_20260830_v9.md",
            "fair_stability_v9_runtime_monitor_termination_race_abandonment_report_20260830_v10.md",
            "v9_runtime_monitor_termination_race_abandonment_report",
            "v9_abandonment_report",
            "v9_results_imported",
            "v9_runtime_namespace_read",
            "v9_attempts_reused",
            "v9_cache_reused",
            "valid_v9_non_import_claims",
        )
        allowed_v10_provenance = (
            "fair_stability_v9_runtime_monitor_termination_race_abandonment_report_20260830_v10.md",
            "fair_stability_v10_proc_tail_closure_abandonment_report_20260830_v11.md",
            "v10_proc_tail_closure_abandonment_report",
            "v10_abandonment_report",
            "v10_results_imported",
            "v10_runtime_namespace_read",
            "v10_attempts_reused",
            "v10_cache_reused",
            "valid_v10_non_import_claims",
            "V10_NON_IMPORT_CLAIM_KEYS",
        )
        allowed_v11_provenance = (
            "fair_stability_v10_proc_tail_closure_abandonment_report_20260830_v11.md",
            "fair_stability_v11_watchdog_pending_reap_abandonment_report_20260830_v12.md",
            "v11_watchdog_pending_reap_abandonment_report",
            "v11_abandonment_report",
            "v11_results_imported",
            "v11_runtime_namespace_read",
            "v11_attempts_reused",
            "v11_cache_reused",
            "valid_v11_non_import_claims",
            "V11_NON_IMPORT_CLAIM_KEYS",
        )
        for name in controls:
            source = (SCRIPTS / name).read_text(encoding="utf-8")
            with self.subTest(name=name):
                self.assertFalse(any(value in source for value in forbidden))
                self.assertIn("v12", source)
                for line in source.splitlines():
                    if "v9" in line:
                        self.assertTrue(
                            any(value in line for value in allowed_v9_provenance),
                            line,
                        )
                    if "v10" in line:
                        self.assertTrue(
                            any(value in line for value in allowed_v10_provenance),
                            line,
                        )
                    if "v11" in line:
                        self.assertTrue(
                            any(value in line for value in allowed_v11_provenance),
                            line,
                        )

    def test_v11_non_import_flags_fail_closed_in_freeze_readers(self) -> None:
        claims = {
            **{
                f"v{version}_results_imported": False
                for version in range(1, 12)
        ...
        self.assertTrue(ORDINAL.valid_v11_non_import_claims(claims))
        with tempfile.TemporaryDirectory() as temporary:
            freeze_path = Path(temporary) / "backend_freeze.json"
            for key in sorted(ORDINAL.V11_NON_IMPORT_CLAIM_KEYS):
                for mutation in ("missing", "true"):
                    with self.subTest(key=key, mutation=mutation):
                        invalid_claims = dict(claims)
                        if mutation == "missing":
                            invalid_claims.pop(key)
                        else:
                            invalid_claims[key] = True
                        self.assertFalse(
                            ORDINAL.valid_v11_non_import_claims(invalid_claims)
                        )
                        freeze_path.write_bytes(
                            ORDINAL.canonical_json(
                                {
                                    "schema_version": (
                                        "aqua-fe-dev-nativeq-schedfix-"
                                        "runtimeexcl-backend-freeze-v12"
                                    ),
                                    "experiment_id": ORDINAL.EXPERIMENT_ID,
                                    "backend_id": (
                                        "DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V12"
                                    ),
                        ...
                        with mock.patch.object(
                            SYSTEMD, "BACKEND_FREEZE", freeze_path
                        ), self.assertRaisesRegex(
                            SYSTEMD.SystemdSupervisorError,
                            "PRIOR_RESULTS_IMPORT_CLAIM_INVALID",
                        ):
                            SYSTEMD.freeze_context()
                        with mock.patch.object(
                            NEXT, "BACKEND_FREEZE", freeze_path
                        ), self.assertRaisesRegex(
                            NEXT.ControllerError,
                            "PRIOR_RESULTS_IMPORT_CLAIM_INVALID",
                        ):
                            NEXT.verify_control_freeze()
                        with mock.patch.object(
                            HFNET, "BACKEND_FREEZE", freeze_path
                        ), self.assertRaisesRegex(
                            HFNET.ContractError,
                            "PRIOR_RESULTS_IMPORT_CLAIM_INVALID",
                        ):
                            HFNET.backend_freeze_identity()
                        with mock.patch.object(
                            VINS, "BACKEND_FREEZE", freeze_path
                        ), mock.patch.object(
                            VINS, "verify_common_inputs"
                        ), self.assertRaisesRegex(
                            VINS.ContractError,
                            "BACKEND_FREEZE_CLAIMS_MISMATCH",
                        ):
                            VINS.load_freeze()
    ...
    def _runtime_fixture(
        ...
        closing_proc = {
            "sequence": 1,
            "probe_role": RESOURCE.CLOSING_PROC_PROBE_ROLE,
            "probe_started_at_utc": "2026-08-29T10:00:01+00:00",
            "probe_started_monotonic_ns": 1_080_000_000,
            "probe_finished_monotonic_ns": 1_090_000_000,
            "duration_seconds": 0.01,
            "process_count": 0,
            "owned_identities": [],
            "supervised_pgid": pid,
            ...
            "external_conflicting_processes": [],
            "coverage_close_ownership_empty": True,
            "coverage_close_recheck_proven": True,
            "probe_errors": [],
        }
        gpu = {
            "sequence": 0,
        ...
        receipt = {
            ...
            "sampling_contract": {
                "proc": RESOURCE._sampling_gap_report(
                    [proc, closing_proc], start_ns, end_ns,
            ...
            "post_samples": {"proc": closing_proc, "gpu": gpu},
            "samples": {"proc": [proc, closing_proc], "gpu": [gpu]},
            "observed_intrusions": {
                "proc_occurrences": [],
                "gpu_occurrences": [],
                "stale_owned_gpu_rows": [],
    ...
    def test_common_validator_cases_7_to_9_closing_proc_evidence(self) -> None:
        def publish(
            root: Path,
            result: dict[str, object],
        ...
        # Case 7: the evidence-bearing closing row keeps the unchanged proc
        # tail contract truthful even when the old invisible-scan shape fails.
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / "attempt"
            cell, result, receipt = self._runtime_fixture(
                root, "hfnet_openloop_350"
            )
            start_ns = receipt["coverage"]["start_monotonic_ns"]
            ordinary, closing = receipt["samples"]["proc"]
            ordinary.update(
                {
                    "probe_started_monotonic_ns": 1_010_000_000,
                    "probe_finished_monotonic_ns": 1_239_000_000,
                    "duration_seconds": 0.229,
                }
            )
            closing.update(
                {
                    "probe_started_monotonic_ns": 1_240_000_000,
                    "probe_finished_monotonic_ns": 1_439_000_000,
                    "duration_seconds": 0.199,
                }
            )
            end_ns = 1_440_000_000
            receipt["coverage"]["end_monotonic_ns"] = end_ns
            receipt["coverage"]["duration_seconds"] = 0.44
            receipt["sampling_contract"]["proc"] = RESOURCE._sampling_gap_report(
                [ordinary, closing],
                start_ns,
                end_ns,
                RESOURCE.PROC_TARGET_INTERVAL_SECONDS,
            ...
            receipt["sampling_contract"]["gpu"] = RESOURCE._sampling_gap_report(
                receipt["samples"]["gpu"],
                start_ns,
                end_ns,
                RESOURCE.GPU_TARGET_INTERVAL_SECONDS,
            ...
            self.assertTrue(receipt["sampling_contract"]["proc"]["proven"])

        # Case 8: a closing scan whose start-to-end tail exceeds 0.25 s is a
        # validly evidenced fail-closed pipeline outcome, never a clean pass.
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / "attempt"
            cell, result, receipt = self._runtime_fixture(
                root, "hfnet_openloop_350"
            ...
            start_ns = receipt["coverage"]["start_monotonic_ns"]
            closing = receipt["samples"]["proc"][-1]
            closing.update(
                {
                    "probe_started_monotonic_ns": 1_200_000_000,
                    "probe_finished_monotonic_ns": 1_250_000_000,
                    "duration_seconds": 0.05,
                }
            )
            end_ns = 1_500_000_000
            receipt["coverage"]["end_monotonic_ns"] = end_ns
            receipt["coverage"]["duration_seconds"] = 0.5
            for stream, target, maximum in (
                ...
                receipt["sampling_contract"][stream] = (
                    RESOURCE._sampling_gap_report(
                        receipt["samples"][stream],
                        start_ns,
                        end_ns,
                        target,
                        maximum,
            ...
            receipt.update(
                {
                    ...
                    "required_attempt_classification": "PIPELINE_INVALID",
                }
            )
            ...
            result["runtime_resource_monitor_decision"] = {
                "intrusion_detected": False,
                "monitor_proven": False,
                "pipeline_failure_reasons": [
                    "RUNTIME_RESOURCE_MONITOR_UNPROVEN"
            ...
            self.assertFalse(receipt["sampling_contract"]["proc"]["proven"])
            self.assertTrue(
                receipt["sampling_contract"]["proc"][
                    "tail_coverage_gap_violation"
                ]
            )

        # Case 9 and public-schema counterexamples: the role is mandatory,
        # and missing, duplicate, misordered, or unproven closing evidence is
        # rejected even when all receipt hashes and derived summaries are repinned.
        def duplicate_closing(value: dict[str, object]) -> None:
            rows = value["samples"]["proc"]
            duplicate = json.loads(json.dumps(rows[-1]))
            duplicate.update(
                {
                    "sequence": 2,
                    "probe_started_monotonic_ns": 1_095_000_000,
                    "probe_finished_monotonic_ns": 1_099_000_000,
                    "duration_seconds": 0.004,
                }
            )
            rows.append(duplicate)

        def misorder_closing(value: dict[str, object]) -> None:
            rows = value["samples"]["proc"]
            rows.reverse()
            for sequence, row in enumerate(rows):
                row["sequence"] = sequence

        def add_closing_error(value: dict[str, object]) -> None:
            error = "COVERAGE_CLOSE_OWNERSHIP_RECHECK_UNPROVEN"
            value["samples"]["proc"][-1]["probe_errors"] = [error]
            value["probe_errors"] = [error]

        mutations = {
            "ordinary_role_missing": lambda value: value["samples"]["proc"][0].pop(
                "probe_role"
            ),
            "closing_missing": lambda value: value["samples"]["proc"].pop(),
            "closing_duplicated": duplicate_closing,
            "closing_misordered": misorder_closing,
            "closing_error": add_closing_error,
            "closing_unproven": lambda value: value["samples"]["proc"][-1].update(
                {
                    "coverage_close_ownership_empty": False,
        ...
        for label, mutate in mutations.items():
            with self.subTest(label=label), tempfile.TemporaryDirectory() as temporary:
                ...
                mutate(receipt)
                proc_rows = receipt["samples"]["proc"]
                receipt["pre_samples"]["proc"] = proc_rows[0]
                receipt["post_samples"]["proc"] = proc_rows[-1]
                receipt["sampling_contract"]["proc"] = (
                    RESOURCE._sampling_gap_report(
                        proc_rows,
                        receipt["coverage"]["start_monotonic_ns"],
                        receipt["coverage"]["end_monotonic_ns"],
                        RESOURCE.PROC_TARGET_INTERVAL_SECONDS,
                        RESOURCE.PROC_MAX_START_GAP_SECONDS,
                    )
                )
    ...
    def test_runtime_receipt_rejects_sampling_coverage_and_code_forgery(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            finish_after_end = json.loads(json.dumps(receipt))
            proc_row = finish_after_end["samples"]["proc"][0]
            proc_row["probe_finished_monotonic_ns"] = (
                finish_after_end["coverage"]["end_monotonic_ns"] + 1
            )
            proc_row["duration_seconds"] = (
                proc_row["probe_finished_monotonic_ns"]
                - proc_row["probe_started_monotonic_ns"]
            ) / 1e9
            (root / "runtime_resource_monitor.json").write_bytes(
                ORDINAL.canonical_json(finish_after_end)
            )
            self._repin_runtime(root, result)
        ...
        for label, mutate in mutations.items():
            with self.subTest(label=label), tempfile.TemporaryDirectory() as temporary:
                ...
                if "samples" in receipt:
                    ...
                    receipt["post_samples"] = {
                        stream: json.loads(json.dumps(receipt["samples"][stream][-1]))
                        for stream in ("proc", "gpu")
                    }
                publish(root, result, receipt)
                with self.assertRaises(ORDINAL.OrdinalError):
    ...
    def test_vins_roscore_interpreter_is_fixed_not_manifest_selected(self) -> None:
        ...
        wrapper = (
            SCRIPTS / "run_fair_stability_vins_replay_child_v12.sh"
    ...
    CHILD = SCRIPTS / "run_fair_stability_vins_replay_child_v12.sh"
    ...
    def test_vins_rejects_prior_result_import_claims(self) -> None:
        ...
        manifest = {
            "schema_version": "aqua-fe-fair-stability-hfnet-input-freeze-v12",
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch.object(VINS, "EXPERIMENT_ROOT", root), mock.patch.object(
                ...
                VINS, "read_csv_rows", side_effect=[roster, historical] * 32
            ), mock.patch.object(
    ...
    def test_runner_summaries_delegate_to_receipt_state_machine(self) -> None:
        ...
        with mock.patch(
            "summarize_fair_stability_v12.summarize", return_value=sentinel
        ) as summarize:
    ...
    def test_postprocess_signals_remain_fail_stop_through_result_publication(self) -> None:
        ...
        for path in (
            SCRIPTS / "run_fair_stability_hfnet_openloop_v12.py",
            SCRIPTS / "run_fair_stability_vins_replay_v12.py",
        ):
    ...
    def test_result_published_then_runner_signal_is_controller_fail_stop(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            matrix = base / "attempt_matrix_freeze_v12.json"
            matrix.write_text("{}\n", encoding="ascii")
            dispatch_claim = attempt / "ordinal_dispatch_claim.json"
            dispatch_claim.write_text("{}\n", encoding="ascii")
            attempt_manifest = {
                "schema_version": "synthetic-attempt-v12",
                "experiment_id": ORDINAL.EXPERIMENT_ID,
                "port": 26100,
            }
            (attempt / "attempt_manifest.json").write_bytes(
                ORDINAL.canonical_json(attempt_manifest)
            )
            ready_gate = {
                "schema_version": ORDINAL.PRESTART_RESOURCE_GATE_SCHEMA,
                "experiment_id": ORDINAL.EXPERIMENT_ID,
                "checked_at_utc": "2026-08-30T00:00:00+00:00",
                "probe_started_monotonic_ns": 1,
                "probe_finished_monotonic_ns": 2,
                "ready": True,
                "errors": [],
                "proc_probe_errors": [],
                ...
                "port": 26100,
                "port_available": True,
                "experiment_disk_probe": "/tmp",
                "experiment_free_bytes": 5 * 1024**3,
                "workspace_disk_probe": "/tmp",
                "workspace_free_bytes": 512 * 1024**2,
            ...
            def fake_run(
                ...
                (attempt / "run_result.json").write_bytes(
                    ORDINAL.canonical_json(
                        {
                            "schema_version": "aqua-fe-fair-stability-vins-result-v12",
                            "experiment_id": ORDINAL.EXPERIMENT_ID,
                            "case_id": "a05_3300_3700",
                            "arm": "learned_klt_vins",
                            "repeat": 1,
    ...
    def test_resource_check_stdout_then_signal_cannot_start_estimator(self) -> None:
        state = {
            "state": "READY",
            "attempt_root": "/tmp/never-started-v12",
            "attempt_index": 1,
            "cell": {
                "ordinal": 1,
                "case_id": "a05_3300_3700",
        ...
        gate = {
            "schema_version": "aqua-fe-fair-stability-prestart-resource-gate-v12",
            "experiment_id": ORDINAL.EXPERIMENT_ID,
            "ready": True,
        }
    ...
    def test_pipeline_fail_stop_state_is_never_resubmitted(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            experiment_root.mkdir()
            with mock.patch.object(
                SYSTEMD, "EXPERIMENT_ROOT", experiment_root
            ), mock.patch.object(
                SYSTEMD,
                "SUPERVISION_ROOT",
    ...
    def test_pipeline_adjudication_is_disabled_and_writes_nothing(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch.object(NEXT, "verify_control_freeze", return_value={}), mock.patch.object(
                ...
                NEXT.ControllerError,
                "PIPELINE_ADJUDICATION_DISABLED_ZERO_REPLACEMENT_V12",
            ):
    ...
    def test_terminal_adoption_has_bound_monitor_and_two_validations(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            claim = {
                "schema_version": "aqua-fe-fair-stability-systemd-adoption-claim-v12",
                ...
                "controller_outcome": ORDINAL.TERMINAL_SYSTEMD_OUTCOME,
            }
            fake_chain = {
                "submission_claim": {"unit": "unit.service"},
                "systemd_execution_receipt": {
                    "controller_returncode": 0,
                    "ordinal_state_after_controller": state,
                    "controller_result": {
                        "outcome": ORDINAL.TERMINAL_SYSTEMD_OUTCOME,
                        "state": state,
                    },
                },
            }
            ...
            with mock.patch.object(
                ...
                NEXT, "validate_systemd_chain", return_value=fake_chain
            ), mock.patch.object(
    ...
    def test_second_validation_failure_prevents_both_receipt_writes(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            fake_chain = {
                ...
                "systemd_execution_receipt": {
                    "controller_returncode": 0,
                    "ordinal_state_after_controller": terminal_state,
                    "controller_result": {
                        "outcome": ORDINAL.TERMINAL_SYSTEMD_OUTCOME,
    ...
    def test_matrix_freeze_requires_exact_pristine_prepare_tree(self) -> None:
        ...
        def prepare_fixture(experiment_root: Path) -> tuple[Path, Path]:
            ...
            manifest = {
                "schema_version": "aqua-fe-fair-stability-hfnet-attempt-v12",
                "experiment_id": ORDINAL.EXPERIMENT_ID,
                "case_id": cell["case_id"],
                "arm": cell["arm"],
                "repeat": 1,
...
class SystemdControlTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        # Systemd-chain tests use isolated roots rather than the immutable
        # formal 120-cell schedule.  Supply an equivalent synthetic schedule
        # so final-adoption validation still exercises exact recomputation.
        def schedule(root: Path) -> list[dict[str, object]]:
            cells: list[dict[str, object]] = [
                {"ordinal": ordinal} for ordinal in range(1, 121)
            ]
            for path in root.glob(
                "systemd_supervision/ordinal_*/submission_*/"
                "submission_claim.json"
            ):
                value = json.loads(path.read_text(encoding="utf-8"))
                state = value.get("ordinal_state_before_submit")
                cell = state.get("cell") if isinstance(state, dict) else None
                ordinal = value.get("planned_ordinal")
                if (
        ...
        def inspect(root: Path, cell: Mapping[str, object]):
            ...
            return {
                ...
                "attempt_index": 1,
                "attempt_root": str(
                    root / f"attempt_ordinal_{ordinal:03d}"
                ),
            }

        canonical_common = sys.modules[
            SYSTEMD.validate_finalized_systemd_adoption.__module__
        ]
        modules = {ORDINAL, canonical_common}
        self._common_state_patchers = []
        for module in modules:
            schedule_patch = mock.patch.object(
                module, "load_schedule", side_effect=schedule
            )
            inspect_patch = mock.patch.object(
                module, "inspect_cell", side_effect=inspect
            )
            schedule_patch.start()
            inspect_patch.start()
            self._common_state_patchers.extend(
                [schedule_patch, inspect_patch]
            )
        for patcher in reversed(self._common_state_patchers):
            self.addCleanup(patcher.stop)

    def ready_resource_gate(
        ...
        return {
            ...
            "external_gpu_todesk_exemption": False,
            "port": port,
            "port_available": None if port is None else True,
            "experiment_disk_probe": "/tmp",
            "experiment_free_bytes": 5 * 1024**3,
            "workspace_disk_probe": "/tmp",
            "workspace_free_bytes": 512 * 1024**2,
    ...
    def blocked_resource_gate(
        self, port: int | None = 26100
    ) -> dict[str, object]:
        gate = self.ready_resource_gate(port)
        if port is None:
            gate["experiment_free_bytes"] = 5 * 1024**3 - 1
            gate["errors"] = ["EXPERIMENT_FREE_SPACE_BELOW_5_GIB"]
        else:
            gate["port_available"] = False
            gate["errors"] = ["ROS_PORT_UNAVAILABLE"]
        gate["ready"] = False
        return gate

    def check_attempt_manifest(
        self, state: Mapping[str, object]
    ) -> dict[str, object]:
        root = Path(str(state["attempt_root"]))
        path = root / "attempt_manifest.json"
        if not path.exists():
            root.mkdir(parents=True, exist_ok=True)
            path.write_bytes(
                ORDINAL.canonical_json(
    ...
    def ready_state(self, experiment_root: Path) -> dict[str, object]:
        return {
            "state": "READY",
            "cell": {
                "ordinal": 1,
    ...
    def stopped_observation(
        self, unit: str, *, after_reset: bool
    ) -> dict[str, object]:
        if after_reset:
            properties = {
                "Id": unit,
        ...
        return {
            "queried_at_utc": "2026-08-30T00:00:00+00:00",
            "returncode": 0,
            "properties": properties,
        }

    def make_resubmit_authorization(
        self,
        ...
        claim = json.loads(
            (prior_directory / "submission_claim.json").read_text(
                encoding="utf-8"
            )
        )
        unit = claim["unit"]
        state = self.ready_state(experiment_root)
        before = self.stopped_observation(unit, after_reset=False)
        after = self.stopped_observation(unit, after_reset=True)
        reset_stdout = (
            prior_directory
        ...
        for path in (reset_stdout, reset_stderr, check_stderr):
            path.write_text("", encoding="utf-8")
        attempt = self.check_attempt_manifest(state)
        gate = self.ready_resource_gate(int(attempt["port"]))
        check_result = {
            "attempt": attempt,
            "ready": True,
            "resource_gate": gate,
        }
        check_stdout.write_bytes(ORDINAL.canonical_json(check_result))
        stamp = "2026-08-30T00:00:00+00:00"
        authorization = {
            "schema_version": SYSTEMD.RESUBMIT_AUTHORIZATION_SCHEMA,
            "experiment_id": ORDINAL.EXPERIMENT_ID,
            "authorized_at_utc": stamp,
            "authorization_command": SYSTEMD._authorization_command(
                prior_directory
            ),
            "planned_ordinal": 1,
            "attempt_index": 1,
            "attempt_root": str(experiment_root / "attempt"),
            "prior_submission_index": authorized_submission_index - 1,
            "authorized_submission_index": authorized_submission_index,
            "prior_unit": unit,
            "prior_invocation_id": self.INVOCATION,
            "prior_submission_claim": ORDINAL.identity(
                prior_directory / "submission_claim.json"
            ),
            "prior_systemd_start_receipt": ORDINAL.identity(
                prior_directory / "systemd_start_receipt.json"
            ),
            "prior_systemd_adoption_receipt": ORDINAL.identity(
            ...
            "stopped_unit_contract_before_reset": (
                SYSTEMD._stopped_unit_contract(
                    unit, before, self.INVOCATION, after_reset=False
                )
            ),
            "reset_failed": {
                "sequence_index": 1,
                ...
                "returncode": 0,
                "stdout": ORDINAL.identity(reset_stdout),
                "stderr": ORDINAL.identity(reset_stderr),
            },
            "stopped_unit_observation_after_reset": after,
            "stopped_unit_contract_after_reset": (
                SYSTEMD._stopped_unit_contract(
                    unit, after, self.INVOCATION, after_reset=True
                )
            ),
            "resource_check": {
                "sequence_index": 2,
                "started_at_utc": stamp,
                "completed_at_utc": stamp,
                "argv": SYSTEMD._runner_check_argv(state),
                "environment": dict(ORDINAL.FIXED_SERVICE_ENVIRONMENT),
                "returncode": 0,
                "stdout": ORDINAL.identity(check_stdout),
                "stderr": ORDINAL.identity(check_stderr),
                "ready": True,
                "resource_gate": gate,
                "runner_result": check_result,
            },
        }
        path = SYSTEMD._authorization_path(
            prior_directory, authorized_submission_index
        )
        path.write_bytes(ORDINAL.canonical_json(authorization))
        return ORDINAL.identity(path)

    def make_resubmit_consumption(
        self,
        prior_directory: Path,
        new_directory: Path,
        new_submission_index: int,
    ) -> dict[str, object]:
        authorization_path = SYSTEMD._authorization_path(
        ...
        new_claim = json.loads(new_claim_path.read_text(encoding="utf-8"))
        consumption = {
            "schema_version": (
                ORDINAL.RESUBMIT_AUTHORIZATION_CONSUMPTION_SCHEMA
            ),
            "experiment_id": ORDINAL.EXPERIMENT_ID,
            "consumed_at_utc": "2026-08-30T00:00:00+00:00",
            "resubmit_authorization": ORDINAL.identity(authorization_path),
            "prior_submission_directory": str(prior_directory),
            "prior_submission_index": new_submission_index - 1,
            "prior_submission_claim": ORDINAL.identity(prior_claim_path),
            "new_submission_directory": str(new_directory),
            "new_submission_index": new_submission_index,
            "new_submission_claim": ORDINAL.identity(new_claim_path),
            "new_unit": new_claim["unit"],
            "planned_ordinal": new_claim["planned_ordinal"],
            "attempt_index": new_claim["attempt_index"],
            "attempt_root": new_claim["attempt_root"],
        }
        self.assertEqual(
            prior_claim["submission_index"], new_submission_index - 1
        )
        path = SYSTEMD._authorization_consumption_path(
            prior_directory, new_submission_index
        )
        path.write_bytes(ORDINAL.canonical_json(consumption))
        return ORDINAL.identity(path)
    ...
    def build_chain(
        ...
        planned_ordinal: int = 1,
        attempt_root_override: Path | None = None,
    ) -> tuple[Path, dict[str, dict[str, object]]]:
        attempt_root = (
            experiment_root / "attempt"
            if attempt_root_override is None
            else attempt_root_override
        )
        directory = (
            experiment_root
            / "systemd_supervision"
            / f"ordinal_{planned_ordinal:03d}"
            / f"submission_{submission_index:03d}"
        )
        authorization = None
        prior_directory = None
        if submission_index > 1:
            prior_directory = directory.parent / (
                f"submission_{submission_index - 1:03d}"
            )
            authorization = self.make_resubmit_authorization(
                experiment_root, prior_directory, submission_index
            )
        directory.mkdir(parents=True)
        unit = (
            f"aqua-fe-fair-stability-v12-o{planned_ordinal:03d}-a001-"
        ...
        claim = {
            ...
            "ordinal_state_before_submit": (
                {
                    **{
                        **self.ready_state(experiment_root),
                        "cell": {
                            **self.ready_state(experiment_root)["cell"],
            ...
            "resubmit_authorization": authorization,
        ...
        if prior_directory is not None:
            self.make_resubmit_consumption(
                prior_directory, directory, submission_index
            )
        submit = {
        ...
        return directory, self.rewrite_chain_as_terminal_result(directory)
    ...
    def finalize_chain(
        ...
        execution = json.loads(
            (directory / "systemd_execution_receipt.json").read_text(
                encoding="utf-8"
            )
        )
        controller_returncode = execution["controller_returncode"]
        actual_outcome = execution["controller_result"]["outcome"]
        self.assertEqual(controller_outcome, actual_outcome)
        before_state = execution["ordinal_state_after_controller"]
        adoption_claim = {
            "schema_version": SYSTEMD.ADOPTION_CLAIM_SCHEMA,
            "experiment_id": ORDINAL.EXPERIMENT_ID,
            "created_at_utc": "2026-08-30T00:00:00+00:00",
            "unit": claim["unit"],
            "planned_ordinal": claim["planned_ordinal"],
            "attempt_index": claim["attempt_index"],
            "attempt_root": claim["attempt_root"],
            "ordinal_state_before_adoption": before_state,
            "controller_returncode": controller_returncode,
            "controller_outcome": actual_outcome,
        ...
        if actual_outcome == ORDINAL.TERMINAL_SYSTEMD_OUTCOME:
            ordinal_path = (
                experiment_root
                / "ordinal_receipts"
                / f"ordinal_{int(claim['planned_ordinal']):03d}_terminal.json"
            )
            ordinal_path.parent.mkdir(parents=True, exist_ok=True)
            ordinal_path.write_bytes(
                ORDINAL.canonical_json(
                    {
                        "schema_version": (
                            "aqua-fe-fair-stability-ordinal-terminal-v12"
                        ),
                        "experiment_id": ORDINAL.EXPERIMENT_ID,
                        "planned_ordinal": claim["planned_ordinal"],
                        "accepted_attempt_index": claim["attempt_index"],
                        "run_result": ORDINAL.identity(
                            Path(claim["attempt_root"])
                            / "run_result.json"
                        ),
                        "systemd_adoption_claim": ORDINAL.identity(
                            adoption_claim_path
                        ),
                    }
                )
            )
            ordinal_terminal_receipt = ORDINAL.identity(ordinal_path)
            if int(claim["planned_ordinal"]) == 120:
                after_state = {"state": "COMPLETE", "completed": 120}
            else:
                after_state = {
                    "state": "READY",
                    "cell": {
                        "ordinal": int(claim["planned_ordinal"]) + 1
                    },
                    "attempt_index": 1,
                    "attempt_root": str(
                        experiment_root
                        / (
                            "attempt_ordinal_"
                            f"{int(claim['planned_ordinal']) + 1:03d}"
                        )
                    ),
                }
        else:
        ...
        final = {
            ...
            "experiment_id": ORDINAL.EXPERIMENT_ID,
            "adopted_at_utc": "2026-08-30T00:00:00+00:00",
            "unit": claim["unit"],
            "controller_returncode": controller_returncode,
            "controller_outcome": actual_outcome,
            **records,
            "systemd_adoption_claim": ORDINAL.identity(adoption_claim_path),
            "ordinal_terminal_receipt": ordinal_terminal_receipt,
            "ordinal_state_before_final_adoption_receipt": after_state,
            "estimator_retry_authorized": False,
            "next_submission_requires_new_submit": (
                actual_outcome in ORDINAL.NO_START_OUTCOMES
            ),
        }
    ...
    def refresh_chain_records(
        self, directory: Path
    ) -> dict[str, dict[str, object]]:
        execution_path = directory / "systemd_execution_receipt.json"
        terminal_path = directory / "systemd_terminal_receipt.json"
        terminal = json.loads(terminal_path.read_text(encoding="utf-8"))
        terminal["systemd_execution_receipt"] = ORDINAL.identity(execution_path)
        terminal_path.write_bytes(ORDINAL.canonical_json(terminal))
        return {
            key: ORDINAL.identity(directory / filename)
            for key, (filename, _schema) in ORDINAL.SYSTEMD_CHAIN_FILES.items()
        }

    def rewrite_chain_as_no_start(
        self,
        experiment_root: Path,
        directory: Path,
        outcome: str,
        ...
        execution = json.loads(execution_path.read_text(encoding="utf-8"))
        attempt = Path(claim["attempt_root"])
        attempt.mkdir(exist_ok=True)
        manifest = self.check_attempt_manifest(
            claim["ordinal_state_before_submit"]
        )
        for candidate in (
            attempt / "start_claim.json",
            attempt / "run_result.json",
        ):
            if candidate.exists() or candidate.is_symlink():
                candidate.unlink()
        prior_controller_result = execution.get("controller_result")
        generated_dispatch = (
            prior_controller_result.get("dispatch_execution")
            if isinstance(prior_controller_result, dict)
            else None
        )
        generated_path = Path(
            str(
                generated_dispatch.get("path", "")
                if isinstance(generated_dispatch, dict)
                else ""
            )
        )
        if generated_path.parent == attempt and generated_path.exists():
            match = re.fullmatch(
            ...
            if match is not None:
                for suffix in ("stdout.log", "stderr.log"):
                    log_path = (
                        attempt
                        / f"controller_dispatch_{match.group(1)}.{suffix}"
                    )
                    if log_path.exists() or log_path.is_symlink():
                        log_path.unlink()
        if not list(attempt.glob("controller_dispatch_execution_*.json")):
            dispatch_claim_path = attempt / "ordinal_dispatch_claim.json"
            if dispatch_claim_path.exists() or dispatch_claim_path.is_symlink():
                dispatch_claim_path.unlink()
        post_state = {
            "state": "READY",
            "attempt_root": str(attempt),
            "attempt_index": claim["attempt_index"],
        ...
        resource_gate = None
        resource_check_returncode = None
        resource_check_ready = None
        if outcome == "RESOURCE_BLOCKED_NO_START":
            resource_gate = self.blocked_resource_gate(
                int(manifest["port"])
            )
            ...
            resource_check_returncode = 2
            resource_check_ready = False
        elif outcome == "PRESTART_DISPATCH_BLOCKED_NO_ESTIMATOR_START":
            dispatch_claim_path = attempt / "ordinal_dispatch_claim.json"
            if not dispatch_claim_path.exists():
                dispatch_claim_path.write_text("{}\n", encoding="ascii")
            existing = sorted(
                attempt.glob("controller_dispatch_execution_*.json")
            )
            index = len(existing) + 1
            dispatch_path = (
                attempt / f"controller_dispatch_execution_{index:03d}.json"
            )
            stdout_path = (
                attempt / f"controller_dispatch_{index:03d}.stdout.log"
            )
            stderr_path = (
                attempt / f"controller_dispatch_{index:03d}.stderr.log"
            )
            stdout_path.write_text("", encoding="utf-8")
            stderr_path.write_text("prestart blocked\n", encoding="utf-8")
            dispatch = {
                "schema_version": ORDINAL.CONTROLLER_DISPATCH_EXECUTION_SCHEMA,
                "experiment_id": ORDINAL.EXPERIMENT_ID,
                "dispatch_index": index,
                "returncode": 2,
                "stdout": ORDINAL.identity(stdout_path),
                "stderr": ORDINAL.identity(stderr_path),
                "dispatch_claim": ORDINAL.identity(dispatch_claim_path),
                "controller": ORDINAL.identity(
                    SCRIPTS / "run_fair_stability_next_v12.py"
                ),
                "run_result": None,
                "run_result_status": None,
                "expected_runner_returncode_from_result_status": None,
                "runner_result_returncode_binding_valid": None,
                "run_result_parse_error": None,
            }
            dispatch_path.write_bytes(ORDINAL.canonical_json(dispatch))
            dispatch_record = ORDINAL.identity(dispatch_path)
            controller_result["dispatch_execution"] = dispatch_record
            controller_result["runner_stderr_tail"] = "prestart blocked"
        else:
            raise AssertionError(outcome)
        absent_start = {
            "path": str(attempt / "start_claim.json"),
            "exists": False,
            "lexists": False,
        ...
        absent_result = {
            "path": str(attempt / "run_result.json"),
            "exists": False,
            "lexists": False,
            "is_regular_file": False,
            "is_symlink": False,
        ...
        execution.update(
            {
                "controller_returncode": 75,
                "controller_result": controller_result,
                "ordinal_state_after_controller": post_state,
                "no_start_provenance": {
                    "schema_version": ORDINAL.NO_START_PROVENANCE_SCHEMA,
                    "experiment_id": ORDINAL.EXPERIMENT_ID,
                    "recorded_at_utc": "2026-08-30T00:00:00+00:00",
                    "unit": claim["unit"],
                    "submission_claim": ORDINAL.identity(claim_path),
                    "systemd_start_receipt": ORDINAL.identity(start_path),
                    "attempt_root": str(attempt),
                    "ordinal_state_before_controller": claim[
                        "ordinal_state_before_submit"
                    ],
                    "ordinal_state_after_controller": post_state,
                    "controller_outcome": outcome,
                    "controller_returncode": 75,
                    "estimator_start_claim_observation": absent_start,
                    "estimator_run_result_observation": absent_result,
                    "estimator_start_authority_present": False,
                    "dispatch_execution": dispatch_record,
                    "resource_gate": resource_gate,
        ...
        terminal.update(
            {
                "service_result_environment": "exit-code",
                "exit_status_environment": "75",
                "observed_tuple": {
                    "Result": "exit-code",
                    "ExecMainCode": "1",
                    "ExecMainStatus": "75",
        ...
        terminal_path.write_bytes(ORDINAL.canonical_json(terminal))
        return self.refresh_chain_records(directory)

    def rewrite_chain_as_terminal_result(
        self,
        directory: Path,
    ) -> dict[str, dict[str, object]]:
        claim_path = directory / "submission_claim.json"
        ...
        for candidate in sorted(
            ...
            if candidate_value.get("run_result") is not None:
                candidate.unlink()
        existing = sorted(attempt.glob("controller_dispatch_execution_*.json"))
        index = len(existing) + 1
        dispatch_path = attempt / f"controller_dispatch_execution_{index:03d}.json"
        stdout_path = attempt / f"controller_dispatch_{index:03d}.stdout.log"
        stderr_path = attempt / f"controller_dispatch_{index:03d}.stderr.log"
        stdout_path.write_text("terminal\n", encoding="utf-8")
        ...
        execution.update(
            {
                ...
                "controller_result": {
                    ...
                    "state": post_state,
                    "runner_returncode": 0,
                    "dispatch_execution": ORDINAL.identity(dispatch_path),
                    "systemd_authority": execution["controller_result"][
                        "systemd_authority"
                    ],
                },
                "ordinal_state_after_controller": post_state,
    ...
    def test_service_entry_no_start_builder_covers_both_outcomes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            self.check_attempt_manifest(claim["ordinal_state_before_submit"])
            claim_path.write_text("{}\n", encoding="ascii")
            start_path.write_text("{}\n", encoding="ascii")
            post = dict(claim["ordinal_state_before_submit"])
            resource_gate = self.blocked_resource_gate(26100)
            resource = SYSTEMD._no_start_provenance(
                unit="unit.service",
                directory=directory,
                claim=claim,
            ...
            self.assertEqual(
                resource["schema_version"], SYSTEMD.NO_START_PROVENANCE_SCHEMA
            )
            self.assertIs(resource["resource_check_ready"], False)
            self.assertIsNone(resource["dispatch_execution"])
            prestart = SYSTEMD._no_start_provenance(
                unit="unit.service",
                directory=directory,
                claim=claim,
                start_path=start_path,
                controller_result={
                    ...
                    "dispatch_execution": {
                        "path": str(attempt / "dispatch.json"),
                        "size_bytes": 1,
                        "sha256": "x",
                    },
                },
            ...
            self.assertIsNone(prestart["resource_gate"])
            self.assertIsNotNone(prestart["dispatch_execution"])

    def test_resource_no_start_remains_valid_after_later_terminal_history(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            first, _ = self.build_chain(root, submission_index=1)
            first_records = self.rewrite_chain_as_no_start(
                root, first, "RESOURCE_BLOCKED_NO_START"
            )
            ORDINAL.validate_systemd_chain(root, first_records, first)
            self.finalize_chain(
                root, first, first_records, "RESOURCE_BLOCKED_NO_START"
            )

            second, _ = self.build_chain(root, submission_index=2)
            second_records = self.rewrite_chain_as_terminal_result(second)
            ORDINAL.validate_systemd_chain(root, second_records, second)
            self.finalize_chain(root, second, second_records)

            # The later start/result in the same prepared attempt must not
            # reinterpret the first submission's immutable absence snapshot.
            ORDINAL.validate_systemd_chain(root, first_records, first)
            with mock.patch.object(
                SYSTEMD, "EXPERIMENT_ROOT", root
            ), mock.patch.object(
                SYSTEMD, "SUPERVISION_ROOT", root / "systemd_supervision"
            ):
                self.assertEqual(SYSTEMD.unadopted_submissions(), [])
            ...
            def fake_inspect(_root, cell):
                return (
                    ...
                    if cell["ordinal"] == 1
                        ...
                        "cell": cell,
                        "attempt_index": 1,
                        "attempt_root": str(root / "attempt_ordinal_002"),
                        "invalid_chain": [],
                    }
                )

            with mock.patch.object(
                ORDINAL, "load_attempt_matrix"
            ), mock.patch.object(
                ORDINAL, "load_schedule", return_value=[first_cell, second_cell]
            ), mock.patch.object(
                ORDINAL, "inspect_cell", side_effect=fake_inspect
            ):
                next_value = ORDINAL.next_state(root)
            self.assertEqual(next_value["state"], "READY")
            self.assertEqual(next_value["cell"]["ordinal"], 2)

    def test_two_no_starts_then_third_terminal_all_revalidate(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            histories = []
            for index in (1, 2):
                directory, _ = self.build_chain(root, submission_index=index)
                records = self.rewrite_chain_as_no_start(
                    root, directory, "RESOURCE_BLOCKED_NO_START"
                )
                ...
                histories.append((directory, records))
            third, _ = self.build_chain(root, submission_index=3)
            third_records = self.rewrite_chain_as_terminal_result(third)
            ORDINAL.validate_systemd_chain(root, third_records, third)
            self.finalize_chain(root, third, third_records)
            for directory, records in histories:
                ORDINAL.validate_systemd_chain(root, records, directory)

    def test_prestart_no_start_binds_its_dispatch_then_later_result(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            first, _ = self.build_chain(root, submission_index=1)
            first_records = self.rewrite_chain_as_no_start(
            ...
            executions = sorted(
                (root / "attempt").glob(
                    "controller_dispatch_execution_*.json"
                )
            )
            self.assertEqual(
                [path.name for path in executions],
                [
                    "controller_dispatch_execution_001.json",
                    "controller_dispatch_execution_002.json",
                ],
            )

    def test_no_start_provenance_field_tampering_fails_closed(self) -> None:
        mutations = {
            "schema": ("schema_version", "forged"),
            "experiment": ("experiment_id", "forged"),
            "timestamp": ("recorded_at_utc", "not-utc"),
            "unit": ("unit", "forged.service"),
            "claim": ("submission_claim", None),
            "start_receipt": ("systemd_start_receipt", None),
            "attempt_root": ("attempt_root", "/tmp/forged"),
            "pre_state": ("ordinal_state_before_controller", {"state": "TERMINAL"}),
            "post_state": ("ordinal_state_after_controller", {"state": "TERMINAL"}),
            "outcome": ("controller_outcome", "TERMINAL_RESULT_PENDING_SYSTEMD_ADOPTION"),
            "returncode": ("controller_returncode", 0),
            "start_observation": ("estimator_start_claim_observation", None),
            "result_observation": ("estimator_run_result_observation", None),
            "start_authority": ("estimator_start_authority_present", True),
            "dispatch": ("dispatch_execution", {"path": "forged"}),
            "resource_gate": ("resource_gate", {"ready": True}),
            "resource_rc": ("resource_check_returncode", 0),
            "resource_ready": ("resource_check_ready", True),
        }
        for label, (field, forged) in mutations.items():
            with self.subTest(label=label), tempfile.TemporaryDirectory() as temporary:
                root = Path(temporary)
                directory, _ = self.build_chain(root)
                self.rewrite_chain_as_no_start(
                    root, directory, "RESOURCE_BLOCKED_NO_START"
                )
                execution_path = directory / "systemd_execution_receipt.json"
                execution = json.loads(
                ...
                execution["no_start_provenance"][field] = forged
                execution_path.write_bytes(ORDINAL.canonical_json(execution))
                records = self.refresh_chain_records(directory)
                with self.assertRaisesRegex(
                    ORDINAL.OrdinalError, "SYSTEMD_NO_START"
                ):
                    ORDINAL.validate_systemd_chain(root, records, directory)

    def test_no_start_missing_none_terminal_mix_and_dispatch_forgery_rejected(self) -> None:
        for mutation in ("missing", "none", "terminal_post", "terminal_mix"):
            with self.subTest(mutation=mutation), tempfile.TemporaryDirectory() as temporary:
                ...
                if mutation == "missing":
                    del execution["no_start_provenance"]
                elif mutation == "none":
                    execution["no_start_provenance"] = None
                elif mutation == "terminal_post":
                    terminal_state = {
                        "state": "TERMINAL",
                        "attempt_root": str(root / "attempt"),
                        "attempt_index": 1,
                    }
                    execution["ordinal_state_after_controller"] = terminal_state
                    execution["no_start_provenance"][
                        "ordinal_state_after_controller"
                    ] = terminal_state
                else:
                    execution["controller_result"]["outcome"] = (
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            execution = json.loads(execution_path.read_text(encoding="utf-8"))
            dispatch_path = Path(
                execution["controller_result"]["dispatch_execution"]["path"]
            )
            dispatch = json.loads(dispatch_path.read_text(encoding="utf-8"))
            dispatch["run_result"] = {"path": "forged"}
            dispatch_path.write_bytes(ORDINAL.canonical_json(dispatch))
            forged_identity = ORDINAL.identity(dispatch_path)
            execution["controller_result"]["dispatch_execution"] = forged_identity
            execution["no_start_provenance"]["dispatch_execution"] = forged_identity
            execution_path.write_bytes(ORDINAL.canonical_json(execution))
            records = self.refresh_chain_records(directory)
            with self.assertRaisesRegex(
                ORDINAL.OrdinalError,
                "SYSTEMD_NO_START_DISPATCH_RECEIPT_INVALID",
            ):
                ORDINAL.validate_systemd_chain(root, records, directory)
    ...
    def test_systemd_unit_contract_and_submit_only_argv(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            unit = "aqua-fe-fair-stability-v12-o001-a001-s001-aabbcc.service"
    ...
    def test_systemd_user_control_plane_uses_fixed_bus_under_empty_ambient_env(
        ...
        with mock.patch.dict(os.environ, {}, clear=True), mock.patch.object(
            ...
            observed = SYSTEMD.query_unit("aqua-fe-fair-stability-v12-test.service")
        self.assertEqual(
    ...
    def test_pipeline_invalid_second_submission_is_rejected_zero_retry(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            first_execution["ordinal_state_after_controller"] = pipeline_state
            ...
            pipeline_finalized = ORDINAL.validate_finalized_systemd_adoption(
                root, first_directory
            )
            self.assertIs(
                pipeline_finalized["receipt"][
                    "next_submission_requires_new_submit"
                ],
                False,
            )
            ...
            invalid_receipt = {
                "schema_version": (
                    "aqua-fe-fair-stability-pipeline-invalid-adjudication-v12"
                ),
                "experiment_id": ORDINAL.EXPERIMENT_ID,
            ...
            with self.assertRaisesRegex(
                ...
                (
                    "PIPELINE_ADJUDICATION_DISABLED_ZERO_REPLACEMENT_V12|"
                    "SYSTEMD_RESUBMIT_AUTHORIZATION_BINDING_INVALID"
                ),
            ):
                ORDINAL.validate_systemd_chain(
    ...
    def test_orphaned_later_result_and_deleted_history_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            first, _ = self.build_chain(root)
            first_records = self.rewrite_chain_as_no_start(
                root, first, "RESOURCE_BLOCKED_NO_START"
            )
            self.finalize_chain(
            ...
            # Keep the shared attempt's full start/result/dispatch evidence,
            # but delete the strictly-later accepted/finalized submission.
            shutil.rmtree(second)
            with self.assertRaisesRegex(
                ORDINAL.OrdinalError,
                "(?:SYSTEMD_NO_START_(?:DISPATCH_ATTRIBUTION_INVALID|"
                "CURRENT_ABSENCE_VIOLATED)|"
                "SYSTEMD_RESUBMIT_CONSUMPTION_SEQUENCE_INVALID)",
            ):
                ORDINAL.validate_systemd_chain(root, first_records, first)
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            second, _ = self.build_chain(root, submission_index=2)
            second_records = self.rewrite_chain_as_no_start(
                root, second, "RESOURCE_BLOCKED_NO_START"
            )
            self.finalize_chain(
                root, second, second_records, "RESOURCE_BLOCKED_NO_START"
            )
            third, third_records = self.build_chain(
                root, submission_index=3
            )
            self.finalize_chain(root, third, third_records)
            ORDINAL.validate_systemd_chain(root, third_records, third)
            (second / "systemd_terminal_receipt.json").unlink()
            with self.assertRaises(ORDINAL.OrdinalError):
                ORDINAL.validate_systemd_chain(root, third_records, third)

    def test_terminal_outcome_requires_result_dispatch_state_and_ordinal(self) -> None:
        for missing in (
            "result",
            "dispatch",
            "state",
            "state_result",
            "bool_controller_returncode",
        ):
            with self.subTest(missing=missing), tempfile.TemporaryDirectory() as temporary:
                root = Path(temporary)
                directory, records = self.build_chain(root)
                if missing == "result":
                    (root / "attempt" / "run_result.json").unlink()
                else:
                    execution_path = (
                        directory / "systemd_execution_receipt.json"
                    )
                    execution = json.loads(
                        execution_path.read_text(encoding="utf-8")
                    )
                    if missing == "dispatch":
                        del execution["controller_result"][
                            "dispatch_execution"
                        ]
                    elif missing == "state":
                        del execution["controller_result"]["state"]
                    elif missing == "state_result":
                        del execution["controller_result"]["state"][
                            "result"
                        ]
                        del execution["ordinal_state_after_controller"][
                            "result"
                        ]
                    else:
                        execution["controller_returncode"] = False
                    execution_path.write_bytes(
                        ORDINAL.canonical_json(execution)
                    )
                    records = self.refresh_chain_records(directory)
                with self.assertRaises(ORDINAL.OrdinalError):
                    ORDINAL.validate_systemd_chain(root, records, directory)
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            directory, records = self.build_chain(root)
            self.finalize_chain(root, directory, records)
            final_path = directory / "systemd_adoption_receipt.json"
            final = json.loads(final_path.read_text(encoding="utf-8"))
            final["ordinal_terminal_receipt"] = None
            final_path.write_bytes(ORDINAL.canonical_json(final))
            with mock.patch.object(
                SYSTEMD, "EXPERIMENT_ROOT", root
            ), mock.patch.object(
                SYSTEMD,
                "SUPERVISION_ROOT",
                root / "systemd_supervision",
            ), self.assertRaisesRegex(
                SYSTEMD.SystemdSupervisorError,
                "TERMINAL_ADOPTION_CLOSURE_INVALID",
            ):
                SYSTEMD.validate_final_adoption(directory)
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            final_path = directory / "systemd_adoption_receipt.json"
            final = json.loads(final_path.read_text(encoding="utf-8"))
            original = Path(final["ordinal_terminal_receipt"]["path"])
            forged = original.with_name("ordinal_999_terminal.json")
            original.rename(forged)
            final["ordinal_terminal_receipt"] = ORDINAL.identity(forged)
            final_path.write_bytes(ORDINAL.canonical_json(final))
            with self.assertRaisesRegex(
                ORDINAL.OrdinalError,
                "SYSTEMD_ORDINAL_TERMINAL_IDENTITY_DRIFT",
            ):
                ORDINAL.validate_finalized_systemd_adoption(root, directory)
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            final_path = directory / "systemd_adoption_receipt.json"
            final = json.loads(final_path.read_text(encoding="utf-8"))
            final["ordinal_state_before_final_adoption_receipt"] = {
                "state": "READY",
                "cell": {"ordinal": 2, "case_id": "wrong"},
                "attempt_index": 1,
                "attempt_root": "/outside/experiment",
            }
            final_path.write_bytes(ORDINAL.canonical_json(final))
            with self.assertRaisesRegex(
                ORDINAL.OrdinalError,
                "SYSTEMD_TERMINAL_ADOPTION_CLOSURE_INVALID",
            ):
                ORDINAL.validate_finalized_systemd_adoption(root, directory)
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            terminal["service_result_environment"] = "exit-code"
            terminal["observed_tuple"]["Result"] = "exit-code"
            terminal_path.write_bytes(ORDINAL.canonical_json(terminal))
            records["systemd_terminal_receipt"] = ORDINAL.identity(
                terminal_path
            )
            with self.assertRaisesRegex(
                ORDINAL.OrdinalError,
                "SYSTEMD_CHAIN_TERMINAL_TUPLE_INVALID",
            ):
                ORDINAL.validate_systemd_chain(root, records, directory)

        for field, forged_value in (
            ("estimator_retry_authorized", True),
            ("next_submission_requires_new_submit", True),
            ("adopted_at_utc", "not-a-time"),
        ):
            with self.subTest(
                final_semantic_field=field
                ...
                final[field] = forged_value
                final_path.write_bytes(ORDINAL.canonical_json(final))
                with self.assertRaisesRegex(
                    ORDINAL.OrdinalError,
                    "SYSTEMD_FINAL_ADOPTION_BINDING_INVALID",
                ):
                    ORDINAL.validate_finalized_systemd_adoption(
                        root, directory
                    )

    def test_no_start_adoption_never_calls_terminal_adoption(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            directory, _ = self.build_chain(root)
            self.rewrite_chain_as_no_start(
            ...
            state = execution["ordinal_state_after_controller"]
            import run_fair_stability_next_v12 as canonical_next
            ...
            with mock.patch.object(
                ...
                canonical_next, "adopt_terminal"
            ) as terminal_adoption:
                receipt = SYSTEMD.adopt(directory)
            terminal_adoption.assert_not_called()
            self.assertIsNone(receipt["ordinal_terminal_receipt"])
            self.assertEqual(receipt["controller_returncode"], 75)
            self.assertIs(
                receipt["next_submission_requires_new_submit"], True
            )
            self.assertIs(receipt["estimator_retry_authorized"], False)
            final_path = directory / "systemd_adoption_receipt.json"
            forged = json.loads(final_path.read_text(encoding="utf-8"))
            forged["next_submission_requires_new_submit"] = False
            final_path.chmod(0o644)
            final_path.write_bytes(ORDINAL.canonical_json(forged))
            with self.assertRaisesRegex(
                ORDINAL.OrdinalError,
                "SYSTEMD_FINAL_ADOPTION_BINDING_INVALID",
            ):
    ...
    def test_authorize_resubmit_orders_reset_gate_before_systemd_run(self) -> None:
        with mock.patch("sys.stderr"), self.assertRaises(SystemExit):
            SYSTEMD.parse_args(["authorize-resubmit"])
        parsed = SYSTEMD.parse_args(
            ["authorize-resubmit", "--submission-dir", "/tmp/prior"]
        )
        self.assertEqual(parsed.submission_dir, Path("/tmp/prior"))
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            first, _ = self.build_chain(root)
            first_records = self.rewrite_chain_as_no_start(
            ...
            claim = json.loads(
                (first / "submission_claim.json").read_text(
                    encoding="utf-8"
                )
            )
            state = self.ready_state(root)
            events: list[str] = []
            observations = [
                self.stopped_observation(
                    claim["unit"], after_reset=False
                ),
                self.stopped_observation(
                    claim["unit"], after_reset=True
                ),
                {"returncode": 0, "properties": {}},
            ]

            def query(_unit):
                index = len([event for event in events if event.startswith("query")])
                events.append(
                    ("query-pre", "query-post", "query-submit")[index]
                )
                observation = dict(observations[index])
                observation["queried_at_utc"] = SYSTEMD.now_utc()
                return observation
            ...
            def command(argv, **_kwargs):
                ...
                if list(argv) == SYSTEMD._runner_check_argv(state):
                    events.append("resource-check")
                    gate["checked_at_utc"] = SYSTEMD.now_utc()
                    return subprocess.CompletedProcess(
                        argv,
                        0,
                        json.dumps(
                ...
                self.assertEqual(argv[0], str(SYSTEMD.SYSTEMD_RUN))
                consumption_path = SYSTEMD._authorization_consumption_path(
                    first, 2
                )
                second_directory = (
                    root
                    / "systemd_supervision/ordinal_001/submission_002"
                )
                self.assertTrue(consumption_path.is_file())
                self.assertTrue(
                    (second_directory / "submission_claim.json").is_file()
                )
                SYSTEMD.validate_resubmit_authorization_consumption(
                    root, first, second_directory, 2
                )
                events.append("consumption-before-systemd-run")
                events.append("systemd-run")
            ...
            with mock.patch.object(
                ...
                SYSTEMD, "query_unit", side_effect=query
            ), mock.patch.object(
                SYSTEMD, "run_checked", side_effect=command
            ), mock.patch.object(
                SYSTEMD,
                "host_preflight",
                ...
                authorization = SYSTEMD.authorize_resubmit(first)
                submit_receipt = SYSTEMD.submit()
            self.assertEqual(
                events,
                [
                    "query-pre",
                    ...
                    "query-post",
                    "resource-check",
                    "consumption-before-systemd-run",
                    "systemd-run",
                    "query-submit",
                ],
            )
            self.assertTrue(submit_receipt["accepted"])
            ...
            self.assertEqual(
                second_claim["resubmit_authorization"],
                ORDINAL.identity(
                    SYSTEMD._authorization_path(first, 2)
                ),
            )
            self.assertEqual(
                authorization["resource_check"]["sequence_index"], 2
            )

    def test_resubmit_consumption_blocks_deleted_successor_rollback(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            marker = SYSTEMD._authorization_consumption_path(first, 2)
            ...
            with mock.patch.object(
                ...
                SYSTEMD.SystemdSupervisorError,
                "RESUBMIT_AUTHORIZATION_ALREADY_CONSUMED",
            ):
                SYSTEMD.validate_resubmit_authorization(
                    first, self.ready_state(root), 2
                )
            with self.assertRaisesRegex(
                ORDINAL.OrdinalError,
                "SYSTEMD_RESUBMIT_CONSUMPTION_SEQUENCE_INVALID",
            ):
                ORDINAL.validate_finalized_systemd_adoption(root, first)
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.make_resubmit_authorization(root, first, 2)
            marker = SYSTEMD._authorization_consumption_path(first, 2)
            marker.symlink_to(marker.with_name("missing-consumption.json"))
            with mock.patch.object(
                SYSTEMD, "EXPERIMENT_ROOT", root
            ), mock.patch.object(
                SYSTEMD,
    ...
    def test_resubmit_consumption_marker_tampering_fails_closed(
        self,
    ) -> None:
        attacks = (
            ("missing", None, None),
            ("missing_field", "new_unit", None),
            ("extra", "extra", True),
            ("index", "new_submission_index", 3),
            ("ordinal", "planned_ordinal", 2),
            ("attempt", "attempt_index", 2),
            ("root", "attempt_root", "/tmp/forged"),
            ("unit", "new_unit", "forged.service"),
            ("claim", "new_submission_claim", None),
            ("authorization", "resubmit_authorization", None),
            ("timestamp", "consumed_at_utc", "not-a-time"),
            ("whitespace", None, None),
            ("key_order", None, None),
            ("symlink", None, None),
        )
        for attack, field, value in attacks:
            with self.subTest(attack=attack), tempfile.TemporaryDirectory() as temporary:
                ...
                second, second_records = self.build_chain(
                    root, submission_index=2
                )
                marker = SYSTEMD._authorization_consumption_path(first, 2)
                if attack == "missing":
                    marker.unlink()
                    ...
                    marker.unlink()
                    marker.symlink_to(marker.with_name("missing-marker.json"))
                    expected = "SYSTEMD_RESUBMIT_CONSUMPTION_FILE_INVALID"
                elif attack in {"whitespace", "key_order"}:
                    ...
                    expected = (
                        "SYSTEMD_RESUBMIT_CONSUMPTION_CANONICAL_BYTES_INVALID"
                    )
                else:
                    ...
                    if attack == "missing_field":
                        ...
                        payload[field] = value
                    marker.write_bytes(ORDINAL.canonical_json(payload))
                    expected = (
                        "SYSTEMD_RESUBMIT_CONSUMPTION_BINDING_INVALID"
                    )
                with self.assertRaisesRegex(ORDINAL.OrdinalError, expected):
                    ORDINAL.validate_systemd_chain(
                        root, second_records, second
                    )

    def test_prepublished_resubmit_consumption_requires_canonical_bytes(
        self,
    ) -> None:
        for attack in ("whitespace", "key_order"):
            with self.subTest(attack=attack), tempfile.TemporaryDirectory() as temporary:
                root = Path(temporary)
                first, _ = self.build_chain(root)
                first_records = self.rewrite_chain_as_no_start(
                ...
                self.build_chain(root, submission_index=2)
                marker = SYSTEMD._authorization_consumption_path(first, 2)
                ...
                with mock.patch.object(
                    ...
                    SYSTEMD.SystemdSupervisorError,
                    "RESUBMIT_AUTHORIZATION_CONSUMPTION_PUBLISH_DRIFT",
                ):
                    SYSTEMD._validate_prepublished_authorization_consumption(
                        marker, expected
                    )

    def test_resubmit_consumption_failures_leave_nonreplayable_evidence(
        self,
    ) -> None:
        for phase in ("claim_publish", "systemd_run"):
            with self.subTest(phase=phase), tempfile.TemporaryDirectory() as temporary:
                ...
                self.make_resubmit_authorization(root, first, 2)
                state = self.ready_state(root)
                original_write = SYSTEMD.write_exclusive
                systemd_run = mock.Mock()

                def write(path, payload, mode=0o440):
                    if (
                        phase == "claim_publish"
                        and path.name == "submission_claim.json"
                        and path.parent.name == "submission_002"
                    ):
                        raise SYSTEMD.SystemdSupervisorError(
                            "INJECTED_CLAIM_PUBLISH_FAILURE"
                        )
                    return original_write(path, payload, mode)

                if phase == "systemd_run":
                    systemd_run.side_effect = RuntimeError(
                        "INJECTED_SYSTEMD_RUN_FAILURE"
                    )
                with mock.patch.object(
                    SYSTEMD, "EXPERIMENT_ROOT", root
                ), mock.patch.object(
                    ...
                    SYSTEMD, "write_exclusive", side_effect=write
                ), mock.patch.object(
                    SYSTEMD, "run_checked", systemd_run
                ), self.assertRaises(Exception):
                    SYSTEMD.submit()
                ...
                self.assertTrue(marker.is_file())
                if phase == "claim_publish":
                    systemd_run.assert_not_called()
                    self.assertFalse(
                        (second / "submission_claim.json").exists()
                    )
                else:
                    systemd_run.assert_called_once()
                    self.assertTrue(
                        (second / "submission_claim.json").is_file()
                    )
                if second.exists():
                    shutil.rmtree(second)
                with mock.patch.object(
                    ...
                    SYSTEMD.validate_resubmit_authorization(first, state, 2)

    def test_submit_without_authorization_and_authorization_replay_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            first, _ = self.build_chain(root)
            first_records = self.rewrite_chain_as_no_start(
            ...
            with mock.patch.object(
                ...
                SYSTEMD, "freeze_context", return_value={}
            ), mock.patch.object(SYSTEMD, "run_checked") as systemd_run:
                with self.assertRaisesRegex(
                    SYSTEMD.SystemdSupervisorError,
                    "JSON_REGULAR_FILE_REQUIRED",
                ):
                    SYSTEMD.submit()
            systemd_run.assert_not_called()
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            original = self.make_resubmit_authorization

            def invalid_timestamp(*args, **kwargs):
                record = original(*args, **kwargs)
                path = Path(str(record["path"]))
                value = json.loads(path.read_text(encoding="utf-8"))
                value["authorized_at_utc"] = "not-a-timestamp"
                path.write_bytes(ORDINAL.canonical_json(value))
                return ORDINAL.identity(path)

            with mock.patch.object(
                self,
                "make_resubmit_authorization",
                side_effect=invalid_timestamp,
            ):
                second, second_records = self.build_chain(
                    root, submission_index=2
                )
            with self.assertRaisesRegex(
                ORDINAL.OrdinalError,
                "SYSTEMD_RESUBMIT_AUTHORIZATION_EVIDENCE_INVALID",
            ):
                ORDINAL.validate_systemd_chain(
                    root, second_records, second
                )

    def test_resubmit_authorization_tampering_fails_closed(self) -> None:
        mutations = {
            "reset_argv": lambda value: value["reset_failed"].update(
                {"argv": ["forged"]}
            ),
            "resource_gate": lambda value: value["resource_check"][
                "resource_gate"
            ].update({"ready": False}),
            "resource_gate_truncated": lambda value: value[
                "resource_check"
            ]["resource_gate"].pop("checked_at_utc"),
            "order": lambda value: value["reset_failed"].update(
                {"started_at_utc": "2026-08-30T00:00:01+00:00"}
            ),
            "post_unit": lambda value: value[
                "stopped_unit_observation_after_reset"
            ]["properties"].update({"MainPID": "42"}),
        }
        for label, mutate in mutations.items():
            with self.subTest(label=label), tempfile.TemporaryDirectory() as temporary:
                root = Path(temporary)
                ...
                self.make_resubmit_authorization(root, first, 2)
                path = SYSTEMD._authorization_path(first, 2)
                value = json.loads(path.read_text(encoding="utf-8"))
                mutate(value)
                path.write_bytes(ORDINAL.canonical_json(value))
                with mock.patch.object(
                    ...
                    "SUPERVISION_ROOT",
                    root / "systemd_supervision",
                ), self.assertRaises(SYSTEMD.SystemdSupervisorError):
                    SYSTEMD.validate_resubmit_authorization(
                        first, self.ready_state(root), 2
                    )

        # Synchronizing the authorization and its check log must not make a
        # schema-truncated, forged ready gate into start authority.
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            first, _ = self.build_chain(root)
            first_records = self.rewrite_chain_as_no_start(
            ...
            path = SYSTEMD._authorization_path(first, 2)
            value = json.loads(path.read_text(encoding="utf-8"))
            forged_gate = self.ready_resource_gate(26100)
            forged_gate["gpu_query"]["stdout"] = "4242, ToDesk, 512\n"
            forged_result = {
                "attempt": self.check_attempt_manifest(
                    self.ready_state(root)
                ),
                "resource_gate": forged_gate,
                "ready": True,
            }
            check_stdout = first / "resubmit_002_check.stdout.log"
            check_stdout.write_bytes(ORDINAL.canonical_json(forged_result))
            value["resource_check"].update(
                {
                    "resource_gate": forged_gate,
                    "runner_result": forged_result,
                    "stdout": ORDINAL.identity(check_stdout),
                }
            )
            path.write_bytes(ORDINAL.canonical_json(value))
    ...
    def test_submission_index_coordinate_and_gap_validation(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            directory, records = self.build_chain(root)
            gap = directory.parent / "submission_003"
            gap.mkdir()
            with self.assertRaisesRegex(
                ORDINAL.OrdinalError,
                "SYSTEMD_SUBMISSION_INDEX_SEQUENCE_INVALID",
            ):
                ORDINAL.validate_systemd_chain(root, records, directory)
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            claim = json.loads(claim_path.read_text(encoding="utf-8"))
            claim["submission_index"] = True
            claim_path.write_bytes(ORDINAL.canonical_json(claim))
            receipt = json.loads(receipt_path.read_text(encoding="utf-8"))
            receipt["submission_claim"] = ORDINAL.identity(claim_path)
            receipt_path.write_bytes(ORDINAL.canonical_json(receipt))
            with self.assertRaisesRegex(
                ORDINAL.OrdinalError,
                "SYSTEMD_SUBMISSION_INDEX_OR_COORDINATE_DRIFT",
            ):
                ORDINAL._validated_submission_sequence(
                    root, directory, claim
                )

    def test_terminal_final_adoption_accepts_next_ordinal_and_last_complete(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            directory, records = self.build_chain(root)
            self.finalize_chain(root, directory, records)
            validated = ORDINAL.validate_finalized_systemd_adoption(
                root, directory
            )
            self.assertEqual(
                validated["receipt"][
            ...
            claim = json.loads(
                (directory / "submission_claim.json").read_text(
                    encoding="utf-8"
                )
            )
            current_cell = claim["ordinal_state_before_submit"]["cell"]
            next_cell = {
                "ordinal": 2,
                "case_id": "next",
                "arm": "pure_klt_vins",
            ...
            expected_followup = {
                ...
                "cell": next_cell,
                "attempt_index": 1,
                "attempt_root": str(root / "next_attempt"),
                "invalid_chain": [],
            }
            with mock.patch.object(
                SYSTEMD,
                "load_schedule",
                return_value=[current_cell, next_cell],
            ), mock.patch.object(
                SYSTEMD,
                "inspect_cell",
                return_value=expected_followup,
            ):
                self.assertEqual(
                    SYSTEMD._expected_terminal_followup_state(claim),
                    expected_followup,
                )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                ...
                {"state": "COMPLETE", "completed": 120},
            )
            last_claim = json.loads(
                (directory / "submission_claim.json").read_text(
                    encoding="utf-8"
                )
            )
            last_cell = last_claim["ordinal_state_before_submit"]["cell"]
            schedule = [
                {"ordinal": index} for index in range(1, 120)
            ] + [last_cell]
            with mock.patch.object(
                SYSTEMD, "load_schedule", return_value=schedule
            ):
                self.assertEqual(
                    SYSTEMD._expected_terminal_followup_state(last_claim),
                    {"state": "COMPLETE", "completed": 120},
                )

        # A later terminal next ordinal must not invalidate the immutable
        # immediate READY snapshot recorded by the previous ordinal.
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            first, first_records = self.build_chain(root)
            self.finalize_chain(root, first, first_records)
            first_final = json.loads(
                (first / "systemd_adoption_receipt.json").read_text(
                    encoding="utf-8"
                )
            )
            frozen_followup = first_final[
                "ordinal_state_before_final_adoption_receipt"
            ]
            second, second_records = self.build_chain(
                root,
                planned_ordinal=2,
                ordinal_state_before_submit=frozen_followup,
                attempt_root_override=Path(
                    str(frozen_followup["attempt_root"])
                ),
            )
            self.finalize_chain(root, second, second_records)
            ORDINAL.validate_finalized_systemd_adoption(root, second)
            ORDINAL.validate_finalized_systemd_adoption(root, first)

    def test_submit_rechecks_pending_history_then_state_inside_lock(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            events: list[str] = []
            state = {"state": "PIPELINE_INVALID_FAIL_STOP_ZERO_RETRY"}

            def pending():
                events.append("pending-history")
                return []

            def reread(_root):
                events.append("next-state")
                return state
            ...
            with mock.patch.object(
                ...
                SYSTEMD, "unadopted_submissions", side_effect=pending
            ), mock.patch.object(
                SYSTEMD, "next_state", side_effect=reread
            ), self.assertRaisesRegex(
                SYSTEMD.SystemdSupervisorError,
                "ORDINAL_STATE_NOT_SUBMITTABLE",
            ):
                SYSTEMD.submit()
            self.assertEqual(events, ["pending-history", "next-state"])

    def test_blocked_resource_gate_is_complete_at_all_three_boundaries(
        self,
    ) -> None:
        def attacked_gates() -> list[tuple[str, dict[str, object]]]:
            baseline = self.blocked_resource_gate(26100)
            minimal = {
                "schema_version": ORDINAL.PRESTART_RESOURCE_GATE_SCHEMA,
                "experiment_id": ORDINAL.EXPERIMENT_ID,
                "ready": False,
            ...
            del deleted["workspace_free_bytes"]
            gpu_stdout = json.loads(json.dumps(baseline))
            gpu_stdout["gpu_query"]["stdout"] = "123, python, 1 MiB\n"
            port = json.loads(json.dumps(baseline))
            port["port_available"] = True
            space = json.loads(json.dumps(baseline))
            space["experiment_free_bytes"] = 5 * 1024**3 - 1
            return [
                ("minimal", minimal),
                ("boolean", boolean),
                ("deleted", deleted),
        ...
        for label, gate in attacked_gates():
            with self.subTest(layer="shared", attack=label):
                self.assertFalse(
                    ORDINAL.valid_prestart_resource_gate_v12(
                        gate,
            ...
            with self.subTest(
                layer="controller_dispatch", attack=label
            ), tempfile.TemporaryDirectory() as temporary:
                root = Path(temporary)
                state = self.ready_state(root)
                manifest = self.check_attempt_manifest(state)
                calls: list[list[str]] = []
                ...
                self.assertFalse(
                    (Path(str(state["attempt_root"]))
                     / "ordinal_dispatch_claim.json").exists()
                )

            with self.subTest(
                layer="supervisor_builder", attack=label
            ), tempfile.TemporaryDirectory() as temporary:
                root = Path(temporary)
                directory = root / "submission"
                directory.mkdir()
                state = self.ready_state(root)
                self.check_attempt_manifest(state)
                claim = {
                ...
                start_path = directory / "systemd_start_receipt.json"
                (directory / "submission_claim.json").write_text(
                    "{}\n", encoding="ascii"
                )
                start_path.write_text("{}\n", encoding="ascii")
            ...
            with self.subTest(
                layer="common_receipt", attack=label
            ), tempfile.TemporaryDirectory() as temporary:
                root = Path(temporary)
                directory, _ = self.build_chain(root)
                records = self.rewrite_chain_as_no_start(
                ...
                execution["controller_result"]["resource_gate"] = gate
                execution["no_start_provenance"]["resource_gate"] = gate
                execution["no_start_provenance"][
                ...
                with self.assertRaises(ORDINAL.OrdinalError):
                    ORDINAL.validate_systemd_chain(
                        root, records, directory
                    )

    def test_no_start_dispatch_artifacts_require_exact_submission_owner(
        self,
    ) -> None:
        def inject_execution(root: Path, index: int) -> None:
            attempt = root / "attempt"
            claim_path = attempt / "ordinal_dispatch_claim.json"
            ...
            stdout_path.write_text("orphan\n", encoding="utf-8")
            stderr_path.write_text("", encoding="utf-8")
            receipt = {
                "schema_version": (
                    ORDINAL.CONTROLLER_DISPATCH_EXECUTION_SCHEMA
                ),
                "experiment_id": ORDINAL.EXPERIMENT_ID,
                "dispatch_index": index,
                "returncode": 2,
                "stdout": ORDINAL.identity(stdout_path),
                "stderr": ORDINAL.identity(stderr_path),
                "dispatch_claim": ORDINAL.identity(claim_path),
                "controller": ORDINAL.identity(
                    SCRIPTS / "run_fair_stability_next_v12.py"
                ),
                "run_result": None,
            ...
            path.write_bytes(ORDINAL.canonical_json(receipt))
        ...
        attacks = ("claim", "log", "execution", "gap", "runtime")
        for attack in attacks:
            with self.subTest(attack=attack), tempfile.TemporaryDirectory() as temporary:
                root = Path(temporary)
                directory, records = make_resource_history(root)
                attempt = root / "attempt"
                if attack == "claim":
                    (attempt / "ordinal_dispatch_claim.json").write_text(
                        "{}\n", encoding="ascii"
                    )
                elif attack == "log":
                    (attempt / "controller_dispatch_001.stdout.log").write_text(
                        "orphan\n", encoding="utf-8"
                    )
                elif attack == "execution":
                    inject_execution(root, 1)
                elif attack == "gap":
                    inject_execution(root, 2)
                else:
        ...
        # An orphan claim blocks both the explicit authorization command and
        # submit before either can invoke reset-failed or systemd-run.
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            directory, _ = make_resource_history(root)
            (root / "attempt" / "ordinal_dispatch_claim.json").write_text(
                "{}\n", encoding="ascii"
            )
            ...
            with mock.patch.object(
                ...
                SYSTEMD, "run_checked", run_checked
            ):
                with self.assertRaises(Exception):
                    SYSTEMD.authorize_resubmit(directory)
                with self.assertRaises(Exception):
                    SYSTEMD.submit()
            run_checked.assert_not_called()

    def test_terminal_revalidates_prior_no_start_dispatch_artifacts(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            (root / "attempt" / "controller_dispatch_001.stdout.log").write_text(
                "tampered prior no-start log\n", encoding="utf-8"
            )
            with self.assertRaisesRegex(
                ORDINAL.OrdinalError,
                "SYSTEMD_NO_START_DISPATCH_ARTIFACT_BINDING_INVALID",
            ):
                ORDINAL.validate_finalized_systemd_adoption(root, second)

    def test_prestart_then_resource_no_start_owns_full_dispatch_history(
        self,
    ) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            build_history(Path(temporary))

        for attack in ("orphan", "tamper"):
            with self.subTest(attack=attack), tempfile.TemporaryDirectory() as temporary:
                root = Path(temporary)
                second = build_history(root)
                attempt = root / "attempt"
                if attack == "tamper":
                    (
                        attempt / "controller_dispatch_001.stdout.log"
                    ).write_text("tampered\n", encoding="utf-8")
                    expected_error = (
                        "SYSTEMD_NO_START_DISPATCH_ARTIFACT_BINDING_INVALID"
                    )
                else:
                    stdout = attempt / "controller_dispatch_002.stdout.log"
                    stderr = attempt / "controller_dispatch_002.stderr.log"
                    stdout.write_text("orphan\n", encoding="utf-8")
                    stderr.write_text("", encoding="utf-8")
                    execution = json.loads(
                        (
                            attempt
                    ...
                    execution.update(
                        {
                            "dispatch_index": 2,
                            "stdout": ORDINAL.identity(stdout),
                            "stderr": ORDINAL.identity(stderr),
                        }
                    )
                    (
                        attempt / "controller_dispatch_execution_002.json"
                    ).write_bytes(ORDINAL.canonical_json(execution))
                    expected_error = (
                        "SYSTEMD_NO_START_DISPATCH_ATTRIBUTION_INVALID"
                    )
                with self.assertRaisesRegex(
                    ORDINAL.OrdinalError, expected_error
                ):
                    ORDINAL.validate_finalized_systemd_adoption(root, second)

    def test_nested_authorization_tamper_prevents_systemd_run(self) -> None:
        mutations = (
            ("reset_extra", "reset_failed", "extra", True),
            ("reset_missing", "reset_failed", "stderr", None),
            ("resource_extra", "resource_check", "extra", True),
            ("resource_missing", "resource_check", "runner_result", None),
        )
        for label, section, field, value in mutations:
            with self.subTest(attack=label), tempfile.TemporaryDirectory() as temporary:
                root = Path(temporary)
                first, _ = self.build_chain(root)
                first_records = self.rewrite_chain_as_no_start(
                    root, first, "RESOURCE_BLOCKED_NO_START"
                ...
                authorization_record = self.make_resubmit_authorization(
                    root, first, 2
                )
                authorization_path = Path(
                    str(authorization_record["path"])
                )
                authorization = json.loads(
                    authorization_path.read_text(encoding="utf-8")
                )
                if "missing" in label:
                    del authorization[section][field]
                else:
                    authorization[section][field] = value
                authorization_path.write_bytes(
                    ORDINAL.canonical_json(authorization)
                )
                run_checked = mock.Mock()
                with mock.patch.object(
                    SYSTEMD, "EXPERIMENT_ROOT", root
                    ...
                    SYSTEMD, "next_state", return_value=self.ready_state(root)
                ), mock.patch.object(
                    SYSTEMD, "host_preflight", return_value={"ready": True}
                ), mock.patch.object(
                    SYSTEMD, "freeze_context", return_value={}
                ), mock.patch.object(
                    SYSTEMD, "run_checked", run_checked
                    ...
                    SYSTEMD.SystemdSupervisorError,
                    "RESUBMIT_AUTHORIZATION_BINDING_INVALID",
                ):
                    SYSTEMD.submit()
                run_checked.assert_not_called()
...
class MainStateAdoptionIntegrationTests(unittest.TestCase):
    """Exercise finalized systemd closure through the public state path."""

    def cell(self, ordinal: int) -> dict[str, object]:
        return {
            "ordinal": ordinal,
    ...
    def state(
        self, root: Path, cell: Mapping[str, object]
    ) -> dict[str, object]:
        return {
            "state": "READY",
            "cell": dict(cell),
            "attempt_index": 1,
            "attempt_root": str(ORDINAL.attempt_root(root, cell, 1)),
            "invalid_chain": [],
        }

    def boundary(self, root: Path) -> dict[str, dict[str, object]]:
        freeze_path = (
            root
            / "vins_dev_nativeq_schedfix_runtimeexcl_v12"
            / "backend_freeze.json"
        ...
        return {
            "common": ORDINAL.identity(
                SCRIPTS / "fair_stability_ordinal_common_v12.py"
            ),
            "controller": ORDINAL.identity(
                SCRIPTS / "run_fair_stability_next_v12.py"
            ),
            "supervisor": ORDINAL.identity(
                SCRIPTS / "run_fair_stability_systemd_supervisor_v12.py"
            ),
            "control_freeze": ORDINAL.identity(freeze_path),
            "attempt_matrix_freeze": ORDINAL.identity(matrix_path),
        }
    ...
    def build_terminal(
        ...
        helper = SystemdControlTests()
        state = self.state(root, cell)
        attempt = Path(str(state["attempt_root"]))
        attempt.parent.mkdir(parents=True, exist_ok=True)
        directory, _ = helper.build_chain(
            root,
            planned_ordinal=int(cell["ordinal"]),
            ordinal_state_before_submit=dict(state),
            attempt_root_override=attempt,
        )
        ...
        manifest_path.write_bytes(
            ORDINAL.canonical_json(
                {
                    "schema_version": (
                        "aqua-fe-fair-stability-vins-attempt-v12"
                    ),
                    "experiment_id": ORDINAL.EXPERIMENT_ID,
                    "case_id": cell["case_id"],
                    "arm": cell["arm"],
        ...
        result.update(
            {
                "schema_version": (
                    "aqua-fe-fair-stability-vins-result-v12"
                ),
                "experiment_id": ORDINAL.EXPERIMENT_ID,
                "case_id": cell["case_id"],
                "arm": cell["arm"],
        ...
        result_path.write_bytes(ORDINAL.canonical_json(result))
        dispatch_path = sorted(
            attempt.glob("controller_dispatch_execution_*.json")
        )[-1]
        dispatch = json.loads(dispatch_path.read_text(encoding="utf-8"))
        dispatch["run_result"] = ORDINAL.identity(result_path)
        dispatch_path.write_bytes(ORDINAL.canonical_json(dispatch))
        execution_path = directory / "systemd_execution_receipt.json"
        execution = json.loads(
            execution_path.read_text(encoding="utf-8")
        ...
        execution["controller_result"]["state"] = post_state
        execution["controller_result"]["dispatch_execution"] = (
            ORDINAL.identity(dispatch_path)
        )
        execution["ordinal_state_after_controller"] = post_state
        execution_path.write_bytes(ORDINAL.canonical_json(execution))
        records = helper.refresh_chain_records(directory)
        helper.finalize_chain(root, directory, records)

        runtime_path = attempt / "runtime_resource_monitor.json"
        runtime_path.write_text("runtime\n", encoding="ascii")
        semantic_path = attempt / "semantic_evidence.txt"
        semantic_path.write_text("semantic\n", encoding="ascii")
        semantic = {
            "synthetic": {
                "state": "FILE",
                **ORDINAL.identity(semantic_path),
            }
        }
        boundary = self.boundary(root)
        ordinal_path = (
            root
            / "ordinal_receipts"
        ...
        ordinal.update(
            {
                ...
                "arm": cell["arm"],
                "repeat": cell["repeat"],
                "start_claim": ORDINAL.identity(
                    attempt / "start_claim.json"
                ),
                "attempt_manifest": ORDINAL.identity(manifest_path),
                "ordinal_dispatch_claim": ORDINAL.identity(
                    attempt / "ordinal_dispatch_claim.json"
                ),
                "runtime_resource_monitor": ORDINAL.identity(runtime_path),
                "semantic_artifacts": semantic,
                "terminal_status": "SUCCESS",
                "clean_success": True,
                "schedule_sha256": ORDINAL.SCHEDULE_SHA256,
                "invalid_attempt_chain": [],
                "controller": boundary["controller"],
                "systemd_supervisor": boundary["supervisor"],
                "control_freeze": boundary["control_freeze"],
                "attempt_matrix_freeze": boundary[
                    "attempt_matrix_freeze"
                ],
                "adopted_at_utc": "2026-08-30T00:00:00+00:00",
            }
        )
        ordinal_path.write_bytes(ORDINAL.canonical_json(ordinal))
        final_path = directory / "systemd_adoption_receipt.json"
        final = json.loads(final_path.read_text(encoding="utf-8"))
        final["ordinal_terminal_receipt"] = ORDINAL.identity(ordinal_path)
        final["ordinal_state_before_final_adoption_receipt"] = dict(
            followup
        )
        final_path.write_bytes(ORDINAL.canonical_json(final))
        return {
            ...
            "attempt": attempt,
            "directory": directory,
            "adoption_claim": directory / "systemd_adoption_claim.json",
            "final": final_path,
            "ordinal": ordinal_path,
            "runtime_identity": ORDINAL.identity(runtime_path),
            "semantic": semantic,
            "boundary": boundary,
        }

    def sync_adoption_claim_identity(
        self, fixture: Mapping[str, object]
    ) -> None:
        adoption_path = Path(str(fixture["adoption_claim"]))
        ordinal_path = Path(str(fixture["ordinal"]))
        final_path = Path(str(fixture["final"]))
        ordinal = json.loads(ordinal_path.read_text(encoding="utf-8"))
        ordinal["systemd_adoption_claim"] = ORDINAL.identity(adoption_path)
        ordinal_path.write_bytes(ORDINAL.canonical_json(ordinal))
        final = json.loads(final_path.read_text(encoding="utf-8"))
        final["systemd_adoption_claim"] = ORDINAL.identity(adoption_path)
        final["ordinal_terminal_receipt"] = ORDINAL.identity(ordinal_path)
        final_path.write_bytes(ORDINAL.canonical_json(final))
    ...
    def inspection_context(
        ...
        runtime_by_root = {
            str(fixture["attempt"]): fixture["runtime_identity"]
            for fixture in fixtures
        }
        semantic_by_root = {
            str(fixture["attempt"]): fixture["semantic"]
            for fixture in fixtures
        }
        boundary = fixtures[0]["boundary"]

        def runtime(
            attempt: Path,
            _result: Mapping[str, object],
            _cell: Mapping[str, object],
            _index: int,
        ) -> object:
            return runtime_by_root[str(attempt)]
        ...
        def semantic(
            ...
            return semantic_by_root[str(attempt)]

        return (
            mock.patch.object(
                ORDINAL, "load_schedule", return_value=schedule
            ),
            mock.patch.object(ORDINAL, "load_attempt_matrix"),
            mock.patch.object(
                ORDINAL, "result_contract_valid", return_value=True
            ),
            mock.patch.object(
                ORDINAL,
            ...
            mock.patch.object(
                ORDINAL, "control_boundary", return_value=boundary
            ),
        )

    def test_direct_state_path_rejects_finalized_adoption_attacks(
        self,
    ) -> None:
        attacks = (
            "claim_extra",
            ...
            "final_retry",
            "terminal_followup",
            "no_start_terminal_mix",
        )
        for attack in attacks:
            with self.subTest(attack=attack), tempfile.TemporaryDirectory() as temporary:
                root = Path(temporary)
                cell = self.cell(1)
                fixture = self.build_terminal(
                    root, cell, {"state": "COMPLETE", "completed": 1}
                )
                adoption_path = Path(str(fixture["adoption_claim"]))
                final_path = Path(str(fixture["final"]))
                if attack.startswith("claim_"):
                    adoption = json.loads(
                        adoption_path.read_text(encoding="utf-8")
                    )
                    if attack == "claim_extra":
                        adoption["forged"] = True
                    elif attack == "claim_timeout":
                        adoption["timeout_or_signal_observed"] = True
                    else:
                        adoption["estimator_retry_authorized"] = True
                    adoption_path.write_bytes(
                        ORDINAL.canonical_json(adoption)
                    )
                    self.sync_adoption_claim_identity(fixture)
                else:
                    final = json.loads(
                        final_path.read_text(encoding="utf-8")
                    )
                    if attack == "final_extra":
                        final["forged"] = True
                    elif attack == "final_retry":
                        final["estimator_retry_authorized"] = True
                    elif attack == "terminal_followup":
                        final[
                            "ordinal_state_before_final_adoption_receipt"
                        ] = {"state": "TERMINAL"}
                    else:
                        final["controller_outcome"] = (
                            "RESOURCE_BLOCKED_NO_START"
                        )
                    final_path.write_bytes(ORDINAL.canonical_json(final))
                patches = self.inspection_context(
                    root, [dict(cell)], [fixture]
                )
                with patches[0], patches[1], patches[2], patches[3], patches[4], patches[5], mock.patch.object(
                    ORDINAL,
                    "validate_finalized_systemd_adoption",
                    wraps=ORDINAL.validate_finalized_systemd_adoption,
                ) as centralized:
                    with self.assertRaises(ORDINAL.OrdinalError):
                        ORDINAL.inspect_cell(root, cell)
                    self.assertGreaterEqual(centralized.call_count, 1)
                    with self.assertRaises(ORDINAL.OrdinalError):
                        ORDINAL.next_state(root)

    def test_historical_terminal_successor_stays_valid_in_next_state(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            first_cell = self.cell(1)
            second_cell = self.cell(2)
            second_state = self.state(root, second_cell)
            first = self.build_terminal(root, first_cell, second_state)
            second = self.build_terminal(
                root,
                second_cell,
                {"state": "COMPLETE", "completed": 2},
            )
            schedule = [first_cell, second_cell]
            patches = self.inspection_context(
                root, schedule, [first, second]
            )
            with patches[0], patches[1], patches[2], patches[3], patches[4], patches[5]:
                self.assertEqual(
                    ORDINAL.inspect_cell(root, first_cell)["state"],
                    "TERMINAL",
                )
                self.assertEqual(
                    ORDINAL.inspect_cell(root, second_cell)["state"],
                    "TERMINAL",
                )
                self.assertEqual(ORDINAL.next_state(root)["state"], "COMPLETE")
...
class ZeroKfWatchdogTests(unittest.TestCase):
    ...
    def reap_receipt(
        ...
        *,
        ...
        return {
            "schema_version": "aqua-fe-fair-stability-same-popen-reap-v12",
            "source": source,
            "same_popen": True,
    ...
    def test_refused_final_gate_is_preserved_but_is_not_an_action(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            outcome = {
                "schema_version": (
                    "aqua-fe-fair-stability-hfnet-zero-kf-watchdog-outcome-v12"
                ),
                "monitor_proven": False,
                "monitoring_errors": [
                    "FINAL_SIGNAL_GATE:"
        ...
        with mock.patch.object(
            ...
            returncode, reaped, timed_out, outcome = (
                HFNET.wait_with_zero_kf_watchdog(
                    ...
                    timeout=31.0,
                    monotonic=clock,
                    monotonic_ns=lambda: int(clock.value * 1e9) + 1,
                )
            )
        self.assertEqual(returncode, -6)
        self.assertTrue(reaped)
    ...
    def test_final_gate_pending_transition_bounded_same_popen_reap_is_passive(
        ...
        self.assertEqual(
            passive["reap"]["source"], "PENDING_REAP_BOUNDED_WAIT"
        )
    ...
    def test_common_watchdog_chain_rejects_contradictory_signal_proofs(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            def runtime_receipt(returncode, cleanup_actions):
                return {
                    "schema_version": (
                        "aqua-fe-fair-stability-runtime-resource-monitor-v12"
                    ),
                    "experiment_id": ORDINAL.EXPERIMENT_ID,
                    "coverage": {"process_group_empty": True},
                    "supervised_process": {
            ...
            outcome = {
                ...
                "watchdog_started_monotonic_ns": 1_000_000_000,
                "global_deadline_monotonic_ns": 1_801_000_000_000,
                "monitor_proven": True,
                "monitoring_errors": [],
                "sample_count": 151,
                "target_poll_interval_seconds": 0.20,
                ...
                "pending_reap_adjudication": None,
            ...
            receipt = {
                "schema_version": (
                    "aqua-fe-fair-stability-hfnet-zero-kf-post-shutdown-watchdog-v12"
                ),
                "experiment_id": ORDINAL.EXPERIMENT_ID,
            ...
            passive = {
                ...
                "reap": {
                    "schema_version": (
                        "aqua-fe-fair-stability-same-popen-reap-v12"
                    ),
            ...
            passive_outcome = {
                ...
                "watchdog_started_monotonic_ns": 1,
                "global_deadline_monotonic_ns": 1_800_000_000_001,
                "monitor_proven": True,
                "monitoring_errors": [],
                "sample_count": 1,
                "target_poll_interval_seconds": 0.20,
            ...
            pending_reap = {
                "schema_version": "aqua-fe-fair-stability-same-popen-reap-v12",
                "source": "PENDING_REAP_BOUNDED_WAIT",
                "same_popen": True,
                "frozen_process_identity": {
                    "pid": 4242,
                    "start_ticks": 777,
            ...
            pending_passive = {
                ...
                "last_exact_group_snapshot": prior,
                "adjudicated_at_utc": "2026-08-29T10:00:32+00:00",
                "adjudicated_monotonic_ns": 30_065_000_000,
                "child_returncode": -6,
                "reaped_before_next_sample": True,
                "reap": pending_reap,
                "signal_attempted": False,
            ...
            pending_outcome["pending_reap_adjudication"] = {
                "schema_version": (
                    "aqua-fe-fair-stability-hfnet-watchdog-pending-reap-v12"
                ),
                "phase": "FINAL_SIGNAL_GATE:FIRST_FINAL_GROUP_PROOF",
                "status": "PASSIVE_REAP_PROVEN",
                "classification": (
                    "SAME_POPEN_REAPED_WITHIN_INCLUSIVE_DEADLINE"
                ),
                "candidate_evidence": pending_gate_evidence,
                "anchor_source": (
                    "candidate_evidence.terminal_transition_snapshot."
                    "observed_monotonic_ns"
                ),
                "anchor_completed_monotonic_ns": 30_050_000_000,
                "global_deadline_monotonic_ns": 1_800_000_000_001,
                "local_maximum_deadline_monotonic_ns": 30_300_000_000,
                "effective_deadline_monotonic_ns": 30_300_000_000,
                ...
                "wait_events": [
                    {
                        "sequence": 0,
                        "started_monotonic_ns": 30_061_000_000,
                        "requested_timeout_seconds": 0.20,
                        "completed_monotonic_ns": 30_065_000_000,
                        "outcome": "RETURNED",
                        "returncode": -6,
                    }
                ],
                "poll_events": [
                    ...
                    {
                        "sequence": 1,
                        "source": "WAIT_RETURNED_SAME_POPEN",
                        "observed_monotonic_ns": 30_065_000_000,
                        "returncode": -6,
                        "group_recheck": None,
                    },
                ],
            ...
            forged_adjudication_time = json.loads(json.dumps(pending_receipt))
            for wrapper in (
                forged_adjudication_time["passive_exit_adjudication"],
                forged_adjudication_time["outcome"][
                    "passive_exit_adjudication"
                ],
            ):
                wrapper["adjudicated_monotonic_ns"] += 1
            publish_passive(forged_adjudication_time)
            with self.assertRaises(ORDINAL.OrdinalError):
                ORDINAL.validate_hfnet_watchdog_evidence(
                    root, manifest, passive_result, cell, 1
                )
...
class RuntimeResourceMonitorTests(unittest.TestCase):
    ...
    def test_proc_and_gpu_start_gap_contracts_are_independent(self) -> None:
        def samples(*starts: int) -> list[dict[str, object]]:
            return [
                {
                    "probe_started_monotonic_ns": value,
                    "probe_finished_monotonic_ns": value + 1,
                }
                for value in starts
            ]
        ...
        finish_after_end = RESOURCE._sampling_gap_report(
            [
                {
                    "probe_started_monotonic_ns": 50_000_000,
                    "probe_finished_monotonic_ns": 100_000_001,
                }
            ],
            0,
            100_000_000,
        ...
        self.assertFalse(finish_after_end["proven"])
        self.assertEqual(
            finish_after_end["sample_finish_contract_violations"],
            [
                {
    ...
    @staticmethod
    def _bare_loop_monitor() -> object:
        monitor = RESOURCE.RuntimeResourceMonitor.__new__(
            RESOURCE.RuntimeResourceMonitor
        )
        monitor.process = mock.Mock()
        ...
        monitor._cleanup_actions = []
        monitor._owned_empty = mock.Mock(return_value=True)
        return monitor

    @staticmethod
    def _reserved_sample(
        stream: str,
        sequence: int,
        started_ns: int,
        started_utc: str,
        ...
        common: dict[str, object] = {
            ...
            "probe_finished_monotonic_ns": started_ns + 1,
    ...
    def test_atomic_probe_reservation_and_coverage_close_for_both_streams(
        self,
    ) -> None:
        for stream in ("proc", "gpu"):
            with self.subTest(stream=stream, winner="reservation"):
                monitor = self._bare_loop_monitor()
                entered = threading.Event()
                release = threading.Event()
                probe_name = f"_{stream}_probe"
                loop = getattr(monitor, f"_{stream}_loop")
                ...
                def probe(
                    ...
                    return self._reserved_sample(
                        stream, sequence, started_ns, started_utc
                    )
                ...
                with mock.patch.object(
                    ...
                    side_effect=[
                        ...
                        "2026-08-30T00:00:02+00:00",
                        "2026-08-30T00:00:03+00:00",
                    ],
                ), mock.patch.object(
                    RESOURCE, "_scan_processes", return_value=([], [])
                ) as close_scan, mock.patch.object(
                    monitor, probe_name, side_effect=probe
                ):
                ...
                rows = getattr(monitor, f"_{stream}_samples")
                self.assertEqual(len(rows), 2 if stream == "proc" else 1)
                self.assertEqual(rows[0]["probe_started_monotonic_ns"], 200)
                self.assertEqual(monitor.coverage_end_monotonic_ns, 500)
                close_scan.assert_called_once_with()
                self.assertLessEqual(
                    rows[0]["probe_started_monotonic_ns"],
                    monitor.coverage_end_monotonic_ns,
                )
                self.assertLessEqual(
                    rows[0]["probe_finished_monotonic_ns"],
                    monitor.coverage_end_monotonic_ns,
                )
                closing = [
                    row
                ...
                self.assertTrue(closing[0]["coverage_close_ownership_empty"])
                self.assertTrue(closing[0]["coverage_close_recheck_proven"])

            with self.subTest(stream=stream, winner="coverage_close"):
                monitor = self._bare_loop_monitor()
                probe = mock.Mock()
                with mock.patch.object(
                    RESOURCE.time, "monotonic_ns", return_value=100
                ), mock.patch.object(
                    RESOURCE,
                    "_utc_now",
                    return_value="2026-08-30T00:00:01+00:00",
                    ...
                    monitor, f"_{stream}_probe", probe
                ):
                    self.assertTrue(monitor._mark_coverage_end_if_empty([]))
                    getattr(monitor, f"_{stream}_loop")()
                probe.assert_not_called()
                close_scan.assert_called_once_with()
                if stream == "gpu":
                    self.assertEqual(monitor._gpu_samples, [])
                self.assertEqual(len(monitor._proc_samples), 1)
                closing = monitor._proc_samples[0]
                self.assertEqual(
                    closing["probe_role"], RESOURCE.CLOSING_PROC_PROBE_ROLE
                )
                self.assertTrue(closing["coverage_close_ownership_empty"])
                self.assertTrue(closing["coverage_close_recheck_proven"])

    def test_probe_exception_keeps_reserved_timestamp_for_both_streams(
        self,
    ) -> None:
        for stream in ("proc", "gpu"):
            with self.subTest(stream=stream):
                ...
                def explode(
                    ...
                    entered.set()
                    self.assertTrue(release.wait(2.0))
                    raise RuntimeError("deterministic-probe-fault")
                ...
                with mock.patch.object(
                    ...
                    monitor, f"_{stream}_probe", side_effect=explode
                ):
                    thread = threading.Thread(
                        target=getattr(monitor, f"_{stream}_loop")
                    )
                    thread.start()
                    self.assertTrue(entered.wait(2.0))
                    self.assertTrue(monitor._mark_coverage_end_if_empty([]))
                ...
                self.assertEqual(rows[0]["probe_started_monotonic_ns"], 200)
                self.assertEqual(
                    rows[0]["probe_started_at_utc"],
                    "2026-08-30T00:00:01+00:00",
                )
                self.assertEqual(rows[0]["probe_finished_monotonic_ns"], 300)
                self.assertEqual(monitor.coverage_end_monotonic_ns, 600)
                close_scan.assert_called_once_with()
                self.assertTrue(
                    rows[0]["probe_errors"][0].startswith(
                        f"{stream.upper()}_PROBE_EXCEPTION:RuntimeError:"
                    )
                )
    ...
    def test_post_reservation_stall_uses_truthful_actual_start_gap(self) -> None:
        for stream, maximum_seconds, target_seconds in (
            (
                "proc",
                RESOURCE.PROC_MAX_START_GAP_SECONDS,
                RESOURCE.PROC_TARGET_INTERVAL_SECONDS,
            ...
            with self.subTest(stream=stream):
                ...
                release = threading.Event()
                original_reserve = monitor._reserve_probe_start
                stalled_start_ns = int((maximum_seconds + 1.0) * 1e9)

                def reserve_then_stall(
                    stream_name: str, sequence: int
                ) -> tuple[str, int] | None:
                    token = original_reserve(stream_name, sequence)
                ...
                with mock.patch.object(
                    ...
                    monitor, "_reserve_probe_start", side_effect=reserve_then_stall
                ), mock.patch.object(
                    monitor, f"_{stream}_probe", side_effect=probe
                ):
                    thread = threading.Thread(
                        target=getattr(monitor, f"_{stream}_loop")
                    )
                    thread.start()
                    self.assertTrue(reserved.wait(2.0))
                    self.assertTrue(monitor._mark_coverage_end_if_empty([]))
                    self.assertIsNone(monitor.coverage_end_monotonic_ns)
                    release.set()
                    thread.join(2.0)
                ...
                self.assertEqual(len(rows), 2 if stream == "proc" else 1)
                self.assertEqual(
                    rows[0]["probe_started_monotonic_ns"], stalled_start_ns
                )
                self.assertLessEqual(
                    rows[0]["probe_finished_monotonic_ns"],
                    monitor.coverage_end_monotonic_ns,
                )
                sampling = RESOURCE._sampling_gap_report(
                    rows,
                ...
                self.assertFalse(sampling["proven"])
                self.assertEqual(
                    sampling["start_gap_violations"],
                    [
                        {
                            "label": "coverage_start_to_first_probe_start",
                            "gap_seconds": stalled_start_ns / 1e9,
                        }
                    ],
                )
                closing = monitor._proc_samples[-1]
    ...
    def test_concurrent_close_callers_commit_one_coverage_end(self) -> None:
        monitor = self._bare_loop_monitor()
        callers_ready = threading.Barrier(3)
        scan_started = threading.Event()
        scan_release = threading.Event()
        one_caller_returned = threading.Event()
        results: list[bool] = []

        def scan() -> tuple[list[object], list[str]]:
            scan_started.set()
            self.assertTrue(scan_release.wait(2.0))
            return [], []

        def close() -> None:
            callers_ready.wait()
            results.append(monitor._mark_coverage_end_if_empty([]))
            one_caller_returned.set()

        with mock.patch.object(
            RESOURCE, "_scan_processes", side_effect=scan
        ) as close_scan, mock.patch.object(
            RESOURCE.time, "monotonic_ns", return_value=777
        ) as end_clock, mock.patch.object(
            RESOURCE,
            "_utc_now",
            return_value="2026-08-30T00:00:00+00:00",
        ) as end_utc:
            callers = [threading.Thread(target=close) for _index in range(2)]
            for caller in callers:
                caller.start()
            callers_ready.wait()
            self.assertTrue(scan_started.wait(2.0))
            self.assertTrue(one_caller_returned.wait(2.0))
            self.assertIsNone(monitor.coverage_end_monotonic_ns)
            scan_release.set()
            for caller in callers:
                caller.join(2.0)
        ...
        self.assertEqual(monitor.coverage_end_monotonic_ns, 777)
        self.assertTrue(monitor._stop.is_set())
        self.assertEqual(monitor._active_probe_reservations, {})
        self.assertEqual(monitor._monitor_errors, [])
        close_scan.assert_called_once_with()
        self.assertEqual(end_clock.call_count, 3)
        self.assertEqual(end_utc.call_count, 2)
        self.assertEqual(len(monitor._proc_samples), 1)
        closing = monitor._proc_samples[0]
        self.assertEqual(
            closing["probe_role"], RESOURCE.CLOSING_PROC_PROBE_ROLE
    ...
    def test_short_child_has_valid_atomic_probe_coverage(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            output = Path(temporary) / "runtime_resource_monitor.json"
            clean_query = {
                "returncode": 0,
            ...
            with mock.patch.object(
                RESOURCE, "_run_gpu_compute_query", return_value=clean_query
            ):
                process = subprocess.Popen(
                ...
                leader, leader_errors = RESOURCE._read_process_record(process.pid)
                self.assertIsNotNone(leader, leader_errors)
                ...
                with mock.patch.object(
                    RESOURCE, "_scan_processes", side_effect=fast_exact_scan
                ):
                    monitor = RESOURCE.RuntimeResourceMonitor(process, output)
                    returncode, reaped, timed_out = monitor.wait(5.0)
                    self.assertEqual(returncode, 0)
                    ...
                    receipt = monitor.finalize()
        self.assertTrue(
            receipt["monitor_proven"],
            {
                "probe_errors": receipt["probe_errors"],
                "sampling_contract": receipt["sampling_contract"],
            },
        )
        self.assertTrue(receipt["pipeline_valid"])
        self.assertEqual(receipt["pipeline_failure_reasons"], [])
        end_ns = receipt["coverage"]["end_monotonic_ns"]
        for stream in ("proc", "gpu"):
            self.assertGreaterEqual(len(receipt["samples"][stream]), 1)
            self.assertTrue(
                all(
                    row["probe_started_monotonic_ns"] <= end_ns
                    for row in receipt["samples"][stream]
                )
            )
