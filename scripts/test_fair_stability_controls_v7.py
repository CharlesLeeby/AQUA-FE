...
VINS = load("fair_vins_v7", SCRIPTS / "run_fair_stability_vins_replay_v7.py")
HFNET = load("fair_hfnet_v7", SCRIPTS / "run_fair_stability_hfnet_openloop_v7.py")
ORDINAL = load("fair_ordinal_v7", SCRIPTS / "fair_stability_ordinal_common_v7.py")
NEXT = load("fair_next_v7", SCRIPTS / "run_fair_stability_next_v7.py")
SUMMARY = load("fair_summary_v7", SCRIPTS / "summarize_fair_stability_v7.py")
RESOURCE = load(
    "fair_runtime_resource_monitor",
    SCRIPTS / "fair_stability_runtime_resource_monitor_v7.py",
)
SYSTEMD = load(
    "fair_systemd_supervisor_v7",
    SCRIPTS / "run_fair_stability_systemd_supervisor_v7.py",
)
...
class LifecycleAndScheduleTests(unittest.TestCase):
    ...
    def test_v7_schedule_and_pristine_attempt_matrix(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            matrix_path = root / "attempt_matrix_freeze_v7.json"
            matrix_path.write_bytes(
                ORDINAL.canonical_json(
                    {
                        "schema_version": "aqua-fe-fair-stability-attempt-matrix-freeze-v7",
                        "experiment_id": ORDINAL.EXPERIMENT_ID,
                        "status": "FROZEN_BEFORE_ANY_V7_ESTIMATOR_START",
...
class V7ControlRegressionTests(unittest.TestCase):
    ...
    def test_vins_roscore_interpreter_is_fixed_not_manifest_selected(self) -> None:
        ...
        wrapper = (
            SCRIPTS / "run_fair_stability_vins_replay_child_v7.sh"
    ...
    CHILD = SCRIPTS / "run_fair_stability_vins_replay_child_v7.sh"
    ...
    def test_vins_rejects_prior_result_import_claims(self) -> None:
        ...
        manifest = {
            "schema_version": "aqua-fe-fair-stability-hfnet-input-freeze-v7",
    ...
    def test_runner_summaries_delegate_to_receipt_state_machine(self) -> None:
        ...
        with mock.patch(
            "summarize_fair_stability_v7.summarize", return_value=sentinel
        ) as summarize:
    ...
    def test_postprocess_signals_remain_fail_stop_through_result_publication(self) -> None:
        ...
        for path in (
            SCRIPTS / "run_fair_stability_hfnet_openloop_v7.py",
            SCRIPTS / "run_fair_stability_vins_replay_v7.py",
        ):
    ...
    def test_result_published_then_runner_signal_is_controller_fail_stop(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            attempt.mkdir()
            matrix = base / "attempt_matrix_freeze_v7.json"
            matrix.write_text("{}\n", encoding="ascii")
    ...
    def test_resource_check_stdout_then_signal_cannot_start_estimator(self) -> None:
        state = {
            "state": "READY",
            "attempt_root": "/tmp/never-started-v7",
            "attempt_index": 1,
            "cell": {
                "ordinal": 1,
                "case_id": "a05_3300_3700",
        ...
        gate = {
            "schema_version": "aqua-fe-fair-stability-prestart-resource-gate-v7",
            "experiment_id": ORDINAL.EXPERIMENT_ID,
            "ready": True,
        }
    ...
    def test_terminal_adoption_has_bound_monitor_and_two_validations(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            claim = {
                "schema_version": "aqua-fe-fair-stability-systemd-adoption-claim-v7",
    ...
    def test_matrix_freeze_requires_exact_pristine_prepare_tree(self) -> None:
        ...
        def prepare_fixture(experiment_root: Path) -> tuple[Path, Path]:
            ...
            manifest = {
                "schema_version": "aqua-fe-fair-stability-hfnet-attempt-v7",
                "experiment_id": ORDINAL.EXPERIMENT_ID,
                "case_id": cell["case_id"],
                "arm": cell["arm"],
                "repeat": 1,
...
class SystemdControlTests(unittest.TestCase):
    ...
    def build_chain(
        ...
        unit = (
            "aqua-fe-fair-stability-v7-o001-a001-"
    ...
    def test_systemd_unit_contract_and_submit_only_argv(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            unit = "aqua-fe-fair-stability-v7-o001-a001-s001-aabbcc.service"
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            dispatch_execution = {
                ...
                "controller": ORDINAL.identity(
                    SCRIPTS / "run_fair_stability_next_v7.py"
                ),
...
class ZeroKfWatchdogTests(unittest.TestCase):
    ...
    def test_common_watchdog_chain_rejects_contradictory_signal_proofs(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            launch = {
                "schema_version": "aqua-fe-fair-stability-hfnet-launch-receipt-v7",
                "experiment_id": ORDINAL.EXPERIMENT_ID,
                "valid": True,
                "errors": [],
                "pid": 4242,
