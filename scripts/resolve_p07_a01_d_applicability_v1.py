#!/usr/bin/env python3
"""Append the locked A01 D=NOT_APPLICABLE resolution to the canonical stream."""
...
LOCK_PATH = governance.P07 / "a01_d_resolution_lock_v1.json"
APPLICABILITY = governance.BUNDLE / "arm_applicability.csv"
OUTPUT = governance.P07 / "d_resolutions/a01_0018_not_applicable_v1.json"
P_RUN_ID = (
    "isj-nativeq-v3_P07_aqualoc_archaeology-A01_16200-17100_"
    "P_f0_b00_20260806T084500Z"
)


class ResolutionViolation(RuntimeError):
    """Raised when the A01 applicability append violates the lock."""
...
def read_csv_bytes(content: bytes) -> tuple[list[str], list[dict[str, str]]]:
    ...
    if not fields or len(fields) != len(set(fields)) or any(None in row for row in rows):
        raise ResolutionViolation("invalid canonical CSV")
    return fields, rows


def load_and_validate_lock() -> dict[str, object]:
    if not LOCK_PATH.is_file():
        raise ResolutionViolation(f"missing A01 D resolution lock: {LOCK_PATH}")
    payload = json.loads(LOCK_PATH.read_text(encoding="utf-8"))
    if (
        payload.get("schema_version") != "isj-p07-a01-d-resolution-lock-v1"
        or payload.get("status") != "FROZEN_READY_TO_APPEND_NOT_APPLICABLE"
        or payload.get("resolution_lock_hash") != lock_hash(payload)
    ):
        raise ResolutionViolation("A01 D lock schema, status, or hash mismatch")
    for record in payload["artifacts"]:
        ...
        if (
            ...
            raise ResolutionViolation(f"locked A01 D artifact drift: {path}")
...
def is_a01_row(row: dict[str, str]) -> bool:
    return (
        ...
        and row["sequence"] == "A01"
        and row["window_start"] == "810.0"
        and row["window_end"] == "855.0"
    ...
    row.update(
        {
            ...
            "evidence_path": (
                ...
                "queue_005_isj_p07_aqualoc_archaeology_a01_0018_p_attempt01/audit_v3.json"
            ),
...
def validate_registry(lock: dict[str, object]) -> list[dict[str, str]]:
    _fields, rows = read_csv_bytes((governance.BUNDLE / "run_registry.csv").read_bytes())
    events = []
    for parent in lock["parent_runs"]:
        chain = [row for row in rows if row["run_id"] == parent["run_id"]]
        if (
            len(chain) != 3
            or [row["status"] for row in chain] != ["PLANNED", "RUNNING", "COMPLETED"]
            or chain[-1]["registry_event_id"] != parent["latest_registry_event_id"]
            or chain[-1]["output_hash_manifest"] != parent["output_hash_manifest"]
        ):
            raise ResolutionViolation(
                f"parent registry chain is not locked completion: {parent['run_id']}"
            )
        events.append(chain[-1])
    p_event = next(event for event in events if event["run_id"] == P_RUN_ID)
    if p_event["accepted_lineage_count"] != "0" or p_event["active"] != "false":
        raise ResolutionViolation("parent P registry event is not zero-lineage inactive")
    return events
...
def append_resolution(lock: dict[str, object]) -> dict[str, object]:
    ...
    with APPLICABILITY.open("r+b", buffering=0) as handle:
        ...
        try:
            ...
            a01_rows = [row for row in rows if is_a01_row(row)]
            pending = [row for row in a01_rows if row["resolution"] == "PENDING_APPLICABILITY"]
            terminal = [row for row in a01_rows if row["resolution"] != "PENDING_APPLICABILITY"]
            if len(pending) != 1 or terminal:
                raise ResolutionViolation(
                    f"A01 applicability is not one pending/no terminal: {a01_rows}"
                )
            ...
            closeout = {
                "schema_version": "isj-p07-a01-d-applicability-resolution-v1",
...
def preflight(lock: dict[str, object]) -> dict[str, object]:
    events = validate_registry(lock)
    _fields, rows = read_csv_bytes(APPLICABILITY.read_bytes())
    a01_rows = [row for row in rows if is_a01_row(row)]
    return {
        "slot_index": 3,
        "parent_latest_statuses": {
            event["arm"]: event["status"] for event in events
        },
        "parent_p_lineage_count": next(
            event["accepted_lineage_count"] for event in events if event["run_id"] == P_RUN_ID
        ),
        "a01_resolutions": [row["resolution"] for row in a01_rows],
...
def main() -> int:
    ...
    if args.preflight_only:
        ...
        return 0 if (
            ...
            and report["a01_resolutions"] == ["PENDING_APPLICABILITY"]
            and not report["output_collision"]
            and set(report["parent_latest_statuses"].values()) == {"COMPLETED"}
        ) else 1
