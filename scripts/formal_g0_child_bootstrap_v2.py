...
_PROBE_OPTIONAL_PERSISTENT_ELF = {
    "realpath": "/usr/lib/x86_64-linux-gnu/libtbbmalloc.so.2",
    "size_bytes": 132_976,
    "sha256": "14147fcfadccacb4ddf94738143a3fa999a065a303dda898da8ab057cfb483b4",
    "stat": {
...
def _probe_module_identity(row: Mapping[str, Any]) -> dict[str, Any]:
    """Build the validated probe/run comparison view for one module row.

    The receipt itself remains lexical and exact.  Both receipts have already
    passed :func:`validate_runtime_receipt` before this helper is called, so a
    persistent path may be reduced to its typed realpath alias only while all
    of its stat, inode, hash, size, symlink, and module metadata remain in the
    comparison.
    """
    ...
    if not isinstance(identities, Mapping):
        raise FormalG0RuntimeError("probe module file identities are malformed")
    ...
    for field, identity in identities.items():
        if field not in _MODULE_PATH_FIELDS or field in bindings:
            raise FormalG0RuntimeError("probe module persistent binding differs")
        if not isinstance(identity, Mapping) or clone.get(field) != identity.get(
            "lexical_path"
        ):
            raise FormalG0RuntimeError("probe module persistent path differs")
        normalized = _probe_persistent_file_identity(
            identity, f"probe module {field}"
        )
...
def _probe_realpath_alias(value: object, label: str) -> str:
    """Return one live absolute realpath without treating hardlinks as aliases."""
    ...
    if (
        ...
        raise FormalG0RuntimeError(f"{label} alias path is malformed")
    ...
    if (
        ...
        raise FormalG0RuntimeError(f"{label} realpath alias is malformed")
    try:
        lexical_stat = os.stat(value)
        normalized_stat = os.stat(normalized)
    except OSError as error:
        raise FormalG0RuntimeError(f"{label} realpath alias is unavailable") from error
    if (lexical_stat.st_dev, lexical_stat.st_ino) != (
        normalized_stat.st_dev,
        normalized_stat.st_ino,
    ):
        raise FormalG0RuntimeError(f"{label} realpath alias identity differs")
    return normalized


def _probe_persistent_file_identity(
    value: Mapping[str, Any], label: str
) -> dict[str, Any]:
    """Normalize only the two path leaves of a deeply validated file identity."""

    clone = json.loads(canonical_json_bytes(value))
    if not isinstance(clone, dict) or set(clone) != _FILE_IDENTITY_KEYS:
        raise FormalG0RuntimeError(f"{label} file identity shape differs")
    lexical = _probe_realpath_alias(clone.get("lexical_path"), label)
    resolved = _probe_realpath_alias(clone.get("resolved_path"), label)
    if lexical != resolved:
        raise FormalG0RuntimeError(f"{label} realpath aliases disagree")
    stat_record = clone.get("stat")
    if not isinstance(stat_record, Mapping):
        raise FormalG0RuntimeError(f"{label} stat identity is malformed")
    live = os.stat(lexical)
    if (int(live.st_dev), int(live.st_ino)) != (
        stat_record.get("device"),
        stat_record.get("inode"),
    ):
        raise FormalG0RuntimeError(f"{label} realpath/stat cross-binding differs")
    clone["lexical_path"] = lexical
    clone["resolved_path"] = resolved
    return clone


def _probe_mapped_closure_identity(value: Mapping[str, Any]) -> dict[str, Any]:
    """Normalize persistent mapped-file aliases after deep map validation."""

    files = value.get("files")
    rows = value.get("rows")
    sealed_rows = value.get("sealed_rows")
    if not isinstance(files, list) or not isinstance(rows, list) or not isinstance(
        sealed_rows, list
    ):
        raise FormalG0RuntimeError("probe mapped closure is malformed")
    normalized_files: dict[str, dict[str, Any]] = {}
    aliases: dict[str, str] = {}
    for index, record in enumerate(files):
        if not isinstance(record, Mapping):
            raise FormalG0RuntimeError("probe mapped file identity is malformed")
        original = record.get("lexical_path")
        normalized = _probe_persistent_file_identity(
            record, f"probe mapped file {index}"
        )
        key = normalized["lexical_path"]
        if not isinstance(original, str) or key in normalized_files:
            raise FormalG0RuntimeError("probe mapped realpath alias is duplicated")
        aliases[original] = key
        normalized_files[key] = normalized
    normalized_rows: list[dict[str, Any]] = []
    for row in rows:
        if not isinstance(row, Mapping):
            raise FormalG0RuntimeError("probe mapped row is malformed")
        clone = dict(row)
        original = clone.get("lexical_path")
        normalized = aliases.get(str(original)) if isinstance(original, str) else None
        if normalized is None or _probe_realpath_alias(
            original, "probe mapped row"
        ) != normalized:
            raise FormalG0RuntimeError("probe mapped row alias differs")
        clone["lexical_path"] = normalized
        normalized_rows.append(clone)
    return {
        "files": [normalized_files[key] for key in sorted(normalized_files)],
        "rows": sorted(
            normalized_rows,
            key=lambda row: (
        ...
        "sealed_rows": list(sealed_rows),
    }


def _probe_native_identity(row: Mapping[str, Any]) -> dict[str, Any]:
    """Normalize only a persistent ELF file leaf; sealed native rows stay exact."""

    clone = json.loads(canonical_json_bytes(row))
    kind = clone.get("mapping_kind")
    if kind == "PERSISTENT_ELF":
        identity = clone.get("file")
        if not isinstance(identity, Mapping):
            raise FormalG0RuntimeError("probe native persistent identity is malformed")
        clone["file"] = _probe_persistent_file_identity(
            identity, "probe native ELF"
        )
    elif kind != "SEALED_MAPPED_FILE":
        raise FormalG0RuntimeError("probe native mapping kind differs")
    return clone


def _probe_optional_persistent_elf(value: Mapping[str, Any]) -> bool:
    """Identify the sole probe-only leaf admitted by sealed incident evidence.

    This is deliberately not a generic SONAME or optional-library rule.  Only
    the exact realpath, content hash, byte count, and stat-cross-bound map rows
    for this frozen ``libtbbmalloc`` object may be absent from an evaluator
    role after the probe has loaded it transiently.
    """

    return (
        value.get("lexical_path") == _PROBE_OPTIONAL_PERSISTENT_ELF["realpath"]
        and value.get("resolved_path")
        == _PROBE_OPTIONAL_PERSISTENT_ELF["realpath"]
        and value.get("size_bytes")
        == _PROBE_OPTIONAL_PERSISTENT_ELF["size_bytes"]
        and value.get("sha256") == _PROBE_OPTIONAL_PERSISTENT_ELF["sha256"]
        and value.get("stat") == _PROBE_OPTIONAL_PERSISTENT_ELF["stat"]
        and value.get("symlink") is None
    )
...
def _assert_expected_closure_subset(
    ...
    expected_maps = _probe_mapped_closure_identity(expected_maps)
    actual_maps = _probe_mapped_closure_identity(actual_maps)
    ...
    for key, row in expected_files.items():
        observed = actual_files.get(key)
        if observed == row:
            continue
        if observed is None and _probe_optional_persistent_elf(row):
            continue
        if observed != row:
    ...
    absent_optional_files = {
        key
        for key, row in expected_files.items()
        if key not in actual_files and _probe_optional_persistent_elf(row)
    }
    required_expected_rows = {
        canonical_json_bytes(dict(row))
        for row in expected_maps.get("rows", [])
        if isinstance(row, Mapping)
        and row.get("lexical_path") not in absent_optional_files
    }
    if not required_expected_rows.issubset(actual_rows):
    ...
    expected_native = {
        canonical_json_bytes(row) for row in expected_native_rows
    }
    actual_native = {canonical_json_bytes(row) for row in actual_native_rows}
    ...
    required_expected_native = {
        canonical_json_bytes(row)
        for row in expected_native_rows
        if not (
            row.get("mapping_kind") == "PERSISTENT_ELF"
            and isinstance(row.get("file"), Mapping)
            and row["file"].get("lexical_path") in absent_optional_files
            and _probe_optional_persistent_elf(row["file"])
        )
    }
    if not required_expected_native.issubset(actual_native):
