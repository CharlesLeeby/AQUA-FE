#!/usr/bin/python3
"""Fail-closed schedule state for fair-stability runtime-exclusive v5."""
...
EXPERIMENT_ID = "fair-stability-positive-roster-openloop-runtimeexcl-v5"
...
def control_boundary(experiment_root: Path) -> dict[str, dict[str, object]]:
    ...
    controller = common.parent / "run_fair_stability_next_v5.py"
    supervisor = common.parent / "run_fair_stability_systemd_supervisor_v5.py"
    freeze_path = (
        experiment_root / "vins_dev_nativeq_schedfix_runtimeexcl_v5" / "backend_freeze.json"
    )
    ...
    return {
        ...
        "attempt_matrix_freeze": identity(experiment_root / "attempt_matrix_freeze_v5.json"),
    }
...
SYSTEMD_CHAIN_FILES = {
    ...
    "submission_receipt": (
        ...
        "aqua-fe-fair-stability-systemd-submit-receipt-v5",
    ),
    ...
    "systemd_execution_receipt": (
        ...
        "aqua-fe-fair-stability-systemd-execution-v5",
    ),
    "systemd_terminal_receipt": (
        "systemd_terminal_receipt.json",
        "aqua-fe-fair-stability-systemd-terminal-v5",
    ),
}
...
def load_attempt_matrix(experiment_root: Path) -> dict[str, Any]:
    """Verify the immutable 120-attempt v5 matrix without freezing later retries."""
    path = experiment_root / "attempt_matrix_freeze_v5.json"
    require_no_symlink_components(path, experiment_root)
    if path.is_symlink() or not path.is_file():
    ...
    if (
        ...
        != "aqua-fe-fair-stability-attempt-matrix-freeze-v5"
        or value.get("experiment_id") != EXPERIMENT_ID
        or value.get("status") != "FROZEN_BEFORE_ANY_V5_ESTIMATOR_START"
    ):
...
def attempt_root(experiment_root: Path, cell: Mapping[str, Any], attempt_index: int) -> Path:
    ...
    if arm.startswith("hfnet_openloop_"):
        ...
        base = (
            experiment_root / "vins_dev_nativeq_schedfix_runtimeexcl_v5" / "attempts"
...
def validate_hfnet_watchdog_evidence(
    ...
    if (
        ...
        != "aqua-fe-fair-stability-hfnet-launch-receipt-v5"
    ...
    if (
        ...
        != "aqua-fe-fair-stability-hfnet-zero-kf-watchdog-outcome-v5"
...
def inspect_cell(experiment_root: Path, cell: Mapping[str, Any]) -> dict[str, Any]:
    ...
    while attempt_index <= MAX_ATTEMPTS_PER_CELL:
        ...
        expected_manifest_schema = (
            "aqua-fe-fair-stability-hfnet-attempt-v5"
            if hfnet_arm
            else "aqua-fe-fair-stability-vins-attempt-v5"
        )
        if (
        ...
        if result_path.is_file():
            ...
            expected_result_schema = (
                "aqua-fe-fair-stability-hfnet-result-v5"
                if hfnet_arm
                else "aqua-fe-fair-stability-vins-result-v5"
            )
            if (
            ...
            if status in TERMINAL_STATUSES:
                ...
                if terminal_receipt.is_file():
                    ...
                    if (
                        ...
                        != "aqua-fe-fair-stability-ordinal-terminal-v5"
                        or not strict_json_equal(
                            [
                    ...
                    if (
                        ...
                        != "aqua-fe-fair-stability-systemd-adoption-claim-v5"
                    ...
                    if (
                        ...
                        != "aqua-fe-fair-stability-systemd-adoption-v5"
            ...
            if (
                ...
                != "aqua-fe-fair-stability-pipeline-invalid-adjudication-v5"
                or invalid_receipt.get("automatic_retry_policy")
                != "EXPLICIT_EXTERNAL_TRANSIENT_ONLY_V5"
...
def verify_dispatch_claim(
    ...
    if claim.get("schema_version") != "aqua-fe-fair-stability-ordinal-dispatch-v5":
