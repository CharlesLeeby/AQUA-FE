#!/usr/bin/python3
"""Durable systemd-user supervision for one fair-stability v11 ordinal.
...
from fair_stability_ordinal_common_v11 import (  # noqa: E402
    ...
    valid_ready_prestart_resource_gate_v11,
...
CONTROLLER = ROOT / "scripts/run_fair_stability_next_v11.py"
...
UNIT_PREFIX = "aqua-fe-fair-stability-v11"

SUBMISSION_SCHEMA = "aqua-fe-fair-stability-systemd-submission-v11"
SUBMIT_RECEIPT_SCHEMA = "aqua-fe-fair-stability-systemd-submit-receipt-v11"
START_RECEIPT_SCHEMA = "aqua-fe-fair-stability-systemd-start-receipt-v11"
EXECUTION_SCHEMA = "aqua-fe-fair-stability-systemd-execution-v11"
TERMINAL_SCHEMA = "aqua-fe-fair-stability-systemd-terminal-v11"
ADOPTION_CLAIM_SCHEMA = "aqua-fe-fair-stability-systemd-adoption-claim-v11"
ADOPTION_SCHEMA = "aqua-fe-fair-stability-systemd-adoption-v11"
...
def host_preflight() -> dict[str, object]:
    ...
    return {
        "schema_version": "aqua-fe-fair-stability-systemd-host-preflight-v11",
        ...
        "login": {
            ...
            "limitation": (
                "LAST_LOGIN_SESSION_EXIT_MAY_STOP_USER_MANAGER_AND_INVALIDATE_V11"
...
def systemd_run_argv(unit: str, directory: Path) -> list[str]:
    ...
    return [
        ...
        f"--description=AQUA-FE fair-stability v11 one ordinal {unit}",
...
def _runner_check_argv(state: Mapping[str, Any]) -> list[str]:
    from run_fair_stability_next_v11 import runner_argv
...
def poll(directory: Path | None = None) -> dict[str, object]:
    ...
    claim = load_json(selected / "submission_claim.json", SUBMISSION_SCHEMA)
    return {
        "schema_version": "aqua-fe-fair-stability-systemd-poll-v11",
        "experiment_id": EXPERIMENT_ID,
...
def adopt(directory: Path | None = None) -> dict[str, object]:
    ...
    if service_result == "timeout" or exit_code in {"killed", "dumped"}:
        raise SystemdSupervisorError("SYSTEMD_TIMEOUT_OR_SIGNAL_FAIL_STOP_ABANDON_V11")
    ...
    if outcome == TERMINAL_SYSTEMD_OUTCOME:
        from run_fair_stability_next_v11 import adopt_terminal
