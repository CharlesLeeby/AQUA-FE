#!/usr/bin/python3
"""Run paired July Learned+KLT/KLT bags under the runtime-exclusive v2 protocol.
...
DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V2 identity so stability remains paired.
"""
...
from fair_stability_ordinal_common_v2 import (
    EXPERIMENT_ID,
    authorize as authorize_ordinal,
    load_schedule,
    verify_dispatch_claim,
)
from fair_stability_runtime_resource_monitor_v2 import (
...
RUNTIME_ADDENDUM = ROOT / "papers/fair_stability_runtime_exclusivity_addendum_v2.md"
...
VINS_ROOT = EXPERIMENT_ROOT / "vins_dev_nativeq_schedfix_runtimeexcl_v2"
BACKEND_FREEZE = VINS_ROOT / "backend_freeze.json"
REPLAY_WRAPPER = ROOT / "scripts/run_fair_stability_vins_replay_child_v2.sh"
RECORD_ENV = ROOT / "scripts/record_vins_env.sh"
WAIT_SUBSCRIBERS = ROOT / "scripts/wait_for_ros_subscribers.py"
RUNTIME_MONITOR = ROOT / "scripts/fair_stability_runtime_resource_monitor_v2.py"
ORDINAL_COMMON = ROOT / "scripts/fair_stability_ordinal_common_v2.py"
ORDINAL_CONTROLLER = ROOT / "scripts/run_fair_stability_next_v2.py"
HFNET_RUNNER = ROOT / "scripts/run_fair_stability_hfnet_openloop_v2.py"
CONTROL_TESTS = ROOT / "scripts/test_fair_stability_controls_v2.py"
UNIFIED_SUMMARY = ROOT / "scripts/summarize_fair_stability_v2.py"
...
def verify_common_inputs() -> tuple[dict[str, dict[str, str]], dict[str, dict[str, str]], dict[str, Any]]:
    ...
    if (
        ...
        or experiment["protocol"]["sha256"] != EXPECTED_PROTOCOL_SHA256
        or experiment["roster"]["sha256"] != EXPECTED_ROSTER_SHA256
    ):
...
def freeze_backend() -> dict[str, object]:
    ...
    freeze = {
        "schema_version": "aqua-fe-dev-nativeq-schedfix-runtimeexcl-backend-freeze-v2",
        "experiment_id": EXPERIMENT_ID,
        "status": "FROZEN_BEFORE_NEW_VINS_REPLAY",
        "frozen_at_utc": now_utc(),
        "backend_id": "DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V2",
        "identity_boundary": {
...
def load_freeze() -> dict[str, Any]:
    ...
    if (
        ...
        != "aqua-fe-dev-nativeq-schedfix-runtimeexcl-backend-freeze-v2"
        or freeze.get("experiment_id") != EXPERIMENT_ID
        or freeze.get("backend_id") != "DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V2"
    ):
...
def prepare_attempt(
    case_id: str, arm: str, repeat: int, attempt_index: int = 1
) -> dict[str, object]:
    freeze = load_freeze()
    ...
    if attempt_index > 1:
        ...
        if (
            ...
            or json.loads(previous.read_text(encoding="utf-8")).get("status")
            != "PIPELINE_INVALID"
        ):
    ...
    tag = (
        f"fair_stability_runtimeexcl_v2_{case_id}_{arm}_"
        f"repeat_{repeat:03d}_attempt_{attempt_index:03d}"
    )
    attempt = {
        "schema_version": "aqua-fe-fair-stability-vins-attempt-v2",
        "experiment_id": EXPERIMENT_ID,
...
def verify_attempt(
    ...
    if (
        attempt.get("schema_version") != "aqua-fe-fair-stability-vins-attempt-v2"
        or attempt.get("experiment_id") != EXPERIMENT_ID
        or attempt.get("backend_id") != "DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V2"
...
def write_unproven_monitor_receipt(
    ...
    receipt: dict[str, Any] = {
        "schema_version": "aqua-fe-fair-stability-runtime-resource-monitor-v2",
        "experiment_id": EXPERIMENT_ID,
...
def run_attempt(case_id: str, arm: str, repeat: int, attempt_index: int = 1) -> dict[str, object]:
    ...
    with lock_path.open("r+b") as lock_stream:
        ...
        claim = {
            "schema_version": "aqua-fe-fair-stability-vins-start-claim-v2",
            "experiment_id": EXPERIMENT_ID,
        ...
        if (
            monitor_receipt.get("monitor_proven") is not True
        ...
        if (
            ...
            or start_values.get("tag") != attempt["tag"]
            or not isinstance(lifecycle_values, Mapping)
            or lifecycle_values.get("tag") != attempt["tag"]
        ):
            pipeline_failures.append("ATTEMPT_TAG_BINDING_UNPROVEN")
        ...
        result = {
            "schema_version": "aqua-fe-fair-stability-vins-result-v2",
            "experiment_id": EXPERIMENT_ID,
...
def summary() -> dict[str, object]:
    ...
    report = {
        "schema_version": "aqua-fe-fair-stability-vins-progress-summary-v2",
        "experiment_id": EXPERIMENT_ID,
        "generated_at_utc": now_utc(), "rows": rows, "aggregate": aggregate,
        "claim_boundary": (
            "DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V2_NOT_P07_REPRODUCTION_"
            "NO_SYSTEM_RANKING"
        ),
    }
