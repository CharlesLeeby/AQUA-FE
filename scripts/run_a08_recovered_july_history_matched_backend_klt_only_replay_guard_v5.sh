...
# Additive v5 replay-only guard for R04--R05.  It reuses the frozen v4
# launcher/wrapper and binds all three consumed infrastructure dispositions.
...
: "${A08_KLT_ONLY_REMAINING_REPLAYS_V5:?missing KLT-only v5 authorization}"
...
: "${A08_PRIOR_R03_DISPOSITION:?missing consumed R03 disposition}"
...
: "${ROS_HOSTNAME:?missing locked ROS hostname}"
...
EXPECTED_OUTPUT_ROOT="/mnt/data/AQUA-FE_WS/experiments/a08_history_matched_controls_v1/backend_klt_only_remaining_replays_v5"
...
if [[ "$A08_STRICT_REPLAY_ONLY_GUARD" != "1" || \
      ...
      "$ROS_HOSTNAME" != "localhost" ]]; then
  echo "A08 KLT-only v5 replay policy mismatch" >&2
  exit 65
fi
if [[ "$A08_PRIOR_R01_DISPOSITION" != \
  ...
  echo "A08 KLT-only v5 scientific boundary mismatch" >&2
  exit 65
fi
if [[ "$DATASET" != "aqualoc_archaeo" || "$SEQ" != "8" || \
      "$START" != "0" || "$END" != "4660" || "$EVERY_N" != "2" || \
      "$METHOD" != "klt" ]]; then
  echo "A08 KLT-only v5 history or method mismatch" >&2
  exit 65
fi
if [[ ! "$A08_ITEM_ID" =~ ^KLT_R0[4-5]$ ]]; then
  echo "A08 KLT-only v5 item mismatch" >&2
  exit 65
fi
REPEAT="${A08_ITEM_ID#KLT_R}"
EXPECTED_TAG="a08_recoveredjuly_hist0000_4660_backend_klt_only_r${REPEAT}_v5"
...
if [[ "$A08_VINS_LIFECYCLE_WRAPPER_V4" != "1" || \
  ...
  echo "A08 KLT-only v5 lifecycle executable mismatch" >&2
  exit 65
fi
if [[ "$A08_UNBLOCKED_OVERLAY_EXEC_V4" != "1" || \
  ...
  echo "A08 KLT-only v5 signal-mask launcher mismatch" >&2
  exit 65
fi
if [[ "$A08_VINS_LIFECYCLE_START_MANIFEST" != \
  ...
  echo "A08 KLT-only v5 lifecycle manifest path mismatch" >&2
  exit 65
fi
if [[ "$A08_SIGNAL_MASK_MANIFEST" != \
      "$EXPECTED_OUTPUT/overlay_signal_mask_manifest_v4.json" ]]; then
  echo "A08 KLT-only v5 signal-mask manifest path mismatch" >&2
  exit 65
fi
if [[ ! -d "$A08_ITEM_OUTPUT_DIR" || -L "$A08_ITEM_OUTPUT_DIR" || \
  ...
  echo "A08 KLT-only v5 evidence workspace binding mismatch" >&2
  exit 65
fi
...
payload = {
    ...
    "prior_dispositions": {
        ...
        "KLT_R03": os.environ["A08_PRIOR_R03_DISPOSITION"],
    },
    "runtime_environment_contract": {
        "locked_ros_hostname": os.environ["ROS_HOSTNAME"],
...
if [[ ! -f "$A08_VINS_LIFECYCLE_START_MANIFEST" || \
  ...
  echo "A08 v5 lifecycle manifest missing after backend" >&2
  exit 70
fi
if [[ ! -f "$A08_SIGNAL_MASK_MANIFEST" || \
      -L "$A08_SIGNAL_MASK_MANIFEST" ]]; then
  echo "A08 v5 signal-mask manifest missing after backend" >&2
  exit 70
fi
