#!/usr/bin/python3
"""Single ordinal dispatcher for the systemd-supervised v6 experiment."""
...
from fair_stability_ordinal_common_v6 import (  # noqa: E402
...
MATRIX_FREEZE = EXPERIMENT_ROOT / "attempt_matrix_freeze_v6.json"
HFNET_RUNNER = ROOT / "scripts/run_fair_stability_hfnet_openloop_v6.py"
VINS_RUNNER = ROOT / "scripts/run_fair_stability_vins_replay_v6.py"
...
def verify_systemd_authority() -> dict[str, object]:
    ...
    if (
        claim.get("schema_version") != "aqua-fe-fair-stability-systemd-submission-v6"
...
def verify_control_freeze() -> dict[str, Any]:
    ...
    for path in (
        ...
        HFNET_RUNNER,
        SYSTEMD_SUPERVISOR,
        SCRIPT_DIR / "fair_stability_ordinal_common_v6.py",
    ):
...
def adopt_terminal(
    ...
    if str(cell["arm"]).startswith("hfnet_openloop_"):
        manifest = _load_json_object(root / "attempt_manifest.json")
        reread_result = _load_json_object(root / "run_result.json")
        if not strict_json_equal(reread_result, result):
            raise ControllerError("HFNET_RESULT_CHANGED_BEFORE_ADOPTION")
        validate_hfnet_watchdog_evidence(
            root,
            manifest,
            reread_result,
    ...
    payload = {
        "schema_version": "aqua-fe-fair-stability-ordinal-terminal-v6",
...
def adjudicate_pipeline_invalid(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        "schema_version": "aqua-fe-fair-stability-pipeline-invalid-adjudication-v6",
        "experiment_id": EXPERIMENT_ID,
        "adjudicated_at_utc": now_utc(),
        "accepted_as_external_pipeline_fault": True,
        "automatic_retry_policy": "EXPLICIT_EXTERNAL_TRANSIENT_ONLY_V6",
...
def dispatch_claim(state: Mapping[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    cell = state["cell"]
    claim = {
        "schema_version": "aqua-fe-fair-stability-ordinal-dispatch-v6",
        "experiment_id": EXPERIMENT_ID,
...
def dispatch(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    check_contract_valid = (
        ...
        == "aqua-fe-fair-stability-prestart-resource-gate-v6"
    ...
    execution = {
        "schema_version": "aqua-fe-fair-stability-controller-dispatch-execution-v6",
        "experiment_id": EXPERIMENT_ID,
