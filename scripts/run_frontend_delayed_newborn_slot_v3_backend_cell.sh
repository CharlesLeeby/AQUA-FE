...
RUNTIME_ROOT="/media/ma/Data/AQUA-FE_WS_storage_offload/frontend_delayed_newborn_slot_v3"
...
case "$SCRATCH_DIR" in
  "$RUNTIME_ROOT"/backend_scratch/"$RUN_SLUG") ;;
...
case "$DECLARED_OUTPUT_DIR" in
  "/media/ma/Data/AQUA-FE_WS_storage_offload/frontend_coverage_monotone_router_v2/backend_scratch/$RUN_SLUG/vins_output") ;;
  *) echo "unexpected frozen YAML output path: $DECLARED_OUTPUT_DIR" >&2; exit 64 ;;
esac
DECLARED_SCRATCH_DIR="$(dirname "$DECLARED_OUTPUT_DIR")"
mkdir -p "$RUN_DIR" "$SCRATCH_DIR" "$DECLARED_SCRATCH_DIR"
...
{
  printf 'schema_version=aqua-fe-delayed-newborn-slot-v3-backend-cell-v1\n'
  printf 'window_id=%s\n' "$WINDOW_ID"
  printf 'run_slug=%s\n' "$RUN_SLUG"
  printf 'cell_id=%s\n' "$CELL_ID"
  printf 'repeat=%s\n' "$REPEAT"
