...
RUNTIME_ROOT="$ROOT/artifacts/frontend_persistence_conditioned_replacement_v1"
...
case "$RUN_DIR" in
  "$RUNTIME_ROOT"/replays/"$WINDOW_ID"/persistence_replace/repeat"$REPEAT") ;;
  *) echo "unsafe run path: $RUN_DIR" >&2; exit 64 ;;
esac

[[ -s "$FEATURE_BAG" ]] || { echo "missing feature bag: $FEATURE_BAG" >&2; exit 66; }
[[ -s "$CANONICAL_CONFIG" ]] || { echo "missing config: $CANONICAL_CONFIG" >&2; exit 66; }
[[ -x "$VINS_NODE" ]] || { echo "missing VINS node: $VINS_NODE" >&2; exit 66; }
if [[ -s "$RUN_DIR/replay_receipt.txt" ]]; then
  echo "resume backend $WINDOW_ID persistence_replace repeat$REPEAT"
  exit 0
fi
...
{
  printf 'schema_version=aqua-fe-persistence-replace-backend-cell-v1\n'
  printf 'window_id=%s\n' "$WINDOW_ID"
  printf 'arm=persistence_replace\n'
  printf 'repeat=%s\n' "$REPEAT"
...
echo "finish backend $WINDOW_ID persistence_replace repeat$REPEAT"
