#!/usr/bin/python3
"""Fail-closed schedule state for fair-stability runtime-exclusive v12."""
...
from fair_stability_runtime_resource_monitor_v12 import (
...
V10_NON_IMPORT_CLAIM_KEYS = V9_NON_IMPORT_CLAIM_KEYS | frozenset(
    {
...
V11_NON_IMPORT_CLAIM_KEYS = V10_NON_IMPORT_CLAIM_KEYS | frozenset(
    {
...
def valid_v10_non_import_claims(value: object) -> bool:
    return bool(
        isinstance(value, Mapping)
        and all(value.get(key) is False for key in V10_NON_IMPORT_CLAIM_KEYS)
    )


def valid_v11_non_import_claims(value: object) -> bool:
    return bool(
        isinstance(value, Mapping)
        and all(value.get(key) is False for key in V11_NON_IMPORT_CLAIM_KEYS)
    )
...
CONTROLLER_DISPATCH_EXECUTION_SCHEMA = (
    "aqua-fe-fair-stability-controller-dispatch-execution-v12"
)
NO_START_PROVENANCE_SCHEMA = (
    "aqua-fe-fair-stability-systemd-no-start-provenance-v12"
)
NO_START_OUTCOMES = {
...
SYSTEMD_ADOPTION_CLAIM_SCHEMA = (
    "aqua-fe-fair-stability-systemd-adoption-claim-v12"
)
SYSTEMD_ADOPTION_SCHEMA = "aqua-fe-fair-stability-systemd-adoption-v12"
RESUBMIT_AUTHORIZATION_SCHEMA = (
    "aqua-fe-fair-stability-systemd-resubmit-authorization-v12"
)
RESUBMIT_AUTHORIZATION_CONSUMPTION_SCHEMA = (
    "aqua-fe-fair-stability-systemd-resubmit-authorization-consumption-v12"
)
...
def valid_ready_prestart_resource_gate_v12(value: object) -> bool:
    """Recognize the complete, internally consistent v12 ready gate."""
    return prestart_resource_gate_contract_valid(
        value, expected_ready=True
    )


def valid_prestart_resource_gate_v12(
    value: object,
    *,
    expected_ready: bool,
...
def control_boundary(experiment_root: Path) -> dict[str, dict[str, object]]:
    ...
    controller = common.parent / "run_fair_stability_next_v12.py"
    supervisor = common.parent / "run_fair_stability_systemd_supervisor_v12.py"
    freeze_path = (
        experiment_root / "vins_dev_nativeq_schedfix_runtimeexcl_v12" / "backend_freeze.json"
    )
    ...
    ) or not valid_v11_non_import_claims(claims):
    ...
    return {
        ...
        "attempt_matrix_freeze": identity(experiment_root / "attempt_matrix_freeze_v12.json"),
    }
...
SYSTEMD_CHAIN_FILES = {
    "submission_claim": (
        ...
        "aqua-fe-fair-stability-systemd-submission-v12",
    ),
    "submission_receipt": (
        "submission_receipt.json",
        "aqua-fe-fair-stability-systemd-submit-receipt-v12",
    ),
    "systemd_start_receipt": (
        "systemd_start_receipt.json",
        "aqua-fe-fair-stability-systemd-start-receipt-v12",
    ),
    "systemd_execution_receipt": (
        "systemd_execution_receipt.json",
        "aqua-fe-fair-stability-systemd-execution-v12",
    ),
    "systemd_terminal_receipt": (
        "systemd_terminal_receipt.json",
        "aqua-fe-fair-stability-systemd-terminal-v12",
    ),
}
...
def _validate_pipeline_adjudication_submission(
    ...
    """V12 has one attempt per coordinate; pipeline adjudication is disabled."""
    raise OrdinalError(
        "PIPELINE_ADJUDICATION_DISABLED_ZERO_REPLACEMENT_V12:"
        f"{submission_directory}"
    )

    # Retained below as unreachable historical validation logic so older v12
    ...
    if (
        ...
        != "aqua-fe-fair-stability-systemd-adoption-v12"
...
_SYSTEMD_UNIT_COORDINATE = re.compile(
    r"aqua-fe-fair-stability-v12-o(?P<ordinal>[0-9]{3})-"
...
def _runner_check_argv_from_authorized_state(
    ...
    if arm.startswith("hfnet_openloop_"):
        return [
            "/usr/bin/python3",
            str(scripts / "run_fair_stability_hfnet_openloop_v12.py"),
            "check",
            *common,
            "--budget",
            arm.rsplit("_", 1)[1],
    ...
    return [
        "/usr/bin/python3",
        str(scripts / "run_fair_stability_vins_replay_v12.py"),
        "check",
        *common,
        "--arm",
        arm,
...
def _validate_resubmit_authorization_evidence(
    ...
    if (
        ...
        or not valid_ready_prestart_resource_gate_v12(gate)
    ):
        raise OrdinalError(
            f"SYSTEMD_RESUBMIT_AUTHORIZATION_EVIDENCE_INVALID:"
            f"{authorization_path}"
...
def _validate_no_start_dispatch_artifact_attribution(
    ...
    controller_path = (
        Path(__file__).resolve().parent / "run_fair_stability_next_v12.py"
    )
...
def _validate_no_start_provenance(
    ...
    if outcome == "RESOURCE_BLOCKED_NO_START":
        ...
        if (
            ...
            or not valid_prestart_resource_gate_v12(
                resource_gate,
...
def load_schedule(experiment_root: Path) -> list[dict[str, Any]]:
    ...
    if not isinstance(parent_claims, Mapping) or any(
        ...
        for version in range(1, 12)
    ) or not valid_v11_non_import_claims(parent_claims):
...
def load_attempt_matrix(experiment_root: Path) -> dict[str, Any]:
    """Verify the immutable 120-attempt v12 matrix without freezing later retries."""
    path = experiment_root / "attempt_matrix_freeze_v12.json"
    require_no_symlink_components(path, experiment_root)
    if path.is_symlink() or not path.is_file():
    ...
    if (
        ...
        != "aqua-fe-fair-stability-attempt-matrix-freeze-v12"
        or value.get("experiment_id") != EXPERIMENT_ID
        or value.get("status") != "FROZEN_BEFORE_ANY_V12_ESTIMATOR_START"
    ):
...
def attempt_root(experiment_root: Path, cell: Mapping[str, Any], attempt_index: int) -> Path:
    ...
    if arm.startswith("hfnet_openloop_"):
        ...
        base = (
            experiment_root / "vins_dev_nativeq_schedfix_runtimeexcl_v12" / "attempts"
...
def validate_result_semantic_evidence(
    ...
    try:
        if arm in {"learned_klt_vins", "pure_klt_vins"}:
            from run_fair_stability_vins_replay_v12 import (
            ...
            from run_fair_stability_hfnet_openloop_v12 import (
                accepted_events,
                parse_log,
                parse_trajectory,
                read_ns_lines,
...
def validate_runtime_resource_monitor_evidence(
    ...
    fail-stop pipeline result, and v12 has no replacement attempts.
    """
    ...
    if hfnet_arm:
        ...
        if vins_child_present:
            ...
            if (
                ...
                != "aqua-fe-fair-stability-vins-child-start-v12"
    ...
    def valid_proc_sample(sample: Mapping[str, Any]) -> bool:
        role = sample.get("probe_role")
        expected_keys = (
            closing_proc_sample_keys
            if role == CLOSING_PROC_PROBE_ROLE
            else ordinary_proc_sample_keys
            if role == ORDINARY_PROC_PROBE_ROLE
            else None
        )
        if expected_keys is None or set(sample) != expected_keys:
            return False
        ...
        if (
            ...
            return False
        if role == CLOSING_PROC_PROBE_ROLE and (
            sample.get("coverage_close_ownership_empty") is not True
            or sample.get("coverage_close_recheck_proven") is not True
            or owned != []
            or group != []
            or errors != []
        ):
    ...
    ordinary_proc_sample_keys = {
        ...
        "external_conflicting_processes", "probe_errors", "probe_role",
    }
    closing_proc_sample_keys = ordinary_proc_sample_keys | {
        "coverage_close_ownership_empty", "coverage_close_recheck_proven",
    }
    gpu_sample_keys = {
        "sequence", "probe_started_at_utc", "probe_started_monotonic_ns",
    ...
    proc_rows = samples["proc"]
    closing_indices = [
        index
        for index, row in enumerate(proc_rows)
        if row.get("probe_role") == CLOSING_PROC_PROBE_ROLE
    ]
    closing_contract_proven = bool(
        proc_rows
        and group_empty is True
        and closing_indices == [len(proc_rows) - 1]
        and all(
            row.get("probe_role") == ORDINARY_PROC_PROBE_ROLE
            for row in proc_rows[:-1]
        )
        and proc_rows[-1].get("coverage_close_ownership_empty") is True
        and proc_rows[-1].get("coverage_close_recheck_proven") is True
        and proc_rows[-1].get("owned_identities") == []
        and proc_rows[-1].get("process_group_members") == []
        and proc_rows[-1].get("probe_errors") == []
        and not any(
            error.startswith("COVERAGE_CLOSE_")
            or error.startswith("CLEAN_COVERAGE_CLOSE_")
            for error in probe_errors
        )
    )
    if not closing_contract_proven:
        raise OrdinalError(
            f"RUNTIME_RESOURCE_MONITOR_CLOSING_PROC_CONTRACT_INVALID:{path}"
        )
...
def validate_controller_dispatch_execution_for_result(
    ...
    dispatch_claim_path = root / "ordinal_dispatch_claim.json"
    controller_path = Path(__file__).resolve().parent / "run_fair_stability_next_v12.py"
...
def validate_hfnet_watchdog_evidence(
    ...
    if (
        ...
        != "aqua-fe-fair-stability-hfnet-launch-receipt-v12"
    ...
    pending_reap = (
        outcome.get("pending_reap_adjudication")
        if isinstance(outcome, Mapping)
        else object()
    )
    ...
    same_popen_reap_sources = {
        ...
        "PENDING_REAP_POST_TIMEOUT_POLL_THEN_WAIT_ZERO",
        "PENDING_REAP_BOUNDED_WAIT",
        "FINAL_GATE_POLL_THEN_WAIT_ZERO",
    ...
    pending_reap_keys = {
        "schema_version", "phase", "status", "classification",
        "candidate_evidence", "anchor_source",
        "anchor_completed_monotonic_ns", "global_deadline_monotonic_ns",
        "local_maximum_deadline_monotonic_ns",
        "effective_deadline_monotonic_ns", "maximum_wait_seconds",
        "deadline_inclusive", "initial_poll", "wait_events", "poll_events",
        "reap", "adjudicated_monotonic_ns", "child_returncode",
        "signal_authorized", "signal_attempted", "signal_sent", "errors",
    }
    pending_initial_poll_keys = {"observed_monotonic_ns", "returncode"}
    pending_poll_event_keys = {
        "sequence", "source", "observed_monotonic_ns", "returncode",
        "group_recheck",
    }
    pending_wait_event_keys = {
        "sequence", "started_monotonic_ns", "requested_timeout_seconds",
        "completed_monotonic_ns", "outcome", "returncode",
    }
    pending_phases = {
        "PERIODIC_EXACT_GROUP_PROOF",
        "FINAL_SIGNAL_GATE:FIRST_FINAL_GROUP_PROOF",
        "FINAL_SIGNAL_GATE:SECOND_FINAL_GROUP_PROOF",
    }
    pending_anchor_sources = {
        "PERIODIC_EXACT_GROUP_PROOF": (
            "candidate_evidence.observed_monotonic_ns"
        ),
        "FINAL_SIGNAL_GATE:FIRST_FINAL_GROUP_PROOF": (
    ...
    def valid_watchdog_global_deadline() -> bool:
        if not isinstance(outcome, Mapping):
            return False
        started_ns = outcome.get("watchdog_started_monotonic_ns")
        global_ns = outcome.get("global_deadline_monotonic_ns")
        total_seconds = fixed_contract.get("estimator_total_timeout_seconds")
        return bool(
            type(started_ns) is int
            and type(global_ns) is int
            and 0 < int(started_ns) < int(global_ns)
            and type(total_seconds) is int
            and int(global_ns) - int(started_ns)
            == int(total_seconds) * 1_000_000_000
        )

    def valid_pending_reap_adjudication(candidate: object) -> bool:
        """Validate the bounded no-signal reap window from its original proof.

        In particular, no poll or wait observation may become a fresh anchor:
        the local deadline is recomputed from the terminal-transition proof.
        """

        if candidate is None:
            final_gate_pending = bool(
                isinstance(final_gate_adjudication, Mapping)
                and final_gate_adjudication.get("classification")
                == "TERMINAL_TRANSITION_PENDING_REAP"
            )
            if not final_gate_pending:
                return True
            ...
            gate_maximum = finite_seconds(
                final_gate_adjudication.get(
                    "maximum_final_gate_duration_seconds"
                )
            )
            if (
                gate_deadline is not None
                and gate_pre_signal is not None
                and gate_elapsed is not None
                and gate_maximum is not None
                and (
                    gate_pre_signal >= gate_deadline
                    or gate_elapsed > gate_maximum
                )
            ):
                return True
            return bool(
                isinstance(passive_exit, Mapping)
                and str(passive_exit.get("phase", "")).startswith(
                    "FINAL_SIGNAL_GATE:"
                )
                and strict_json_equal(
                    passive_exit.get("evidence"), final_gate_adjudication
                )
                and isinstance(passive_exit.get("reap"), Mapping)
                and passive_exit["reap"].get("source")
                == "FINAL_GATE_PENDING_POLL_THEN_WAIT_ZERO"
                and valid_passive_exit(passive_exit)
            )
        if (
            not isinstance(candidate, Mapping)
            or set(candidate) != pending_reap_keys
            or candidate.get("schema_version")
            != "aqua-fe-fair-stability-hfnet-watchdog-pending-reap-v12"
            or candidate.get("phase") not in pending_phases
            or candidate.get("anchor_source")
            != pending_anchor_sources.get(candidate.get("phase"))
            or candidate.get("maximum_wait_seconds") != 0.25
            or candidate.get("deadline_inclusive") is not True
            or candidate.get("signal_authorized") is not False
            or candidate.get("signal_attempted") is not False
            or candidate.get("signal_sent") is not False
            or not isinstance(candidate.get("errors"), list)
            or any(
                not isinstance(error, str) or not error
                for error in candidate.get("errors", [])
            )
            or not valid_watchdog_global_deadline()
        ):
            return False
        ...
        if phase == "PERIODIC_EXACT_GROUP_PROOF":
            if not valid_terminal_transition_proof(evidence):
                return False
            anchor_ns = proof_monotonic_ns(evidence)
        else:
            if (
                ...
                != "TERMINAL_TRANSITION_PENDING_REAP"
                or phase != f"FINAL_SIGNAL_GATE:{evidence.get('phase')}"
            ):
                return False
            transition = evidence.get("terminal_transition_snapshot")
            if not valid_terminal_transition_proof(transition):
                return False
            anchor_ns = proof_monotonic_ns(transition)

        started_ns = outcome.get("watchdog_started_monotonic_ns")
        outcome_global_ns = outcome.get("global_deadline_monotonic_ns")
        recorded_anchor_ns = candidate.get("anchor_completed_monotonic_ns")
        recorded_global_ns = candidate.get("global_deadline_monotonic_ns")
        local_ns = candidate.get("local_maximum_deadline_monotonic_ns")
        effective_ns = candidate.get("effective_deadline_monotonic_ns")
        adjudicated_ns = candidate.get("adjudicated_monotonic_ns")
        if (
            anchor_ns is None
            or type(recorded_anchor_ns) is not int
            or recorded_anchor_ns != anchor_ns
            or type(recorded_global_ns) is not int
            or recorded_global_ns != outcome_global_ns
            or type(local_ns) is not int
            or local_ns != anchor_ns + 250_000_000
            or type(effective_ns) is not int
            or effective_ns != min(int(outcome_global_ns), int(local_ns))
            or type(adjudicated_ns) is not int
            or int(started_ns) > anchor_ns
            or anchor_ns > int(outcome_global_ns)
            or int(adjudicated_ns) < anchor_ns
        ):
            return False
        if phase.startswith("FINAL_SIGNAL_GATE:"):
            gate_deadline = finite_seconds(evidence.get("deadline_monotonic"))
            if (
                gate_deadline is None
                or abs(gate_deadline - int(outcome_global_ns) / 1_000_000_000.0)
                > 0.000001
            ):
                return False

        initial_poll = candidate.get("initial_poll")
        poll_events = candidate.get("poll_events")
        wait_events = candidate.get("wait_events")
        if (
            not isinstance(initial_poll, Mapping)
            or set(initial_poll) != pending_initial_poll_keys
            or type(initial_poll.get("observed_monotonic_ns")) is not int
            or int(initial_poll.get("observed_monotonic_ns", 0)) < anchor_ns
            or initial_poll.get("returncode") is not None
            or not isinstance(poll_events, list)
            or not poll_events
            or not isinstance(wait_events, list)
            or len(wait_events) not in {
                len(poll_events) - 1,
                len(poll_events),
            }
        ):
            return False
        trailing_wait_exception = bool(
            len(wait_events) == len(poll_events)
            and wait_events
            and isinstance(wait_events[-1], Mapping)
            and wait_events[-1].get("outcome") == "EXCEPTION"
        )
        if len(wait_events) == len(poll_events) and not trailing_wait_exception:
            return False

        previous_poll_ns: int | None = None
        for sequence, event in enumerate(poll_events):
            if (
                not isinstance(event, Mapping)
                or set(event) != pending_poll_event_keys
                or type(event.get("sequence")) is not int
                or event.get("sequence") != sequence
                or event.get("source")
                not in {
                    "INITIAL_NONE_POLL",
                    "POST_WAIT_TIMEOUT_POLL",
                    "WAIT_RETURNED_SAME_POPEN",
                }
                or type(event.get("observed_monotonic_ns")) is not int
            ):
                return False
            observed_ns = int(event["observed_monotonic_ns"])
            if previous_poll_ns is not None and observed_ns <= previous_poll_ns:
                return False
            if sequence == 0:
                if (
                    event.get("source") != "INITIAL_NONE_POLL"
                    or event.get("returncode") is not None
                    or event.get("group_recheck") is not None
                    or observed_ns != initial_poll.get("observed_monotonic_ns")
                ):
                    return False
            elif event.get("source") in {
                "POST_WAIT_TIMEOUT_POLL",
            }:
                recheck = event.get("group_recheck")
                recheck_ns = proof_monotonic_ns(recheck)
                poll_returncode = event.get("returncode")
                if type(poll_returncode) is int:
                    if recheck is not None:
                        return False
                elif poll_returncode is None:
                    final_deadline_poll = observed_ns >= int(effective_ns)
                    if recheck is None and not final_deadline_poll:
                        return False
                    if recheck is not None and (
                        recheck_ns is None
                        or recheck_ns < observed_ns
                        or not (
                            valid_terminal_transition_proof(recheck)
                            or valid_group_proof(recheck)
                            or failed_group_proof_valid(recheck)
                        )
                    ):
                        return False
                else:
                ...
                event.get("source") != "WAIT_RETURNED_SAME_POPEN"
                or type(event.get("returncode")) is not int
                or event.get("group_recheck") is not None
            ):
                return False
            previous_poll_ns = observed_ns

        paired_wait_events = (
            wait_events[:-1] if trailing_wait_exception else wait_events
        )
        for sequence, wait_event in enumerate(paired_wait_events):
            prior_poll = poll_events[sequence]
            following_poll = poll_events[sequence + 1]
            if (
                not isinstance(wait_event, Mapping)
                or set(wait_event) != pending_wait_event_keys
                or type(wait_event.get("sequence")) is not int
                or wait_event.get("sequence") != sequence
                or type(wait_event.get("started_monotonic_ns")) is not int
                or type(wait_event.get("completed_monotonic_ns")) is not int
                or type(wait_event.get("requested_timeout_seconds")) is not float
                or not math.isfinite(wait_event["requested_timeout_seconds"])
                or not 0.0 < wait_event["requested_timeout_seconds"] <= 0.25
                or int(wait_event["started_monotonic_ns"])
                < int(prior_poll["observed_monotonic_ns"])
                or int(wait_event["completed_monotonic_ns"])
                < int(wait_event["started_monotonic_ns"])
                or int(following_poll["observed_monotonic_ns"])
                < int(wait_event["completed_monotonic_ns"])
                or wait_event["requested_timeout_seconds"]
                > max(
                    0.0,
                    (int(effective_ns) - int(wait_event["started_monotonic_ns"]))
                    / 1_000_000_000.0,
                )
                + 1e-9
                or wait_event.get("outcome") not in {"RETURNED", "TIMEOUT"}
            ):
                return False
            following_source = following_poll.get("source")
            if wait_event.get("outcome") == "TIMEOUT":
                if (
                    wait_event.get("returncode") is not None
                    or following_source
                    not in {
                        "POST_WAIT_TIMEOUT_POLL",
                    }
                ...
                type(wait_event.get("returncode")) is not int
                or following_source != "WAIT_RETURNED_SAME_POPEN"
                or following_poll.get("returncode")
                != wait_event.get("returncode")
                or sequence != len(paired_wait_events) - 1
            ):
                return False
            recheck = following_poll.get("group_recheck")
            if isinstance(recheck, Mapping) and sequence + 1 < len(wait_events):
                recheck_ns = proof_monotonic_ns(recheck)
                if (
                    recheck_ns is None
                    or int(wait_events[sequence + 1]["started_monotonic_ns"])
                    < recheck_ns
                ):
                    return False

        if trailing_wait_exception:
            exception_event = wait_events[-1]
            prior_poll = poll_events[-1]
            if (
                not isinstance(exception_event, Mapping)
                or set(exception_event) != pending_wait_event_keys
                or exception_event.get("sequence") != len(wait_events) - 1
                or type(exception_event.get("sequence")) is not int
                or type(exception_event.get("started_monotonic_ns")) is not int
                or type(exception_event.get("completed_monotonic_ns")) is not int
                or type(exception_event.get("requested_timeout_seconds")) is not float
                or not math.isfinite(exception_event["requested_timeout_seconds"])
                or not 0.0 < exception_event["requested_timeout_seconds"] <= 0.25
                or int(exception_event["started_monotonic_ns"])
                < int(prior_poll["observed_monotonic_ns"])
                or int(exception_event["completed_monotonic_ns"])
                < int(exception_event["started_monotonic_ns"])
                or exception_event["requested_timeout_seconds"]
                > max(
                    0.0,
                    (
                        int(effective_ns)
                ...
                + 1e-9
                or exception_event.get("outcome") != "EXCEPTION"
                or exception_event.get("returncode") is not None
                or int(adjudicated_ns)
                < int(exception_event["completed_monotonic_ns"])
            ):
                return False

        status = candidate.get("status")
        classification = candidate.get("classification")
        child_returncode = candidate.get("child_returncode")
        errors = candidate.get("errors")
        reap = candidate.get("reap")
        final_poll = poll_events[-1]
        if status == "PASSIVE_REAP_PROVEN":
            if (
                classification
                != "SAME_POPEN_REAPED_WITHIN_INCLUSIVE_DEADLINE"
                or errors != []
                or type(child_returncode) is not int
                or final_poll.get("source")
                not in {
                    "WAIT_RETURNED_SAME_POPEN",
                    "POST_WAIT_TIMEOUT_POLL",
                }
                or final_poll.get("returncode") != child_returncode
                or int(final_poll["observed_monotonic_ns"]) > int(effective_ns)
                or int(adjudicated_ns) > int(effective_ns)
                or not valid_same_popen_reap(
                    reap, int(child_returncode), int(adjudicated_ns)
                )
                or not isinstance(passive_exit, Mapping)
                or passive_exit.get("phase") != phase
                or not strict_json_equal(passive_exit.get("evidence"), evidence)
                or passive_exit.get("child_returncode") != child_returncode
                or not strict_json_equal(passive_exit.get("reap"), reap)
                or passive_exit.get("adjudicated_monotonic_ns")
                != adjudicated_ns
                or passive_exit.get("signal_attempted") is not False
                or passive_exit.get("signal_sent") is not False
                or not isinstance(execution, Mapping)
                or execution.get("raw_returncode") != child_returncode
                or execution.get("child_reaped") is not True
                or execution.get("timed_out") is not False
                or not proven
                or sent
        ...
        if (
            ...
            or classification
            not in {
                "STILL_LIVE_AT_INCLUSIVE_DEADLINE",
                "UNPROVEN_PENDING_REAP",
            }
            or not errors
            or reap is not None
            ...
            or proven
            or sent
            or any(f"PENDING_REAP:{error}" not in watchdog_errors for error in errors)
        ):
            return False
        if trailing_wait_exception and classification != "UNPROVEN_PENDING_REAP":
            return False
        if classification == "STILL_LIVE_AT_INCLUSIVE_DEADLINE":
            return bool(
                child_returncode is None
                and final_poll.get("returncode") is None
                and int(final_poll["observed_monotonic_ns"])
                >= int(effective_ns)
                and int(adjudicated_ns) >= int(effective_ns)
            )
        return type(child_returncode) in (int, type(None))
    ...
    if (
        ...
        or "pending_reap_adjudication" not in value
        or "pending_reap_adjudication" not in outcome
        or not strict_json_equal(
            value.get("pending_reap_adjudication"), pending_reap
        )
        or not valid_watchdog_global_deadline()
        or not valid_pending_reap_adjudication(pending_reap)
    ...
    while attempt_index <= MAX_ATTEMPTS_PER_CELL:
        ...
        if result_path.is_file():
            ...
            expected_result_schema = (
                "aqua-fe-fair-stability-hfnet-result-v12"
                if hfnet_arm
                else "aqua-fe-fair-stability-vins-result-v12"
            )
            if (
            ...
            if invalid_receipt_path.is_file():
                raise OrdinalError(
                    f"PIPELINE_ADJUDICATION_DISABLED_ZERO_REPLACEMENT_V12:{root}"
                )
            ...
            if (
                ...
                != "EXPLICIT_EXTERNAL_TRANSIENT_ONLY_V12"
...
def verify_dispatch_claim(
    ...
    if claim.get("schema_version") != "aqua-fe-fair-stability-ordinal-dispatch-v12":
