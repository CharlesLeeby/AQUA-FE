#!/usr/bin/python3
"""Durable systemd-user supervision for one fair-stability v8 ordinal.
...
CONTROLLER = ROOT / "scripts/run_fair_stability_next_v8.py"
...
UNIT_PREFIX = "aqua-fe-fair-stability-v8"

SUBMISSION_SCHEMA = "aqua-fe-fair-stability-systemd-submission-v8"
SUBMIT_RECEIPT_SCHEMA = "aqua-fe-fair-stability-systemd-submit-receipt-v8"
START_RECEIPT_SCHEMA = "aqua-fe-fair-stability-systemd-start-receipt-v8"
EXECUTION_SCHEMA = "aqua-fe-fair-stability-systemd-execution-v8"
TERMINAL_SCHEMA = "aqua-fe-fair-stability-systemd-terminal-v8"
ADOPTION_CLAIM_SCHEMA = "aqua-fe-fair-stability-systemd-adoption-claim-v8"
ADOPTION_SCHEMA = "aqua-fe-fair-stability-systemd-adoption-v8"
...
def host_preflight() -> dict[str, object]:
    ...
    return {
        "schema_version": "aqua-fe-fair-stability-systemd-host-preflight-v8",
        ...
        "login": {
            ...
            "limitation": (
                "LAST_LOGIN_SESSION_EXIT_MAY_STOP_USER_MANAGER_AND_INVALIDATE_V8"
...
def systemd_run_argv(unit: str, directory: Path) -> list[str]:
    ...
    return [
        ...
        f"--description=AQUA-FE fair-stability v8 one ordinal {unit}",
...
def poll(directory: Path | None = None) -> dict[str, object]:
    ...
    return {
        "schema_version": "aqua-fe-fair-stability-systemd-poll-v8",
        "experiment_id": EXPERIMENT_ID,
...
def adopt(directory: Path | None = None) -> dict[str, object]:
    ...
    if service_result == "timeout" or exit_code in {"killed", "dumped"}:
        raise SystemdSupervisorError("SYSTEMD_TIMEOUT_OR_SIGNAL_FAIL_STOP_ABANDON_V8")
    ...
    if state.get("state") == "STARTED_WITHOUT_RESULT":
        raise SystemdSupervisorError("STARTED_WITHOUT_RESULT_FAIL_STOP_ABANDON_V8")
    ...
    if state.get("state") == "TERMINAL_UNADOPTED":
        from run_fair_stability_next_v8 import adopt_terminal
