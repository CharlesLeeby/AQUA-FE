#!/usr/bin/python3
"""Fail-closed schedule state for fair-stability runtime-exclusive v7."""
...
EXPERIMENT_ID = "fair-stability-positive-roster-openloop-runtimeexcl-v7"
...
CONTROLLER_DISPATCH_EXECUTION_SCHEMA = (
    "aqua-fe-fair-stability-controller-dispatch-execution-v7"
)
...
def control_boundary(experiment_root: Path) -> dict[str, dict[str, object]]:
    ...
    controller = common.parent / "run_fair_stability_next_v7.py"
    supervisor = common.parent / "run_fair_stability_systemd_supervisor_v7.py"
    freeze_path = (
        experiment_root / "vins_dev_nativeq_schedfix_runtimeexcl_v7" / "backend_freeze.json"
    )
    ...
    return {
        ...
        "attempt_matrix_freeze": identity(experiment_root / "attempt_matrix_freeze_v7.json"),
    }
...
SYSTEMD_CHAIN_FILES = {
    ...
    "submission_receipt": (
        ...
        "aqua-fe-fair-stability-systemd-submit-receipt-v7",
    ),
    ...
    "systemd_execution_receipt": (
        ...
        "aqua-fe-fair-stability-systemd-execution-v7",
    ),
    "systemd_terminal_receipt": (
        "systemd_terminal_receipt.json",
        "aqua-fe-fair-stability-systemd-terminal-v7",
    ),
}
...
def _validate_pipeline_adjudication_submission(
    ...
    """V7 has one attempt per coordinate; pipeline adjudication is disabled."""
    raise OrdinalError(
        "PIPELINE_ADJUDICATION_DISABLED_ZERO_REPLACEMENT_V7:"
        f"{submission_directory}"
    )

    # Retained below as unreachable historical validation logic so older v7
    ...
    if (
        ...
        != "aqua-fe-fair-stability-pipeline-invalid-adjudication-v7"
    ...
    if (
        ...
        != "aqua-fe-fair-stability-systemd-adoption-v7"
        ...
        != "aqua-fe-fair-stability-systemd-adoption-claim-v7"
    ...
    if result_path.is_file() and not result_path.is_symlink():
        ...
        if (
            ...
            expected_outcome = (
                ...
                else "PIPELINE_INVALID_FAIL_STOP_MANUAL_REVIEW_ZERO_RETRY"
...
def load_attempt_matrix(experiment_root: Path) -> dict[str, Any]:
    """Verify the immutable 120-attempt v7 matrix without freezing later retries."""
    path = experiment_root / "attempt_matrix_freeze_v7.json"
    require_no_symlink_components(path, experiment_root)
    if path.is_symlink() or not path.is_file():
    ...
    if (
        ...
        != "aqua-fe-fair-stability-attempt-matrix-freeze-v7"
        or value.get("experiment_id") != EXPERIMENT_ID
        or value.get("status") != "FROZEN_BEFORE_ANY_V7_ESTIMATOR_START"
    ):
    ...
    for key, candidate in (
        ...
        (
            "backend_freeze",
            experiment_root
            / "vins_dev_nativeq_schedfix_runtimeexcl_v7"
            / "backend_freeze.json",
        ),
    ):
...
def attempt_root(experiment_root: Path, cell: Mapping[str, Any], attempt_index: int) -> Path:
    ...
    if arm.startswith("hfnet_openloop_"):
        ...
        base = (
            experiment_root / "vins_dev_nativeq_schedfix_runtimeexcl_v7" / "attempts"
...
def validate_result_semantic_evidence(
    ...
    try:
        if arm in {"learned_klt_vins", "pure_klt_vins"}:
            from run_fair_stability_vins_replay_v7 import (
            ...
            from run_fair_stability_hfnet_openloop_v7 import (
                accepted_events,
                parse_log,
                parse_trajectory,
                read_ns_lines,
...
def validate_runtime_resource_monitor_evidence(
    ...
    fail-stop pipeline result, and v7 has no replacement attempts.
    """
    ...
    if hfnet_arm:
        ...
        if vins_child_present:
            ...
            if (
                ...
                != "aqua-fe-fair-stability-vins-child-start-v7"
...
def validate_controller_dispatch_execution_for_result(
    ...
    controller_path = Path(__file__).resolve().parent / "run_fair_stability_next_v7.py"
...
def validate_hfnet_watchdog_evidence(
    ...
    if (
        value.get("schema_version")
        != "aqua-fe-fair-stability-hfnet-zero-kf-post-shutdown-watchdog-v7"
        or value.get("experiment_id") != EXPERIMENT_ID
        or not valid_utc_receipt_timestamp(value.get("finalized_at_utc"))
        or not strict_json_equal(
            {
    ...
    if (
        ...
        != "aqua-fe-fair-stability-hfnet-launch-receipt-v7"
    ...
    if (
        ...
        != "aqua-fe-fair-stability-hfnet-zero-kf-watchdog-outcome-v7"
...
def inspect_cell(experiment_root: Path, cell: Mapping[str, Any]) -> dict[str, Any]:
    ...
    while attempt_index <= MAX_ATTEMPTS_PER_CELL:
        ...
        expected_manifest_schema = (
            "aqua-fe-fair-stability-hfnet-attempt-v7"
            if hfnet_arm
            else "aqua-fe-fair-stability-vins-attempt-v7"
        )
        if (
        ...
        if result_path.is_file():
            ...
            expected_result_schema = (
                "aqua-fe-fair-stability-hfnet-result-v7"
                if hfnet_arm
                else "aqua-fe-fair-stability-vins-result-v7"
            )
            if (
            ...
            if status in TERMINAL_STATUSES:
                ...
                if terminal_receipt.is_file():
                    ...
                    if (
                        ...
                        != "aqua-fe-fair-stability-ordinal-terminal-v7"
                        or not strict_json_equal(
                            [
            ...
            if invalid_receipt_path.is_file():
                raise OrdinalError(
                    f"PIPELINE_ADJUDICATION_DISABLED_ZERO_REPLACEMENT_V7:{root}"
                )
            ...
            if (
                ...
                != "EXPLICIT_EXTERNAL_TRANSIENT_ONLY_V7"
...
def verify_dispatch_claim(
    ...
    if claim.get("schema_version") != "aqua-fe-fair-stability-ordinal-dispatch-v7":
