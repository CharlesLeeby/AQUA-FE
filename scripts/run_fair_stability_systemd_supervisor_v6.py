#!/usr/bin/python3
"""Durable systemd-user supervision for one fair-stability v6 ordinal.
...
CONTROLLER = ROOT / "scripts/run_fair_stability_next_v6.py"
...
UNIT_PREFIX = "aqua-fe-fair-stability-v6"

SUBMISSION_SCHEMA = "aqua-fe-fair-stability-systemd-submission-v6"
SUBMIT_RECEIPT_SCHEMA = "aqua-fe-fair-stability-systemd-submit-receipt-v6"
START_RECEIPT_SCHEMA = "aqua-fe-fair-stability-systemd-start-receipt-v6"
EXECUTION_SCHEMA = "aqua-fe-fair-stability-systemd-execution-v6"
TERMINAL_SCHEMA = "aqua-fe-fair-stability-systemd-terminal-v6"
ADOPTION_CLAIM_SCHEMA = "aqua-fe-fair-stability-systemd-adoption-claim-v6"
ADOPTION_SCHEMA = "aqua-fe-fair-stability-systemd-adoption-v6"
...
def host_preflight() -> dict[str, object]:
    ...
    return {
        "schema_version": "aqua-fe-fair-stability-systemd-host-preflight-v6",
        ...
        "login": {
            ...
            "limitation": (
                "LAST_LOGIN_SESSION_EXIT_MAY_STOP_USER_MANAGER_AND_INVALIDATE_V6"
...
def systemd_run_argv(unit: str, directory: Path) -> list[str]:
    ...
    return [
        ...
        f"--description=AQUA-FE fair-stability v6 one ordinal {unit}",
...
def poll(directory: Path | None = None) -> dict[str, object]:
    ...
    return {
        "schema_version": "aqua-fe-fair-stability-systemd-poll-v6",
        "experiment_id": EXPERIMENT_ID,
...
def adopt(directory: Path | None = None) -> dict[str, object]:
    ...
    if service_result == "timeout" or exit_code in {"killed", "dumped"}:
        raise SystemdSupervisorError("SYSTEMD_TIMEOUT_OR_SIGNAL_FAIL_STOP_ABANDON_V6")
    ...
    if state.get("state") == "STARTED_WITHOUT_RESULT":
        raise SystemdSupervisorError("STARTED_WITHOUT_RESULT_FAIL_STOP_ABANDON_V6")
    ...
    if state.get("state") == "TERMINAL_UNADOPTED":
        from run_fair_stability_next_v6 import adopt_terminal
