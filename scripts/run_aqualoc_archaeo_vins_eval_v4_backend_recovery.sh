...
{
  echo "schema=aqua-fe-a10-v4-roscore-provenance-v1"
  echo "port=$PORT"
  echo "ros_master_uri=$ROS_MASTER_URI"
  echo "network_namespace=$(/usr/bin/readlink /proc/self/ns/net)"
} > "$RUN_DIR/roscore.log"
/usr/bin/setsid roscore -p "$PORT" >> "$RUN_DIR/roscore.log" 2>&1 &
ROSCORE_PID=$!
VINS_PID=""
bounded_stop_group() {
  local leader="$1"
  ...
  echo "cleanup_${label}_complete=1"
}
cleanup() {
  local status=$?
  trap - EXIT
  set +e
  bounded_stop_group "${VINS_PID:-}" "vins"
  bounded_stop_group "$ROSCORE_PID" "roscore"
  exit "$status"
}
trap cleanup EXIT
...
if [[ -n "${VINS_NODE_BIN:-}" ]]; then
  ...
  /usr/bin/setsid "$VINS_NODE_BIN" "$VINS_CONFIG" > "$VINS_LOG" 2>&1 &
else
  ...
  # rospack/rosrun.
  /usr/bin/setsid rosrun vins vins_node "$VINS_CONFIG" > "$VINS_LOG" 2>&1 &
fi
VINS_PID=$!
/usr/bin/sleep 4
