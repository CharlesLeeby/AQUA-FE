#!/usr/bin/python3
"""Single ordinal dispatcher for the systemd-supervised v7 experiment."""
...
from fair_stability_ordinal_common_v7 import (  # noqa: E402
...
MATRIX_FREEZE = EXPERIMENT_ROOT / "attempt_matrix_freeze_v7.json"
HFNET_RUNNER = ROOT / "scripts/run_fair_stability_hfnet_openloop_v7.py"
VINS_RUNNER = ROOT / "scripts/run_fair_stability_vins_replay_v7.py"
...
def verify_systemd_authority() -> dict[str, object]:
    ...
    if (
        claim.get("schema_version") != "aqua-fe-fair-stability-systemd-submission-v7"
...
def verify_control_freeze() -> dict[str, Any]:
    ...
    for path in (
        ...
        HFNET_RUNNER,
        SYSTEMD_SUPERVISOR,
        SCRIPT_DIR / "fair_stability_ordinal_common_v7.py",
    ):
...
def adopt_terminal(
    ...
    payload = {
        "schema_version": "aqua-fe-fair-stability-ordinal-terminal-v7",
...
def automatic_retry_codes(state: Mapping[str, Any]) -> list[str]:
    """Validate the evidence shape, then enforce v7's zero-retry policy."""
    result = state["result"]
    ...
    raise ControllerError(
        "PIPELINE_INVALID_HALT_MANUAL_ABANDONMENT_REQUIRED_ZERO_RETRY_V7:"
        f"{codes}"
    )
...
def adjudicate_pipeline_invalid(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        "schema_version": "aqua-fe-fair-stability-pipeline-invalid-adjudication-v7",
        "experiment_id": EXPERIMENT_ID,
        "adjudicated_at_utc": now_utc(),
        "accepted_as_external_pipeline_fault": True,
        "automatic_retry_policy": "EXPLICIT_EXTERNAL_TRANSIENT_ONLY_V7",
...
def dispatch_claim(state: Mapping[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    cell = state["cell"]
    claim = {
        "schema_version": "aqua-fe-fair-stability-ordinal-dispatch-v7",
        "experiment_id": EXPERIMENT_ID,
...
def dispatch(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    check_contract_valid = (
        type(ready) is bool
        and isinstance(resource_gate_value, Mapping)
        and resource_gate_value.get("schema_version")
        == "aqua-fe-fair-stability-prestart-resource-gate-v7"
        and resource_gate_value.get("experiment_id") == EXPERIMENT_ID
        and resource_gate_value.get("ready") is ready
    ...
    execution = {
        "schema_version": "aqua-fe-fair-stability-controller-dispatch-execution-v7",
        "experiment_id": EXPERIMENT_ID,
