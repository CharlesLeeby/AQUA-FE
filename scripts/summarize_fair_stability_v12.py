#!/usr/bin/python3
"""Receipt-validated progress summary for runtime-exclusive fair stability v12."""
...
def window_level_analysis(
    ...
    authorization_failure_reason = (
        "REQUIRES_ALL_120_TERMINAL_AND_40_COMPLETE_CASE_ARM_CELLS"
    )
    if not (
        complete_120
        ...
        return {
            "schema_version": "aqua-fe-fair-stability-window-analysis-v12",
            "analysis_authorized": False,
            "authorization_failure_reason": authorization_failure_reason,
            "window_arm_scores": None,
            "arm_estimates": None,
            "contrasts": None,
    ...
    result: dict[str, Any] = {
        ...
        "authorization_failure_reason": (
            None if authorized else authorization_failure_reason
        ),
        "design": {
        ...
        "validity_gates": {
            ...
            "no_pipeline_invalid_attempt_entered_analysis": authorized,
        },
        "window_arm_scores": scores if authorized else None,
        "arm_estimates": None,
        "contrasts": None,
        "multiplicity": {
    ...
    if not authorized:
        result["outcome_details_withheld_until_all_120_terminal"] = True
        return result
...
def summarize() -> dict[str, Any]:
    ...
    terminal_cells: list[tuple[Mapping[str, Any], Mapping[str, Any]]] = []
    states: Counter[str] = Counter()
    for cell in schedule:
        state = inspect_cell(EXPERIMENT_ROOT, cell)
        states[str(state["state"])] += 1
        if state["state"] == "TERMINAL":
            terminal_cells.append((cell, state))
    complete = (
        len(terminal_cells) == 120
        and states == Counter({"TERMINAL": 120})
    )
    if complete:
        rows = [normalize_result(cell, state) for cell, state in terminal_cells]
    cell_summaries: list[dict[str, Any]] = []
    arms = sorted({str(cell["arm"]) for cell in schedule})
    cases = list(dict.fromkeys(str(cell["case_id"]) for cell in schedule))
    for case_id in (cases if complete else ()):
        for arm in (arms if complete else ()):
    ...
    return {
        "schema_version": "aqua-fe-fair-stability-receipt-summary-v12",
        ...
        "complete": complete, "terminal_replicates": states.get("TERMINAL", 0),
        "planned_replicates": 120, "schedule_state_counts": dict(sorted(states.items())),
        "analysis_authorized": paired_analysis.get("analysis_authorized") is True,
        "outcome_details_withheld_until_all_120_terminal": not complete,
        "rows": rows if complete else None,
        "case_arm_cells": cell_summaries if complete else None,
        "arms": arm_summaries,
        "window_level_analysis": paired_analysis,
        "analysis_boundary": {
            "outcome_selected_roster": True,
            "repetitions_nested_within_window_not_independent_samples": True,
            "pipeline_invalid_is_experiment_fail_stop_zero_retry": True,
...
def main() -> int:
    report = summarize()
    target = EXPERIMENT_ROOT / "fair_stability_progress_summary_v12.json"
