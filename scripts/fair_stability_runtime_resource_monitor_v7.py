#!/usr/bin/python3
"""Continuous resource-exclusivity evidence for fair-stability v7 runs.
...
def watchdog_terminal_transition_candidate(proof: object) -> bool:
    """Recognize only the narrow Linux /proc shape of our leader exiting.

    This is not accepted as a live identity proof. It merely permits the
    caller to defer judgment until the next already-scheduled sample so Popen
    can reap the same child. Any additional scan/read/member error or any
    non-empty wrong executable/argv remains an immediate hard failure.
    """
...
def _classify_gpu_applications(
    ...
    for source in applications:
        ...
        # Deliberately no ToDesk or desktop-process exemption in v7.
        external.append(row)
    return {
        "external": external,
...
def resource_gate(
    ...
    """Run the strict v7 pre-start exclusivity gate.
...
class RuntimeResourceMonitor:
    ...
    def watchdog_sigterm_exact_group(
        ...
        *,
        ...
        def passive(phase: str, returncode: int) -> dict[str, object]:
            return {
        ...
        if initial_returncode is not None:
            return complete_gate(
                passive("BEFORE_FIRST_FINAL_GROUP_PROOF", initial_returncode)
            )
        ...
        if first_returncode is not None:
            if (
                ...
                result = passive("AFTER_FIRST_FINAL_GROUP_PROOF", first_returncode)
        ...
        if second_returncode is not None:
            if (
                ...
                result = passive("AFTER_SECOND_FINAL_GROUP_PROOF", second_returncode)
        ...
        if leader_returncode is not None:
            ...
            result = passive("BEFORE_KILLPG", leader_returncode)
            result["first_exact_group_proof"] = first
