...
int main(int argc, char** argv) {
    ...
    auto settings = std::make_shared<FeatureExtractorSettings>(KEYP_ORB, DESC_ORB, settings_path);
    std::shared_ptr<FeatureExtractor> extractor =
        std::make_shared<FeatureExtractor_orb32>(4000, settings);
    ...
    while (std::getline(rgb, line)) {
        ...
        int nmatches = matcher.SearchForInitialization(
            *initial, current, prev_matched, matches, 100, DESC_ORB);
