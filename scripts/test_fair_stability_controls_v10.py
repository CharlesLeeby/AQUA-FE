...
VINS = load("fair_vins_v10", SCRIPTS / "run_fair_stability_vins_replay_v10.py")
HFNET = load("fair_hfnet_v10", SCRIPTS / "run_fair_stability_hfnet_openloop_v10.py")
ORDINAL = load("fair_ordinal_v10", SCRIPTS / "fair_stability_ordinal_common_v10.py")
NEXT = load("fair_next_v10", SCRIPTS / "run_fair_stability_next_v10.py")
SUMMARY = load("fair_summary_v10", SCRIPTS / "summarize_fair_stability_v10.py")
RESOURCE = load(
    "fair_runtime_resource_monitor_v10",
    SCRIPTS / "fair_stability_runtime_resource_monitor_v10.py",
)
SYSTEMD = load(
    "fair_systemd_supervisor_v10",
    SCRIPTS / "run_fair_stability_systemd_supervisor_v10.py",
)
...
class LifecycleAndScheduleTests(unittest.TestCase):
    def test_vins_lifecycle_exact_identity(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            lifecycle_values.update(
                {
                    "schema_version": "aqua-fe-fair-stability-vins-child-lifecycle-v10",
    ...
    def test_v10_schedule_and_pristine_attempt_matrix(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            matrix_path = root / "attempt_matrix_freeze_v10.json"
            matrix_path.write_bytes(
                ORDINAL.canonical_json(
                    {
                        "schema_version": "aqua-fe-fair-stability-attempt-matrix-freeze-v10",
                        "experiment_id": ORDINAL.EXPERIMENT_ID,
                        "status": "FROZEN_BEFORE_ANY_V10_ESTIMATOR_START",
...
class V10ControlRegressionTests(unittest.TestCase):
    def test_v10_control_document_identities_and_history_are_pinned(self) -> None:
    ...
    def test_v10_controls_never_read_predecessor_runtime_or_attempt_namespaces(self) -> None:
        controls = (
            "fair_stability_ordinal_common_v10.py",
            "fair_stability_runtime_resource_monitor_v10.py",
            "run_fair_stability_hfnet_openloop_v10.py",
            "run_fair_stability_vins_replay_v10.py",
            "run_fair_stability_next_v10.py",
            "run_fair_stability_systemd_supervisor_v10.py",
            "run_fair_stability_vins_replay_child_v10.sh",
            "summarize_fair_stability_v10.py",
        )
        forbidden = (
    ...
    def test_v9_non_import_flags_fail_closed_in_freeze_readers(self) -> None:
        claims = {
            **{
                f"v{version}_results_imported": False
                for version in range(1, 10)
        ...
        self.assertTrue(ORDINAL.valid_v9_non_import_claims(claims))
        with tempfile.TemporaryDirectory() as temporary:
            freeze_path = Path(temporary) / "backend_freeze.json"
            for key in sorted(ORDINAL.V9_NON_IMPORT_CLAIM_KEYS):
                for mutation in ("missing", "true"):
                    with self.subTest(key=key, mutation=mutation):
                        ...
                        self.assertFalse(
                            ORDINAL.valid_v9_non_import_claims(invalid_claims)
                        )
                            ...
                            ORDINAL.canonical_json(
                                {
                                    "schema_version": (
                                        ...
                                        "runtimeexcl-backend-freeze-v10"
                                    ),
                                    "experiment_id": ORDINAL.EXPERIMENT_ID,
                                    "backend_id": (
                                        "DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V10"
                                    ),
    ...
    def test_vins_roscore_interpreter_is_fixed_not_manifest_selected(self) -> None:
        ...
        wrapper = (
            SCRIPTS / "run_fair_stability_vins_replay_child_v10.sh"
    ...
    CHILD = SCRIPTS / "run_fair_stability_vins_replay_child_v10.sh"
    ...
    def test_vins_rejects_prior_result_import_claims(self) -> None:
        ...
        manifest = {
            "schema_version": "aqua-fe-fair-stability-hfnet-input-freeze-v10",
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch.object(VINS, "EXPERIMENT_ROOT", root), mock.patch.object(
                ...
                VINS, "read_csv_rows", side_effect=[roster, historical] * 16
            ), mock.patch.object(
    ...
    def test_runner_summaries_delegate_to_receipt_state_machine(self) -> None:
        ...
        with mock.patch(
            "summarize_fair_stability_v10.summarize", return_value=sentinel
        ) as summarize:
    ...
    def test_postprocess_signals_remain_fail_stop_through_result_publication(self) -> None:
        ...
        for path in (
            SCRIPTS / "run_fair_stability_hfnet_openloop_v10.py",
            SCRIPTS / "run_fair_stability_vins_replay_v10.py",
        ):
    ...
    def test_result_published_then_runner_signal_is_controller_fail_stop(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            matrix = base / "attempt_matrix_freeze_v10.json"
            matrix.write_text("{}\n", encoding="ascii")
            dispatch_claim = attempt / "ordinal_dispatch_claim.json"
            dispatch_claim.write_text("{}\n", encoding="ascii")
            attempt_manifest = {
                "schema_version": "synthetic-attempt-v10",
                "experiment_id": ORDINAL.EXPERIMENT_ID,
                "port": 26100,
            }
            ...
            def fake_run(
                ...
                (attempt / "run_result.json").write_bytes(
                    ORDINAL.canonical_json(
                        {
                            "schema_version": "aqua-fe-fair-stability-vins-result-v10",
                            "experiment_id": ORDINAL.EXPERIMENT_ID,
                            "case_id": "a05_3300_3700",
                            "arm": "learned_klt_vins",
                            "repeat": 1,
    ...
    def test_resource_check_stdout_then_signal_cannot_start_estimator(self) -> None:
        state = {
            "state": "READY",
            "attempt_root": "/tmp/never-started-v10",
            "attempt_index": 1,
            "cell": {
                "ordinal": 1,
                "case_id": "a05_3300_3700",
        ...
        gate = {
            "schema_version": "aqua-fe-fair-stability-prestart-resource-gate-v10",
            "experiment_id": ORDINAL.EXPERIMENT_ID,
            "ready": True,
        }
    ...
    def test_terminal_adoption_has_bound_monitor_and_two_validations(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            claim = {
                "schema_version": "aqua-fe-fair-stability-systemd-adoption-claim-v10",
    ...
    def test_matrix_freeze_requires_exact_pristine_prepare_tree(self) -> None:
        ...
        def prepare_fixture(experiment_root: Path) -> tuple[Path, Path]:
            ...
            manifest = {
                "schema_version": "aqua-fe-fair-stability-hfnet-attempt-v10",
                "experiment_id": ORDINAL.EXPERIMENT_ID,
                "case_id": cell["case_id"],
                "arm": cell["arm"],
                "repeat": 1,
...
class SystemdControlTests(unittest.TestCase):
    ...
    def build_chain(
        ...
        unit = (
            f"aqua-fe-fair-stability-v10-o{planned_ordinal:03d}-a001-"
        ...
        if outcome == "RESOURCE_BLOCKED_NO_START":
            ...
            dispatch = {
                ...
                "controller": ORDINAL.identity(
                    SCRIPTS / "run_fair_stability_next_v10.py"
                ),
                "run_result": None,
    ...
    def test_systemd_unit_contract_and_submit_only_argv(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            unit = "aqua-fe-fair-stability-v10-o001-a001-s001-aabbcc.service"
    ...
    def test_systemd_user_control_plane_uses_fixed_bus_under_empty_ambient_env(
        ...
        with mock.patch.dict(os.environ, {}, clear=True), mock.patch.object(
            ...
            observed = SYSTEMD.query_unit("aqua-fe-fair-stability-v10-test.service")
        self.assertEqual(
    ...
    def test_no_start_adoption_never_calls_terminal_adoption(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            import run_fair_stability_next_v10 as canonical_next
    ...
    def test_blocked_resource_gate_is_complete_at_all_three_boundaries(
        ...
        for label, gate in attacked_gates():
            with self.subTest(layer="shared", attack=label):
                self.assertFalse(
                    ORDINAL.valid_prestart_resource_gate_v10(
                        gate,
    ...
    def boundary(self, root: Path) -> dict[str, dict[str, object]]:
        ...
        return {
            "common": ORDINAL.identity(
                SCRIPTS / "fair_stability_ordinal_common_v10.py"
            ),
            "controller": ORDINAL.identity(
                SCRIPTS / "run_fair_stability_next_v10.py"
            ),
    ...
    def build_terminal(
        ...
        manifest_path.write_bytes(
            ORDINAL.canonical_json(
                {
                    "schema_version": (
                        "aqua-fe-fair-stability-vins-attempt-v10"
                    ),
                    "experiment_id": ORDINAL.EXPERIMENT_ID,
                    "case_id": cell["case_id"],
                    "arm": cell["arm"],
        ...
        result.update(
            {
                "schema_version": (
                    "aqua-fe-fair-stability-vins-result-v10"
                ),
                "experiment_id": ORDINAL.EXPERIMENT_ID,
                "case_id": cell["case_id"],
                "arm": cell["arm"],
...
class ZeroKfWatchdogTests(unittest.TestCase):
    ...
    def reap_receipt(
        ...
        *,
        ...
        return {
            "schema_version": "aqua-fe-fair-stability-same-popen-reap-v10",
            "source": source,
            "same_popen": True,
    ...
    def test_common_watchdog_chain_rejects_contradictory_signal_proofs(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            launch = {
                "schema_version": "aqua-fe-fair-stability-hfnet-launch-receipt-v10",
                "experiment_id": ORDINAL.EXPERIMENT_ID,
                "valid": True,
                "errors": [],
                "pid": 4242,
...
class RuntimeResourceMonitorTests(unittest.TestCase):
    ...
    def test_atomic_probe_reservation_and_coverage_close_for_both_streams(
        ...
        for stream in ("proc", "gpu"):
            with self.subTest(stream=stream, winner="reservation"):
                ...
                with mock.patch.object(
                    RESOURCE.time, "monotonic_ns", side_effect=[100, 200, 300]
                ), mock.patch.object(
                    RESOURCE,
                    "_utc_now",
                    side_effect=[
                ...
                self.assertEqual(monitor.coverage_end_monotonic_ns, 300)
            ...
            with self.subTest(stream=stream, winner="coverage_close"):
                ...
                close_scan.assert_called_once_with()
                self.assertEqual(getattr(monitor, f"_{stream}_samples"), [])
    ...
    def test_probe_exception_keeps_reserved_timestamp_for_both_streams(
        ...
        for stream in ("proc", "gpu"):
            with self.subTest(stream=stream):
                ...
                self.assertEqual(monitor.coverage_end_monotonic_ns, 400)
                close_scan.assert_called_once_with()
                self.assertTrue(
        ...
        for stream, maximum_seconds, target_seconds in (
            ...
            with self.subTest(stream=stream):
                ...
                with mock.patch.object(
                    RESOURCE.time,
                    "monotonic_ns",
                    side_effect=[100, stalled_start_ns, stalled_start_ns + 2],
                ), mock.patch.object(
                    RESOURCE,
                    "_utc_now",
                    side_effect=[
    ...
    def test_concurrent_close_callers_commit_one_coverage_end(self) -> None:
        ...
        close_scan.assert_called_once_with()
        end_clock.assert_called_once_with()
        end_utc.assert_called_once_with()
