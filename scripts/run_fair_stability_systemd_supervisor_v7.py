#!/usr/bin/python3
"""Durable systemd-user supervision for one fair-stability v7 ordinal.
...
CONTROLLER = ROOT / "scripts/run_fair_stability_next_v7.py"
...
UNIT_PREFIX = "aqua-fe-fair-stability-v7"

SUBMISSION_SCHEMA = "aqua-fe-fair-stability-systemd-submission-v7"
SUBMIT_RECEIPT_SCHEMA = "aqua-fe-fair-stability-systemd-submit-receipt-v7"
START_RECEIPT_SCHEMA = "aqua-fe-fair-stability-systemd-start-receipt-v7"
EXECUTION_SCHEMA = "aqua-fe-fair-stability-systemd-execution-v7"
TERMINAL_SCHEMA = "aqua-fe-fair-stability-systemd-terminal-v7"
ADOPTION_CLAIM_SCHEMA = "aqua-fe-fair-stability-systemd-adoption-claim-v7"
ADOPTION_SCHEMA = "aqua-fe-fair-stability-systemd-adoption-v7"
...
def host_preflight() -> dict[str, object]:
    ...
    return {
        "schema_version": "aqua-fe-fair-stability-systemd-host-preflight-v7",
        ...
        "login": {
            ...
            "limitation": (
                "LAST_LOGIN_SESSION_EXIT_MAY_STOP_USER_MANAGER_AND_INVALIDATE_V7"
...
def systemd_run_argv(unit: str, directory: Path) -> list[str]:
    ...
    return [
        ...
        f"--description=AQUA-FE fair-stability v7 one ordinal {unit}",
...
def poll(directory: Path | None = None) -> dict[str, object]:
    ...
    return {
        "schema_version": "aqua-fe-fair-stability-systemd-poll-v7",
        "experiment_id": EXPERIMENT_ID,
...
def adopt(directory: Path | None = None) -> dict[str, object]:
    ...
    if service_result == "timeout" or exit_code in {"killed", "dumped"}:
        raise SystemdSupervisorError("SYSTEMD_TIMEOUT_OR_SIGNAL_FAIL_STOP_ABANDON_V7")
    ...
    if state.get("state") == "STARTED_WITHOUT_RESULT":
        raise SystemdSupervisorError("STARTED_WITHOUT_RESULT_FAIL_STOP_ABANDON_V7")
    ...
    if state.get("state") == "TERMINAL_UNADOPTED":
        from run_fair_stability_next_v7 import adopt_terminal
    ...
    receipt = {
        ...
        "estimator_retry_authorized": False,
        "next_submission_requires_new_submit": state_after.get("state") in {
            "READY", "NEEDS_PREPARATION"
        },
    }
