...
write_lifecycle() {
  ...
  scan_group_residuals
  {
    printf 'schema_version=aqua-fe-fair-stability-vins-child-lifecycle-v9\n'
...
{
  printf 'schema_version=aqua-fe-fair-stability-vins-child-start-v9\n'
  printf 'experiment_id=%s\n' "$EXPERIMENT_ID"
  printf 'tag=%s\n' "$TAG"
  printf 'wrapper_pid=%s\n' "$WRAPPER_PID"
  printf 'wrapper_start_ticks=%s\n' "$WRAPPER_START_TICKS"
...
{
  printf 'schema_version=aqua-fe-fair-stability-vins-replay-manifest-v9\n'
  printf 'experiment_id=%s\n' "$EXPERIMENT_ID"
  printf 'run_dir=%s\n' "$RUN_DIR"
  printf 'play_bag=%s\n' "$FEATURE_BAG"
  ...
  printf 'shutdown_contract=EXPECTED_BAG_END_SIGTERM_REAP_V9\n'
