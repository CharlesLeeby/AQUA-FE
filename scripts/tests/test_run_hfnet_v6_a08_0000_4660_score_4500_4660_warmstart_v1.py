#!/usr/bin/env python3
"""Process-free tests for the A08 0..4660 / score 4500..4660 runner."""
...
from scripts import run_hfnet_v6_a08_0000_4660_score_4500_4660_warmstart_v1 as runner


def _stamps() -> list[int]:
    return [1_000_000_000_000_000_000 + index * 50_000_000 for index in range(4661)]
...
def _runtime_paths(root: Path) -> dict[str, Path]:
    ...
    return {
        ...
        "score_trajectory": result_dir / "trajectory_score_source_4500_4660.txt",
    }
...
class A08WarmstartRunnerTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_exact_identity_mapping_boundaries_and_namespace(self) -> None:
        ...
        self.assertEqual(
            (runner.FEED_LOCAL_FIRST, runner.FEED_LOCAL_LAST), (0, 4660)
        )
        self.assertEqual(
        ...
        self.assertEqual(
            (runner.SCORE_LOCAL_FIRST, runner.SCORE_LOCAL_LAST), (4500, 4660)
        )
        self.assertEqual(runner.LOCAL_TO_SOURCE_OFFSET, 0)
        self.assertEqual(runner.SOURCE_TO_LOCAL_OFFSET, 0)
        self.assertEqual(runner.PREROLL_LAST_NS, 1542885186057768976)
        self.assertEqual(runner.SCORE_FIRST_NS, 1542885186107583632)
    ...
    def test_selection_and_runability_only_boundary_are_explicit(self) -> None:
        ...
        self.assertEqual(selection["feed_source_frame_indices_inclusive"], [0, 4660])
        self.assertEqual(selection["feed_local_frame_indices_inclusive"], [0, 4660])
        self.assertEqual(selection["score_source_frame_indices_inclusive"], [4500, 4660])
        self.assertEqual(selection["score_local_frame_indices_inclusive"], [4500, 4660])
        self.assertEqual(selection["preroll_camera_count"], 4500)
    ...
    def test_pose_parser_keeps_source_and_local_indices_identical(self) -> None:
        ...
        row = value["rows"][0]
        self.assertEqual(row["local_camera_index"], 4500)
        self.assertEqual(row["source_camera_index"], 4500)

    def test_strict_161_pass_writes_read_only_no_clobber_crop(self) -> None:
        stamps = _stamps()
        ...
        self.assertEqual(result["score"]["pose_count"], 161)
        self.assertTrue(result["score"]["exact_161_of_161_contiguous"])
        self.assertEqual(result["score"]["first_source_index"], 4500)
        self.assertEqual(result["score"]["last_source_index"], 4660)
        crop = path_map["score_trajectory"]
        self.assertEqual(len(crop.read_text(encoding="ascii").splitlines()), 161)
    ...
    def test_configure_wires_hardened_one_shot_supervisor_without_start(self) -> None:
        ...
        self.assertEqual(base.SOURCE_LAST, 4660)
        self.assertEqual(base.CAMERA_COUNT, 4661)
