#!/usr/bin/env python3
"""Synthetic, process-free tests for the A09 natural-history materializer."""
...
from scripts import (
    materialize_hfnet_v6_a09_0000_6200_score_6000_6200_v1 as subject,
)
...
class A09NaturalHistoryMaterializerTests(unittest.TestCase):
    def test_production_contract_and_score_boundary(self) -> None:
        ...
        self.assertEqual(contract.expected_image_total_bytes, 1_523_345_291)
        self.assertEqual(
    ...
    def test_synthetic_materialization_is_atomic_and_process_free(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(
                ...
                return_value={
                    "a08_materializer_bridge": {"sha256": "synthetic-a08"},
                    "a05_materializer_bridge": {"sha256": "synthetic-a05"},
                    "a06_adapter": {"sha256": "synthetic-a06"},
                },
            ), mock.patch.dict(
                subject._ADAPTER_BINDINGS,
                {"validate_selector": mock.Mock(return_value=selector_result)},
            ):
                manifest = subject.materialize(selector, source, output, contract)
            self.assertEqual(manifest["status"], "PASS_PREPARATION_ONLY")
            self.assertEqual(manifest["selection"]["sequence_id"], "A09")
