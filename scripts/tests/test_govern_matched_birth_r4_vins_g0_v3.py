...
from scripts import build_a02_matched_birth_r4_vins_g0_runtime_alias_incident_v1 as incident_builder
from scripts import govern_matched_birth_r4_vins_g0_v3 as g
...
def spawn_signal_guard(
    *, process_started: bool = True, received: list[str] | None = None,
    ...
    return {
        ...
        "cleanup_mode_before_restore": True if deferred else True,
        "child_reaped_before_restore": True,
        "restore_transition_mask_applied": not deferred,
        "handlers_restored": not deferred,
        "post_restore_target_mask": None if deferred else [],
        "handler_lifecycle": (
...
def runtime_alias_incident_fixture() -> dict[str, object]:
    source = g.Path(g._OUTER_SCRIPT).read_bytes()
    digest = hashlib.sha256(source).hexdigest()
    contract = g.incident_adoption_namespace_contract_v1(digest, len(source))
    builder_bytes = g.INCIDENT_BUILDER.read_bytes()
    builder_identity = record(
        g.INCIDENT_BUILDER, hashlib.sha256(builder_bytes).hexdigest(),
        len(builder_bytes),
    )
    return incident_builder.expected_incident_record_v1(
        successor_sha256=digest, successor_size_bytes=len(source),
        v3_job_hash=str(contract["job_hash"]),
        builder_identity=builder_identity,
        successor_namespace_contract=contract,
    )


def rehash_incident(value: dict[str, object]) -> None:
    value["incident_hash"] = "0" * 64
    value["incident_hash"] = hashlib.sha256(
        g._canonical_json_bytes(value) + b"\n"
    ).hexdigest()


def incident_scalar_paths(
    value: object, prefix: tuple[object, ...] = (),
) -> object:
    if isinstance(value, dict):
        for key, child in value.items():
            yield from incident_scalar_paths(child, prefix + (key,))
    elif isinstance(value, list):
        for index, child in enumerate(value):
            yield from incident_scalar_paths(child, prefix + (index,))
    else:
        yield prefix


def mutate_incident_scalar(value: object, path: tuple[object, ...]) -> None:
    parent = value
    for part in path[:-1]:
...
class ContractTests(unittest.TestCase):
    def test_probe_success_closeout_is_typed_and_exact(self) -> None:
        intent = {"probe_launch_intent_hash": "a" * 64}
        freeze = {"freeze_hash": "b" * 64}
        value = g._probe_success_payload(intent=intent, freeze=freeze)
        self.assertEqual(
            g._validate_probe_success(value, intent=intent, freeze=freeze), value
        )
        for key, replacement in (
            ...
            altered["success_hash"] = g._self_hash(altered, "success_hash")
            with self.subTest(key=key), self.assertRaises(g.GovernanceError):
                g._validate_probe_success(altered, intent=intent, freeze=freeze)

    def test_probe_success_loader_rejects_writable_mode(self) -> None:
        intent = {"probe_launch_intent_hash": "a" * 64}
        freeze = {"freeze_hash": "b" * 64}
        value = g._probe_success_payload(intent=intent, freeze=freeze)
        with tempfile.TemporaryDirectory(dir=g.ROOT) as raw:
            path = Path(raw) / "probe_success.json"
            path.write_bytes(g._render_canonical_json(value))
            path.chmod(0o644)
            with mock.patch.object(g, "PROBE_SUCCESS", path), \
                 self.assertRaisesRegex(g.GovernanceError, "mode/identity"):
                g._load_probe_success(intent=intent, freeze=freeze)

    def test_incident_authority_rejects_pending_writable_canonical_leaf(self) -> None:
        with tempfile.TemporaryDirectory(dir=g.ROOT) as raw:
            path = Path(raw) / "incident.json"
            path.write_bytes(b'{}\n')
            path.chmod(0o600)
            with mock.patch.object(g, "INCIDENT", path), \
                 self.assertRaisesRegex(g.GovernanceError, "mode/identity"):
                with g._held_incident_adoption_authority():
                    pass

    def test_outer_carrier_commits_success_only_after_clean_systemexit_zero(self) -> None:
        with tempfile.TemporaryDirectory(dir=g.ROOT) as raw:
            root = Path(raw)
            first = root / "freeze.json"
            second = root / "intent.json"
            marker = root / "success.json"
            first.write_bytes(b"freeze")
            second.write_bytes(b"intent")
            ...
            commit = {
                "path": os.fspath(marker),
                "content": content,
                "sha256": hashlib.sha256(content).hexdigest(),
                "mode": 0o444,
                "required_records": records,
                "required_absent": [os.fspath(root / "failure.json")],
            }
            fixture = root / "governor_fixture.py"
            fixture.write_text(
                "_AQUAFE_G0_FINAL_SUCCESS_COMMIT = " + repr(commit) +
                "\n_AQUAFE_G0_OUTER_ACTION_CLEAN_SUCCESS = "
                + repr({"action": "write-freeze", "exit_code": 0}) + "\n",
                encoding="utf-8",
            )
            digest = hashlib.sha256(fixture.read_bytes()).hexdigest()
            command = [
                g._OUTER_PYTHON, "-I", "-B", "-c", g._OUTER_CARRIER_SOURCE,
                os.fspath(fixture), digest, "--action", "write-freeze",
            ]
            process = subprocess.run(
                command, cwd=g.ROOT, env=dict(g._OUTER_ENVIRONMENT),
    ...
    def test_outer_carrier_never_treats_caught_systemexit_zero_as_success(self) -> None:
        with tempfile.TemporaryDirectory(dir=g.ROOT) as raw:
            root = Path(raw)
            marker = root / "success.json"
            fixture = root / "governor_fixture.py"
            ...
            process = subprocess.run(
                [g._OUTER_PYTHON, "-I", "-B", "-c", g._OUTER_CARRIER_SOURCE,
                 os.fspath(fixture), digest, "--action", "write-freeze"],
                cwd=g.ROOT, env=dict(g._OUTER_ENVIRONMENT),
                stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                check=False, timeout=30,
            )
    ...
    def test_outer_carrier_never_commits_success_on_nonzero_exit(self) -> None:
        with tempfile.TemporaryDirectory(dir=g.ROOT) as raw:
            root = Path(raw)
            marker = root / "success.json"
            fixture = root / "governor_fixture.py"
    ...
    def test_outer_carrier_refuses_success_when_failure_receipt_is_visible(self) -> None:
        with tempfile.TemporaryDirectory(dir=g.ROOT) as raw:
            root = Path(raw)
            ...
            commit = {
                ...
                "mode": 0o444,
                "required_records": records,
                "required_absent": [os.fspath(failure)],
            }
            fixture = root / "governor_fixture.py"
            fixture.write_text(
                "_AQUAFE_G0_FINAL_SUCCESS_COMMIT = " + repr(commit)
    ...
    def test_incident_namespace_api_is_pure_and_matches_live_job_contract(self) -> None:
        source = g.Path(g._OUTER_SCRIPT).read_bytes()
        digest = hashlib.sha256(source).hexdigest()
        live_job_hash = g._current_contract_inputs()[-1]["job_hash"]
        with mock.patch("builtins.open", side_effect=AssertionError("open reached")), \
             mock.patch.object(g.os, "open", side_effect=AssertionError("os.open reached")), \
             mock.patch.object(g.subprocess, "Popen", side_effect=AssertionError("Popen reached")):
            contract = g.incident_adoption_namespace_contract_v1(
                digest, len(source)
            )
        self.assertEqual(
            set(contract),
            {
                ...
                "destination", "staging", "intent", "closeout", "freeze",
                "post", "probe_intent", "probe_failure", "probe_success",
            },
        )
        self.assertEqual(contract["job_hash"], live_job_hash)
        self.assertEqual(contract["job_id"], "matched_birth_r4_vins_g0_v3")

    def test_active_v3_incident_entrypoints_are_ast_unique_and_schema_exact(self) -> None:
        tree = ast.parse(Path(g.__file__).read_bytes())
        names = [node.name for node in tree.body if isinstance(node, ast.FunctionDef)]
        for name in (
            "incident_adoption_namespace_contract_v1",
            "_validate_incident_adoption",
            "_held_incident_adoption_authority",
        ):
    ...
    def test_builder_shaped_incident_fixture_passes_and_successor_mutation_fails(self) -> None:
        incident = runtime_alias_incident_fixture()
        incident_builder.validate_incident_record_v1(
            incident, expected=incident
        )
        g._validate_incident_adoption(incident)
        altered = copy.deepcopy(incident)
        altered["successor_authorization"]["v3_job_hash"] = "a" * 64
        rehash_incident(altered)
        with self.assertRaises(g.GovernanceError):
            g._validate_incident_adoption(altered)

    def test_real_builder_build_record_interoperates_with_governor_validator(self) -> None:
        source = g.Path(g._OUTER_SCRIPT).read_bytes()
        digest = hashlib.sha256(source).hexdigest()
        contract = g.incident_adoption_namespace_contract_v1(digest, len(source))
        builder_source = g.INCIDENT_BUILDER.read_bytes()
        builder_identity = record(
            g.INCIDENT_BUILDER,
            hashlib.sha256(builder_source).hexdigest(), len(builder_source),
        )
        value = incident_builder.build_record(
            successor_sha256=digest, successor_size_bytes=len(source),
            v3_job_hash=str(contract["job_hash"]),
            builder_identity=builder_identity,
        )
        incident_builder.validate_incident_record_v1(value, expected=value)
        self.assertEqual(g._validate_incident_adoption(value), value)

    def test_unfrozen_incident_pins_are_actively_rejected(self) -> None:
        incident = runtime_alias_incident_fixture()
        for name, replacement in (
            ("INCIDENT_BUILDER_SHA256", "0" * 64),
            ("INCIDENT_BUILDER_SIZE", 0),
            ("INCIDENT_BUILDER_CARRIER_SHA256", "0" * 64),
            ("INCIDENT_BUILDER_CARRIER_SIZE", 0),
            ("INCIDENT_HUMAN_SHA256", "0" * 64),
            ("INCIDENT_HUMAN_SIZE", 0),
        ):
            with self.subTest(name=name), mock.patch.object(g, name, replacement), \
                 self.assertRaisesRegex(g.GovernanceError, "hard pins are not frozen"):
                g._validate_incident_adoption(incident)

    def test_every_incident_scalar_leaf_is_exact_after_adversarial_rehash(self) -> None:
        incident = runtime_alias_incident_fixture()
        paths = [
            path for path in incident_scalar_paths(incident)
            if path != ("incident_hash",)
        ]
        self.assertGreater(len(paths), 100)
        for path in paths:
            changed = copy.deepcopy(incident)
            mutate_incident_scalar(changed, path)
            rehash_incident(changed)
            with self.subTest(path=path), self.assertRaises(g.GovernanceError):
                g._validate_incident_adoption(changed)

    def test_incident_all_material_nested_fields_are_fail_closed(self) -> None:
        baseline = runtime_alias_incident_fixture()
        mutations = {
            "scientific_role": lambda x: x.__setitem__("scientific_role", "OTHER"),
            "incident_class": lambda x: x.__setitem__("incident_class", "OTHER"),
            "retained_failure_absence": lambda x: x["retained_v2_evidence"]
                ["runtime_probe"].__setitem__("failure_receipt_absent", False),
            "retained_v2_governor": lambda x: x["retained_v2_evidence"]
                ["governor"].__setitem__("sha256", "a" * 64),
            "builder_identity": lambda x: x["builder_identity"]
                .__setitem__("size_bytes", 1),
            "builder_execution_contract": lambda x: x["builder_execution_contract"]
                .__setitem__("working_directory", "/tmp"),
            "human_record": lambda x: x["human_record"]
                .__setitem__("sha256", "a" * 64),
            "retained_freeze_record": lambda x: x["retained_v2_evidence"]
                ["freeze"]["file"].__setitem__("inode", 1),
            "retained_freeze_self_hash": lambda x: x["retained_v2_evidence"]
                ["freeze"].__setitem__("self_hash", "a" * 64),
            "retained_probe_success": lambda x: x["retained_v2_evidence"]
                ["runtime_probe"]["success_closeout"].__setitem__("size_bytes", 1),
            "science_verification_path": lambda x: x["retained_v2_evidence"]
                ["science_output_pair_identity"]["common_support_summary.json"]
                ["verification"].__setitem__("path", "primary"),
            "diagnosis": lambda x: x["mechanical_runtime_alias_diagnosis"]
                .__setitem__("classification", "OTHER"),
            "diagnosis_facts": lambda x: x["mechanical_runtime_alias_diagnosis"]
                ["facts"].reverse(),
            "diagnosis_legacy_authority": lambda x: x[
                "mechanical_runtime_alias_diagnosis"
            ]["authority"]["legacy_child_bootstrap"].__setitem__("sha256", "a" * 64),
            "diagnosis_optional_elf": lambda x: x[
                "mechanical_runtime_alias_diagnosis"
            ]["probe_only_optional_elf"]["persistent_proc_maps"]
                .__setitem__("expected_row_count", 4),
            "diagnosis_runtime_closure": lambda x: x[
                "mechanical_runtime_alias_diagnosis"
            ]["runtime_closure_comparison"]["roles"]["primary"]
                ["persistent_modules"].__setitem__("typed_policy_match_count", 489),
            "transitive_v1_intent": lambda x: x["retained_v2_evidence"]
                ["transitive_consumed_v1_evidence"]["runtime_probe_launch_intent"]
                .__setitem__("inode", 1),
            "required_absent_count": lambda x: x["builder_execution_contract"]
                .__setitem__("required_absent_at_commit_count", 21),
            "outcome_firewall": lambda x: x["outcome_firewall"]
                .__setitem__("science_metric_values_read_or_recorded", True),
            "claim_boundary": lambda x: x["claim_boundary"]
                .__setitem__("v2_publication_or_post_pass_claimed", True),
            "action_sequence": lambda x: x["successor_authorization"]["authorized_action_sequence"].reverse(),
            "old_namespace_completion": lambda x: x["successor_authorization"]
                .__setitem__(
                    "old_v1_or_v2_namespace_retry_mutation_or_completion_authorized",
                    True,
                ),
            "science_change": lambda x: x["successor_authorization"]
                .__setitem__("scientific_or_evaluation_contract_change_authorized", True),
            "repair_scope": lambda x: x["successor_authorization"]["repair_policy"]
                .__setitem__("runtime_closure_lexical_alias", "GENERAL_RELAXATION"),
        }
        for label, mutate in mutations.items():
            altered = copy.deepcopy(baseline)
            mutate(altered)
            rehash_incident(altered)
            with self.subTest(label=label), self.assertRaises(g.GovernanceError):
                g._validate_incident_adoption(altered)

    def test_consumed_v2_failure_namespace_is_not_a_v3_destination(self) -> None:
        contract = runtime_alias_incident_fixture()["successor_authorization"][
            "successor_namespace_contract"
        ]
        self.assertNotEqual(contract["destination"], os.fspath(g.V2_RESULT))
        self.assertNotEqual(contract["staging"], os.fspath(g.V2_STAGING))
        self.assertTrue(all(
            not os.path.lexists(path) for path in g._v2_failed_namespace_absences()
        ))
        self.assertEqual(
            set(g._v2_failed_namespace_absences()),
            {
                g.V1_FREEZE, g.V1_PROBE_FAILURE, g.V1_RESULT, g.V1_POST,
                g.V1_STAGING, g.V1_PUBLICATION_INTENT,
                g.V1_PUBLICATION_CLOSEOUT, g.V2_PROBE_FAILURE, g.V2_POST,
                g.V2_RESULT, g.V2_PUBLICATION_CLOSEOUT,
            },
        )

    def test_each_transitive_legacy_absence_presence_is_rejected(self) -> None:
        legacy = g._v2_failed_namespace_absences()
        self.assertEqual(len(set(legacy)), 11)
        for index, original in enumerate(legacy):
            with self.subTest(path=original), tempfile.TemporaryDirectory(
                ...
                with self.assertRaisesRegex(
                    g.GovernanceError, "failure absence drifted"
                ):
                    with g._hold_v2_absence(present):
                        self.fail("present legacy namespace passed absence hold")

    def test_transitive_v1_probe_intent_exact_record_and_semantics_are_held(self) -> None:
        raw = os.fspath(g.V1_PROBE_INTENT)
        expected = g.V2_FAILURE_EVIDENCE_RECORDS[raw]
        content = g._live_exact_record(
            g.V1_PROBE_INTENT, expected, "test transitive v1 intent"
        )
        records = {
            path: Path(path).read_bytes() for path in g.V2_FAILURE_EVIDENCE_RECORDS
        }
        self.assertEqual(records[raw], content)
        g._validate_v2_failure_semantics(records)
        changed = copy.deepcopy(expected)
        changed["sha256"] = "a" * 64
        with self.assertRaisesRegex(g.GovernanceError, "byte authority differs"):
            g._live_exact_record(
                g.V1_PROBE_INTENT, changed, "mutated transitive v1 intent"
            )

    def test_consumed_v2_staging_gid_drift_is_rejected(self) -> None:
        changed = copy.deepcopy(g.V2_STAGING_DIRECTORY_RECORDS)
        changed[os.fspath(g.V2_STAGING)]["gid"] = 9999
        with mock.patch.object(g, "V2_STAGING_DIRECTORY_RECORDS", changed), \
             self.assertRaisesRegex(
                 g.GovernanceError, "staging directory identity differs"
             ):
            g._validate_v2_staging_directories()

    def test_legacy_bootstrap_reproduces_alias_failure_current_repairs_it(self) -> None:
        freeze = json.loads(g.V2_FREEZE.read_bytes())
        expected = freeze["expected_runtime_closure"]["runtime_receipt"]
        for role in g.ROLE_NAMES:
            receipt = json.loads(
                (g.V2_STAGING / role / g.RUNTIME_RECEIPT_NAME).read_bytes()
            )
            with self.subTest(role=role), self.assertRaisesRegex(
                g.legacy_bootstrap.FormalG0RuntimeError,
                "actual evaluator module differs: Cryptodome",
            ):
                g.legacy_bootstrap.validate_runtime_receipt_static(
                    receipt, role=role, expected_closure=expected
                )
            g.bootstrap.validate_runtime_receipt_static(
                receipt, role=role, expected_closure=expected
            )

    def test_current_and_legacy_bootstrap_authorities_are_distinct(self) -> None:
        self.assertIsNot(g.bootstrap, g.legacy_bootstrap)
        self.assertEqual(g.BOOTSTRAP.name, "formal_g0_child_bootstrap_v2.py")
        self.assertEqual(g.LEGACY_BOOTSTRAP.name, "formal_g0_child_bootstrap_v1.py")
        self.assertEqual(
            hashlib.sha256(g.BOOTSTRAP.read_bytes()).hexdigest(),
            g.BOOTSTRAP_SHA256,
        )
        self.assertEqual(
            (hashlib.sha256(g.LEGACY_BOOTSTRAP.read_bytes()).hexdigest(),
             g.LEGACY_BOOTSTRAP.stat().st_size),
            (g.LEGACY_BOOTSTRAP_SHA256, g.LEGACY_BOOTSTRAP_SIZE),
        )
        self.assertIn(
            "scripts.formal_g0_child_bootstrap_v1", g._OUTER_WORKSPACE_SOURCES
        )
        self.assertIn(
            "scripts.formal_g0_child_bootstrap_v2", g._OUTER_WORKSPACE_SOURCES
        )

    def test_legacy_bootstrap_hard_pin_drift_fails_static_authority(self) -> None:
        original = g._workspace_record

        def drift(path: Path, label: str) -> dict[str, object]:
            observed = original(path, label)
            if path == g.LEGACY_BOOTSTRAP:
                observed["sha256"] = "a" * 64
            return observed

        with mock.patch.object(g, "_OUTER_GUARD_ACTIVE", False), \
             mock.patch.object(g, "_workspace_record", side_effect=drift), \
             self.assertRaisesRegex(
                 g.GovernanceError, "legacy formal G0 child bootstrap authority"
             ):
    ...
    def test_real_outer_cli_guard_enters_main_and_keeps_prefix_absent(self) -> None:
        ...
        self.assertEqual(process.returncode, 2)
        self.assertIn(b"missing incident adoption", process.stderr)
    ...
    def test_outer_finder_rejects_mutated_allowed_source_before_any_exec(self) -> None:
        original_sources = dict(g._OUTER_WORKSPACE_SOURCES)
        name = "scripts.formal_g0_child_bootstrap_v2"
        with tempfile.TemporaryDirectory() as temporary:
            altered = Path(temporary) / "formal_g0_child_bootstrap_v2.py"
            altered.write_bytes(Path(original_sources[name]).read_bytes() + b"\n# drift\n")
            sources = dict(original_sources)
            sources[name] = os.fspath(altered)
            with mock.patch.object(g, "_OUTER_GUARD_ACTIVE", True), \
                 mock.patch.object(g, "_OUTER_WORKSPACE_FINDER", None), \
                 mock.patch.object(g, "_OUTER_WORKSPACE_SOURCES", sources), \
                 mock.patch.object(
                     g._OuterBoundSourceLoader, "exec_module",
                     side_effect=AssertionError("source executed before hard pin"),
                 ) as execute, self.assertRaisesRegex(
                     RuntimeError, "source hard authority differs"
                 ):
                g._install_outer_workspace_finder()
            self.assertEqual(execute.call_count, 0)

    def test_outer_finder_rejects_mutated_evaluator_before_any_import_exec(self) -> None:
        name = "evaluator_base"
        with tempfile.TemporaryDirectory() as temporary:
            altered = Path(temporary) / "evaluate_vins_common_support.py"
            altered.write_bytes(
                Path(g._OUTER_AUXILIARY_SOURCES[name]).read_bytes() + b"\n# drift\n"
            )
            sources = dict(g._OUTER_AUXILIARY_SOURCES)
            sources[name] = os.fspath(altered)
            with mock.patch.object(g, "_OUTER_GUARD_ACTIVE", True), \
                 mock.patch.object(g, "_OUTER_WORKSPACE_FINDER", None), \
                 mock.patch.object(g, "_OUTER_AUXILIARY_SOURCES", sources), \
                 mock.patch.object(
                     g._OuterBoundSourceLoader, "exec_module",
                     side_effect=AssertionError("module executed before evaluator pin"),
                 ) as execute, self.assertRaisesRegex(
                     RuntimeError, "auxiliary source hard authority differs"
                 ):
                g._install_outer_workspace_finder()
            self.assertEqual(execute.call_count, 0)
    ...
    def test_write_freeze_helper_drift_stops_before_probe_or_publication(self) -> None:
        original = g._workspace_record
        ...
        with mock.patch.object(g, "_OUTER_GUARD_ACTIVE", False), \
             mock.patch.object(g, "_workspace_record", side_effect=drift), \
             mock.patch.object(g, "_capture_runtime_closure") as capture, \
             mock.patch.object(g.subprocess, "Popen") as popen, \
             mock.patch.object(
                 g.publisher, "publish_json_transactional_rooted"
             ) as publish, mock.patch.object(
                 g, "_current_contract_inputs"
             ) as current, self.assertRaisesRegex(
                 g.GovernanceError, "hard-pinned formal helper differs: epoch_wrapper"
             ):
            g.write_freeze()
    ...
    def test_write_freeze_live_job_drift_stops_before_probe_or_publication(self) -> None:
        live = g._current_contract_inputs()
        changed = copy.deepcopy(live)
        changed[-1]["job_hash"] = "a" * 64
        with mock.patch.object(g, "_static_authority_check"), \
             mock.patch.object(g, "_current_contract_inputs", return_value=changed), \
             mock.patch.object(g, "_capture_runtime_closure") as capture, \
             mock.patch.object(g.subprocess, "Popen") as popen, \
             mock.patch.object(
                 g.publisher, "publish_json_transactional_rooted"
                 ...
                 g.GovernanceError,
                 "live successor input/helper/publication authority differs",
             ):
            g.write_freeze()
        capture.assert_not_called()
        popen.assert_not_called()
...
class ProcessReceiptTests(unittest.TestCase):
    def test_child_stderr_allowlist_is_exact_and_closed(self) -> None:
        self.assertEqual(
            g._child_stderr_allowlist(),
            [g._stream_receipt(b""), g._stream_receipt(g.EXPECTED_CHILD_STDERR)],
        )
        self.assertEqual(len(g.EXPECTED_CHILD_STDERR), 88)
        self.assertEqual(
            hashlib.sha256(g.EXPECTED_CHILD_STDERR).hexdigest(),
            g.EXPECTED_CHILD_STDERR_SHA256,
        )
        for content in (b"", g.EXPECTED_CHILD_STDERR):
            g._validate_child_stderr_receipt(
                g._stream_receipt(content), label="fixture"
            )
        for content in (b"unexpected\n", g.EXPECTED_CHILD_STDERR + b"x"):
            with self.assertRaises(g.GovernanceError):
                g._validate_child_stderr_receipt(
                    g._stream_receipt(content), label="fixture"
                )
    ...
    def test_popen_receipt_is_one_attempt_fixed_timeout_and_stream_digest(self) -> None:
        ...
        class Process:
            ...
            def communicate(self, timeout: int) -> tuple[bytes, bytes]:
                ...
                return b"stdout", g.EXPECTED_CHILD_STDERR
...
class ProbeTransactionTests(unittest.TestCase):
    ...
    def test_probe_failure_receipt_matrix_mutations_fail_closed(self) -> None:
        intent = {"probe_launch_intent_hash": "a" * 64}
        failure = g._probe_failure_payload(
            intent,
            pid=4321,
            process_started=True,
            timed_out=False,
            return_code=0,
            classification="POST_RC0_PARENT_VALIDATION_ERROR",
            wait_error=ValueError("late validation"),
            cleanup_errors=[],
            child_reaped=True,
            spawn_signal_guard=spawn_signal_guard(deferred=True),
            stdout=b"receipt",
            stderr=b"",
        )
        g._validate_probe_failure_receipt(failure, intent)
        mutations = {
            "pid": lambda x: x.__setitem__("pid", None),
            "start_count": lambda x: x.__setitem__("process_start_count", 0),
            "timed_out": lambda x: x.__setitem__("timed_out", True),
            "return_code": lambda x: x.__setitem__("return_code", 1),
            "termination": lambda x: x.__setitem__("termination_signal", 9),
            "classification": lambda x: x.__setitem__("classification", "EXITED_RC0"),
            "wait_error": lambda x: x.__setitem__("wait_error", None),
            "cleanup": lambda x: x.__setitem__("cleanup_errors", [{"class": 1, "message": "x"}]),
            "stdout_hash": lambda x: x["stdout"].__setitem__("sha256", "bad"),
            "stderr_size": lambda x: x["stderr"].__setitem__("size_bytes", -1),
            "spawn": lambda x: x["spawn_signal_guard"].__setitem__("child_reaped_before_restore", False),
        }
        for label, mutate in mutations.items():
            altered = copy.deepcopy(failure)
            mutate(altered)
            altered["probe_failure_receipt_hash"] = g.p07gov.canonical_json_hash(
                altered, "probe_failure_receipt_hash"
            )
            with self.subTest(label=label), self.assertRaises(g.GovernanceError):
                g._validate_probe_failure_receipt(altered, intent)
    ...
    def test_any_post_rc0_parent_validation_error_is_terminal_evidence(self) -> None:
        class Process:
            pid = 4199
            returncode = 0

            def communicate(self, timeout: int) -> tuple[bytes, bytes]:
                return b"runtime-json", g.EXPECTED_CHILD_STDERR

        with tempfile.TemporaryDirectory(dir=g.ROOT) as raw:
            root = Path(raw)
            with self.assertRaisesRegex(ValueError, "late parent validation"):
                with ExitStack() as stack:
                    capture, actual, environment, pass_fds = self._capture(stack, root)
                    with mock.patch.object(g.subprocess, "Popen", return_value=Process()):
                        completed = self._run(capture, actual, environment, pass_fds)
                    self.assertEqual(completed.returncode, 0)
                    raise ValueError("late parent validation")
            intent = json.loads((root / "probe_intent.json").read_bytes())
            failure = json.loads((root / "probe_failure.json").read_bytes())
            g._validate_probe_failure_receipt(failure, intent)
            self.assertEqual(
                failure["classification"], "POST_RC0_PARENT_VALIDATION_ERROR"
            )
            self.assertEqual(failure["return_code"], 0)
            self.assertEqual(
                failure["stderr"], g._stream_receipt(g.EXPECTED_CHILD_STDERR)
            )

    def test_probe_handlers_cover_intent_write_and_post_rc0_outer_signal(self) -> None:
        class Process:
            pid = 4197
            returncode = 0
        ...
        with tempfile.TemporaryDirectory(dir=g.ROOT) as raw:
            ...
            handler_seen_during_intent = False

            def observe_intent_write(descriptor: int, content: bytes) -> None:
                nonlocal handler_seen_during_intent
                handler = signal.getsignal(signal.SIGTERM)
                handler_seen_during_intent = (
                    callable(handler)
                    and getattr(handler, "__self__", None) is capture.signal_guard
                )
                original_write(descriptor, content)

            with mock.patch.object(g, "_write_all", side_effect=observe_intent_write), \
                 mock.patch.object(g.subprocess, "Popen", return_value=Process()):
                self._run(capture, actual, environment, pass_fds)
            self.assertTrue(handler_seen_during_intent)
            handler = signal.getsignal(signal.SIGTERM)
            self.assertIs(getattr(handler, "__self__", None), capture.signal_guard)
            with self.assertRaises(g._SpawnTerminationSignal) as caught:
                handler(signal.SIGTERM, None)
    ...
    def test_deferred_outer_final_checkpoint_error_is_terminal_evidence(self) -> None:
        class Process:
            pid = 4198
            returncode = 0
        ...
        with tempfile.TemporaryDirectory(dir=g.ROOT) as raw:
            ...
            with mock.patch.object(g.subprocess, "Popen", return_value=Process()):
                self._run(capture, actual, environment, pass_fds)
            prior = g._DEFERRED_FREEZE_STACK
            try:
                g._DEFERRED_FREEZE_STACK = stack.pop_all()
                g._finish_deferred_freeze_stack(
                    g.GovernanceError("injected outer final checkpoint")
                )
            finally:
                stack.close()
                g._DEFERRED_FREEZE_STACK = prior
            intent = json.loads((root / "probe_intent.json").read_bytes())
            failure = json.loads((root / "probe_failure.json").read_bytes())
            g._validate_probe_failure_receipt(failure, intent)
            self.assertEqual(
    ...
    def test_freeze_stack_handoff_has_no_pop_all_ownership_gap(self) -> None:
        events: list[str] = []
        stack = g._DeferrableExitStack()
        stack.__enter__()
        stack.callback(lambda: events.append("closed"))
        prior = g._DEFERRED_FREEZE_STACK
        try:
            g._DEFERRED_FREEZE_STACK = stack
            stack.defer_to_outer()
            self.assertFalse(stack.__exit__(None, None, None))
            self.assertEqual(events, [])
            g._finish_deferred_freeze_stack(None)
            self.assertEqual(events, ["closed"])
            self.assertIsNone(g._DEFERRED_FREEZE_STACK)
        finally:
            if g._DEFERRED_FREEZE_STACK is not None:
                g._finish_deferred_freeze_stack(
                    g.GovernanceError("test cleanup")
                )
            g._DEFERRED_FREEZE_STACK = prior

    def test_freeze_plus_terminal_failure_is_refused_by_start_gate(self) -> None:
        with tempfile.TemporaryDirectory(dir=g.ROOT) as raw:
            root = Path(raw)
            intent = root / "probe_intent.json"
            failure = root / "probe_failure.json"
            intent.write_bytes(b"retained intent")
            failure.write_bytes(b"terminal failure")
            reserved = {
                "destination": os.fspath(root / "result"),
                "staging": os.fspath(root / "staging"),
                "intent": os.fspath(root / "publication_intent.json"),
                "closeout": os.fspath(root / "publication_closeout.json"),
                "post": os.fspath(root / "post.json"),
                "probe_intent": os.fspath(intent),
                "probe_failure": os.fspath(failure),
                "probe_success": os.fspath(root / "probe_success.json"),
            }
            with mock.patch.object(g, "PROBE_INTENT", intent), \
                 self.assertRaisesRegex(
                     g.GovernanceError,
                     "reserved namespace already contains evidence: probe_failure",
                 ):
                g._assert_reserved_outputs_absent({"reserved_paths": reserved})

    def test_probe_rc0_one_byte_stderr_mutation_is_terminal_evidence(self) -> None:
        class Process:
            pid = 4200
            returncode = 0

            def communicate(self, timeout: int) -> tuple[bytes, bytes]:
                return b"runtime-json", g.EXPECTED_CHILD_STDERR + b"x"
    ...
    def test_probe_intent_fsync_handoff_errors_never_start_or_authorize_retry(self) -> None:
        for failing_call in (1, 2):
            with self.subTest(failing_call=failing_call), \
                ...
                def interrupt_after_fsync(descriptor: int) -> None:
                    nonlocal calls
                    real_fsync(descriptor)
                    calls += 1
                    if calls == failing_call:
                        raise KeyboardInterrupt("intent fsync handoff")

                with ExitStack() as stack:
                    capture, actual, environment, pass_fds = self._capture(stack, root)
                    with mock.patch.object(g.os, "fsync", side_effect=interrupt_after_fsync), \
                         mock.patch.object(
                             g.subprocess, "Popen",
                ...
                with ExitStack() as retry_stack:
                    retry, retry_actual, retry_environment, retry_fds = self._capture(
                        retry_stack, root
                    )
                    with mock.patch.object(
                        g.subprocess, "Popen",
                        side_effect=AssertionError("retry Popen reached"),
                    ) as retry_popen, self.assertRaises(g.GovernanceError):
                        self._run(
                            retry, retry_actual, retry_environment, retry_fds
                        )
    ...
    def test_failure_write_error_still_leaves_intent_alone_no_retry(self) -> None:
        with tempfile.TemporaryDirectory(dir=g.ROOT) as raw:
            root = Path(raw)
            with ExitStack() as stack:
                capture, actual, environment, pass_fds = self._capture(stack, root)
                ...
                def fail_second_write(descriptor: int, content: bytes) -> None:
                    ...
                    writes += 1
                    if writes == 2:
                        raise OSError("synthetic failure receipt disk error")
                    original(descriptor, content)

                with mock.patch.object(g, "_write_all", side_effect=fail_second_write), \
                     mock.patch.object(g.subprocess, "Popen", side_effect=OSError("start")):
                    with self.assertRaises(Exception):
                        self._run(capture, actual, environment, pass_fds)
            self.assertTrue((root / "probe_intent.json").is_file())
            with ExitStack() as retry_stack:
                ...
                ) as popen, self.assertRaises(Exception):
...
class GovernanceFlowTests(unittest.TestCase):
    def test_retained_created_directory_rejects_gid_drift(self) -> None:
        with tempfile.TemporaryDirectory(dir=g.ROOT) as raw:
            root = Path(raw)
            parent_fd = os.open(root, os.O_RDONLY | os.O_DIRECTORY)
            real_fstat = g.os.fstat

            try:
                with g._create_retained_child_directory(
                    parent_fd, "role", label="fixture gid role"
                ) as (role_fd, validate_role):
                    def drift(fd: int) -> object:
                        observed = real_fstat(fd)
                        if fd != role_fd:
                            return observed
                        values = list(observed)
                        values[5] = int(observed.st_gid) + 1
                        return os.stat_result(values)

                    with mock.patch.object(g.os, "fstat", side_effect=drift), \
                         self.assertRaisesRegex(
                             g.GovernanceError, "retained directory drifted"
                         ):
                        validate_role()
            finally:
                os.close(parent_fd)
