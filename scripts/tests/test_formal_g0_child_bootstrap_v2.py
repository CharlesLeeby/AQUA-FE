...
SOURCE = ROOT / "scripts/formal_g0_child_bootstrap_v2.py"
SPEC = importlib.util.spec_from_file_location("formal_g0_child_bootstrap_v2", SOURCE)
...
def _lib_alias(path: str) -> str | None:
    prefix = "/usr/lib/"
    if not path.startswith(prefix):
        return None
    alias = "/lib/" + path[len(prefix):]
    if os.path.exists(alias) and os.path.realpath(alias) == os.path.realpath(path):
        return alias
    return None


def _alias_receipt_usr_lib_to_lib(
    payload: dict[str, object],
) -> tuple[int, int, int]:
    module_count = 0
    for row in payload["persistent_modules"]:
        identities = row["file_identities"]
        for field, identity in list(identities.items()):
            alias = _lib_alias(identity["lexical_path"])
            if alias is None:
                continue
            row[field] = alias
            identities[field] = bootstrap.persistent_file_identity(alias)
            module_count += 1

    maps = payload["persistent_proc_maps"]
    aliases: dict[str, str] = {}
    for index, identity in enumerate(list(maps["files"])):
        alias = _lib_alias(identity["lexical_path"])
        if alias is None:
            continue
        aliases[identity["lexical_path"]] = alias
        maps["files"][index] = bootstrap.persistent_file_identity(alias)
    for row in maps["rows"]:
        alias = aliases.get(row["lexical_path"])
        if alias is not None:
            row["lexical_path"] = alias
    maps["files"].sort(key=lambda row: row["lexical_path"])
    maps["rows"].sort(
        key=lambda row: (
    ...
    payload["native_loaded_elf_closure"] = bootstrap._native_loaded_elf_closure(
        maps, payload["sealed_sources"]
    )
    native_count = sum(
        1
        for row in payload["native_loaded_elf_closure"]
        if row.get("mapping_kind") == "PERSISTENT_ELF"
        and row["file"]["lexical_path"] in aliases.values()
    )
    _rehash(payload)
    return module_count, len(aliases), native_count


def _remove_mapped_realpath(payload: dict[str, object], realpath: str) -> None:
    maps = payload["persistent_proc_maps"]
    removed = {
        row["lexical_path"]
        for row in maps["files"]
        if os.path.realpath(row["lexical_path"]) == realpath
    }
    if not removed:
        raise AssertionError(f"mapped fixture lacks {realpath}")
    maps["files"] = [
        row for row in maps["files"] if row["lexical_path"] not in removed
    ]
    maps["rows"] = [
        row for row in maps["rows"] if row["lexical_path"] not in removed
    ]
    payload["native_loaded_elf_closure"] = bootstrap._native_loaded_elf_closure(
        maps, payload["sealed_sources"]
    )
...
class FormalG0ChildBootstrapTests(unittest.TestCase):
    ...
    def test_probe_to_role_accepts_only_typed_usr_lib_realpath_aliases(self) -> None:
        expected = copy.deepcopy(self.probe)
        module_count, mapped_count, native_count = _alias_receipt_usr_lib_to_lib(
            expected
        )
        ...
        actual = _as_role(self.probe, "primary", 998)
        bootstrap.validate_runtime_receipt_static(
            actual, role="primary", expected_closure=expected
        )

    def test_probe_alias_keeps_hash_inode_and_ordinary_paths_exact(self) -> None:
        actual = _as_role(self.probe, "primary", 998)
        first_module = next(
            row for row in actual["persistent_modules"] if row["file_identities"]
        )
        field = sorted(first_module["file_identities"])[0]
        for label, mutate in (
            (
                "hash",
                lambda identity: identity.__setitem__("sha256", "0" * 64),
            ),
            (
                "inode",
                lambda identity: identity["stat"].__setitem__(
                    "inode", identity["stat"]["inode"] + 1
                ),
            ),
        ):
            drift = copy.deepcopy(actual)
            identity = next(
                row for row in drift["persistent_modules"]
                if row["sys_modules_key"] == first_module["sys_modules_key"]
            )["file_identities"][field]
            mutate(identity)
            _rehash(drift)
            with self.subTest(label=label), self.assertRaises(
                bootstrap.FormalG0RuntimeError
            ):
                bootstrap.validate_runtime_receipt_static(
                    drift, role="primary", expected_closure=self.probe
                )

        with tempfile.TemporaryDirectory() as temporary:
            first = Path(temporary) / "first.so"
            hardlink = Path(temporary) / "hardlink.so"
            other = Path(temporary) / "other.so"
            first.write_bytes(b"same bytes\n")
            os.link(first, hardlink)
            other.write_bytes(first.read_bytes())
            first_identity = bootstrap._probe_persistent_file_identity(
                bootstrap.persistent_file_identity(str(first)), "first"
            )
            hardlink_identity = bootstrap._probe_persistent_file_identity(
                bootstrap.persistent_file_identity(str(hardlink)), "hardlink"
            )
            other_identity = bootstrap._probe_persistent_file_identity(
                bootstrap.persistent_file_identity(str(other)), "other"
            )
            self.assertEqual(
                first_identity["stat"]["inode"],
                hardlink_identity["stat"]["inode"],
            )
            self.assertNotEqual(first_identity, hardlink_identity)
            self.assertNotEqual(first_identity, other_identity)

    def test_exact_tbb_probe_only_leaf_may_be_absent_from_role(self) -> None:
        target = bootstrap._PROBE_OPTIONAL_PERSISTENT_ELF["realpath"]
        actual = _as_role(self.probe, "primary", 998)
        _remove_mapped_realpath(actual, target)
        bootstrap.validate_runtime_receipt_static(
            actual, role="primary", expected_closure=self.probe
        )

        partial = _as_role(self.probe, "primary", 998)
        tbb_rows = [
            row for row in partial["persistent_proc_maps"]["rows"]
            if os.path.realpath(row["lexical_path"]) == target
        ]
        self.assertGreater(len(tbb_rows), 1)
        partial["persistent_proc_maps"]["rows"].remove(tbb_rows[0])
        _rehash(partial)
        with self.assertRaises(bootstrap.FormalG0RuntimeError):
            bootstrap.validate_runtime_receipt_static(
                partial, role="primary", expected_closure=self.probe
            )

    def test_tbb_optional_type_is_exact_and_unrelated_missing_fails(self) -> None:
        target = bootstrap._PROBE_OPTIONAL_PERSISTENT_ELF["realpath"]
        raw = bootstrap.persistent_file_identity(target)
        exact = bootstrap._probe_persistent_file_identity(raw, "exact tbb")
        self.assertTrue(bootstrap._probe_optional_persistent_elf(exact))
        for label, key, value in (
            ("path", "lexical_path", "/usr/lib/not-tbbmalloc.so.2"),
            ("size", "size_bytes", exact["size_bytes"] + 1),
            ("hash", "sha256", "0" * 64),
            (
                "stat",
                "stat",
                {**exact["stat"], "inode": exact["stat"]["inode"] + 1},
            ),
        ):
            drift = copy.deepcopy(exact)
            drift[key] = value
            with self.subTest(label=label):
                self.assertFalse(bootstrap._probe_optional_persistent_elf(drift))
        with self.assertRaises(bootstrap.FormalG0RuntimeError):
            bootstrap._probe_native_identity({"mapping_kind": "OTHER"})

        actual = _as_role(self.probe, "primary", 998)
        unrelated = next(
            row["lexical_path"]
            for row in actual["persistent_proc_maps"]["files"]
            if os.path.realpath(row["lexical_path"]) != target
        )
        _remove_mapped_realpath(actual, os.path.realpath(unrelated))
        with self.assertRaises(bootstrap.FormalG0RuntimeError):
            bootstrap.validate_runtime_receipt_static(
                actual, role="primary", expected_closure=self.probe
            )

    def test_role_semantic_identity_does_not_normalize_optional_tbb(self) -> None:
        target = bootstrap._PROBE_OPTIONAL_PERSISTENT_ELF["realpath"]
        primary = _as_role(self.probe, "primary", 998)
        verification = _as_role(self.probe, "verification", 999)
        _remove_mapped_realpath(verification, target)
        bootstrap.validate_runtime_receipt_static(
            primary, role="primary", expected_closure=self.probe
        )
        bootstrap.validate_runtime_receipt_static(
            verification, role="verification", expected_closure=self.probe
        )
        self.assertNotEqual(
