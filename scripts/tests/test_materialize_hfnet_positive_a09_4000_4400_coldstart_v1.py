#!/usr/bin/env python3
"""Process-free tests for the A09 4000..4400 cold-start preparation."""
...
from scripts import materialize_hfnet_positive_a09_4000_4400_coldstart_v1 as adapter
...
class A09ColdstartMaterializerTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.short_bag.write_bytes(b"not-used-as-image-source")
    ...
    def test_frozen_production_contract_and_provenance(self) -> None:
        ...
        provenance = adapter._require_provenance(
            adapter.DEFAULT_SHORT_BAG, adapter.DEFAULT_CONFIG
        )
        self.assertEqual(
            provenance["source_choice"]["selected"],
            "canonical_raw_archive_png_members",
        )
        self.assertFalse(
            provenance["source_choice"]["rejected_alternative"][
                "canonical_png_container_bytes_available"
            ]
        )
    ...
    def test_no_clobber_rejects_before_source_work(self) -> None:
    ...
    def test_atomic_failure_leaves_neither_output_nor_staging_tree(self) -> None:
        output = self.root / "atomic-failure"
        with self._provenance_patch(), mock.patch.object(
            adapter.core,
            "stream_selected_images",
            side_effect=adapter.ContractError("INJECTED_STREAM_FAILURE"),
        ):
            with self.assertRaisesRegex(
                adapter.ContractError, "INJECTED_STREAM_FAILURE"
            ):
                adapter.materialize(
                    self.source,
                    self.short_bag,
    ...
    def test_script_has_no_process_launcher_import(self) -> None:
