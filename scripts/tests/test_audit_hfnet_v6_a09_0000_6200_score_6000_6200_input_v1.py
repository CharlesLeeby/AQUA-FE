#!/usr/bin/env python3
"""Process-free tests for the independent A09 input auditor."""
...
from scripts import (
    audit_hfnet_v6_a09_0000_6200_score_6000_6200_input_v1 as subject,
)


class A09IndependentAuditorTests(unittest.TestCase):
    ...
    def test_manifest_selection_is_exact_and_tamper_fails(self) -> None:
        ...
        tampered["score_relative_indices_inclusive"] = [5999, 6200]
    ...
    def test_wrapper_audit_repairs_no_scientific_data_and_starts_no_process(self) -> None:
        fake = {
            "auditor": {"sha256": "base"},
            "camera": {"preroll_source_indices_inclusive": [0, 4499]},
            "reporting_boundary": "base",
            "claims": dict(subject.core.CLAIMS),
        }
        with mock.patch.object(
            subject, "_require_reused_auditor_pins", return_value={}
        ), mock.patch.object(subject, "_base_audit", return_value=fake):
            result = subject.audit(
                Path("root"), Path("source"), Path("selector"), Path("config")
            )
        self.assertEqual(result["camera"]["preroll_source_indices_inclusive"], [0, 5999])
        self.assertIn("A09", result["reporting_boundary"])
