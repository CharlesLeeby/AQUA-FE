...
from scripts import govern_matched_birth_r4_vins_g0_v2 as g
...
class ContractTests(unittest.TestCase):
    ...
    def test_incident_namespace_api_is_pure_and_matches_live_job_contract(self) -> None:
        ...
        self.assertEqual(contract["job_id"], "matched_birth_r4_vins_g0_v2")

    def test_builder_shaped_incident_fixture_passes_and_successor_mutation_fails(self) -> None:
        from scripts import build_a02_matched_birth_r4_vins_g0_probe_stderr_incident_v1 as builder

        source = g.Path(g._OUTER_SCRIPT).read_bytes()
        digest = hashlib.sha256(source).hexdigest()
        contract = g.incident_adoption_namespace_contract_v1(digest, len(source))
        incident = builder.build_record(
            successor_sha256=digest,
            successor_size_bytes=len(source),
            v2_job_hash=contract["job_hash"],
            builder_identity=builder._read_only_builder_identity(),
        )
        g._validate_incident_adoption(incident)
        altered = copy.deepcopy(incident)
        altered["successor_authorization"]["v2_job_hash"] = "a" * 64
        altered["incident_hash"] = builder._self_hash(altered)
    ...
    def test_incident_all_material_nested_fields_are_fail_closed(self) -> None:
        ...
        mutations = {
            ...
            "builder_execution": lambda x: x["builder_execution_contract"]["environment"].__setitem__("PATH", "/tmp"),
            "intent_semantics": lambda x: x["retained_v1_evidence"]["intent_semantics"].__setitem__("attempt_count", 2),
            "diagnosis": lambda x: x["static_control_flow_diagnosis"]["facts"].append("invented"),
            "code_authority": lambda x: x["retained_code_authorities"]["backend"].__setitem__("sha256", "a" * 64),
            "outcome_firewall": lambda x: x["outcome_firewall"].__setitem__("vio_metric_files_read", True),
            "claim_boundary": lambda x: x["claim_boundary"].__setitem__("v1_freeze_pass_claimed", True),
            "action_sequence": lambda x: x["successor_authorization"]["authorized_action_sequence"].reverse(),
            "intent_without_success": lambda x: x["successor_authorization"].__setitem__(
                "visible_intent_without_exact_success_closeout_consumes_namespace_and_forbids_retry_or_continuation", False),
            "success_required": lambda x: x["successor_authorization"].__setitem__(
                "success_closeout_required_before_later_actions", False),
            "terminal_evidence_boundary": lambda x: x["successor_authorization"]
                ["terminal_evidence_boundary"].__setitem__(
                    "failure_receipt_durability_unconditional_claimed", True),
        }
    ...
    def test_consumed_v1_intent_semantics_and_authorized_argv_are_exact(self) -> None:
        intent = json.loads(g.OLD_PROBE_INTENT.read_bytes())
        g._validate_consumed_v1_intent(intent)
        for label, mutate in (
            ("attempt", lambda x: x.__setitem__("attempt_count", 2)),
            ("start_count", lambda x: x.__setitem__("authorized_process_start_count", 2)),
            ("no_retry", lambda x: x.__setitem__("no_retry_after_pending_evidence", False)),
            ("status", lambda x: x.__setitem__("status", "OTHER")),
            ("canonical_argv", lambda x: x["authorized_canonical_argv"].__setitem__(0, "/tmp/python")),
            ("actual_argv", lambda x: x["actual_procfd_argv"].__setitem__(4, "--other")),
        ):
            altered = copy.deepcopy(intent)
            mutate(altered)
            altered["probe_launch_intent_hash"] = g.p07gov.canonical_json_hash(
                altered, "probe_launch_intent_hash"
            )
            with self.subTest(label=label), self.assertRaises(g.GovernanceError):
                g._validate_consumed_v1_intent(altered)
