#!/usr/bin/env python3
"""Process-free tests for the additive A08 ToDesk co-run overlay."""
...
class CorunOverlayTest(unittest.TestCase):
    ...
    def test_resource_helper_and_a09_success_precedent_are_frozen(self) -> None:
        rows = (
            (
    ...
    def test_namespace_token_indices_and_timeout_are_a08_specific(self) -> None:
        ...
        self.assertEqual(subject.formal.FEED_SOURCE_LAST, 4660)
        self.assertEqual(subject.formal.FEED_LOCAL_FIRST, 0)
        self.assertEqual(subject.formal.FEED_LOCAL_LAST, 4660)
        self.assertEqual(subject.formal.SCORE_SOURCE_FIRST, 4500)
        self.assertEqual(subject.formal.SCORE_SOURCE_LAST, 4660)
        self.assertEqual(subject.formal.SCORE_COUNT, 161)
        ...
        self.assertEqual(
            ...
            "HFNET_V6_A08_0000_4660_SCORE_4500_4660_TODESK_CORUN_"
            "DEVELOPMENT_ONLY_ATTEMPT_001_START_EXACTLY_ONCE",
        )
        self.assertEqual(
            subject.paths()["score_trajectory"].name,
            "trajectory_score_source_4500_4660.txt",
        )
    ...
    def test_todesk_ros_and_vins_are_allowed_when_memory_floor_holds(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(
                subject.proven_resource.subprocess, "run", side_effect=responses
            ):
    ...
    def test_prepared_manifest_injection_and_validation_are_exact(self) -> None:
        ...
        with mock.patch.object(subject, "_FORMAL_VALIDATE_PREPARED_LAUNCH"):
            ...
            with self.assertRaisesRegex(
                ...
                "A08_CORUN_PREPARED_RESOURCE_CONTRACT_MISMATCH",
            ):
    ...
    def test_check_rebinds_gate_after_installer_before_real_base_check(self) -> None:
        def strict_gate() -> dict[str, object]:
            return {"ready": False}
    ...
    def test_terminal_decoration_retains_duration_but_forbids_runtime_claim(self) -> None:
        value = subject._decorate_terminal(
            {
                "status": "PASS_DEVELOPMENT_RUNABILITY",
                "execution": {"duration_seconds": 321.0},
                "claim_boundary": {"development_only": True},
            }
        )
        self.assertEqual(value["execution"]["duration_seconds"], 321.0)
    ...
    def test_full_code_authority_includes_formal_helper_and_overlay_test(self) -> None:
        ...
        self.assertEqual(
            authority["frozen_a08_controller"]["sha256"],
            subject.FORMAL_RUNNER_SHA256,
        )
        self.assertEqual(
            authority["frozen_a08_controller_test"]["sha256"],
            subject.FORMAL_RUNNER_TEST_SHA256,
        )
        self.assertEqual(
