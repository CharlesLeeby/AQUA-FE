#!/usr/bin/python3
"""Run paired July Learned+KLT/KLT bags under the runtime-exclusive v3 protocol.
...
DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V3 identity so stability remains paired.
"""
...
from fair_stability_ordinal_common_v3 import (
    EXPERIMENT_ID,
...
from fair_stability_runtime_resource_monitor_v3 import (
...
RUNTIME_ADDENDUM = ROOT / "papers/fair_stability_runtime_exclusivity_addendum_v3.md"
...
REPLAY_WRAPPER = ROOT / "scripts/run_fair_stability_vins_replay_child_v3.sh"
RECORD_ENV = ROOT / "scripts/record_vins_env.sh"
WAIT_SUBSCRIBERS = ROOT / "scripts/wait_for_ros_subscribers.py"
RUNTIME_MONITOR = ROOT / "scripts/fair_stability_runtime_resource_monitor_v3.py"
ORDINAL_COMMON = ROOT / "scripts/fair_stability_ordinal_common_v3.py"
ORDINAL_CONTROLLER = ROOT / "scripts/run_fair_stability_next_v3.py"
HFNET_RUNNER = ROOT / "scripts/run_fair_stability_hfnet_openloop_v3.py"
CONTROL_TESTS = ROOT / "scripts/test_fair_stability_controls_v3.py"
UNIFIED_SUMMARY = ROOT / "scripts/summarize_fair_stability_v3.py"
CONTROL_SUPERSESSION = ROOT / "papers/fair_stability_control_supersession_v3.md"
...
def verify_common_inputs() -> tuple[dict[str, dict[str, str]], dict[str, dict[str, str]], dict[str, Any]]:
    ...
    if (
        ...
        != "aqua-fe-fair-stability-hfnet-input-freeze-v3"
...
def freeze_backend() -> dict[str, object]:
    ...
    freeze = {
        "schema_version": "aqua-fe-dev-nativeq-schedfix-runtimeexcl-backend-freeze-v3",
        "experiment_id": EXPERIMENT_ID,
        "status": "FROZEN_BEFORE_NEW_VINS_REPLAY",
        "frozen_at_utc": now_utc(),
        "backend_id": "DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V3",
        "identity_boundary": {
...
def load_freeze() -> dict[str, Any]:
    ...
    if (
        ...
        != "aqua-fe-dev-nativeq-schedfix-runtimeexcl-backend-freeze-v3"
        or freeze.get("experiment_id") != EXPERIMENT_ID
        or freeze.get("backend_id") != "DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V3"
...
def prepare_attempt(
    ...
    *,
    ...
    tag = (
        f"fair_stability_runtimeexcl_v3_{case_id}_{arm}_"
...
def verify_attempt(
    ...
    if (
        attempt.get("schema_version") != "aqua-fe-fair-stability-vins-attempt-v3"
        or attempt.get("experiment_id") != EXPERIMENT_ID
        or attempt.get("backend_id") != "DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V3"
...
def write_unproven_monitor_receipt(
    ...
    receipt: dict[str, Any] = {
        "schema_version": "aqua-fe-fair-stability-runtime-resource-monitor-v3",
        "experiment_id": EXPERIMENT_ID,
...
def run_attempt(case_id: str, arm: str, repeat: int, attempt_index: int = 1) -> dict[str, object]:
    ...
    with lock_path.open("r+b") as lock_stream:
        ...
        claim = {
            "schema_version": "aqua-fe-fair-stability-vins-start-claim-v3",
            "experiment_id": EXPERIMENT_ID,
        ...
        if (
            ...
            != "aqua-fe-fair-stability-runtime-resource-monitor-v3"
...
def summary() -> dict[str, object]:
    ...
    from summarize_fair_stability_v3 import summarize as summarize_receipts
