#!/usr/bin/env python3
"""Execute only new rows in the frozen admission-continuation backend plan."""
...
CELL_RUNNER = ROOT / "scripts/run_frontend_admission_continuation_v1_backend_cell.sh"
...
def main() -> int:
    ...
    for row in rows:
        ...
        if row["execution"] == "REUSED_FROZEN_V2_REPLAY":
            ...
            continue
        replay = Path(row["replay_dir"])
        if (replay / "replay_receipt.txt").is_file():
            validate_reused(row)
            completed += 1
            print(f"validated completed replay {row['run_slug']} {row['cell_id']} repeat{row['repeat']}", flush=True)
            continue
        command = [
            "bash",
            str(CELL_RUNNER),
