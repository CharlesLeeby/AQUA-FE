#!/usr/bin/env python3
"""Fail-closed one-shot supervisor for the frozen A10 same-history v2 run.
...
EXP_ROOT = Path("/mnt/data/AQUA-FE_WS/experiments/a10_samehistory_system_v2")
DEFAULT_LOCK = ROOT / "papers/samehistory_system_comparison_a10_finalonline_v2_execution_lock.json"
...
DEPENDENCIES = {
    "klt_export": (),
    "aquafe_build": ("klt_export",),
    "vanilla_origin_native_image_context": (),
    "external_klt_finalonline_backbone": ("klt_export",),
    "aquafe_finalonline_xfeat_lineage": ("aquafe_build",),
}
BACKENDS = frozenset(ITEM_ORDER[2:])
OUTPUT_DIRS = {
    "klt_export": EXP_ROOT / "frontends/klt_export",
...
WORKSPACE_LINKS = {
    "klt_export": ROOT / "logs/aqualoc_archaeo_vins/external_klt_every2_systemfair_a10_finalonline_v2_feed0000_2800_klt_export",
    "aquafe_build": ROOT / "logs/aqualoc_archaeo_vins/aquafe_finalonline_xfeat_lineage_systemfair_a10_finalonline_v2_feed0000_2800_build",
    "vanilla_origin_native_image_context": ROOT / "logs/aqualoc_archaeo_vins/origin_klt_every1_systemfair_a10_finalonline_v2_feed0000_2800_score2400_2800_vanilla_origin",
    "external_klt_finalonline_backbone": ROOT / "logs/aqualoc_archaeo_vins/external_klt_every2_systemfair_a10_finalonline_v2_feed0000_2800_score2400_2800_external_klt",
    "aquafe_finalonline_xfeat_lineage": ROOT / "logs/aqualoc_archaeo_vins/external_hybrid_xfeat_every2_systemfair_a10_finalonline_v2_feed0000_2800_score2400_2800_aquafe_finalonline_xfeat_lineage",
}
...
# cycle.
EXPECTED_CONTRACT_SHA256 = "8d765090c62eb4b85916b2f11f75cd80e77a9455398a0f1ba93b84cbba521046"
...
def validate_items(lock: Mapping[str, Any], verified: Mapping[str, Mapping[str, Any]]) -> None:
    ...
    required_static_paths = {
        ...
        ROOT / "scripts/analyze_samehistory_system_a10_finalonline_v2.py",
...
def verify_lock(path: Path) -> tuple[dict[str, Any], dict[str, dict[str, Any]], dict[str, Any]]:
    ...
    if builder is None or Path(builder["path"]).absolute() != ROOT / "scripts/build_samehistory_system_a10_finalonline_v2_execution_lock.py":
...
def forbidden_command(argv: Sequence[str]) -> bool:
    ...
    for index, name in enumerate(names):
        if name in {"rosbag", "rosbag.py"} and "play" in argv[index + 1:]:
            return True
    text = " ".join(argv).lower()
    return "hfnet_slam" in text or "mono_tum" in text
...
def finish_claimed(
    ...
    try:
        ...
        try:
            ...
            postflight = {
                "status": "PASS" if not hits and port_error is None else "FAIL",
                "checked_at_local": now_local(), "forbidden_processes": hits,
                "declared_port_error": port_error,
            ...
            if hits or port_error is not None:
