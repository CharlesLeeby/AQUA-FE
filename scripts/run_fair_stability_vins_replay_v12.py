#!/usr/bin/python3
"""Run paired July Learned+KLT/KLT bags under the runtime-exclusive v12 protocol.
...
DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V12 identity so stability remains paired.
"""
...
RUNTIME_ADDENDUM = ROOT / "papers/fair_stability_runtime_exclusivity_addendum_v12.md"
...
REPLAY_WRAPPER = ROOT / "scripts/run_fair_stability_vins_replay_child_v12.sh"
RECORD_ENV = ROOT / "scripts/record_vins_env.sh"
WAIT_SUBSCRIBERS = ROOT / "scripts/wait_for_ros_subscribers.py"
RUNTIME_MONITOR = ROOT / "scripts/fair_stability_runtime_resource_monitor_v12.py"
ORDINAL_COMMON = ROOT / "scripts/fair_stability_ordinal_common_v12.py"
ORDINAL_CONTROLLER = ROOT / "scripts/run_fair_stability_next_v12.py"
HFNET_RUNNER = ROOT / "scripts/run_fair_stability_hfnet_openloop_v12.py"
CONTROL_TESTS = ROOT / "scripts/test_fair_stability_controls_v12.py"
PENDING_REAP_TESTS = ROOT / "scripts/test_fair_stability_watchdog_pending_v12.py"
UNIFIED_SUMMARY = ROOT / "scripts/summarize_fair_stability_v12.py"
...
ROSCORE_COMMAND_INTERPRETER = Path("/usr/bin/python3")
ROSCORE_RESOLVED_INTERPRETER = ROSCORE_COMMAND_INTERPRETER.resolve()
ROSCORE_SCRIPT = Path("/opt/ros/noetic/bin/roscore")
...
def verify_common_inputs() -> tuple[dict[str, dict[str, str]], dict[str, dict[str, str]], dict[str, Any]]:
    ...
    if (
        ...
        != "aqua-fe-fair-stability-hfnet-input-freeze-v12"
        ...
        or claims.get("v5_results_imported") is not False
        or claims.get("v6_results_imported") is not False
        or claims.get("v7_results_imported") is not False
        or claims.get("v8_results_imported") is not False
    ...
    if (
        ...
        or not identity_matches(
            execution_controls.get("watchdog_pending_tests"),
            identity(PENDING_REAP_TESTS),
        )
    ):
    ...
    expected_parent_documents = {
        ...
        "v5_watchdog_exit_race_abandonment_report": V5_ABANDONMENT_REPORT,
        "v6_final_gate_timing_abandonment_report": V6_ABANDONMENT_REPORT,
        "v7_teardown_snapshot_abandonment_report": V7_ABANDONMENT_REPORT,
        ...
        "v10_proc_tail_closure_abandonment_report": V10_ABANDONMENT_REPORT,
        "v11_watchdog_pending_reap_abandonment_report": V11_ABANDONMENT_REPORT,
...
def freeze_backend() -> dict[str, object]:
    ...
    backend = {
        ...
        "roscore_launch_control": {
            "command_interpreter": str(ROSCORE_COMMAND_INTERPRETER),
            "resolved_interpreter": identity(ROSCORE_RESOLVED_INTERPRETER),
            "script": identity(ROSCORE_SCRIPT),
        },
    }
    ...
    freeze = {
        "schema_version": "aqua-fe-dev-nativeq-schedfix-runtimeexcl-backend-freeze-v12",
        "experiment_id": EXPERIMENT_ID,
        "status": "FROZEN_BEFORE_NEW_VINS_REPLAY",
        "frozen_at_utc": now_utc(),
        "backend_id": "DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V12",
        "identity_boundary": {
        ...
        "v5_abandonment_report": identity(V5_ABANDONMENT_REPORT),
        "v6_abandonment_report": identity(V6_ABANDONMENT_REPORT),
        "v7_abandonment_report": identity(V7_ABANDONMENT_REPORT),
        "v8_abandonment_report": identity(V8_ABANDONMENT_REPORT),
        "v9_abandonment_report": identity(V9_ABANDONMENT_REPORT),
        "v10_abandonment_report": identity(V10_ABANDONMENT_REPORT),
        "v11_abandonment_report": identity(V11_ABANDONMENT_REPORT),
...
def load_freeze() -> dict[str, Any]:
    ...
    if (
        ...
        != "aqua-fe-dev-nativeq-schedfix-runtimeexcl-backend-freeze-v12"
        or freeze.get("experiment_id") != EXPERIMENT_ID
        or freeze.get("backend_id") != "DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V12"
    ...
    for key, path in (
        ...
        ("v5_abandonment_report", V5_ABANDONMENT_REPORT),
        ("v6_abandonment_report", V6_ABANDONMENT_REPORT),
        ("v7_abandonment_report", V7_ABANDONMENT_REPORT),
        ("v8_abandonment_report", V8_ABANDONMENT_REPORT),
        ("v9_abandonment_report", V9_ABANDONMENT_REPORT),
        ("v10_abandonment_report", V10_ABANDONMENT_REPORT),
        ("v11_abandonment_report", V11_ABANDONMENT_REPORT),
        ("roster", ROSTER),
        ("historical_results", HISTORICAL_RESULTS),
        ("historical_manifest", HISTORICAL_MANIFEST),
    ...
    roscore_launch = freeze["backend"].get("roscore_launch_control")
    if (
        not isinstance(roscore_launch, Mapping)
        or set(roscore_launch) != {
            "command_interpreter", "resolved_interpreter", "script"
        }
        or roscore_launch.get("command_interpreter")
        != str(ROSCORE_COMMAND_INTERPRETER)
        or ROSCORE_COMMAND_INTERPRETER.resolve()
        != ROSCORE_RESOLVED_INTERPRETER
        or not identity_matches(
            identity(ROSCORE_RESOLVED_INTERPRETER),
            roscore_launch.get("resolved_interpreter"),
        )
        or not identity_matches(
            identity(ROSCORE_SCRIPT), roscore_launch.get("script")
        )
    ):
        raise ContractError("FROZEN_ROSCORE_LAUNCH_CONTROL_DRIFT")
...
def prepare_attempt(
    ...
    *,
    ...
    tag = (
        f"fair_stability_runtimeexcl_v12_{case_id}_{arm}_"
        f"repeat_{repeat:03d}_attempt_{attempt_index:03d}"
    )
    attempt = {
        "schema_version": "aqua-fe-fair-stability-vins-attempt-v12",
        "experiment_id": EXPERIMENT_ID,
        "status": "PREPARED_NOT_STARTED",
        "prepared_at_utc": now_utc(),
        "backend_id": "DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V12",
        "backend_freeze": identity(BACKEND_FREEZE),
        "roscore_launch_control": freeze["backend"]["roscore_launch_control"],
        "case_id": case_id,
        "arm": arm,
        "repeat": repeat,
...
def verify_attempt(
    ...
    if (
        attempt.get("schema_version") != "aqua-fe-fair-stability-vins-attempt-v12"
        or attempt.get("experiment_id") != EXPERIMENT_ID
        or attempt.get("backend_id") != "DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V12"
    ...
    if attempt.get("roscore_launch_control") != freeze["backend"].get(
        "roscore_launch_control"
    ):
        raise ContractError("ATTEMPT_ROSCORE_LAUNCH_CONTROL_DRIFT")
    for key in (
        "source_config",
...
def parse_vio(path: Path, stamps: Sequence[int]) -> dict[str, object]:
    result: dict[str, object] = {
        "exists": path.is_file(),
        "file_nonempty": bool(path.is_file() and path.stat().st_size > 0),
        "valid": False,
        "pose_count": 0,
        "errors": [],
    }
...
def parse_child_start(
    ...
    for key in ("wrapper_pid", "roscore_pid", "vins_pid"):
        try:
            parsed_pids.append(int(values.get(key, "")))
        except ValueError:
            parsed_pids.append(-1)
    if len(set(parsed_pids)) != 3 or any(pid <= 0 for pid in parsed_pids):
        errors.append("CHILD_PROCESS_PIDS_NOT_DISTINCT")
    if values.get("roscore_executable") != str(ROSCORE_RESOLVED_INTERPRETER):
        errors.append("ROSCORE_EXECUTABLE_INVALID")
    if values.get("roscore_command", "").split() != [
        str(ROSCORE_COMMAND_INTERPRETER),
        str(ROSCORE_SCRIPT),
        "-p",
        str(expected_port),
    ...
    if command != [str(expected_executable), str(expected_config)]:
...
def run_attempt(case_id: str, arm: str, repeat: int, attempt_index: int = 1) -> dict[str, object]:
    ...
    with open_regular_lock(lock_path) as lock_stream:
        ...
        claim = {
            "schema_version": "aqua-fe-fair-stability-vins-start-claim-v12",
            "experiment_id": EXPERIMENT_ID,
        ...
        if not timed_out and isinstance(lifecycle_values, Mapping):
            ...
            if lifecycle_nonempty in {"0", "1"} and (
                (lifecycle_nonempty == "1")
                is not bool(trajectory.get("file_nonempty"))
            ):
