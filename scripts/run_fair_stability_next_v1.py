#!/usr/bin/python3
"""Single ordinal dispatcher for the frozen fair-stability experiment."""
...
VINS_RUNNER = ROOT / "scripts/run_fair_stability_vins_replay_v1.py"
...
def verify_control_freeze() -> dict[str, Any]:
    ...
    for path in (CONTROLLER, HFNET_RUNNER, SCRIPT_DIR / "fair_stability_ordinal_common_v1.py"):
...
def adopt_terminal(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        "schema_version": "aqua-fe-fair-stability-ordinal-terminal-v1",
...
def adjudicate_pipeline_invalid(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        "schema_version": "aqua-fe-fair-stability-pipeline-invalid-adjudication-v1",
...
def dispatch_claim(state: Mapping[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    cell = state["cell"]
    claim = {
        "schema_version": "aqua-fe-fair-stability-ordinal-dispatch-v1",
...
def dispatch(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    execution = {
        "schema_version": "aqua-fe-fair-stability-controller-dispatch-execution-v1",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("command", choices=("status", "run-next"))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = next_state(EXPERIMENT_ROOT) if args.command == "status" else run_next()
