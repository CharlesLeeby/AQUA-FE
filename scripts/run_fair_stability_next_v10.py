#!/usr/bin/python3
"""Single ordinal dispatcher for the systemd-supervised v10 experiment."""
...
VINS_RUNNER = ROOT / "scripts/run_fair_stability_vins_replay_v10.py"
...
def verify_systemd_authority() -> dict[str, object]:
    ...
    if (
        claim.get("schema_version") != "aqua-fe-fair-stability-systemd-submission-v10"
...
def verify_control_freeze() -> dict[str, Any]:
    ...
    for path in (
        ...
        HFNET_RUNNER,
        SYSTEMD_SUPERVISOR,
        SCRIPT_DIR / "fair_stability_ordinal_common_v10.py",
    ):
...
def adopt_terminal(
    ...
    payload = {
        "schema_version": "aqua-fe-fair-stability-ordinal-terminal-v10",
...
def automatic_retry_codes(state: Mapping[str, Any]) -> list[str]:
    """Validate the evidence shape, then enforce v10's zero-retry policy."""
    result = state["result"]
    ...
    raise ControllerError(
        "PIPELINE_INVALID_HALT_MANUAL_ABANDONMENT_REQUIRED_ZERO_RETRY_V10:"
        f"{codes}"
    )
...
def adjudicate_pipeline_invalid(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        "schema_version": "aqua-fe-fair-stability-pipeline-invalid-adjudication-v10",
        "experiment_id": EXPERIMENT_ID,
        "adjudicated_at_utc": now_utc(),
        "accepted_as_external_pipeline_fault": True,
        "automatic_retry_policy": "EXPLICIT_EXTERNAL_TRANSIENT_ONLY_V10",
...
def dispatch_claim(state: Mapping[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    cell = state["cell"]
    claim = {
        "schema_version": "aqua-fe-fair-stability-ordinal-dispatch-v10",
        "experiment_id": EXPERIMENT_ID,
...
def dispatch(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    check_contract_valid = (
        ...
        and valid_prestart_resource_gate_v10(
            resource_gate_value,
    ...
    execution = {
        "schema_version": "aqua-fe-fair-stability-controller-dispatch-execution-v10",
        "experiment_id": EXPERIMENT_ID,
