#!/usr/bin/env python3
"""Build native-q candidate v3 after P06 window-selection v2 calibration."""
...
DEFAULT_OUTPUT = BUNDLE / "method_lock_nativeq_legacy_candidate_v3.json"
...
P06_CALIBRATION = BUNDLE / "p06/development_calibration_v2.json"
...
def build_payload() -> dict[str, object]:
    ...
    local_paths = [
        ...
        "papers/ieee_sensors_journal_experiments/window_selection_protocol_v2.md",
        "papers/ieee_sensors_journal_experiments/p06/b1_screening_code_hashes_v2.sha256",
        "papers/ieee_sensors_journal_experiments/p06/development_score_audit_v2.csv",
        "papers/ieee_sensors_journal_experiments/p06/development_calibration_v2.json",
        "papers/ieee_sensors_journal_experiments/p06/window_selection_v2_calibration_report.md",
        "papers/ieee_sensors_journal_experiments/p06/development_equivalence/aqualoc_a06_2210_2460/equivalence_audit.json",
        "papers/ieee_sensors_journal_experiments/p06/development_equivalence/aqualoc_a06_2210_2460/frame_equivalence_audit.csv",
        ...
        "scripts/build_nativeq_legacy_method_lock_v3.py",
        "scripts/audit_p06_screening_equivalence.py",
        "scripts/build_p06_window_selection_v2.py",
        "scripts/p06_window_selection_v2.py",
        "scripts/run_p06_development_equivalence.sh",
        "scripts/run_p06_development_calibration_v2.sh",
        ...
        "scripts/tests/test_p06_window_screening_contract.py",
    ...
    previous_nativeq_candidate = file_record(
        "papers/ieee_sensors_journal_experiments/method_lock_nativeq_legacy_candidate_v2.json",
        role="superseded_nativeq_candidate",
    )
    p05_status = read_p05_status()
    p06_status = read_p06_status()
    missing = [
        str(record["path"])
        for record in artifacts
        + development_evidence
        + [old_candidate, previous_nativeq_candidate]
    ...
    if p06_status != "PASS":
        blockers.append("P06 window-selection v2 equivalence/calibration audit has not passed")
    blockers.append("P06 outcome-blind full-sequence screening and final manifest are pending")

    payload: dict[str, object] = {
        "schema_version": "isj-method-lock-nativeq-legacy-candidate-v3",
        "protocol_version": "isj-nativeq-legacy-candidate-v3",
        "status": (
            "READY_FOR_P06_SCREENING"
            if p05_status == "PASS" and p06_status == "PASS" and not missing
        ...
        "supersedes_nativeq_candidate": {
            "path": previous_nativeq_candidate["path"],
            "sha256": previous_nativeq_candidate["sha256"],
            "disposition": (
                "retained_unchanged; superseded because the ROS exporter hash "
                "changed during outcome-blind P06 metric contract repair"
            ),
        },
        "method_identity": {
            "name": "AQUA-FE native-q learned-seeded legacy candidate",
            "profile": "isj-nativeq-xfeat-seedchain-legacy-v3",
        ...
        "shared_contract": {
            ...
            "window_selection_protocol": "isj-window-selection-v2",
            "window_selection_thresholds": {"tau_low": 0.17, "tau_normal": 0.10},
        },
        "p05_status": p05_status,
        "p06_window_selection_calibration_status": p06_status,
        "artifacts": artifacts,
...
def validate_payload(payload: dict[str, object]) -> None:
    ...
    if shared["window_selection_protocol"] != "isj-window-selection-v2":
        raise ValueError("window-selection protocol drift")
    if payload["p06_window_selection_calibration_status"] != "PASS":
        raise ValueError("P06 window-selection v2 calibration is not PASS")
...
def read_p06_status() -> str:
    if not P06_CALIBRATION.is_file():
        return "NOT_RUN"
    try:
        return str(
            json.loads(P06_CALIBRATION.read_text(encoding="utf-8")).get(
                "status", "INVALID"
            )
        )
    except (OSError, json.JSONDecodeError):
