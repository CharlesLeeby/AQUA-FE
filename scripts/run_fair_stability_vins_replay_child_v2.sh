...
group_residuals() {
  ps -eo pid=,pgid=,stat= | awk -v pgid="$WRAPPER_PGID" -v self="$$" \
    '$2 == pgid && $1 != self {print $1 ":" $3}' | paste -sd, -
}
...
write_lifecycle() {
  ...
  DESCENDANT_RESIDUALS="$(group_residuals)"
  if [[ -n "$DESCENDANT_RESIDUALS" ]]; then
    DESCENDANT_RESIDUAL_COUNT="$(awk -F, '{print NF}' <<< "$DESCENDANT_RESIDUALS")"
  else
    DESCENDANT_RESIDUAL_COUNT=0
  fi
  {
    printf 'schema_version=aqua-fe-fair-stability-vins-child-lifecycle-v2\n'
...
{
  printf 'schema_version=aqua-fe-fair-stability-vins-child-start-v2\n'
  printf 'experiment_id=%s\n' "$EXPERIMENT_ID"
  printf 'tag=%s\n' "$TAG"
  printf 'wrapper_pid=%s\n' "$WRAPPER_PID"
  printf 'wrapper_start_ticks=%s\n' "$WRAPPER_START_TICKS"
...
{
  printf 'schema_version=aqua-fe-fair-stability-vins-replay-manifest-v2\n'
  printf 'experiment_id=%s\n' "$EXPERIMENT_ID"
  printf 'run_dir=%s\n' "$RUN_DIR"
  printf 'play_bag=%s\n' "$FEATURE_BAG"
  ...
  printf 'shutdown_contract=EXPECTED_BAG_END_SIGTERM_REAP_V2\n'
