#!/usr/bin/env python3
"""Analysis-only audit for the frozen A10 final-online v3 recovery run.

The five project items are one-shot executions.  The first item adopts one
predeclared, byte-pinned KLT payload from the retained failed v2 attempt; it is
not a new KLT generation and it never reclassifies v2 as successful.  A missing
terminal receipt is therefore PENDING even if partial output files exist.  The
common-support CPU evaluator is allowed to run only after all five receipts are
terminal-success, recovery provenance passes, and all four systems pass the
preregistered score-window usability gate.
...
imputation is computed.  Any result remains pipeline-debug/secondary-
sensitivity evidence and is not primary paper evidence.
"""
...
DEFAULT_OUTPUT = (
    ROOT / "papers/samehistory_system_comparison_a10_finalonline_v3_recovery_analysis"
)
EXECUTION_LOCK = (
    ROOT / "papers/samehistory_system_comparison_a10_finalonline_v3_recovery_execution_lock.json"
)
PROTOCOL = ROOT / "papers/samehistory_system_comparison_a10_finalonline_v3_recovery_protocol.md"
KLT_ADOPTION_MANIFEST_SOURCE = (
    ROOT
    / "papers/samehistory_system_comparison_a10_finalonline_v3_recovery_klt_adoption_manifest.json"
)
...
V2_KLT_CLAIM = V2_KLT_DIR / "process_start_claim.json"
V2_KLT_RECEIPT = V2_KLT_DIR / "formal_run_receipt_v2.json"
V2_KLT_LOG = V2_KLT_DIR / "supervisor_process.log"
V2_EXECUTION_LOCK = (
    ROOT / "papers/samehistory_system_comparison_a10_finalonline_v2_execution_lock.json"
)
V2_KLT_FEATURES = V2_KLT_DIR / "features.bag"
V2_KLT_METRICS = V2_KLT_DIR / "frontend_metrics.csv"
V2_KLT_CAMERA = V2_KLT_DIR / "aqualoc_archaeo10_pinhole.yaml"

RECOVERY_MANIFEST_SCHEMA = (
    "aqua-fe-a10-samehistory-klt-input-adoption-manifest-v1"
)
RECOVERY_EVIDENCE_SCOPE = "PIPELINE_DEBUG_AND_SECONDARY_SENSITIVITY_ONLY"
RECOVERY_SOURCE_FAILURE = "POSTFLIGHT_PROCESS_PRESENT"
RECOVERY_LOCK_IDENTITY_PATHS: Mapping[str, Path] = {
    "klt_adoption_manifest": KLT_ADOPTION_MANIFEST_SOURCE,
    "v2_klt_claim": V2_KLT_CLAIM,
    "v2_klt_receipt": V2_KLT_RECEIPT,
    "v2_klt_log": V2_KLT_LOG,
    "v2_execution_lock": V2_EXECUTION_LOCK,
    "v2_klt_features_bag": V2_KLT_FEATURES,
    "v2_klt_frontend_metrics": V2_KLT_METRICS,
    "v2_klt_camera_yaml": V2_KLT_CAMERA,
}
...
ITEM_DIRS: Mapping[str, Path] = {
    "klt_input_adoption": EXP / "frontends/klt_input_adoption",
...
REQUIRED_OUTPUTS: Mapping[str, tuple[Path, ...]] = {
    "klt_input_adoption": (
        ITEM_DIRS["klt_input_adoption"] / "features.bag",
        ITEM_DIRS["klt_input_adoption"] / "frontend_metrics.csv",
        ITEM_DIRS["klt_input_adoption"] / "aqualoc_archaeo10_pinhole.yaml",
        ITEM_DIRS["klt_input_adoption"] / "adoption_manifest.json",
    ),
    "aquafe_build": (
    ...
    if not isinstance(items, Mapping) or set(items) != set(ITEM_ORDER):
        ...
        for item_id in ITEM_ORDER:
            ...
            required_output_names = {
                path.relative_to(ITEM_DIRS[item_id]).as_posix()
                for path in REQUIRED_OUTPUTS[item_id]
            }
            if set(outputs) != required_output_names:
                errors.append(f"EXECUTION_LOCK_REQUIRED_OUTPUT_SET_DRIFT:{item_id}")
            unsafe = False
...
def audit_recovery_provenance(
    ...
    """Re-audit why the v2 payload may be adopted without calling v2 a PASS."""
    errors: list[str] = []
    actual_identities: dict[str, dict[str, Any]] = {}
    static_manifest: Optional[dict[str, Any]] = None
    ...
    if lock is None or lock_audit.get("state") != "PASS":
        return {
            "state": "BLOCKED_EXECUTION_LOCK_EVIDENCE_INVALID",
            "provenance_valid": False,
            "adoption_accepted_for_v3": False,
            "evidence_scope": RECOVERY_EVIDENCE_SCOPE,
            "paper_primary_evidence_permitted": False,
            "v2_source_attempt_successful": False,
            "v2_source_attempt_disposition": "EXECUTION_INTEGRITY_FAILED",
            "errors": ["RECOVERY_REQUIRES_VALID_V3_EXECUTION_LOCK"],
        }

    locked_identities = lock.get("identities")
    if not isinstance(locked_identities, Mapping):
        fail("RECOVERY_LOCK_IDENTITIES_MISSING")
        locked_identities = {}
    for key, path in RECOVERY_LOCK_IDENTITY_PATHS.items():
        try:
            actual = identity(path)
            actual_identities[key] = actual
            if locked_identities.get(key) != actual:
                fail(f"RECOVERY_LOCK_IDENTITY_DRIFT:{key}")
        except EvidenceError as error:
            fail(f"RECOVERY_SOURCE_IDENTITY_INVALID:{key}:{error}")

    try:
        static_manifest = read_json(KLT_ADOPTION_MANIFEST_SOURCE)
    except EvidenceError as error:
        fail(f"RECOVERY_STATIC_MANIFEST_INVALID:{error}")

    required_manifest_keys = {
        "schema_version",
        "adoption_disposition",
        "evidence_scope",
        "paper_primary_evidence_permitted",
        "selection_policy",
        "source_attempt",
    ...
    if isinstance(static_manifest, Mapping):
        if set(static_manifest) != required_manifest_keys:
            fail("RECOVERY_STATIC_MANIFEST_KEY_SET_DRIFT")
        if static_manifest.get("schema_version") != RECOVERY_MANIFEST_SCHEMA:
            fail("RECOVERY_STATIC_MANIFEST_SCHEMA_DRIFT")
        if (
            static_manifest.get("adoption_disposition")
            != "SEALED_BYTE_PRESERVING_INPUT_ONLY"
        ):
            fail("RECOVERY_ADOPTION_DISPOSITION_DRIFT")
        if static_manifest.get("evidence_scope") != RECOVERY_EVIDENCE_SCOPE:
            fail("RECOVERY_EVIDENCE_SCOPE_DRIFT")
        if static_manifest.get("paper_primary_evidence_permitted") is not False:
            fail("RECOVERY_PRIMARY_EVIDENCE_MUST_BE_FORBIDDEN")

        expected_selection = {
            "candidate_count": 1,
            "backend_trajectories_available_when_adoption_was_frozen": 0,
            "result_informed_selection": False,
            "fallback_to_regeneration_within_v3": False,
        }
        if static_manifest.get("selection_policy") != expected_selection:
            fail("RECOVERY_SELECTION_POLICY_DRIFT")

        expected_source_attempt = {
            "experiment": "a10_samehistory_system_v2",
            "item_id": "klt_export",
            "terminal_process_status": "TERMINAL_PROCESS_RC0",
            "raw_return_code": 0,
            "artifact_contract_status": "PASS",
            "execution_integrity_status": "FAIL",
            "execution_failure_reason": RECOVERY_SOURCE_FAILURE,
            "overall_disposition": "EXECUTION_INTEGRITY_FAILED",
            "overall_pass": False,
            "must_not_be_described_as_pass": True,
        }
        if static_manifest.get("source_attempt") != expected_source_attempt:
            fail("RECOVERY_SOURCE_ATTEMPT_DECLARATION_DRIFT")

        required_forbidden = {
            "v2 execution passed",
            "v2 is a formal successful attempt",
            "v3 adoption repairs or reclassifies the v2 receipt",
            "v3 recovery is primary paper evidence without a later exclusive regeneration",
        }
        forbidden = static_manifest.get("forbidden_interpretations")
        if (
            ...
            fail("RECOVERY_FORBIDDEN_INTERPRETATIONS_DRIFT")

        evidence_key_map = {
            "claim": "v2_klt_claim",
            "receipt": "v2_klt_receipt",
            "process_log": "v2_klt_log",
            "execution_lock": "v2_execution_lock",
        }
        evidence = static_manifest.get("source_attempt_evidence")
        if not isinstance(evidence, Mapping) or set(evidence) != set(evidence_key_map):
            fail("RECOVERY_SOURCE_EVIDENCE_MAP_DRIFT")
        else:
            for manifest_key, lock_key in evidence_key_map.items():
                actual = actual_identities.get(lock_key)
                if actual is None or evidence.get(manifest_key) != actual:
                    fail(f"RECOVERY_MANIFEST_SOURCE_IDENTITY_DRIFT:{manifest_key}")

        adopted_specs = {
            "features_bag": (
                "v2_klt_features_bag", V2_KLT_FEATURES, "features.bag"
            ),
            "frontend_metrics": (
                "v2_klt_frontend_metrics", V2_KLT_METRICS, "frontend_metrics.csv"
            ),
            "camera_yaml": (
                "v2_klt_camera_yaml", V2_KLT_CAMERA,
        ...
        adopted = static_manifest.get("adopted_files")
        if not isinstance(adopted, Mapping) or set(adopted) != set(adopted_specs):
            fail("RECOVERY_ADOPTED_FILE_MAP_DRIFT")
        else:
            for manifest_key, (lock_key, source_path, output_name) in adopted_specs.items():
                actual = actual_identities.get(lock_key)
                entry = adopted.get(manifest_key)
                expected_entry = None
                if actual is not None:
                    expected_entry = {
                        ...
                        "expected_output_sha256": actual["sha256"],
                        "expected_output_size_bytes": actual["size_bytes"],
                    }
                if expected_entry is None or entry != expected_entry:
                    fail(f"RECOVERY_ADOPTED_FILE_DECLARATION_DRIFT:{manifest_key}")

    v2_receipt: Optional[dict[str, Any]] = None
    try:
        v2_receipt = read_json(V2_KLT_RECEIPT)
    except EvidenceError as error:
        fail(f"RECOVERY_V2_RECEIPT_INVALID:{error}")
    if isinstance(v2_receipt, Mapping):
        terminal = v2_receipt.get("terminal_process")
        artifact = v2_receipt.get("artifact_contract")
        execution = v2_receipt.get("execution_integrity")
        score = v2_receipt.get("score_usability")
        if (
            v2_receipt.get("item_id") != "klt_export"
            or v2_receipt.get("status") != "TERMINAL_PROCESS_RC0"
            or v2_receipt.get("overall_disposition") != "EXECUTION_INTEGRITY_FAILED"
        ):
            fail("RECOVERY_V2_RECEIPT_TOP_LEVEL_DISPOSITION_DRIFT")
        if (
            ...
            fail("RECOVERY_V2_TERMINAL_RC0_NOT_ESTABLISHED")
        if (
            not isinstance(artifact, Mapping)
            or artifact.get("status") != "PASS"
            or artifact.get("issues") != []
        ):
            fail("RECOVERY_V2_ARTIFACT_PASS_NOT_ESTABLISHED")
        if (
            not isinstance(execution, Mapping)
            or execution.get("status") != "FAIL"
            or execution.get("supervisor_error") != RECOVERY_SOURCE_FAILURE
            or not isinstance(execution.get("postflight"), Mapping)
            or execution["postflight"].get("status") != "FAIL"
        ):
            fail("RECOVERY_V2_EXECUTION_FAILURE_NOT_ESTABLISHED")
        if score != {"status": "NOT_APPLICABLE", "failure_codes": []}:
            fail("RECOVERY_V2_SCORE_LAYER_DRIFT")
        ...
        for receipt_key, lock_key in receipt_links.items():
            actual = actual_identities.get(lock_key)
            if actual is None or not identity_claim_matches(
                v2_receipt.get(receipt_key), actual
            ):
                fail(f"RECOVERY_V2_RECEIPT_LINK_DRIFT:{receipt_key}")

        recorded_outputs = artifact.get("outputs") if isinstance(artifact, Mapping) else None
        source_outputs = {
            V2_KLT_FEATURES: "v2_klt_features_bag",
            V2_KLT_METRICS: "v2_klt_frontend_metrics",
            V2_KLT_CAMERA: "v2_klt_camera_yaml",
        }
        if not isinstance(recorded_outputs, Mapping) or set(recorded_outputs) != {
            str(path) for path in source_outputs
        }:
            fail("RECOVERY_V2_ARTIFACT_OUTPUT_SET_DRIFT")
        else:
            for source_path, lock_key in source_outputs.items():
                actual = actual_identities.get(lock_key)
                if actual is None or not identity_claim_matches(
                    recorded_outputs.get(str(source_path)), actual
                ):
                    fail(f"RECOVERY_V2_ARTIFACT_OUTPUT_IDENTITY_DRIFT:{source_path.name}")

    try:
        v2_claim = read_json(V2_KLT_CLAIM)
        v2_lock = read_json(V2_EXECUTION_LOCK)
        v2_lock_identity = actual_identities.get("v2_execution_lock")
        if (
            v2_claim.get("item_id") != "klt_export"
            or v2_claim.get("retry_count") != 0
            or v2_claim.get("popen_invocation_count_at_claim") != 0
            or v2_claim.get("launch_allowance_consumed") is not True
            or v2_lock_identity is None
            or not identity_claim_matches(v2_claim.get("execution_lock"), v2_lock_identity)
        ):
            fail("RECOVERY_V2_CLAIM_CONTRACT_DRIFT")
        v2_items = v2_lock.get("items")
        if (
            v2_lock.get("status") != "FROZEN_BEFORE_SCORED_RUNS"
            or not isinstance(v2_items, Mapping)
            or not isinstance(v2_items.get("klt_export"), Mapping)
        ):
            fail("RECOVERY_V2_EXECUTION_LOCK_CONTRACT_DRIFT")
    except EvidenceError as error:
        fail(f"RECOVERY_V2_CLAIM_OR_LOCK_INVALID:{error}")

    adopted_manifest_path = (
        ITEM_DIRS["klt_input_adoption"] / "adoption_manifest.json"
    )
    adopted_manifest_identity: Optional[dict[str, Any]] = None
    if adopted_manifest_path.exists() or adopted_manifest_path.is_symlink():
        try:
            adopted_manifest_identity = identity(adopted_manifest_path)
            source_identity = actual_identities.get("klt_adoption_manifest")
            if source_identity is None or (
                adopted_manifest_identity["size_bytes"] != source_identity["size_bytes"]
                or adopted_manifest_identity["sha256"] != source_identity["sha256"]
            ):
                fail("RECOVERY_ADOPTED_MANIFEST_NOT_BYTE_IDENTICAL")
            if static_manifest is None or read_json(adopted_manifest_path) != static_manifest:
                fail("RECOVERY_ADOPTED_MANIFEST_VALUE_DRIFT")
        except EvidenceError as error:
            fail(f"RECOVERY_ADOPTED_MANIFEST_INVALID:{error}")

    adoption_accepted = adoption_receipt.get("successful") is True
    if adoption_accepted:
        if adopted_manifest_identity is None:
            fail("RECOVERY_SUCCESSFUL_ADOPTION_MISSING_MANIFEST")
        artifact_outputs = adoption_receipt.get("artifact_outputs")
        if (
            adopted_manifest_identity is None
            or not isinstance(artifact_outputs, Mapping)
            or not identity_claim_matches(
                artifact_outputs.get(str(adopted_manifest_path.absolute())),
                adopted_manifest_identity,
            )
        ):
            fail("RECOVERY_ADOPTION_RECEIPT_MANIFEST_BINDING_INVALID")

    provenance_valid = not errors
    state = (
        "PASS_SEALED_ADOPTION_PROVENANCE"
        if provenance_valid and adoption_accepted
        else "PASS_SOURCE_PROVENANCE_ADOPTION_PENDING"
        if provenance_valid
        and adoption_receipt.get("state") == "PENDING_MISSING_RECEIPT"
        else "PASS_SOURCE_PROVENANCE_ADOPTION_NOT_ACCEPTED"
        if provenance_valid
        else "FAIL_RECOVERY_PROVENANCE"
    )
    return clean_json(
        {
            "state": state,
            "provenance_valid": provenance_valid,
            "adoption_accepted_for_v3": bool(provenance_valid and adoption_accepted),
            "adoption_is_new_generation": False,
            "adoption_reclassifies_v2": False,
            "evidence_scope": RECOVERY_EVIDENCE_SCOPE,
            "paper_primary_evidence_permitted": False,
            "winner_permitted": False,
            "ranking_permitted": False,
            "significance_test_permitted": False,
            "v2_source_attempt_successful": False,
            "v2_terminal_process_status": "TERMINAL_PROCESS_RC0",
            "v2_raw_return_code": 0,
            "v2_artifact_contract_status": "PASS",
            "v2_execution_integrity_status": "FAIL",
            "v2_execution_failure_reason": RECOVERY_SOURCE_FAILURE,
            "v2_source_attempt_disposition": "EXECUTION_INTEGRITY_FAILED",
            "source_evidence_identities": actual_identities,
            "adopted_manifest_identity": adopted_manifest_identity,
            "selection_policy": static_manifest.get("selection_policy")
            if isinstance(static_manifest, Mapping)
            else None,
            "forbidden_interpretations": static_manifest.get(
                "forbidden_interpretations"
            )
            if isinstance(static_manifest, Mapping)
            else None,
...
def frozen_supervisor_audit_namespace(lock: Mapping[str, Any]) -> dict[str, Any]:
    ...
    supervisor_path = ROOT / "scripts/run_samehistory_system_a10_finalonline_v3_recovery.py"
...
def evaluator_input_manifest(
    ...
    for name, source_path in RECOVERY_LOCK_IDENTITY_PATHS.items():
        files[f"recovery_{name}"] = identity(source_path)
    files["klt_adoption_manifest_output"] = identity(
        ITEM_DIRS["klt_input_adoption"] / "adoption_manifest.json"
    )
    for arm in VINS_ARMS:
...
def build_evaluator_command(output_dir: Path, hfnet_canonical_path: Path) -> list[str]:
    ...
    command.extend(
        [
            ...
            str(MIN_RPE_PAIRS),
            "--contrast-name",
            "SAMEHISTORY_SYSTEM_A10_FINALONLINE_V3_RECOVERY_SCORE_2400_2800",
            "--output-dir",
            str(output_dir),
        ]
    )
...
def adjudicate_common_support(
    ...
    exact_protocol_values = {
        "contrast_name": "SAMEHISTORY_SYSTEM_A10_FINALONLINE_V3_RECOVERY_SCORE_2400_2800",
...
def build_bundle(
    ...
    *,
    ...
    recovery_provenance = audit_recovery_provenance(
        lock, lock_audit, receipts["klt_input_adoption"]
    )
    missing_receipts = [
        item_id
    ...
    if (
        lock_audit.get("state") == "PASS"
        and recovery_provenance.get("provenance_valid") is not True
    ):
        analysis_state = "BLOCKED_RECOVERY_PROVENANCE_INVALID"
        evaluator_audit = {
            "state": "BLOCKED_RECOVERY_PROVENANCE_INVALID",
            "errors": recovery_provenance.get("errors") or [],
        }
        rows = [
            receipt_only_system_row(
                arm, receipts[arm], global_reason="RECOVERY_PROVENANCE_INVALID"
            )
            for arm in VINS_ARMS
        ]
        ...
        rows.append(hfnet_usability(bridge_audit, canonical_audit))
    elif recovery_provenance.get("adoption_accepted_for_v3") is not True:
        analysis_state = "BLOCKED_RECOVERY_ADOPTION_NOT_ACCEPTED"
        evaluator_audit = {"state": "BLOCKED_RECOVERY_ADOPTION_NOT_ACCEPTED"}
        rows = [
            receipt_only_system_row(
                arm, receipts[arm], global_reason="RECOVERY_ADOPTION_NOT_ACCEPTED"
            )
            for arm in VINS_ARMS
        ]
        rows.append(hfnet_usability(bridge_audit, canonical_audit))
    ...
    bundle = {
        ...
        "question": (
            "A10 source 0..2800 v3 recovery same-history system comparison, "
            "scored only on source 2400..2800."
        ),
        "analysis_only": True,
        "evidence_scope": RECOVERY_EVIDENCE_SCOPE,
        "paper_primary_evidence_permitted": False,
        ...
        "frozen_paths": {
            ...
            "klt_adoption_manifest_source": str(KLT_ADOPTION_MANIFEST_SOURCE),
            "klt_adoption_manifest_output": str(
                ITEM_DIRS["klt_input_adoption"] / "adoption_manifest.json"
            ),
        },
        ...
        "recovery_provenance_audit": recovery_provenance,
        ...
        "interpretation_policy": {
            ...
            "v2_source_attempt_successful": False,
            "v2_source_attempt_may_be_described_as_pass": False,
            "v3_adoption_reclassifies_v2": False,
            "formal_ape_gate_open": False,
            "formal_winner_permitted": False,
            "significance_test_permitted": False,
            "ranking_permitted": False,
            ...
            "cross_system_disclosures": [
                "v3 is a pipeline-debug/secondary-sensitivity recovery and is not primary paper evidence",
                "the adopted KLT files come from the sole predeclared v2 candidate; v2 ended TERMINAL_PROCESS_RC0 with artifact PASS but execution FAIL (POSTFLIGHT_PROCESS_PRESENT), so v2 is not a successful attempt",
                "byte-preserving adoption does not repair or reclassify v2; later machine-exclusive KLT regeneration is required for primary paper evidence",
...
def write_report(path: Path, bundle: Mapping[str, Any]) -> None:
    lines = [
        "# A10 same-history final-online v3 recovery analysis",
        "",
        f"Analysis state: `{bundle['analysis_state']}`.",
        "",
        (
            "This is a pipeline-debug/secondary-sensitivity recovery, not primary "
            "paper evidence. The score interval has 21 native COLMAP proxy rows, so "
            "the 30-pose formal APE gate is closed. No winner, significance test, "
            "ranking, or failure-as-zero penalty is permitted."
        ),
        "",
        "## Recovery provenance",
        "",
    ]
    recovery = bundle.get("recovery_provenance_audit") or {}
    if recovery.get("v2_terminal_process_status") is None:
        source_disposition_text = (
            "Recovery provenance is not validated, so no source-attempt success "
            "claim is admissible. In particular, v2 must not be described as a "
            "successful attempt."
        )
    else:
        source_disposition_text = (
            "The retained v2 source attempt is not a successful attempt: terminal "
            f"process `{recovery.get('v2_terminal_process_status')}`, raw return "
            f"code `{recovery.get('v2_raw_return_code')}`, artifact contract "
            f"`{recovery.get('v2_artifact_contract_status')}`, execution integrity "
            f"`{recovery.get('v2_execution_integrity_status')}` with "
            f"`{recovery.get('v2_execution_failure_reason')}`, overall disposition "
            f"`{recovery.get('v2_source_attempt_disposition')}`."
        )
    lines.extend(
        [
            f"State: `{recovery.get('state')}`.",
            "",
            source_disposition_text,
            "",
            (
                "v3 copies the sole predeclared KLT candidate byte for byte. Adoption "
                f"accepted for v3: `{recovery.get('adoption_accepted_for_v3')}`; "
                "it does not repair or reclassify the v2 receipt."
            ),
            "",
            (
                f"Evidence scope: `{recovery.get('evidence_scope')}`; primary paper "
                f"evidence permitted: `{recovery.get('paper_primary_evidence_permitted')}`. "
                "A later machine-exclusive regeneration is required before primary use."
            ),
            "",
        ]
    )
    recovery_identities = recovery.get("source_evidence_identities")
    evidence_labels = (
        ("static adoption manifest", "klt_adoption_manifest"),
        ("v2 process-start claim", "v2_klt_claim"),
        ("v2 terminal receipt", "v2_klt_receipt"),
        ("v2 supervisor log", "v2_klt_log"),
        ("v2 execution lock", "v2_execution_lock"),
    )
    if isinstance(recovery_identities, Mapping):
        lines.extend(
            [
        ...
        for label, key in evidence_labels:
            claim = recovery_identities.get(key)
            if isinstance(claim, Mapping):
                lines.append(
                    f"| {label} | `{claim.get('path')}` | "
                    f"{claim.get('size_bytes')} | `{claim.get('sha256')}` |"
                )
        lines.append("")
    lines.extend(
        [
...
def validate_reused_external_evidence(bundle: Mapping[str, Any]) -> None:
    ...
    recovery_claim = bundle.get("recovery_provenance_audit")
    if not isinstance(recovery_claim, Mapping):
        raise EvidenceError("REUSED_ANALYSIS_RECOVERY_PROVENANCE_MISSING")
    current_lock: Optional[dict[str, Any]] = None
    if lock_audit.get("state") == "PASS":
        current_lock = read_json(EXECUTION_LOCK)
    recomputed_recovery = audit_recovery_provenance(
        current_lock,
        lock_audit,
        receipt_audits["klt_input_adoption"],
    )
    if dict(recovery_claim) != recomputed_recovery:
        raise EvidenceError("REUSED_ANALYSIS_RECOVERY_PROVENANCE_CHANGED")
