#!/usr/bin/env python3
"""Build one outcome-blind matched XFeat/GFTT pre-run freeze.

This builder reads immutable inputs/static code identities plus the sealed r2
incident evidence required to authorize an exact science-preserving r3
continuation.  Retired outcomes are verified only as governance evidence and
are never projected into the candidate configuration.  It never imports a
future r3 manifest, feature bag, diagnostics stream, attempt, or launcher
...
SCHEMA_VERSION = "aqua-fe-matched-birth-pair-outcome-blind-freeze-builder-v1"
MODE_CONTRACTS = {
    "probe16": {"allow_prefix": True, "expected_frames": 16},
...
def _governed_future_paths(kwargs: Mapping[str, object]) -> list[Path]:
    ...
    if len(paths) != 21 or len({str(path) for path in paths}) != 21:
        raise FreezeBuildError("governed r2 path set is not exact 21")
    return paths
...
def build_payload(
    *,
    ...
    incident_gate = audit._validate_infrastructure_incident()
    ...
    audit._validate_continuation_mode_and_inputs(
...
def build_and_write(**kwargs) -> dict[str, object]:
    ...
    live_incident = audit._validate_infrastructure_incident()["identity"]
    if audit._canonical_bytes(live_incident) != audit._canonical_bytes(
        freeze["infrastructure_incident"]
    ):
        raise FreezeBuildError("infrastructure incident drift before freeze commit")
    ...
    return {
        ...
        "retired_r2_outcomes_used_for_candidate_projection": False,
        "producer_started": False,
    }
