#!/usr/bin/python3
"""Single ordinal dispatcher for the systemd-supervised v12 experiment."""
...
from fair_stability_ordinal_common_v12 import (  # noqa: E402
    ...
    valid_prestart_resource_gate_v12,
...
MATRIX_FREEZE = EXPERIMENT_ROOT / "attempt_matrix_freeze_v12.json"
HFNET_RUNNER = ROOT / "scripts/run_fair_stability_hfnet_openloop_v12.py"
VINS_RUNNER = ROOT / "scripts/run_fair_stability_vins_replay_v12.py"
...
def verify_systemd_authority() -> dict[str, object]:
    ...
    if (
        claim.get("schema_version") != "aqua-fe-fair-stability-systemd-submission-v12"
...
def verify_control_freeze() -> dict[str, Any]:
    ...
    for path in (
        ...
        HFNET_RUNNER,
        SYSTEMD_SUPERVISOR,
        SCRIPT_DIR / "fair_stability_ordinal_common_v12.py",
    ):
...
def adopt_terminal(
    ...
    payload = {
        "schema_version": "aqua-fe-fair-stability-ordinal-terminal-v12",
...
def automatic_retry_codes(state: Mapping[str, Any]) -> list[str]:
    """Validate the evidence shape, then enforce v12's zero-retry policy."""
    result = state["result"]
    ...
    raise ControllerError(
        "PIPELINE_INVALID_HALT_MANUAL_ABANDONMENT_REQUIRED_ZERO_RETRY_V12:"
        f"{codes}"
    )
...
def adjudicate_pipeline_invalid(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        "schema_version": "aqua-fe-fair-stability-pipeline-invalid-adjudication-v12",
        "experiment_id": EXPERIMENT_ID,
        "adjudicated_at_utc": now_utc(),
        "accepted_as_external_pipeline_fault": True,
        "automatic_retry_policy": "EXPLICIT_EXTERNAL_TRANSIENT_ONLY_V12",
...
def dispatch_claim(state: Mapping[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    cell = state["cell"]
    claim = {
        "schema_version": "aqua-fe-fair-stability-ordinal-dispatch-v12",
        "experiment_id": EXPERIMENT_ID,
...
def dispatch(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    check_contract_valid = (
        ...
        and valid_prestart_resource_gate_v12(
            resource_gate_value,
    ...
    execution = {
        "schema_version": "aqua-fe-fair-stability-controller-dispatch-execution-v12",
        "experiment_id": EXPERIMENT_ID,
