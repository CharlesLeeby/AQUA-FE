#!/usr/bin/python3
"""Single ordinal dispatcher for the runtime-exclusive v2 experiment."""
...
from fair_stability_ordinal_common_v2 import (  # noqa: E402
    EXPERIMENT_ID,
...
MATRIX_FREEZE = EXPERIMENT_ROOT / "attempt_matrix_freeze_v2.json"
HFNET_RUNNER = ROOT / "scripts/run_fair_stability_hfnet_openloop_v2.py"
VINS_RUNNER = ROOT / "scripts/run_fair_stability_vins_replay_v2.py"
CONTROLLER = Path(__file__).resolve()

ALLOWED_PIPELINE_FAILURES = {
    "SUPERVISOR_LAUNCH_OR_REAP_FAILED",
    "ROSBAG_PLAY_NONZERO",
    "WRAPPER_NONZERO_DESPITE_VALID_LIFECYCLE",
    "GPU_DRIVER_OR_RUNTIME_LOAD_FAILURE",
    "ONNX_MODEL_DRIFT",
    "RUNTIME_CONFIG_DRIFT",
    "SELECTED_TIMESTAMPS_DRIFT",
    "POSTRUN_FROZEN_INPUT_OR_STACK_REVERIFICATION_FAILED",
    "POSTRUN_FULL_BACKEND_FREEZE_REVERIFICATION_FAILED",
    "MIDRUN_EXTERNAL_RESOURCE_INTRUSION",
    "RUNTIME_RESOURCE_MONITOR_UNPROVEN",
    "ESTIMATOR_START_IDENTITY_UNPROVEN",
    "SUPERVISED_SHUTDOWN_CONTRACT_UNPROVEN",
    "ATTEMPT_TAG_BINDING_UNPROVEN",
}
ALLOWED_PIPELINE_PREFIXES = (
    "POSTRUN_IDENTITY_DRIFT:",
    "POSTRUN_IDENTITY_UNREADABLE:",
    "POSTRUN_CONTROL_DRIFT:",
    "POSTRUN_CONTROL_UNREADABLE:",
)
...
def verify_control_freeze() -> dict[str, Any]:
    ...
    for path in (CONTROLLER, HFNET_RUNNER, SCRIPT_DIR / "fair_stability_ordinal_common_v2.py"):
...
def adopt_terminal(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        "schema_version": "aqua-fe-fair-stability-ordinal-terminal-v2",
...
def adjudicate_pipeline_invalid(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    result = state["result"]
    codes = [str(value) for value in result.get("pipeline_failure_codes", [])]
    if not codes:
        raise ControllerError("PIPELINE_INVALID_WITHOUT_PIPELINE_FAILURE_CODE")
    rejected = [
        code for code in codes
        if code not in ALLOWED_PIPELINE_FAILURES
        and not any(code.startswith(prefix) for prefix in ALLOWED_PIPELINE_PREFIXES)
    ]
    if rejected:
        raise ControllerError(f"PIPELINE_FAILURE_REQUIRES_MANUAL_ADJUDICATION:{rejected}")
    root = Path(state["attempt_root"])
    ...
    payload = {
        "schema_version": "aqua-fe-fair-stability-pipeline-invalid-adjudication-v2",
        "experiment_id": EXPERIMENT_ID,
...
def dispatch_claim(state: Mapping[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    cell = state["cell"]
    claim = {
        "schema_version": "aqua-fe-fair-stability-ordinal-dispatch-v2",
        "experiment_id": EXPERIMENT_ID,
...
def dispatch(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    execution = {
        "schema_version": "aqua-fe-fair-stability-controller-dispatch-execution-v2",
        "experiment_id": EXPERIMENT_ID,
...
def freeze_attempt_matrix() -> dict[str, Any]:
    ...
    for cell in schedule:
        ...
        expected = [
            EXPERIMENT_ID, str(cell["case_id"]), str(cell["arm"]),
            int(cell["repeat"]), 1,
        ]
        ...
        observed = [
            value.get("experiment_id"), value.get("case_id"), observed_arm,
            value.get("repeat"), value.get("attempt_index", 1),
        ]
