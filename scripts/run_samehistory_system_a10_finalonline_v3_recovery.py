#!/usr/bin/env python3
"""Fail-closed one-shot supervisor for the frozen A10 v3 recovery run.
...
EXP_ROOT = Path("/mnt/data/AQUA-FE_WS/experiments/a10_samehistory_system_v3_recovery")
DEFAULT_LOCK = ROOT / "papers/samehistory_system_comparison_a10_finalonline_v3_recovery_execution_lock.json"
...
SCHEMA = "aqua-fe-a10-samehistory-finalonline-supervisor-v3"
...
KINDS = {
    "klt_input_adoption": "sealed_input_adoption",
    "aquafe_build": "learned_sidecar_build",
    "vanilla_origin_native_image_context": "backend_replay",
    "external_klt_finalonline_backbone": "backend_replay",
    "aquafe_finalonline_xfeat_lineage": "backend_replay",
}
ISOLATION_POLICIES = {
...
BACKENDS = frozenset(ITEM_ORDER[2:])
OUTPUT_DIRS = {
    "klt_input_adoption": EXP_ROOT / "frontends/klt_input_adoption",
    "aquafe_build": EXP_ROOT / "frontends/aquafe_finalonline",
    "vanilla_origin_native_image_context": EXP_ROOT / "backends/vanilla_origin_native_image_context",
    "external_klt_finalonline_backbone": EXP_ROOT / "backends/external_klt_finalonline_backbone",
    "aquafe_finalonline_xfeat_lineage": EXP_ROOT / "backends/aquafe_finalonline_xfeat_lineage",
}
WORKSPACE_LINKS = {
    "klt_input_adoption": ROOT / "logs/aqualoc_archaeo_vins/external_klt_every2_systemfair_a10_finalonline_v3_recovery_feed0000_2800_klt_input_adoption",
    "aquafe_build": ROOT / "logs/aqualoc_archaeo_vins/aquafe_finalonline_xfeat_lineage_systemfair_a10_finalonline_v3_recovery_feed0000_2800_build",
    "vanilla_origin_native_image_context": ROOT / "logs/aqualoc_archaeo_vins/origin_klt_every1_systemfair_a10_finalonline_v3_recovery_feed0000_2800_score2400_2800_vanilla_origin",
    "external_klt_finalonline_backbone": ROOT / "logs/aqualoc_archaeo_vins/external_klt_every2_systemfair_a10_finalonline_v3_recovery_feed0000_2800_score2400_2800_external_klt",
    "aquafe_finalonline_xfeat_lineage": ROOT / "logs/aqualoc_archaeo_vins/external_hybrid_xfeat_every2_systemfair_a10_finalonline_v3_recovery_feed0000_2800_score2400_2800_aquafe_finalonline_xfeat_lineage",
}
...
# cycle.
EXPECTED_CONTRACT_SHA256 = "ed0bc347095122ee5c6465863bad3572bd8d56294039649ac4ac214f9d7197d1"
CHECK_TYPES = frozenset(
    {
        "rosbag_topic", "rosbag_timeline_equal", "rosbag_feature_extension",
        "csv_rows", "file_sha256", "text_contains", "replay_manifest", "ape_kv",
        "rosbag_topic_equal",
        "vins_trajectory",
    }
)
...
FORMAL_ROS_PORTS = frozenset({11971, 11973, 11974})
...
def validate_items(lock: Mapping[str, Any], verified: Mapping[str, Mapping[str, Any]]) -> None:
    ...
    for index, item_id in enumerate(ITEM_ORDER):
        ...
        if item_id == "klt_input_adoption":
            required_signatures = {
                ("rosbag_topic", "features.bag", "/feature_tracker/feature"),
                ("rosbag_topic_equal", "features.bag", "/rtimulib_node/imu"),
                ("rosbag_topic_equal", "features.bag", "/aqualoc/colmap_gt"),
                ("csv_rows", "frontend_metrics.csv", ""),
                ("file_sha256", "aqualoc_archaeo10_pinhole.yaml", ""),
                ("file_sha256", "adoption_manifest.json", ""),
            }
        elif item_id == "aquafe_build":
            required_signatures = {
                ("rosbag_topic", "full_merged.bag", "/feature_tracker/feature"),
                ("rosbag_topic_equal", "full_merged.bag", "/rtimulib_node/imu"),
                ("rosbag_topic_equal", "full_merged.bag", "/aqualoc/colmap_gt"),
                ("rosbag_topic", "sidecar.bag", "/feature_tracker/sidecar"),
                ("rosbag_feature_extension", "full_merged.bag", "/feature_tracker/feature"),
                ("csv_rows", "stats.csv", ""),
            }
        else:
            config_name = (
    ...
    if len(set(ports)) != len(ports):
        raise ContractError("PORTS_NOT_DISTINCT")
    if set(ports) != set(FORMAL_ROS_PORTS):
        raise ContractError(f"FORMAL_ROS_PORT_SET_MISMATCH:{sorted(ports)}")

    required_static_paths = {
        ROOT / "scripts/run_paper_sidecar_profiles.sh",
        ...
        ROOT / "scripts/analyze_samehistory_system_a10_finalonline_v3_recovery.py",
        ROOT / "uw_frontend/ros/export_vins_features.py",
        ROOT / "uw_frontend/ros/xfeat_seed_sidecar_node.py",
        ROOT / "uw_frontend/ros/causal_lineage_shadow_node.py",
        ROOT / "uw_frontend/configs/experiments/formal_loftr_mirror_contribution_v33_frontend.yaml",
        ROOT / "uw_frontend/configs/experiments/low_texture_lineage_safe_dense_start_frontend.yaml",
        (ROOT / "external_tools/accelerated_features/weights/xfeat.pt").resolve(strict=True),
...
def verify_lock(path: Path) -> tuple[dict[str, Any], dict[str, dict[str, Any]], dict[str, Any]]:
    ...
    if builder is None or Path(builder["path"]).absolute() != ROOT / "scripts/build_samehistory_system_a10_finalonline_v3_recovery_execution_lock.py":
...
def quiescence_snapshot(
    ...
    rosout_baseline: Sequence[Mapping[str, Any]] | None = None, *, phase: str,
) -> dict[str, Any]:
...
def check_quiescence(
    ...
    result = quiescence_snapshot(
        item, expected_rosout_executable, rosout_baseline, phase=phase
    )
...
def minimal_receipt(
    ...
    try:
        ...
        assert claim_data is not None
        claim_value = parse_json(claim_data, claim_path).get("masterless_rosout_exemption")
        if isinstance(claim_value, dict):
            claim_rosout_exemption = dict(claim_value)
    except BaseException:
        pass
