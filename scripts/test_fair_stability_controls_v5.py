...
VINS = load("fair_vins_v5", SCRIPTS / "run_fair_stability_vins_replay_v5.py")
HFNET = load("fair_hfnet_v5", SCRIPTS / "run_fair_stability_hfnet_openloop_v5.py")
ORDINAL = load("fair_ordinal_v5", SCRIPTS / "fair_stability_ordinal_common_v5.py")
NEXT = load("fair_next_v5", SCRIPTS / "run_fair_stability_next_v5.py")
SUMMARY = load("fair_summary_v5", SCRIPTS / "summarize_fair_stability_v5.py")
RESOURCE = load(
    "fair_runtime_resource_monitor",
    SCRIPTS / "fair_stability_runtime_resource_monitor_v5.py",
)
SYSTEMD = load(
    "fair_systemd_supervisor_v5",
    SCRIPTS / "run_fair_stability_systemd_supervisor_v5.py",
)
...
class LifecycleAndScheduleTests(unittest.TestCase):
    ...
    def test_v5_schedule_and_pristine_attempt_matrix(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            matrix_path = root / "attempt_matrix_freeze_v5.json"
            matrix_path.write_bytes(
                ORDINAL.canonical_json(
                    {
                        "schema_version": "aqua-fe-fair-stability-attempt-matrix-freeze-v5",
                        "experiment_id": ORDINAL.EXPERIMENT_ID,
                        "status": "FROZEN_BEFORE_ANY_V5_ESTIMATOR_START",
...
class V5ControlRegressionTests(unittest.TestCase):
    ...
    CHILD = SCRIPTS / "run_fair_stability_vins_replay_child_v5.sh"
    ...
    def test_vins_rejects_prior_result_import_claims(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch.object(VINS, "EXPERIMENT_ROOT", root), mock.patch.object(
                ...
                VINS, "read_csv_rows", side_effect=[roster, historical] * 5
            ), mock.patch.object(
    ...
    def test_runner_summaries_delegate_to_receipt_state_machine(self) -> None:
        ...
        with mock.patch(
            "summarize_fair_stability_v5.summarize", return_value=sentinel
        ) as summarize:
    ...
    def test_adjudication_allowed_path_has_bound_result(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(receipt["automatic_retry_policy"], "EXPLICIT_EXTERNAL_TRANSIENT_ONLY_V5")
...
class SystemdControlTests(unittest.TestCase):
    ...
    def build_chain(
        ...
        unit = "aqua-fe-fair-stability-v5-o001-a001-s001-aabbcc.service"
        claim = {
