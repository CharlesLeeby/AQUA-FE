#!/usr/bin/python3
"""Prepare and run the open-loop HFNet arms of runtime-exclusive v7.
...
RUNTIME_EXCLUSIVITY_ADDENDUM = (
    ROOT / "papers/fair_stability_runtime_exclusivity_addendum_v7.md"
)
...
RUNTIME_RESOURCE_MONITOR = (
    ROOT / "scripts/fair_stability_runtime_resource_monitor_v7.py"
)
...
EXPECTED_CONTROL_DOCUMENTS = {
    "runtime_exclusivity_addendum": (
        RUNTIME_EXCLUSIVITY_ADDENDUM,
        25_474,
        "2220e1aaf0b61dc94284dd1f407ba541328f64246c56653561fe0107e01b3f8c",
    ),
    "v1_runtime_contamination_report": (
        V1_RUNTIME_CONTAMINATION_REPORT,
        3_436,
    ...
    "control_supersession": (
        CONTROL_SUPERSESSION,
        15_080,
        "a2c01ba67828388a9c693145640512d95236f18720e8db3f49c09071b57268dd",
    ),
...
def load_experiment() -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != "aqua-fe-fair-stability-hfnet-input-freeze-v7":
...
def verify_attempt(
    ...
    if attempt.get("schema_version") != "aqua-fe-fair-stability-hfnet-attempt-v7":
...
def wait_with_zero_kf_watchdog(
    ...
    *,
    ...
    def outcome() -> dict[str, object]:
        return {
            "schema_version": "aqua-fe-fair-stability-hfnet-zero-kf-watchdog-outcome-v7",
...
def run_attempt(case_id: str, budget: int, repeat: int, attempt_index: int = 1) -> dict[str, object]:
    ...
    with open_regular_lock(lock_path) as lock_stream:
        ...
        claim = {
            "schema_version": "aqua-fe-fair-stability-hfnet-start-claim-v7",
            "experiment_id": EXPERIMENT_ID,
        ...
        result = {
            "schema_version": "aqua-fe-fair-stability-hfnet-result-v7",
            "experiment_id": EXPERIMENT_ID,
            "status": status,
