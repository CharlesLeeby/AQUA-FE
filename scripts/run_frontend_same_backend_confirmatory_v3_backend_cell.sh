...
# Run one immutable serial VINS replay for confirmatory-v3.  Every arm and
# repeat for a window reads the same canonical YAML bytes.
...
SCRATCH_DIR="${7:?scratch dir}"
RUN_DIR="${8:?run dir}"
PORT="${9:?ROS port}"

ROOT="/home/ma/AQUA-FE_WS"
RUNTIME_ROOT="/media/ma/Data/AQUA-FE_WS_storage_offload/frontend_same_backend_confirmatory_v3"
...
case "$SCRATCH_DIR" in
  "$RUNTIME_ROOT"/scratch/"$RUN_SLUG") ;;
  *) echo "unsafe scratch path: $SCRATCH_DIR" >&2; exit 64 ;;
esac
case "$RUN_DIR" in
  "$RUNTIME_ROOT"/replays/"$RUN_SLUG"/"$ARM"/repeat"$REPEAT") ;;
...
if [[ -s "$RUN_DIR/replay_receipt.txt" ]]; then
  ...
  [[ "$saved_bag_hash" == "$(sha256sum "$FEATURE_BAG" | awk '{print $1}')" ]] || {
    echo "resume feature-bag hash mismatch: $RUN_DIR" >&2; exit 65;
  }
  [[ "$saved_config_hash" == "$(sha256sum "$CANONICAL_CONFIG" | awk '{print $1}')" ]] || {
    echo "resume backend-config hash mismatch: $RUN_DIR" >&2; exit 65;
  }
  echo "resume backend $RUN_SLUG $ARM repeat$REPEAT"
  exit 0
fi
...
[[ -n "$IMU_TOPIC" ]] || { echo "could not parse imu_topic: $CANONICAL_CONFIG" >&2; exit 65; }
...
{
  printf 'schema_version=aqua-fe-confirmatory-v3-backend-cell-v1\n'
  printf 'window_id=%s\n' "$WINDOW_ID"
  printf 'run_slug=%s\n' "$RUN_SLUG"
  printf 'arm=%s\n' "$ARM"
  printf 'repeat=%s\n' "$REPEAT"
...
echo "finish backend $RUN_SLUG $ARM repeat$REPEAT"
