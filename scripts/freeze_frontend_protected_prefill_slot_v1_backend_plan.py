#!/usr/bin/env python3
"""Freeze identity-audited backend replays for EXP-20260906-009."""
...
def main() -> int:
                ...
                rows.append(
                    {
                        ...
                        "backend_role": role,
                        "repeat": repeat,
                        "execution": "NEW_PROTECTED_PREFILL_REPLAY",
                        "feature_bag": str(bag),
                        ...
                        "vins_lib_sha256": lib_hash,
                        "replay_dir": str(replay),
                        "source_experiment": "EXP-20260906-009",
                    }
                )
    ...
    if sum(row["execution"] == "NEW_PROTECTED_PREFILL_REPLAY" for row in rows) != expected_new:
    ...
    identity_paths = [
        ...
        PLAN,
        ROOT / "scripts/run_frontend_protected_prefill_slot_v1_backend.py",
        ROOT / "scripts/run_frontend_protected_prefill_slot_v1_backend_cell.sh",
    ]
    lock = {
        "schema_version": "aqua-fe-protected-prefill-slot-v1-backend-lock-v1",
