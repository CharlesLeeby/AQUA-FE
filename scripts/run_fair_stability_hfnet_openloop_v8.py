#!/usr/bin/python3
"""Prepare and run the open-loop HFNet arms of runtime-exclusive v8.
...
RUNTIME_EXCLUSIVITY_ADDENDUM = (
    ROOT / "papers/fair_stability_runtime_exclusivity_addendum_v8.md"
)
...
RUNTIME_RESOURCE_MONITOR = (
    ROOT / "scripts/fair_stability_runtime_resource_monitor_v8.py"
)
...
EXPECTED_CONTROL_DOCUMENTS = {
    "runtime_exclusivity_addendum": (
        RUNTIME_EXCLUSIVITY_ADDENDUM,
        27_963,
        "7e2475bd72869d80a103667d62d884e4bce27f20bfdef9e7bf357f065384e2f3",
    ),
    "v1_runtime_contamination_report": (
        V1_RUNTIME_CONTAMINATION_REPORT,
        3_436,
    ...
    "control_supersession": (
        CONTROL_SUPERSESSION,
        17_008,
        "47aea45770e2ca964caa8273a1f6f2180b5adef5961993af3c7ad648c32b747f",
    ),
...
def load_experiment() -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != "aqua-fe-fair-stability-hfnet-input-freeze-v8":
...
def verify_attempt(
    ...
    if attempt.get("schema_version") != "aqua-fe-fair-stability-hfnet-attempt-v8":
...
def wait_with_zero_kf_watchdog(
    ...
    *,
    ...
    def outcome() -> dict[str, object]:
        return {
            "schema_version": "aqua-fe-fair-stability-hfnet-zero-kf-watchdog-outcome-v8",
...
def run_attempt(case_id: str, budget: int, repeat: int, attempt_index: int = 1) -> dict[str, object]:
    ...
    with open_regular_lock(lock_path) as lock_stream:
        ...
        claim = {
            "schema_version": "aqua-fe-fair-stability-hfnet-start-claim-v8",
            "experiment_id": EXPERIMENT_ID,
        ...
        result = {
            "schema_version": "aqua-fe-fair-stability-hfnet-result-v8",
            "experiment_id": EXPERIMENT_ID,
            "status": status,
