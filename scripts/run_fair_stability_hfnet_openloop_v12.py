#!/usr/bin/python3
"""Prepare and run the open-loop HFNet arms of runtime-exclusive v12.
...
from fair_stability_ordinal_common_v12 import (
    EXPERIMENT_ID,
...
RUNTIME_EXCLUSIVITY_ADDENDUM = (
    ROOT / "papers/fair_stability_runtime_exclusivity_addendum_v12.md"
)
...
CONTROL_SUPERSESSION = ROOT / "papers/fair_stability_control_supersession_v12.md"
...
V9_ABANDONMENT_REPORT = (
    ROOT
    / "papers/fair_stability_v9_runtime_monitor_termination_race_abandonment_report_20260830_v10.md"
)
V10_ABANDONMENT_REPORT = (
    ROOT
    / "papers/fair_stability_v10_proc_tail_closure_abandonment_report_20260830_v11.md"
)
V11_ABANDONMENT_REPORT = (
    ROOT
    / "papers/fair_stability_v11_watchdog_pending_reap_abandonment_report_20260830_v12.md"
)
SHUTDOWN_ADDENDUM = (
...
VINS_ROOT = EXPERIMENT_ROOT / "vins_dev_nativeq_schedfix_runtimeexcl_v12"
BACKEND_FREEZE = VINS_ROOT / "backend_freeze.json"
ORDINAL_COMMON = ROOT / "scripts/fair_stability_ordinal_common_v12.py"
RUNTIME_RESOURCE_MONITOR = (
    ROOT / "scripts/fair_stability_runtime_resource_monitor_v12.py"
)
SYSTEMD_SUPERVISOR = ROOT / "scripts/run_fair_stability_systemd_supervisor_v12.py"
WATCHDOG_PENDING_TESTS = (
    ROOT / "scripts/test_fair_stability_watchdog_pending_v12.py"
)
...
EXPECTED_CONTROL_DOCUMENTS = {
    "runtime_exclusivity_addendum": (
        RUNTIME_EXCLUSIVITY_ADDENDUM,
        25_135,
        "a6cc74cc39c4e278a1156ec5f7866d884c22bfbfafccb5cd1588d84a9107687c",
    ),
    "v1_runtime_contamination_report": (
        V1_RUNTIME_CONTAMINATION_REPORT,
        3_436,
    ...
    "control_supersession": (
        CONTROL_SUPERSESSION,
        21_579,
        "44e9d47311ccd4191bb97565b966734c356fa75dc712894a9b049572d030044b",
    ),
    ...
    "v7_teardown_snapshot_abandonment_report": (
        V7_ABANDONMENT_REPORT,
        5_053,
        "8ab95a76948c1a99a9a9600e32a7a80515a2ea24b1fb3a948422d41a44420c2d",
    ),
    ...
    "v9_runtime_monitor_termination_race_abandonment_report": (
        V9_ABANDONMENT_REPORT,
        8_047,
        "c26d03b25c1d64c9387739f66371bc12e21b90dd84224e029e5d8fa843b4d895",
    ),
    "v10_proc_tail_closure_abandonment_report": (
        V10_ABANDONMENT_REPORT,
        6_980,
        "d1d58f3c2a5b7166aea7498e1137e32166b49c4032c28e801e5a7bc5ce769c22",
    ),
    "v11_watchdog_pending_reap_abandonment_report": (
        V11_ABANDONMENT_REPORT,
        8_865,
        "20321a981c7bc240e1d982e8aedbf33bc1ba20b7fa815ddeeee558a966f78e08",
    ),
    "shutdown_addendum": (
        SHUTDOWN_ADDENDUM,
        1_645,
...
ZERO_KF_WATCHDOG_PENDING_REAP_SECONDS = 0.25
    ...
    try:
        ...
        manifest = {
            ...
            "execution_controls": {
                ...
                "watchdog_pending_tests": identity(WATCHDOG_PENDING_TESTS),
            },
...
def load_experiment() -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != "aqua-fe-fair-stability-hfnet-input-freeze-v12":
    ...
    if (
        ...
        or not valid_v11_non_import_claims(manifest.get("claims"))
    ...
    for label, path in (
        ...
        ("watchdog_pending_tests", WATCHDOG_PENDING_TESTS),
    ):
        require_recorded_identity(
    ...
    try:
        ...
        attempt = {
            ...
            "zero_kf_post_shutdown_watchdog": {
                ...
                "pending_reap_maximum_seconds": (
                    ZERO_KF_WATCHDOG_PENDING_REAP_SECONDS
                ),
...
def verify_attempt(
    ...
    if attempt.get("schema_version") != "aqua-fe-fair-stability-hfnet-attempt-v12":
    ...
    *,
    ...
    if monotonic_ns is None:
        monotonic_ns = (
            ...
            else lambda: int(float(monotonic()) * 1e9)
        )
    started = float(monotonic())
    watchdog_started_monotonic_ns = int(started * 1e9)
    global_deadline_monotonic_ns = (
        watchdog_started_monotonic_ns + int(float(timeout) * 1e9)
    )
    deadline = global_deadline_monotonic_ns / 1e9
    next_sample = started
    ...
    pending_reap_adjudication: dict[str, object] | None = None
    ...
    def resolve_pending_reap(
        phase: str,
        evidence: Mapping[str, object],
        initial_poll_observed_monotonic_ns: int,
    ) -> tuple[int | None, bool, bool]:
        """Adjudicate a legal teardown candidate without authorizing a signal."""

        nonlocal pending_reap_adjudication, permanently_disabled
        evidence_copy = json.loads(json.dumps(evidence, allow_nan=False))
        if phase == "PERIODIC_EXACT_GROUP_PROOF":
            candidate = evidence
            anchor_source = "candidate_evidence.observed_monotonic_ns"
        elif phase in {
            "FINAL_SIGNAL_GATE:FIRST_FINAL_GROUP_PROOF",
            "FINAL_SIGNAL_GATE:SECOND_FINAL_GROUP_PROOF",
        }:
            candidate = evidence.get("terminal_transition_snapshot")
            anchor_source = (
        ...
        anchor = (
            candidate.get("observed_monotonic_ns")
            if isinstance(candidate, Mapping)
            else None
        )
        entered = int(monotonic_ns())
        local_deadline = (
            int(anchor) + int(ZERO_KF_WATCHDOG_PENDING_REAP_SECONDS * 1e9)
        ...
        initial_poll = {
            "observed_monotonic_ns": initial_poll_observed_monotonic_ns,
            "returncode": None,
        }
        adjudication: dict[str, object] = {
            ...
            "phase": phase,
            "status": "PENDING_REAP",
            "classification": "PENDING_REAP",
            "candidate_evidence": evidence_copy,
            "anchor_source": anchor_source,
            "anchor_completed_monotonic_ns": anchor,
            "global_deadline_monotonic_ns": global_deadline_monotonic_ns,
            "local_maximum_deadline_monotonic_ns": local_deadline,
            "effective_deadline_monotonic_ns": effective_deadline,
            "maximum_wait_seconds": ZERO_KF_WATCHDOG_PENDING_REAP_SECONDS,
        ...
        def publish_failure(
            errors: Sequence[str],
            *,
            classification: str = "UNPROVEN_PENDING_REAP",
            ...
            adjudication.update(
                {
                    ...
                    "reap": None,
                    "adjudicated_monotonic_ns": adjudicated,
                    "child_returncode": child_returncode,
                    "signal_authorized": False,
                    "signal_attempted": False,
                    "signal_sent": False,
                    "errors": unique_errors
                    or ["PENDING_REAP_UNPROVEN_WITHOUT_REASON"],
                }
            )
            pending_reap_adjudication = json.loads(
                json.dumps(adjudication, allow_nan=False)
            )
            monitoring_errors.extend(
                f"PENDING_REAP:{value}"
                for value in pending_reap_adjudication["errors"]
            )
            permanently_disabled = True
            return child_returncode, child_returncode is not None, False
        ...
        def publish_success(
            ...
            nonlocal pending_reap_adjudication
            if (
                completed_monotonic_ns > effective_deadline
                or reap.get("same_popen") is not True
                or reap.get("wait_returncode") != returncode
            ):
                return publish_failure(
                    ["PENDING_REAP_RETURN_OR_REAP_AFTER_INCLUSIVE_DEADLINE"],
            ...
            adjudication.update(
                {
                    ...
                    "adjudicated_monotonic_ns": completed_monotonic_ns,
                    "child_returncode": returncode,
                    "signal_authorized": False,
                    "signal_attempted": False,
                    "signal_sent": False,
            ...
            record_passive_exit(
                ...
                frozen_adjudicated_monotonic_ns=completed_monotonic_ns,
            )
            if passive_exit_adjudication is None:
                return publish_failure(
                    ["PENDING_REAP_PASSIVE_EXIT_ADJUDICATION_UNPROVEN"],
                    child_returncode=returncode,
                    completed_monotonic_ns=completed_monotonic_ns,
                )
        ...
        if (
            ...
            or not watchdog_terminal_transition_candidate(candidate)
        ):
            initial_errors.append("PENDING_REAP_CANDIDATE_INVALID")
        if type(anchor) is not int or anchor <= 0:
            initial_errors.append("PENDING_REAP_ANCHOR_INVALID")
        if (
            type(initial_poll_observed_monotonic_ns) is not int
            or initial_poll_observed_monotonic_ns < int(anchor or 0)
        ):
            initial_errors.append("PENDING_REAP_INITIAL_POLL_TIMING_INVALID")
        if entered < initial_poll_observed_monotonic_ns:
            initial_errors.append("PENDING_REAP_ENTRY_CLOCK_REGRESSION")
        if initial_poll_observed_monotonic_ns > effective_deadline:
            initial_errors.append("PENDING_REAP_ENTERED_AFTER_DEADLINE")
        if (
            initial_poll_observed_monotonic_ns - int(anchor or 0)
            > int(ZERO_KF_WATCHDOG_POLL_SECONDS * 1e9)
        ):
            initial_errors.append("PENDING_REAP_POLL_GAP_EXCEEDED")
        if initial_errors:
            return publish_failure(initial_errors)

        anchor_identity = (
            candidate.get("leader_pid"),
            candidate.get("leader_start_ticks"),
            candidate.get("pgid"),
            candidate.get("session"),
        )
        previous_observation = initial_poll_observed_monotonic_ns
        target_grid_deadline = (
            int(anchor) + int(ZERO_KF_WATCHDOG_TARGET_POLL_SECONDS * 1e9)
        )
        next_poll_deadline = (
            effective_deadline
            if initial_poll_observed_monotonic_ns >= target_grid_deadline
            else min(target_grid_deadline, effective_deadline)
        )
        while True:
            wait_started = int(monotonic_ns())
            if wait_started < previous_observation:
                return publish_failure(["PENDING_REAP_CLOCK_REGRESSION"])
            if wait_started > effective_deadline:
                return publish_failure(
                    ["PENDING_REAP_DEADLINE_EXCEEDED_BEFORE_WAIT"]
                )
            if wait_started == effective_deadline:
                last_poll = adjudication["poll_events"][-1]
                if (
                    last_poll.get("returncode") is None
                    and last_poll.get("observed_monotonic_ns")
                    == effective_deadline
                ):
                    return publish_failure(
                        ["PENDING_REAP_STILL_LIVE_AT_INCLUSIVE_DEADLINE"],
                        classification="STILL_LIVE_AT_INCLUSIVE_DEADLINE",
                        completed_monotonic_ns=wait_started,
                    )
                return publish_failure(
                    ["PENDING_REAP_DEADLINE_REACHED_WITHOUT_FINAL_POLL_PROOF"],
                    completed_monotonic_ns=wait_started,
                )
            if (
                wait_started >= next_poll_deadline
                and next_poll_deadline < effective_deadline
            ):
                next_poll_deadline = effective_deadline
            requested_timeout = max(
                0.0,
                (next_poll_deadline - wait_started) / 1e9,
            )
            wait_event: dict[str, object] = {
                "sequence": len(adjudication["wait_events"]),
            ...
            try:
                waited_returncode = process.wait(timeout=requested_timeout)
                ...
                adjudication["wait_events"].append(wait_event)
                if (
                    wait_completed < wait_started
                    or wait_completed - previous_observation
                    > int(ZERO_KF_WATCHDOG_POLL_SECONDS * 1e9)
                ):
                    return publish_failure(
                        ["PENDING_REAP_POLL_GAP_EXCEEDED"],
                        completed_monotonic_ns=wait_completed,
                    )
                try:
                    poll_returncode = process.poll()
                except BaseException as error:
                    return publish_failure(
                        [
                            "PENDING_REAP_POLL_EXCEPTION:"
                            f"{type(error).__name__}:{error}"
                        ]
                    )
                poll_observed = int(monotonic_ns())
                poll_event: dict[str, object] = {
                    "sequence": len(adjudication["poll_events"]),
                    "source": "POST_WAIT_TIMEOUT_POLL",
                    "observed_monotonic_ns": poll_observed,
                    "returncode": poll_returncode,
                    "group_recheck": None,
                }
                adjudication["poll_events"].append(poll_event)
                if (
                    poll_observed < wait_completed
                    or poll_observed - previous_observation
                    > int(ZERO_KF_WATCHDOG_POLL_SECONDS * 1e9)
                ):
                    return publish_failure(
                        ["PENDING_REAP_POLL_GAP_EXCEEDED"],
                        ...
                        completed_monotonic_ns=poll_observed,
                    )
                if poll_returncode is not None:
                    returncode, reap, reap_error = reap_after_same_popen_poll(
                        "PENDING_REAP_POST_TIMEOUT_POLL_THEN_WAIT_ZERO",
                        int(poll_returncode),
                        poll_observed,
                    )
                    completed = int(monotonic_ns())
                    if reap_error is not None or reap is None or returncode is None:
                        return publish_failure(
                            [
                                "PENDING_REAP_SAME_POPEN_REAP_FAILED:"
                                f"{reap_error or 'UNPROVEN'}"
                            ],
                            child_returncode=returncode,
                            completed_monotonic_ns=completed,
                        )
                    return publish_success(returncode, reap, completed)
                if poll_observed >= effective_deadline:
                    return publish_failure(
                        ["PENDING_REAP_STILL_LIVE_AT_INCLUSIVE_DEADLINE"],
                        classification="STILL_LIVE_AT_INCLUSIVE_DEADLINE",
                        completed_monotonic_ns=poll_observed,
                    )
                recheck = runtime_monitor.watchdog_exact_group_snapshot(
                    expected_executable,
                    expected_argv,
                )
                poll_event["group_recheck"] = json.loads(
                    json.dumps(recheck, allow_nan=False)
                )
                recheck_observed = recheck.get("observed_monotonic_ns")
                recheck_identity = (
                    recheck.get("leader_pid"),
                    recheck.get("leader_start_ticks"),
                    recheck.get("pgid"),
                    recheck.get("session"),
                )
                recheck_errors: list[str] = []
                if (
                    type(recheck_observed) is not int
                    or recheck_observed < poll_observed
                    or recheck_observed > effective_deadline
                ):
                    recheck_errors.append("PENDING_REAP_RECHECK_TIMING_INVALID")
                if recheck_identity != anchor_identity:
                    recheck_errors.append("PENDING_REAP_FROZEN_IDENTITY_CHANGED")
                if not watchdog_terminal_transition_candidate(recheck):
                    recheck_errors.extend(
                        f"PENDING_REAP_RECHECK:{value}"
                        for value in recheck.get("errors", [])
                    )
                    if not recheck.get("errors"):
                        recheck_errors.append("PENDING_REAP_RECHECK_UNPROVEN")
                if (
                    type(recheck_observed) is int
                    and recheck_observed - previous_observation
                    > int(ZERO_KF_WATCHDOG_POLL_SECONDS * 1e9)
                ):
                    recheck_errors.append("PENDING_REAP_POLL_GAP_EXCEEDED")
                if recheck_errors:
                    return publish_failure(
                        recheck_errors,
                        completed_monotonic_ns=(
                            int(recheck_observed)
                            if type(recheck_observed) is int
                            else None
                        ),
                    )
                previous_observation = int(recheck_observed)
                next_poll_deadline = effective_deadline
                continue
            except (OSError, subprocess.SubprocessError) as error:
                wait_completed = int(monotonic_ns())
            ...
            wait_event.update(
                {
                    "completed_monotonic_ns": wait_completed,
                    "outcome": "RETURNED",
                    "returncode": waited_returncode,
                }
            )
            adjudication["wait_events"].append(wait_event)
            adjudication["poll_events"].append(
                {
                    "sequence": len(adjudication["poll_events"]),
                    "source": "WAIT_RETURNED_SAME_POPEN",
                    "observed_monotonic_ns": wait_completed,
                    "returncode": waited_returncode,
                    "group_recheck": None,
                }
            )
            reap = build_same_popen_reap(
                ...
                poll_observed_monotonic_ns=(
                    int(adjudication["poll_events"][-2]["observed_monotonic_ns"])
                ),
                poll_returncode=None,
                wait_started_monotonic_ns=wait_started,
                wait_completed_monotonic_ns=wait_completed,
                wait_timeout_seconds=float(requested_timeout),
                wait_returncode=waited_returncode,
            )
            if reap is None:
                return publish_failure(
                    ["PENDING_REAP_SAME_POPEN_REAP_UNPROVEN"],
                    child_returncode=(
                        int(waited_returncode)
                        if type(waited_returncode) is int
                        else None
                    ),
                    completed_monotonic_ns=wait_completed,
                )
            return publish_success(int(waited_returncode), reap, wait_completed)
    ...
    def record_passive_exit(
        ...
        *,
        ...
        frozen_adjudicated_monotonic_ns: int | None = None,
    ) -> None:
        nonlocal passive_exit_adjudication
        observation_phase = (
            "FINAL_GATE"
        ...
        passive_exit_adjudication = {
            ...
            "adjudicated_monotonic_ns": (
                int(monotonic_ns())
                if frozen_adjudicated_monotonic_ns is None
                else int(frozen_adjudicated_monotonic_ns)
            ),
    ...
    def ordinary_exit_evidence(
        ...
        return {
            ...
            "observed_monotonic_ns": int(monotonic_ns()),
    ...
    def outcome() -> dict[str, object]:
        return {
            "schema_version": "aqua-fe-fair-stability-hfnet-zero-kf-watchdog-outcome-v12",
            "watchdog_started_monotonic_ns": watchdog_started_monotonic_ns,
            "global_deadline_monotonic_ns": global_deadline_monotonic_ns,
            ...
            "pending_reap_adjudication": pending_reap_adjudication,
    ...
    while True:
        ...
        loop_poll_observed_monotonic_ns = int(monotonic_ns())
        if returncode is not None:
            ...
            if action is None and not monitoring_errors:
                phase = "LOOP_POLL"
                record_passive_exit(
        ...
        if action is None:
            ...
            if signature.get("parser_proven") is not True:
                ...
                if group.get("proven") is not True:
                    ...
                    group_poll_observed_monotonic_ns = int(monotonic_ns())
        ...
        scheduled_wait_started_monotonic_ns = int(monotonic_ns())
        try:
            returncode = process.wait(timeout=delay)
            scheduled_wait_completed_monotonic_ns = int(monotonic_ns())
...
def build_zero_kf_watchdog_receipt(
    ...
    pending_reap = outcome.get("pending_reap_adjudication")
    passive_exit = outcome.get("passive_exit_adjudication")
    if isinstance(pending_reap, Mapping):
        if (
            pending_reap.get("signal_authorized") is not False
            or pending_reap.get("signal_attempted") is not False
            or pending_reap.get("signal_sent") is not False
        ):
            errors.append("PENDING_REAP_SIGNAL_CONTRACT_VIOLATION")
        if pending_reap.get("status") == "PASSIVE_REAP_PROVEN":
            if (
                not isinstance(passive_exit, Mapping)
                or passive_exit.get("phase") != pending_reap.get("phase")
                or not strict_json_equal(
                    passive_exit.get("evidence"),
                    pending_reap.get("candidate_evidence"),
                )
                or passive_exit.get("child_returncode")
                != pending_reap.get("child_returncode")
                or not strict_json_equal(
                    passive_exit.get("reap"), pending_reap.get("reap")
                )
            ):
                errors.append("PENDING_REAP_PASSIVE_EXIT_BINDING_INVALID")
    ...
    return {
        ...
        "pending_reap_adjudication": (
            dict(pending_reap) if isinstance(pending_reap, Mapping) else None
        ),
        "sigterm_sent": sent,
...
def run_attempt(case_id: str, budget: int, repeat: int, attempt_index: int = 1) -> dict[str, object]:
    ...
    with open_regular_lock(lock_path) as lock_stream:
        ...
        claim = {
            "schema_version": "aqua-fe-fair-stability-hfnet-start-claim-v12",
            "experiment_id": EXPERIMENT_ID,
        ...
        result = {
            "schema_version": "aqua-fe-fair-stability-hfnet-result-v12",
            "experiment_id": EXPERIMENT_ID,
            "status": status,
...
def summary() -> dict[str, object]:
    ...
    from summarize_fair_stability_v12 import summarize as summarize_receipts
