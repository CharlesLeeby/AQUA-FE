#!/usr/bin/python3
"""Fail-closed schedule state for fair-stability runtime-exclusive v9."""
...
EXPERIMENT_ID = "fair-stability-positive-roster-openloop-runtimeexcl-v9"
...
CONTROLLER_DISPATCH_EXECUTION_SCHEMA = (
    "aqua-fe-fair-stability-controller-dispatch-execution-v9"
)
...
SYSTEMD_ADOPTION_CLAIM_SCHEMA = (
    "aqua-fe-fair-stability-systemd-adoption-claim-v9"
)
SYSTEMD_ADOPTION_SCHEMA = "aqua-fe-fair-stability-systemd-adoption-v9"
RESUBMIT_AUTHORIZATION_SCHEMA = (
    "aqua-fe-fair-stability-systemd-resubmit-authorization-v9"
)
RESUBMIT_AUTHORIZATION_CONSUMPTION_SCHEMA = (
    "aqua-fe-fair-stability-systemd-resubmit-authorization-consumption-v9"
)
...
def valid_ready_prestart_resource_gate_v9(value: object) -> bool:
    """Recognize the complete, internally consistent v9 ready gate."""
    return prestart_resource_gate_contract_valid(
        value, expected_ready=True
    )


def valid_prestart_resource_gate_v9(
    value: object,
    *,
    expected_ready: bool,
...
def control_boundary(experiment_root: Path) -> dict[str, dict[str, object]]:
    ...
    controller = common.parent / "run_fair_stability_next_v9.py"
    supervisor = common.parent / "run_fair_stability_systemd_supervisor_v9.py"
    freeze_path = (
        experiment_root / "vins_dev_nativeq_schedfix_runtimeexcl_v9" / "backend_freeze.json"
    )
    ...
    return {
        ...
        "attempt_matrix_freeze": identity(experiment_root / "attempt_matrix_freeze_v9.json"),
    }
...
SYSTEMD_CHAIN_FILES = {
    ...
    "submission_receipt": (
        ...
        "aqua-fe-fair-stability-systemd-submit-receipt-v9",
    ),
    ...
    "systemd_execution_receipt": (
        ...
        "aqua-fe-fair-stability-systemd-execution-v9",
    ),
    "systemd_terminal_receipt": (
        "systemd_terminal_receipt.json",
        "aqua-fe-fair-stability-systemd-terminal-v9",
    ),
}
...
def _validate_pipeline_adjudication_submission(
    ...
    """V9 has one attempt per coordinate; pipeline adjudication is disabled."""
    raise OrdinalError(
        "PIPELINE_ADJUDICATION_DISABLED_ZERO_REPLACEMENT_V9:"
        f"{submission_directory}"
    )

    # Retained below as unreachable historical validation logic so older v9
    ...
    if (
        ...
        != "aqua-fe-fair-stability-systemd-adoption-v9"
...
_SYSTEMD_UNIT_COORDINATE = re.compile(
    r"aqua-fe-fair-stability-v9-o(?P<ordinal>[0-9]{3})-"
...
def _runner_check_argv_from_authorized_state(
    ...
    if arm.startswith("hfnet_openloop_"):
        return [
            "/usr/bin/python3",
            str(scripts / "run_fair_stability_hfnet_openloop_v9.py"),
            "check",
            *common,
            "--budget",
            arm.rsplit("_", 1)[1],
    ...
    return [
        "/usr/bin/python3",
        str(scripts / "run_fair_stability_vins_replay_v9.py"),
        "check",
        *common,
        "--arm",
        arm,
...
def _validate_no_start_dispatch_artifact_attribution(
    ...
    controller_path = (
        Path(__file__).resolve().parent / "run_fair_stability_next_v9.py"
    )
...
def load_attempt_matrix(experiment_root: Path) -> dict[str, Any]:
    """Verify the immutable 120-attempt v9 matrix without freezing later retries."""
    path = experiment_root / "attempt_matrix_freeze_v9.json"
    require_no_symlink_components(path, experiment_root)
    if path.is_symlink() or not path.is_file():
    ...
    if (
        ...
        != "aqua-fe-fair-stability-attempt-matrix-freeze-v9"
        or value.get("experiment_id") != EXPERIMENT_ID
        or value.get("status") != "FROZEN_BEFORE_ANY_V9_ESTIMATOR_START"
    ):
...
def attempt_root(experiment_root: Path, cell: Mapping[str, Any], attempt_index: int) -> Path:
    ...
    if arm.startswith("hfnet_openloop_"):
        ...
        base = (
            experiment_root / "vins_dev_nativeq_schedfix_runtimeexcl_v9" / "attempts"
...
def validate_result_semantic_evidence(
    ...
    try:
        if arm in {"learned_klt_vins", "pure_klt_vins"}:
            from run_fair_stability_vins_replay_v9 import (
            ...
            from run_fair_stability_hfnet_openloop_v9 import (
                accepted_events,
                parse_log,
                parse_trajectory,
                read_ns_lines,
...
def validate_runtime_resource_monitor_evidence(
    ...
    fail-stop pipeline result, and v9 has no replacement attempts.
    """
    ...
    if hfnet_arm:
        ...
        if vins_child_present:
            ...
            if (
                ...
                != "aqua-fe-fair-stability-vins-child-start-v9"
    ...
    for stream in ("proc", "gpu"):
        ...
        for index, row in enumerate(rows):
            if (
                ...
                or row.get("probe_finished_monotonic_ns")
...
def validate_controller_dispatch_execution_for_result(
    ...
    dispatch_claim_path = root / "ordinal_dispatch_claim.json"
    controller_path = Path(__file__).resolve().parent / "run_fair_stability_next_v9.py"
...
def validate_hfnet_watchdog_evidence(
    ...
    if (
        ...
        != "aqua-fe-fair-stability-hfnet-launch-receipt-v9"
    ...
    while attempt_index <= MAX_ATTEMPTS_PER_CELL:
        ...
        if result_path.is_file():
            ...
            expected_result_schema = (
                "aqua-fe-fair-stability-hfnet-result-v9"
                if hfnet_arm
                else "aqua-fe-fair-stability-vins-result-v9"
            )
            if (
            ...
            if invalid_receipt_path.is_file():
                raise OrdinalError(
                    f"PIPELINE_ADJUDICATION_DISABLED_ZERO_REPLACEMENT_V9:{root}"
                )
            ...
            if (
                ...
                != "EXPLICIT_EXTERNAL_TRANSIENT_ONLY_V9"
...
def verify_dispatch_claim(
    ...
    if claim.get("schema_version") != "aqua-fe-fair-stability-ordinal-dispatch-v9":
