#!/usr/bin/python3
"""Continuous resource-exclusivity evidence for fair-stability v10 runs.
...
PRESTART_RESOURCE_GATE_SCHEMA = (
    "aqua-fe-fair-stability-prestart-resource-gate-v10"
)
...
def _classify_gpu_applications(
    ...
    for source in applications:
        ...
        # Deliberately no ToDesk or desktop-process exemption in v10.
        external.append(row)
    return {
        "external": external,
...
def resource_gate(
    ...
    """Run the strict v10 pre-start exclusivity gate.
...
class RuntimeResourceMonitor:
    ...
    def _complete_coverage_close_if_ready(self) -> bool:
        """Commit coverage end only after all reserved probes are published."""
        with self._lock:
            if self.coverage_end_monotonic_ns is not None:
                return True
            if (
        ...
        observed_records, scan_errors = _scan_processes()

        with self._lock:
            self._coverage_close_recheck_in_progress = False
            self._monitor_errors.extend(scan_errors)
            if (
                self.coverage_end_monotonic_ns is not None
                or self._coverage_phase != "CLOSE_PENDING"
                or self._active_probe_reservations
            ):
                self._probe_condition.notify_all()
                return self.coverage_end_monotonic_ns is not None
            if (
                self.process.returncode is None
                or not self._owned_empty(observed_records)
            ):
                self._monitor_errors.append(
                    "COVERAGE_CLOSE_OWNERSHIP_EMPTY_RECHECK_FAILED"
                )
                self._probe_condition.notify_all()
                return False
