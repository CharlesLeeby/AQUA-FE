#!/usr/bin/env python3
"""Independent exact audit for the promoted formal900 matched birth pair.
...
SCHEMA_VERSION = "aqua-fe-detector-birth-rawlk-matched-pair-formal-audit-v2"
...
CONTINUATION_CONTRACT_SCHEMA = (
    "aqua-fe-matched-birth-r3-probe16-to-r4-formal900-transition-v2"
)
ADOPTION_SCHEMA_VERSION = "aqua-fe-detector-birth-rawlk-formal900-adoption-v1"
ADOPTION_STATUS = "ADOPTED_R3_PASS_FOR_FRESH_FORMAL900_FREEZE_ONLY"
STATIC_PROJECTION_SCHEMA = (
    "aqua-fe-matched-birth-r3-outcome-blind-static-projection-v1"
)
PROMOTED_R3_ROOT = (
    WORKSPACE_ROOT / "experiments/matched_birth_a02_4500_6300_r3"
)
FORMAL900_ADOPTION_PATH = (
    WORKSPACE_ROOT
    / "papers/a02_4500_6300_matched_birth_r3_pass_formal900_adoption_v1.json"
)
FORMAL900_ADOPTION_HUMAN_RECORD_PATH = (
    WORKSPACE_ROOT
    / "papers/a02_4500_6300_matched_birth_r3_pass_formal900_adoption_v1.md"
)
# Filled after the one-shot adoption certificate is published.  Empty values
# are rejected by the live validator; tests inject an explicit expected
# identity and never weaken this production fail-closed boundary.
FORMAL900_ADOPTION_SIZE = 43004
FORMAL900_ADOPTION_SHA256 = (
    "6b011fd6bcf64a5b85049a2f9c6ef5d40da818cb28c5a8304f4dbef72a3df86d"
)
TERMINATED_R2_ROOT = (
...
def _file_identity(path: Path, *, label: str = "file") -> dict[str, object]:
    held = _held_active_identity_or_none(path)
    if held is not None:
        return held
...
def _load_canonical_object(path: Path, *, label: str) -> dict[str, object]:
    held = _held_active_bytes_or_none(path)
    if held is not None:
        try:
            payload = json.loads(held.decode("utf-8"))
        except (UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise AuditFailure(f"{label} is not valid UTF-8 JSON: {path}") from exc
        if type(payload) is not dict or held != _canonical_bytes(payload):
...
R3_ARM_FILE_NAMES = R2_ARM_FILE_NAMES
R3_ARM_DIRECTORIES = {
    XFEAT_ARM: "xfeat_r3",
    GFTT_ARM: "gftt_r3",
}
...
R4_ROOT_ARTIFACT_NAMES = {
    "freeze": "formal900_freeze.json",
    "pre_run_start_receipt": "formal900_pre_run_start_receipt.json",
    "post_run_pair_seal": "formal900_post_run_pair_seal.json",
}
R3_PASS_GATE_KEYS = frozenset(
    {
        "manifest_pair",
...
def _held_tree_snapshot(
    *,
    root: Path,
    arm_directories: Mapping[str, str],
    root_file_names: frozenset[str],
    arm_file_names: frozenset[str],
    expected_inventory: object,
    label: str,
) -> dict[str, object]:
    """Open and retain every governed inode through semantic validation."""

    if type(expected_inventory) is not list or any(
        type(row) is not dict for row in expected_inventory
    ):
        raise AuditFailure(f"{label} expected inventory is malformed")
    expected_by_path = {row.get("path"): row for row in expected_inventory}
    if len(expected_by_path) != 24 or None in expected_by_path:
        raise AuditFailure(f"{label} expected inventory is not exact 24")
    if root.expanduser().absolute() != root or root.resolve(strict=True) != root:
        raise AuditFailure(f"{label} root is not canonical and symlink-free")
    directories: dict[Path, int] = {}
    files: dict[Path, int] = {}
    payloads: dict[str, bytes] = {}

    def open_directory(path: Path, name: str | None, parent: int | None) -> int:
        raw = str(path) if name is None else name
        visible = (
            os.lstat(path)
            if parent is None
            else os.stat(raw, dir_fd=parent, follow_symlinks=False)
        )
        descriptor = os.open(
            raw,
        ...
        if (
            ...
            os.close(descriptor)
            raise AuditFailure(f"{label} directory open race: {path}")
        return descriptor

    def open_file(path: Path, name: str, parent: int) -> None:
        ...
        held = os.fstat(descriptor)
        identity = (int(held.st_dev), int(held.st_ino))
        if stat.S_ISLNK(visible.st_mode) or identity != (
            int(visible.st_dev), int(visible.st_ino)
        ):
            files.pop(path, None)
            os.close(descriptor)
            raise AuditFailure(f"{label} file open race: {path}")
        try:
            encoded, final_stat = _read_regular_descriptor_bytes(
                descriptor, expected_identity=identity, label=f"{label} {path.name}"
            )
            if not stat.S_ISREG(final_stat.st_mode):
                raise AuditFailure(f"{label} non-regular file: {path}")
        except BaseException:
            files.pop(path, None)
            os.close(descriptor)
            raise
        payloads[str(path)] = encoded

    try:
        root_fd = open_directory(root, None, None)
        if _fstatfs_magic(root_fd) != EXT4_SUPER_MAGIC:
            raise AuditFailure(f"{label} filesystem identity drift")
        exact_root_listing = root_file_names | set(arm_directories.values())
        if set(os.listdir(root_fd)) != exact_root_listing:
            raise AuditFailure(f"{label} root listing drift")
        for arm_name in arm_directories.values():
            arm_path = root / arm_name
            arm_fd = open_directory(arm_path, arm_name, root_fd)
            if set(os.listdir(arm_fd)) != arm_file_names | {"private_work"}:
                raise AuditFailure(f"{label} {arm_name} listing drift")
            private = arm_path / "private_work"
            private_fd = open_directory(private, "private_work", arm_fd)
            if os.listdir(private_fd):
                raise AuditFailure(f"{label} {arm_name} private work not empty")
        for name in sorted(root_file_names):
            open_file(root / name, name, root_fd)
        for arm_name in arm_directories.values():
            arm_path = root / arm_name
            for name in sorted(arm_file_names):
                open_file(arm_path / name, name, directories[arm_path])
            ...
            rows.append(
                {
                    ...
                    "uid": int(observed.st_uid), "gid": int(observed.st_gid),
                    "device_id": int(observed.st_dev), "inode": int(observed.st_ino),
                    "nlink": int(observed.st_nlink), "size_bytes": len(encoded),
                    "sha256": hashlib.sha256(encoded).hexdigest(),
                }
            )
        rows.sort(key=lambda row: str(row["path"]))
        _strict_equal(rows, expected_inventory, label=f"{label} held tree inventory")
        return {
            "root": root, "arm_directories": dict(arm_directories),
            "root_file_names": root_file_names, "arm_file_names": arm_file_names,
            "inventory": rows, "file_payloads": payloads,
            "directory_descriptors": directories, "file_descriptors": files,
            "label": label,
        }
    except BaseException:
        for descriptor in list(files.values()) + list(reversed(tuple(directories.values()))):
            os.close(descriptor)
        raise


def _finish_held_tree_snapshot(snapshot: Mapping[str, object]) -> None:
    """Re-read every held byte and stat before releasing the evidence tree."""
    ...
    exact_root = snapshot["root_file_names"] | set(snapshot["arm_directories"].values())
    if set(os.listdir(directories[root])) != exact_root:
        raise AuditFailure(f"{label} root changed before held close")
    for arm_name in snapshot["arm_directories"].values():
        arm_path = root / arm_name
        if set(os.listdir(directories[arm_path])) != snapshot["arm_file_names"] | {
            "private_work"
        }:
            raise AuditFailure(f"{label} {arm_name} changed before held close")
        if os.listdir(directories[arm_path / "private_work"]):
            raise AuditFailure(f"{label} {arm_name} private work changed")
    ...
    for path, descriptor in files.items():
        ...
        os.lseek(descriptor, 0, os.SEEK_SET)
        encoded, final_stat = _read_regular_descriptor_bytes(
            descriptor, expected_identity=identity, label=f"{label} final {path.name}"
        )
        if encoded != payloads[str(path)]:
            raise AuditFailure(f"{label} held bytes changed: {path}")
        final_rows.append(
            {
                "path": str(path), "kind": "regular",
    ...
    _strict_equal(final_rows, snapshot["inventory"], label=f"{label} final inventory")
    for path, descriptor in {**directories, **files}.items():
        ...
        if stat.S_ISLNK(visible.st_mode) or (
            ...
            raise AuditFailure(f"{label} visible inode drift at close: {path}")


def _close_held_tree_snapshot(snapshot: Mapping[str, object]) -> None:
...
# The active r4 snapshot is installed only by main() while it owns the
# write-once output descriptor.  Centralizing held-file resolution here keeps
# the existing semantic validators usable by the outcome-blind builder (where
# all candidate outputs are necessarily absent) while making both auditor CLI
# actions fail closed on one continuously held set of evidence inodes.
_ACTIVE_FORMAL_SNAPSHOT: dict[str, object] | None = None


def _active_lexical_path(path: Path | str) -> Path:
    return Path(path).expanduser().absolute()


def _held_active_record_or_none(path: Path | str) -> Mapping[str, object] | None:
    snapshot = _ACTIVE_FORMAL_SNAPSHOT
    if snapshot is None:
        return None
    records = snapshot["file_records"]
    lexical = _active_lexical_path(path)
    record = records.get(lexical)
    if record is None and lexical in snapshot["required_file_paths"]:
        raise AuditFailure(f"required held active file is absent from snapshot: {lexical}")
    return record


def _held_active_identity_or_none(path: Path | str) -> dict[str, object] | None:
    record = _held_active_record_or_none(path)
    if record is None:
        return None
    return copy.deepcopy(record["identity"])


def _held_active_bytes_or_none(path: Path | str) -> bytes | None:
    record = _held_active_record_or_none(path)
    if record is None:
        return None
    ...
    identity = tuple(record["inode_identity"])
    os.lseek(descriptor, 0, os.SEEK_SET)
    encoded, _observed = _read_regular_descriptor_bytes(
        descriptor,
        expected_identity=(int(identity[0]), int(identity[1])),
        label=f"held active evidence {record['path'].name}",
    )
    return encoded


def _held_active_proc_path(path: Path | str) -> Path:
    record = _held_active_record_or_none(path)
    if record is None:
        return Path(path)
    return Path(f"/proc/self/fd/{int(record['descriptor'])}")


def _held_active_mode(path: Path | str) -> int:
    record = _held_active_record_or_none(path)
    if record is None:
        return stat.S_IMODE(Path(path).stat().st_mode)
    return stat.S_IMODE(os.fstat(int(record["descriptor"])).st_mode)


def _held_active_directory_record_or_none(
    path: Path | str,
) -> Mapping[str, object] | None:
    snapshot = _ACTIVE_FORMAL_SNAPSHOT
    if snapshot is None:
        return None
    return snapshot["directory_records"].get(_active_lexical_path(path))


def _stream_held_file(
    descriptor: int,
    *,
    expected_identity: tuple[int, int],
    label: str,
    capture: bool,
) -> tuple[bytes | None, dict[str, object], os.stat_result]:
    before = os.fstat(descriptor)
    if (
        not stat.S_ISREG(before.st_mode)
        or int(before.st_nlink) != 1
        ...
        raise AuditFailure(f"{label} is not one held single-link regular inode")
    os.lseek(descriptor, 0, os.SEEK_SET)
    digest = hashlib.sha256()
    ...
    if (
        ...
        or int(after.st_uid) != int(before.st_uid)
        or int(after.st_gid) != int(before.st_gid)
        or int(after.st_nlink) != int(before.st_nlink)
        or int(after.st_mtime_ns) != int(before.st_mtime_ns)
        or int(after.st_ctime_ns) != int(before.st_ctime_ns)
    ):
        raise AuditFailure(f"{label} changed while its held descriptor was read")
...
def _active_stat_fingerprint(observed: os.stat_result) -> tuple[int, ...]:
    return (
        int(observed.st_dev),
        int(observed.st_ino),
...
def _held_active_formal_snapshot(
    *,
    action: str,
    freeze_json: Path,
    source_bag: Path,
    ...
    """Hold the exact active r4 tree and three locked inputs through commit."""

    if action not in {"check-start", "audit"}:
        raise AuditFailure("unknown held formal snapshot action")
    root = ARTIFACT_NAMESPACE_ROOT
    if root.expanduser().absolute() != root or root.resolve(strict=True) != root:
        raise AuditFailure("active r4 root is not canonical and symlink-free")
    expected_output = root / R4_ROOT_ARTIFACT_NAMES[
        "pre_run_start_receipt" if action == "check-start" else "post_run_pair_seal"
    ]
    if output != expected_output:
        raise AuditFailure("active snapshot output path mismatch")
    expected_root_paths = {
        "freeze": root / R4_ROOT_ARTIFACT_NAMES["freeze"],
        "pre_run_start_receipt": root
        / R4_ROOT_ARTIFACT_NAMES["pre_run_start_receipt"],
        "post_run_pair_seal": root
        / R4_ROOT_ARTIFACT_NAMES["post_run_pair_seal"],
    }
    _strict_equal(
        {
            "freeze": str(freeze_json),
            "pre_run_start_receipt": str(pre_run_start_receipt),
            "post_run_pair_seal": str(post_run_pair_seal),
        },
        {key: str(value) for key, value in expected_root_paths.items()},
        label="active snapshot exact r4 root paths",
    )
    arm_paths = {
        XFEAT_ARM: {
    ...
    for arm_id, arm_name in R4_ARM_DIRECTORIES.items():
        _strict_equal(
            {key: str(value) for key, value in arm_paths[arm_id].items()},
            {
                key: str(root / arm_name / name)
                for key, name in exact_arm_names.items()
            },
            label=f"active snapshot exact {arm_id} paths",
        )

    expected_directory_paths = {
        root,
        *(root / arm_name for arm_name in R4_ARM_DIRECTORIES.values()),
    }
    if action == "audit":
        expected_directory_paths.update(
            root / arm_name / "private_work"
            for arm_name in R4_ARM_DIRECTORIES.values()
        )
    expected_governed_file_paths = {
        expected_root_paths["freeze"],
        *(
            root / arm_name / "command_contract.json"
            for arm_name in R4_ARM_DIRECTORIES.values()
        ),
    }
    if action == "audit":
        expected_governed_file_paths.add(
            expected_root_paths["pre_run_start_receipt"]
        )
        expected_governed_file_paths.update(
            root / arm_name / name
            for arm_name in R4_ARM_DIRECTORIES.values()
            for name in (
                "producer_attempt.json",
    ...
    expected_locked_input_paths = {
        _active_lexical_path(path)
        for path in (source_bag, raw_bag, camera_yaml)
    }
    expected_file_paths = (
        expected_governed_file_paths | expected_locked_input_paths
    )
    expected_directory_count = 3 if action == "check-start" else 5
    expected_file_count = 6 if action == "check-start" else 21
    if (
        len(expected_directory_paths) != expected_directory_count
        or len(expected_governed_file_paths)
        != (3 if action == "check-start" else 18)
        or len(expected_locked_input_paths) != 3
        or len(expected_file_paths) != expected_file_count
        or output in expected_file_paths
    ):
        raise AuditFailure("active snapshot independently expected path set is not exact")
    ...
    def hold_directory(
        ...
        try:
            ...
            if (
                ...
                raise AuditFailure(f"active held directory contract failed: {path}")
            directory_records[path] = {
                "path": path,
                "descriptor": descriptor,
                "fingerprint": _active_stat_fingerprint(held),
            }
            return descriptor
        except BaseException:
            os.close(descriptor)
    ...
    def hold_file(
        ...
        *,
        ...
        if lexical != path or lexical.resolve(strict=True) != lexical:
            raise AuditFailure(f"active evidence path is not canonical: {path}")
        raw = str(path) if name is None else name
        visible = (
            os.lstat(path)
            if parent_descriptor is None
        ...
        try:
            held = os.fstat(descriptor)
            inode_identity = (int(held.st_dev), int(held.st_ino))
            if (
                stat.S_ISLNK(visible.st_mode)
                or inode_identity
                != (int(visible.st_dev), int(visible.st_ino))
                ...
                or (
                    governed
                    and (
                        stat.S_IMODE(held.st_mode) != 0o444
                        or int(held.st_uid) != int(os.getuid())
                    )
                )
            ):
                raise AuditFailure(f"active held regular-file contract failed: {path}")
            capture = path.suffix != ".bag"
            payload, digest, final_stat = _stream_held_file(
                descriptor,
                expected_identity=inode_identity,
                label=f"active held {path.name}",
                capture=capture,
            )
            file_records[path] = {
                "path": path,
                ...
                "fingerprint": _active_stat_fingerprint(final_stat),
                "identity": {
                    "path": str(path),
    ...
    try:
        ...
        if _fstatfs_magic(root_fd) != EXT4_SUPER_MAGIC:
            raise AuditFailure("active r4 held root is not ext4")
        if (int(os.fstat(root_fd).st_dev), int(os.fstat(root_fd).st_ino)) != (
            int(os.fstat(output_parent_descriptor).st_dev),
            int(os.fstat(output_parent_descriptor).st_ino),
        ):
            raise AuditFailure("active snapshot root does not match output parent")
        output_visible = os.stat(
            output.name, dir_fd=root_fd, follow_symlinks=False
        )
        output_held = os.fstat(output_descriptor)
        if (
            (int(output_visible.st_dev), int(output_visible.st_ino))
            != output_identity
            or (int(output_held.st_dev), int(output_held.st_ino))
            != output_identity
            or not stat.S_ISREG(output_held.st_mode)
            or int(output_held.st_nlink) != 1
            or stat.S_IMODE(output_held.st_mode) != 0o444
            or int(output_held.st_size) != 0
        ):
            raise AuditFailure("active snapshot output reservation cross-binding failed")

        root_names = (
            {
                R4_ROOT_ARTIFACT_NAMES["freeze"],
                R4_ROOT_ARTIFACT_NAMES["pre_run_start_receipt"],
                *R4_ARM_DIRECTORIES.values(),
            }
            if action == "check-start"
            else {
                *R4_ROOT_ARTIFACT_NAMES.values(),
                *R4_ARM_DIRECTORIES.values(),
            }
        )
        expected_listings[root] = frozenset(root_names)
        if set(os.listdir(root_fd)) != root_names:
            raise AuditFailure("active r4 root topology is not exact")
        for arm_id, arm_name in R4_ARM_DIRECTORIES.items():
            arm_path = root / arm_name
            arm_fd = hold_directory(
                arm_path, name=arm_name, parent_descriptor=root_fd
            )
            arm_names = (
                {"command_contract.json"}
                if action == "check-start"
            ...
            expected_listings[arm_path] = frozenset(arm_names)
            if set(os.listdir(arm_fd)) != arm_names:
                raise AuditFailure(f"active {arm_name} topology is not exact")
            if action == "audit":
                private = arm_path / "private_work"
                ...
                if os.listdir(private_fd):
                    raise AuditFailure(f"active {arm_name} private work is not empty")

        root_evidence = {R4_ROOT_ARTIFACT_NAMES["freeze"]}
        if action == "audit":
            root_evidence.add(R4_ROOT_ARTIFACT_NAMES["pre_run_start_receipt"])
        ...
        for arm_name in R4_ARM_DIRECTORIES.values():
            arm_path = root / arm_name
            arm_fd = int(directory_records[arm_path]["descriptor"])
            for name in sorted(arm_regular_names):
                hold_file(
                    arm_path / name,
                    name=name,
                    parent_descriptor=arm_fd,
                    governed=True,
                )
        for path in (source_bag, raw_bag, camera_yaml):
            hold_file(path, name=None, parent_descriptor=None, governed=False)

        identities = [tuple(record["inode_identity"]) for record in file_records.values()]
        identities.append(output_identity)
        if len(identities) != len(set(identities)):
            raise AuditFailure("active evidence/input/output inodes are not distinct")
        if (
            set(directory_records) != expected_directory_paths
            or len(directory_records) != expected_directory_count
            or set(file_records) != expected_file_paths
            or len(file_records) != expected_file_count
        ):
            raise AuditFailure("active snapshot held path keyset/count is not exact")
        return {
            "action": action,
            "root": root,
            "output": output,
            ...
            "required_file_paths": frozenset(expected_file_paths),
...
def _finish_held_active_formal_snapshot(snapshot: Mapping[str, object]) -> None:
    """Re-read every held inode and exact directory listing without closing."""
    ...
    for path, record in snapshot["directory_records"].items():
        ...
        visible = os.lstat(path)
        if (
            _active_stat_fingerprint(observed) != tuple(record["fingerprint"])
            or stat.S_ISLNK(visible.st_mode)
            or (int(visible.st_dev), int(visible.st_ino))
            != (int(observed.st_dev), int(observed.st_ino))
            or set(os.listdir(descriptor))
            != set(snapshot["expected_listings"][path])
        ):
            raise AuditFailure(f"active held directory drift before commit: {path}")
    for path, record in snapshot["file_records"].items():
        descriptor = int(record["descriptor"])
        inode_identity = tuple(record["inode_identity"])
        _payload, digest, observed = _stream_held_file(
            descriptor,
            expected_identity=(int(inode_identity[0]), int(inode_identity[1])),
            label=f"final active held {path.name}",
            capture=False,
        )
        visible = os.lstat(path)
        if (
            ...
            or (int(visible.st_dev), int(visible.st_ino))
            != (int(observed.st_dev), int(observed.st_ino))
            or digest["size_bytes"] != record["identity"]["size_bytes"]
            or digest["sha256"] != record["identity"]["sha256"]
        ):
            raise AuditFailure(f"active held evidence drift before commit: {path}")


def _close_held_active_formal_snapshot(snapshot: Mapping[str, object]) -> None:
    errors: list[BaseException] = []
    for record in list(snapshot["file_records"].values()) + list(
        reversed(tuple(snapshot["directory_records"].values()))
    ):
        try:
            os.close(int(record["descriptor"]))
    ...
    if errors:
        raise AuditFailure(
            "active snapshot descriptor close failure: "
            + ";".join(f"{type(exc).__name__}:{exc}" for exc in errors)
        )


def _require_borrowed_active_snapshot(
    snapshot: Mapping[str, object] | None, *, action: str
) -> Mapping[str, object]:
    if (
        snapshot is None
        or snapshot is not _ACTIVE_FORMAL_SNAPSHOT
        or snapshot.get("action") != action
    ):
        raise AuditFailure(f"{action} requires the main-owned held active snapshot")
    return snapshot


def _late_validate_active_formal_snapshot(snapshot: Mapping[str, object]) -> None:
    baseline = snapshot.get("semantic_baseline")
    if type(baseline) is not dict:
        raise AuditFailure("active semantic baseline was not recorded")
    _strict_equal(
        _validate_formal900_adoption(),
        baseline["formal900_adoption_gate"],
        label="late formal900 adoption sanitized gate",
    )
    _strict_equal(
        _audit_code_closure(),
        baseline["audit_code_closure"],
        label="late audit code closure",
    )
    _strict_equal(
        _audit_runtime_observation(),
        baseline["audit_runtime"],
        label="late audit runtime",
    )


def _decode_held_canonical_json(
    snapshot: Mapping[str, object], path: Path, *, label: str
) -> dict[str, object]:
    encoded = snapshot["file_payloads"].get(str(path))
    if type(encoded) is not bytes:
        raise AuditFailure(f"{label} is absent from held snapshot")
    try:
        payload = json.loads(encoded.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise AuditFailure(f"{label} is not UTF-8 JSON") from exc
    if type(payload) is not dict or encoded != _canonical_bytes(payload):
        raise AuditFailure(f"{label} is not one canonical JSON object")
    return payload
...
def _inventory_identity(
    ...
    matches = [row for row in snapshot["inventory"] if row["path"] == str(path)]
    if len(matches) != 1 or matches[0].get("kind") != "regular":
        raise AuditFailure(f"{label} missing from held inventory")
    row = matches[0]
    return {
        "path": str(path),
...
def _static_scientific_projection(
    freeze: Mapping[str, object],
    *,
    command_contracts: Mapping[str, Mapping[str, object]],
) -> dict[str, object]:
    """Return only schedule-independent, outcome-blind producer semantics."""
    ...
    fixed = _require_exact_keys(
        ...
        label="static projection fixed semantics",
    )
    locked = _require_exact_keys(
        freeze.get("locked_inputs"),
        {"source_feature_bag", "raw_image_bag", "camera_yaml"},
        label="static projection locked inputs",
    )
    common = freeze.get("expected_common_contract")
    if type(common) is not dict or freeze.get(
        "expected_common_contract_sha256"
    ) != core._canonical_sha256(common):
        raise AuditFailure("static projection common contract/hash mismatch")
    closure = freeze.get("audit_code_closure")
    if type(closure) is not dict or "pair_auditor" not in closure:
        raise AuditFailure("static projection audit code closure malformed")
    production_closure = {
    ...
    arms = _require_exact_keys(
        freeze.get("arms"), set(EXPECTED_ARMS), label="static projection arms"
    )
    contracts = _require_exact_keys(
        command_contracts, set(EXPECTED_ARMS), label="static projection contracts"
    )
    schedule_scaled_maximum_paths = {
        "/detector_runtime/runtime/candidate_total",
        "/detector_runtime/runtime/detect_calls",
        "/detector_runtime/runtime/detect_ms/all/count",
        "/detector_runtime/runtime/detect_ms/steady_state/count",
    }

    def marker_shape(value: object, path: tuple[str, ...] = ()) -> object:
        """Erase only schedule-sized upper bounds from dynamic markers."""

        if type(value) is dict and set(value) == {TEMPLATE_MARKER_KEY}:
            rule = value[TEMPLATE_MARKER_KEY]
            if type(rule) is not dict:
                raise AuditFailure("static projection dynamic marker malformed")
            pointer = "/" + "/".join(path)
            return {
                TEMPLATE_MARKER_KEY: {
                    ...
                    if not (
                        key == "maximum"
                        and pointer in schedule_scaled_maximum_paths
                    )
                }
            }
        if type(value) is dict:
            return {
                key: marker_shape(child, path + (key,))
                for key, child in value.items()
            }
        if type(value) is list:
            return [
                marker_shape(child, path + (str(index),))
    ...
    for arm_id in (XFEAT_ARM, GFTT_ARM):
        arm = _require_exact_keys(
            arms[arm_id], FREEZE_ARM_KEYS, label=f"static projection {arm_id}"
        )
        template = arm.get("manifest_template")
        attempt = arm.get("attempt")
        contract = contracts[arm_id]
        if type(template) is not dict or type(attempt) is not dict or type(contract) is not dict:
            raise AuditFailure(f"static projection {arm_id} payload malformed")
        _require_exact_keys(template, MANIFEST_TOP_KEYS, label=f"static {arm_id} template")
        _require_exact_keys(attempt, ATTEMPT_KEYS, label=f"static {arm_id} attempt")
        _require_exact_keys(
            contract,
            {
                "schema_version", "arm_id", "argv", "environment",
                "working_directory", "start_receipt", "rc_receipt",
                "supervision",
            },
            label=f"static {arm_id} command",
        )
        argv = contract["argv"]
        if type(argv) is not list or any(type(value) is not str for value in argv):
            raise AuditFailure(f"static projection {arm_id} argv malformed")
        option_names = [value for value in argv if value.startswith("--")]
        fixed_options = sorted(
            option for option in option_names
            if option not in {
                "--output-bag", "--manifest-json", "--diagnostics-csv",
                "--legacy-manifest-json", "--work-directory", "--attempt-json",
                "--max-published-frames",
            }
        )
        projected_arms[arm_id] = {
            "arm_contract": copy.deepcopy(template.get("arm_contract")),
            "arm_contract_sha256": template.get("arm_contract_sha256"),
            "pair_difference_policy": copy.deepcopy(
                template.get("pair_difference_policy")
            ),
            "code_artifacts": marker_shape(template.get("code_artifacts")),
            "inputs": copy.deepcopy(template.get("inputs")),
            "manifest_schema_version": template.get("schema_version"),
            "scientific_role": template.get("scientific_role"),
            "attempt": {
                key: copy.deepcopy(attempt[key])
                for key in (
                    "schema_version", "status", "attempt_count",
                    "process_start_count", "no_retry", "arm_id",
                    "requested_inputs", "producer",
                )
            },
            "command": {
                ...
                "supervision": copy.deepcopy(contract["supervision"]),
                "fixed_option_names": fixed_options,
            },
        }
    return {
        "schema_version": STATIC_PROJECTION_SCHEMA,
        "scientific_role": freeze.get("scientific_role"),
        "locked_inputs": copy.deepcopy(locked),
        "fixed_semantics": {
            key: copy.deepcopy(fixed[key])
            for key in (
                "manifest_schema_version", "attempt_schema_version",
                "feature_topic", "image_topic", "expected_arms",
                "diagnostic_fields", "common_diagnostic_fields",
            )
        },
        "expected_common_contract": copy.deepcopy(common),
        "expected_common_contract_sha256": freeze.get(
            "expected_common_contract_sha256"
        ),
        "production_code_closure": production_closure,
        "arms": projected_arms,
        "required_pair_scalar_differences": copy.deepcopy(
            freeze.get("required_pair_scalar_differences")
        ),
        ...
        "infrastructure_incident": copy.deepcopy(
            freeze.get("infrastructure_incident")
        ),
    }


def _outcome_blind_common_schedule_prefix(
    freeze: Mapping[str, object], *, expected_rows: int
) -> list[dict[str, object]]:
    """Extract only raw-input/common-preprocessing leaves, never arm outcomes."""

    arms = _require_exact_keys(
        freeze.get("arms"), set(EXPECTED_ARMS), label="common schedule arms"
    )
    extracted: dict[str, list[dict[str, object]]] = {}
    for arm_id in (XFEAT_ARM, GFTT_ARM):
        template = arms[arm_id].get("manifest_template")
        rows = template.get("raw_frame_diagnostics") if type(template) is dict else None
        if type(rows) is not list or len(rows) < expected_rows:
            raise AuditFailure(f"{arm_id} common schedule is shorter than expected")
        selected = []
        for index, row in enumerate(rows[:expected_rows]):
            if type(row) is not dict:
                raise AuditFailure(f"{arm_id} common schedule row {index} malformed")
            projected = {}
            for field in COMMON_DIAGNOSTIC_FIELDS:
                value = row.get(field)
                if type(value) is dict and set(value) == {TEMPLATE_MARKER_KEY}:
                    raise AuditFailure(
                        f"{arm_id} common schedule field is outcome-dynamic: {field}"
                    )
                projected[field] = copy.deepcopy(value)
            selected.append(projected)
        extracted[arm_id] = selected
    _strict_equal(
        extracted[XFEAT_ARM], extracted[GFTT_ARM],
        label="common schedule exact between detector arms",
    )
    return extracted[XFEAT_ARM]


def _normalized_manifest_static_contract(
    template: Mapping[str, object], *, arm_id: str
) -> dict[str, object]:
    """Normalize the full template by an explicit typed schedule/path whitelist."""

    normalized = copy.deepcopy(dict(template))

    scalable_marker_maxima = {
        "/code_artifacts/detector_runtime/runtime/candidate_total",
        "/code_artifacts/detector_runtime/runtime/detect_calls",
        "/code_artifacts/detector_runtime/runtime/detect_ms/all/count",
        "/code_artifacts/detector_runtime/runtime/detect_ms/steady_state/count",
        "/metrics/detector_candidates",
        "/metrics/observations",
        "/metrics/published_continuations",
        "/metrics/published_first_occurrences",
        "/metrics/raw_births",
        "/metrics/raw_drops",
        "/metrics/unique_ids",
    }

    def remap(value: object, path: tuple[str, ...] = ()) -> object:
        if type(value) is dict and set(value) == {TEMPLATE_MARKER_KEY}:
            rule = value[TEMPLATE_MARKER_KEY]
            pointer = "/" + "/".join(path)
            return {
                TEMPLATE_MARKER_KEY: {
                    key: remap(child, path + (key,))
                    for key, child in rule.items()
                    if not (
                        key == "maximum" and pointer in scalable_marker_maxima
                    )
                }
            }
        if type(value) is dict:
        ...
        if type(value) is list:
            return [
                remap(child, path + (str(index),))
                for index, child in enumerate(value)
            ]
        if type(value) is str:
            mappings = (
                (
                    str(PROMOTED_R3_ROOT / R3_ARM_DIRECTORIES[arm_id]),
                    f"$ARM_ROOT/{arm_id}",
                ),
                (str(PROMOTED_R3_ROOT), "$RUN_ROOT"),
                (
                    str(ARTIFACT_NAMESPACE_ROOT / R4_ARM_DIRECTORIES[arm_id]),
                    f"$ARM_ROOT/{arm_id}",
                ),
                (str(ARTIFACT_NAMESPACE_ROOT), "$RUN_ROOT"),
            )
    ...
    normalized["status"] = "$SCHEDULE_STATUS"
    normalized["formal_eligible"] = "$SCHEDULE_FORMAL_ELIGIBLE"
    normalized["formal_eligibility_reason"] = "$SCHEDULE_REASON"
    normalized["prefix"] = "$SCHEDULE_PREFIX"
    normalized["raw_frame_diagnostics"] = "$SCHEDULE_RAW_ROWS"
    normalized["diagnostics_streams"] = "$SCHEDULE_DIAGNOSTICS_STREAMS"
    normalized["nonfeature_stream_before"] = "$SCHEDULE_NONFEATURE"
    normalized["nonfeature_stream_after"] = "$SCHEDULE_NONFEATURE"
    attempt = normalized.get("attempt")
    payload = attempt.get("payload") if type(attempt) is dict else None
    if (
        type(payload) is not dict
        or type(payload.get("prefix_nonformal")) is not bool
    ):
        raise AuditFailure(f"{arm_id} manifest attempt prefix flag malformed")
    payload["prefix_nonformal"] = "$SCHEDULE_PREFIX_FLAG"
    metrics = normalized.get("metrics")
    if type(metrics) is not dict:
        raise AuditFailure(f"{arm_id} normalized metrics malformed")
    for key in (
        "adaptive_clahe_frames", "published_frames", "raw_frames_processed"
    ):
        metrics[key] = "$SCHEDULE_COUNT"
    return normalized


def _raw_outcome_marker_schema(
    template: Mapping[str, object], *, arm_id: str
) -> dict[str, object]:
    """Prove every scheduled row carries one exact detector-outcome schema."""

    rows = template.get("raw_frame_diagnostics")
    if type(rows) is not list or not rows:
        raise AuditFailure(f"{arm_id} raw diagnostics template is empty/malformed")
    shapes: list[dict[str, object]] = []
    for index, row in enumerate(rows):
        if type(row) is not dict or set(row) != set(core.DIAGNOSTIC_FIELDS):
            raise AuditFailure(f"{arm_id} raw row {index} field set drift")
        shape: dict[str, object] = {}
        for field in sorted(set(core.DIAGNOSTIC_FIELDS) - set(COMMON_DIAGNOSTIC_FIELDS)):
            value = row[field]
            if type(value) is not dict or set(value) != {TEMPLATE_MARKER_KEY}:
                raise AuditFailure(
                    f"{arm_id} raw row {index} outcome marker missing: {field}"
                )
            shape[field] = copy.deepcopy(value)
        shapes.append(shape)
    first = shapes[0]
    for index, shape in enumerate(shapes[1:], start=1):
        _strict_equal(
            shape, first, label=f"{arm_id} raw outcome marker row {index}"
        )
    return first


def _normalized_producer_command(
    command: Mapping[str, object], *, arm_id: str, source_probe: bool
) -> dict[str, object]:
    """Normalize only r3/r4 paths and the exact terminal prefix-limit suffix."""

    normalized = copy.deepcopy(dict(command))
    argv = normalized.get("argv")
    if type(argv) is not list or any(type(value) is not str for value in argv):
        raise AuditFailure(f"{arm_id} producer argv malformed")
    if source_probe:
        if argv[-2:] != ["--max-published-frames", "16"]:
            raise AuditFailure(f"{arm_id} r3 command lacks exact terminal limit suffix")
        argv = argv[:-2]
    elif "--max-published-frames" in argv:
        raise AuditFailure(f"{arm_id} formal command contains a limit option")
...
def _validate_formal900_mode_and_inputs(
    *,
    adoption_gate: Mapping[str, object],
    allow_prefix: bool,
    expected_frames: int,
    locked_inputs: Mapping[str, object],
) -> None:
    """Fail before expensive bag reconstruction on any non-formal request."""

    if allow_prefix or expected_frames != EXPECTED_PUBLISHED_FRAMES:
        raise AuditFailure("formal v2 accepts exactly formal900 (900/1800, no prefix)")
    _strict_equal(
        locked_inputs,
        adoption_gate.get("locked_inputs"),
        label="formal900 locked inputs",
    )
    if adoption_gate.get("authorized_namespace_root") != str(ARTIFACT_NAMESPACE_ROOT):
        raise AuditFailure("formal900 adoption authorizes a different namespace")


def _validate_formal900_transition(
    freeze: Mapping[str, object],
    *,
    adoption_gate: Mapping[str, object],
    allow_prefix: bool,
    ...
    """Check exact static invariants plus the sole typed schedule transition."""

    _validate_formal900_mode_and_inputs(
        adoption_gate=adoption_gate,
        allow_prefix=allow_prefix,
        expected_frames=expected_frames,
        locked_inputs=freeze.get("locked_inputs"),
    )
    fixed = freeze.get("fixed_semantics")
    _strict_equal(
        fixed,
        _fixed_semantics(
            ...
            allow_prefix=False,
            expected_frames=EXPECTED_PUBLISHED_FRAMES,
        ),
        label="formal900 exact mode transition",
    )
    observed_static = _static_scientific_projection(
        freeze, command_contracts=candidate_command_contracts
    )
    _strict_equal(
        observed_static,
        adoption_gate.get("static_projection"),
        label="formal900 static scientific invariants",
    )
    _strict_equal(
        _outcome_blind_common_schedule_prefix(
            freeze, expected_rows=2 * PROBE_PUBLISHED_FRAMES
        ),
        adoption_gate.get("probe_common_schedule_prefix"),
        label="formal900 first-32 common schedule transition",
    )
    arms = freeze["arms"]
    for arm_id in (XFEAT_ARM, GFTT_ARM):
        arm_root = ARTIFACT_NAMESPACE_ROOT / R4_ARM_DIRECTORIES[arm_id]
        exact_paths = {
            "feature_bag": str(arm_root / "features.bag"),
            "manifest_json": str(arm_root / "export_manifest.json"),
            "diagnostics_csv": str(arm_root / "raw_diagnostics.csv"),
            "legacy_primitive_manifest": str(
                arm_root / "legacy_primitive_manifest.json"
            ),
            "private_work_directory": str(arm_root / "private_work"),
            "attempt_json": str(arm_root / "producer_attempt.json"),
        }
        _strict_equal(
            arms[arm_id]["paths"], exact_paths,
            label=f"formal900 {arm_id} exact r4 paths",
        )
        attempt = arms[arm_id]["attempt"]
        if attempt.get("prefix_nonformal") is not False:
            raise AuditFailure(f"formal900 {arm_id} attempt remains prefix")
        template = arms[arm_id]["manifest_template"]
        template_attempt = template.get("attempt")
        template_attempt_payload = (
            template_attempt.get("payload")
            if type(template_attempt) is dict else None
        )
        if (
            type(template_attempt_payload) is not dict
            or template_attempt_payload.get("prefix_nonformal") is not False
        ):
            raise AuditFailure(
                f"formal900 {arm_id} manifest attempt remains prefix"
            )
        _strict_equal(
            _normalized_manifest_static_contract(template, arm_id=arm_id),
            adoption_gate["normalized_manifest_static_contracts"][arm_id],
            label=f"formal900 {arm_id} normalized full-template invariants",
        )
        _strict_equal(
            _raw_outcome_marker_schema(template, arm_id=arm_id),
            adoption_gate["raw_outcome_marker_schemas"][arm_id],
            label=f"formal900 {arm_id} all-row outcome marker schema",
        )
        _strict_equal(
            {
                "status": template.get("status"),
                "formal_eligible": template.get("formal_eligible"),
                "formal_eligibility_reason": template.get(
                    "formal_eligibility_reason"
                ),
                "prefix": template.get("prefix"),
                "raw_row_count": len(template.get("raw_frame_diagnostics", [])),
            },
            {
                "status": "FULL",
                ...
                "prefix": {
                    ...
                    "selected_published_frames": EXPECTED_PUBLISHED_FRAMES,
                    "cutoff_feature_record_stamp_ns": None,
                },
                "raw_row_count": EXPECTED_RAW_FRAMES,
            },
            label=f"formal900 {arm_id} exact schedule leaves",
        )
        contract = candidate_command_contracts[arm_id]
        argv = contract.get("argv")
        if "--max-published-frames" in argv:
            raise AuditFailure(
                f"formal900 {arm_id} command contains forbidden limit flag"
            )
        _strict_equal(
            _normalized_producer_command(
                contract, arm_id=arm_id, source_probe=False
            ),
            adoption_gate["normalized_producer_commands"][arm_id],
            label=f"formal900 {arm_id} exact producer command transition",
        )
    return {
        "schema_version": CONTINUATION_CONTRACT_SCHEMA,
        "from": {
            "root": str(PROMOTED_R3_ROOT),
            "allow_prefix_nonformal": True,
            "expected_published_frames": PROBE_PUBLISHED_FRAMES,
            "expected_raw_frames": 2 * PROBE_PUBLISHED_FRAMES,
        },
        "to": {
            "root": str(ARTIFACT_NAMESPACE_ROOT),
            "allow_prefix_nonformal": False,
            "expected_published_frames": EXPECTED_PUBLISHED_FRAMES,
            "expected_raw_frames": EXPECTED_RAW_FRAMES,
        },
        "static_projection_sha256": core._canonical_sha256(observed_static),
        "r3_outcomes_used_for_formal_configuration": False,
    }


def _validate_formal900_adoption_semantic(
    adoption: Mapping[str, object],
    *,
    expected_identity: Mapping[str, object],
    held_human_identity: Mapping[str, object],
) -> dict[str, object]:
    """Validate r3 PASS under held FDs and return no outcome-bearing payload."""
    _require_exact_keys(
        adoption,
        {
            "schema_version", "status", "scientific_role", "builder_identity",
            "builder_execution_contract", "human_record", "source_namespace",
            "pass_qualification",
            "source_static_projection_sha256", "target",
            "formal_transition_contract", "evidence_use_boundary",
            "claim_boundary",
        },
        label="formal900 adoption",
    )
    if (
        adoption["schema_version"] != ADOPTION_SCHEMA_VERSION
        or adoption["status"] != ADOPTION_STATUS
        or adoption["scientific_role"]
        != "POST_PROBE_CONTRACT_PASS_GATED_FORMAL_SCALE_PROMOTION"
    ):
        raise AuditFailure("formal900 adoption schema/status/role mismatch")
    builder_identity = _validate_identity_claim(
        adoption["builder_identity"], label="formal900 adoption builder"
    )
    expected_adoption_builder = (
        WORKSPACE_ROOT
        / "scripts/build_matched_birth_r3_pass_formal900_adoption_v1.py"
    ).resolve(strict=True)
    if builder_identity["path"] != str(expected_adoption_builder):
        raise AuditFailure("formal900 adoption builder path mismatch")
    execution = _require_exact_keys(
        adoption["builder_execution_contract"],
        {
            "working_directory", "environment", "authorized_write_once_argv",
            "runtime", "runtime_sha256",
        },
        label="formal900 adoption builder execution contract",
    )
    from scripts import run_matched_birth_arm_once_v1 as sealed_launcher
    _strict_equal(
        execution,
        {
            ...
            "authorized_write_once_argv": [
                ...
                "--action", "write-once", "--output",
                str(FORMAL900_ADOPTION_PATH),
            ],
            "runtime": _audit_runtime_observation(),
            "runtime_sha256": core._canonical_sha256(execution["runtime"]),
        },
        label="formal900 adoption exact builder execution contract",
    )
    _strict_equal(
        adoption["human_record"], held_human_identity,
        label="formal900 held human adoption record",
    )
    source = _require_exact_keys(
        adoption["source_namespace"],
        {
            "root", "filesystem", "tree_entry_count", "directory_count",
            "regular_file_count", "exact_tree_inventory", "role_bindings",
        },
        label="formal900 adoption source namespace",
    )
    if source["root"] != str(PROMOTED_R3_ROOT):
        raise AuditFailure("formal900 adoption source root mismatch")
    entries = source["exact_tree_inventory"]
    if (
        source["tree_entry_count"] != 24
        or source["directory_count"] != 5
        or source["regular_file_count"] != 19
        or type(entries) is not list
        or len(entries) != 24
        or source["filesystem"] != {
            "filesystem_type": "ext4", "device_id": 66312
        }
    ):
        raise AuditFailure("formal900 adoption tree inventory/count mismatch")
    target = _require_exact_keys(
        adoption["target"],
        {
            "root", "arm_directories", "mode", "allow_prefix_nonformal",
            "published_frames", "raw_frames",
        },
        label="formal900 adoption target",
    )
    _strict_equal(
        target,
        {
            ...
            "mode": "formal900",
            "allow_prefix_nonformal": False,
            "published_frames": EXPECTED_PUBLISHED_FRAMES,
            "raw_frames": EXPECTED_RAW_FRAMES,
        },
        label="formal900 adoption exact target",
    )
    transition = _require_exact_keys(
        adoption["formal_transition_contract"],
        {
            "transform_id", "path_mapping", "mode_transition",
            "producer_argv_transition",
        },
        label="formal900 adoption transition",
    )
    _strict_equal(
        transition,
        {
            ...
            "path_mapping": {
                str(PROMOTED_R3_ROOT): str(ARTIFACT_NAMESPACE_ROOT),
                str(PROMOTED_R3_ROOT / "xfeat_r3"): str(
                    ARTIFACT_NAMESPACE_ROOT / "xfeat_r4"
                ),
                str(PROMOTED_R3_ROOT / "gftt_r3"): str(
                    ARTIFACT_NAMESPACE_ROOT / "gftt_r4"
                ),
            },
            "mode_transition": {
                "allow_prefix_nonformal": [True, False],
                "published_frames": [PROBE_PUBLISHED_FRAMES, EXPECTED_PUBLISHED_FRAMES],
                "raw_frames": [2 * PROBE_PUBLISHED_FRAMES, EXPECTED_RAW_FRAMES],
            },
            "producer_argv_transition": {
        ...
        label="formal900 adoption authorized transition",
    )
    _strict_equal(
        adoption["evidence_use_boundary"],
        {
        ...
        label="formal900 adoption evidence-use boundary",
    )
    _strict_equal(
        ...
        label="formal900 adoption claim boundary",
    )

    snapshot: dict[str, object] | None = None
    try:
        snapshot = _held_tree_snapshot(
            root=PROMOTED_R3_ROOT,
            arm_directories=R3_ARM_DIRECTORIES,
            root_file_names=R3_ROOT_FILE_NAMES,
            arm_file_names=R3_ARM_FILE_NAMES,
            expected_inventory=entries,
            label="promoted r3",
        )
        freeze_path = PROMOTED_R3_ROOT / "probe16_freeze.json"
        start_path = PROMOTED_R3_ROOT / "probe16_pre_run_start_receipt.json"
        seal_path = PROMOTED_R3_ROOT / "probe16_post_run_pair_seal.json"
        freeze = _decode_held_canonical_json(
            snapshot, freeze_path, label="promoted r3 freeze"
        )
        start = _decode_held_canonical_json(
            snapshot, start_path, label="promoted r3 start receipt"
        )
        seal = _decode_held_canonical_json(
            snapshot, seal_path, label="promoted r3 PASS seal"
        )
        role_bindings = _require_exact_keys(
            source["role_bindings"],
            {"freeze", "pre_run_start_receipt", "post_run_pair_seal", "arms"},
            label="formal900 adoption r3 role bindings",
        )
        for claim, path, label in (
            (role_bindings["freeze"], freeze_path, "r3 freeze"),
            (
                role_bindings["pre_run_start_receipt"], start_path,
                "r3 start receipt",
            ),
            (role_bindings["post_run_pair_seal"], seal_path, "r3 PASS seal"),
        ):
            _strict_equal(
                claim,
                _inventory_identity(snapshot, path, label=label),
                label=f"formal900 adoption {label} identity",
            )
        if (
            freeze.get("schema_version")
            != "aqua-fe-detector-birth-rawlk-matched-pair-audit-freeze-v1"
            or freeze.get("status") != "FROZEN"
            or freeze.get("scientific_role")
            != "post_result_development_exploratory_detector_birth_ablation"
        ):
            raise AuditFailure("promoted r3 freeze status/schema/role drift")
        _strict_equal(
            freeze.get("fixed_semantics"),
            _fixed_semantics(
                feature_topic=FEATURE_TOPIC_DEFAULT,
            ...
            label="promoted r3 frozen probe mode",
        )
        r3_audit_paths = {
            "pre_run_start_receipt": str(start_path),
            "post_run_pair_seal": str(seal_path),
        }
        _strict_equal(
            freeze.get("audit_paths"), r3_audit_paths,
            label="promoted r3 audit paths",
        )
        r3_contracts: dict[str, dict[str, object]] = {}
        for arm_id in (XFEAT_ARM, GFTT_ARM):
            arm_root = PROMOTED_R3_ROOT / R3_ARM_DIRECTORIES[arm_id]
            expected_paths = {
                "feature_bag": str(arm_root / "features.bag"),
                "manifest_json": str(arm_root / "export_manifest.json"),
                "diagnostics_csv": str(arm_root / "raw_diagnostics.csv"),
            ...
            _strict_equal(
                freeze["arms"][arm_id]["paths"], expected_paths,
                label=f"promoted r3 {arm_id} paths",
            )
            source_attempt = freeze["arms"][arm_id].get("attempt")
            source_template = freeze["arms"][arm_id].get("manifest_template")
            source_template_attempt = (
                source_template.get("attempt")
                if type(source_template) is dict else None
            )
            source_template_payload = (
                source_template_attempt.get("payload")
                if type(source_template_attempt) is dict else None
            )
            if (
                type(source_attempt) is not dict
                or source_attempt.get("prefix_nonformal") is not True
                or type(source_template_payload) is not dict
                or source_template_payload.get("prefix_nonformal") is not True
            ):
                raise AuditFailure(
                    f"promoted r3 {arm_id} probe attempt prefix drift"
                )
            command_path = arm_root / "command_contract.json"
            r3_contracts[arm_id] = _decode_held_canonical_json(
                snapshot, command_path, label=f"promoted r3 {arm_id} command"
            )
            _strict_equal(
                freeze["authoritative_commands"]["arms"][arm_id][
                    "command_contract"
                ],
                _inventory_identity(snapshot, command_path, label="r3 command"),
                label=f"promoted r3 {arm_id} frozen command binding",
            )
        _require_exact_keys(
            start,
            {
                "schema_version", "status", "pass", "scientific_role",
                "pre_run_freeze", "audit_code_closure", "locked_inputs",
                "infrastructure_incident", "artifact_filesystem_contract",
                "reserved_paths_absent", "claim_boundary",
            },
            label="promoted r3 start receipt",
        )
        if (
            start["status"] != "PASS_PRE_RUN_FREEZE_AND_ABSENCE"
            or start["pass"] is not True
        ):
            raise AuditFailure("promoted r3 start receipt is not PASS")
        _strict_equal(
            start["pre_run_freeze"],
            _inventory_identity(snapshot, freeze_path, label="r3 freeze"),
            label="promoted r3 start-to-freeze binding",
        )
        _require_exact_keys(
            seal,
            {
                "schema_version", "status", "pass", "scientific_role",
                "allow_prefix_nonformal", "expected_published_frames", "inputs",
                "post_run_outcome_seal", "gates", "claim_boundary",
            },
            label="promoted r3 PASS seal",
        )
        if (
            seal["schema_version"]
            != "aqua-fe-detector-birth-rawlk-matched-pair-audit-v1"
            or seal["status"] != "PASS" or seal["pass"] is not True
            or seal["allow_prefix_nonformal"] is not True
            or seal["expected_published_frames"] != PROBE_PUBLISHED_FRAMES
        ):
            raise AuditFailure("promoted r3 seal is not exact probe16 PASS")
        gates = _require_exact_keys(
            seal["gates"], R3_PASS_GATE_KEYS, label="promoted r3 PASS gates"
        )
        if any(type(gate) is not dict or gate.get("pass") is not True for gate in gates.values()):
            raise AuditFailure("promoted r3 does not pass all exact 12 gates")
        _strict_equal(
            seal["claim_boundary"],
            {
                "detector_birth_source_only_within_this_frozen_carrier": True,
            ...
            label="promoted r3 claim boundary",
        )
        _strict_equal(
            seal["inputs"]["pre_run_freeze"],
            _inventory_identity(snapshot, freeze_path, label="r3 freeze"),
            label="promoted r3 seal input freeze binding",
        )
        post = _require_exact_keys(
            seal["post_run_outcome_seal"],
            {"pre_run_freeze", "pre_run_start_receipt", "arms"},
            label="promoted r3 outcome seal binding",
        )
        _strict_equal(
            post["pre_run_freeze"],
            _inventory_identity(snapshot, freeze_path, label="r3 freeze"),
            label="promoted r3 post-seal freeze binding",
        )
        _strict_equal(
            post["pre_run_start_receipt"],
            _inventory_identity(snapshot, start_path, label="r3 start"),
            label="promoted r3 post-seal start binding",
        )
        post_arms = _require_exact_keys(
            post["arms"], set(EXPECTED_ARMS), label="promoted r3 sealed arms"
        )
        sealed_roles = {
            "feature_bag": "features.bag",
        ...
        for arm_id, arm_name in R3_ARM_DIRECTORIES.items():
            bound_arm = _require_exact_keys(
                _require_exact_keys(
                    role_bindings["arms"], set(EXPECTED_ARMS),
                    label="formal900 adoption role-bound arms",
                )[arm_id],
                set(sealed_roles) | {"command_contract"},
                label=f"formal900 adoption role-bound {arm_id}",
            )
            claims = _require_exact_keys(
                post_arms[arm_id], set(sealed_roles),
                label=f"promoted r3 sealed {arm_id}",
            )
            for role, name in sealed_roles.items():
                live_identity = _inventory_identity(
                    snapshot, PROMOTED_R3_ROOT / arm_name / name,
                    label=f"r3 {arm_id} {role}",
                )
                _strict_equal(
                    claims[role],
                    live_identity,
                    label=f"promoted r3 {arm_id} {role} binding",
                )
                _strict_equal(
                    bound_arm[role], live_identity,
                    label=f"formal900 adoption {arm_id} {role} role binding",
                )
            _strict_equal(
                bound_arm["command_contract"],
                _inventory_identity(
                    snapshot,
                    PROMOTED_R3_ROOT / arm_name / "command_contract.json",
                    label=f"r3 {arm_id} command contract",
                ),
                label=f"formal900 adoption {arm_id} command role binding",
            )
        projection = _static_scientific_projection(
            freeze, command_contracts=r3_contracts
        )
        ...
        if adoption["source_static_projection_sha256"] != hashlib.sha256(
            _canonical_bytes(recorded_projection)
        ).hexdigest():
            raise AuditFailure("formal900 adoption static projection mismatch")
        qualification = _require_exact_keys(
            adoption["pass_qualification"],
            {
                "freeze_status", "start_status", "seal_status", "seal_pass",
                "required_gate_names", "every_gate_pass", "per_arm_process",
            },
            label="formal900 adoption PASS qualification",
        )
        if (
            qualification["freeze_status"] != "FROZEN"
            or qualification["start_status"]
            != "PASS_PRE_RUN_FREEZE_AND_ABSENCE"
            or qualification["seal_status"] != "PASS"
            or qualification["seal_pass"] is not True
            or qualification["required_gate_names"] != sorted(R3_PASS_GATE_KEYS)
            or qualification["every_gate_pass"] is not True
            or type(qualification["per_arm_process"]) is not dict
            or set(qualification["per_arm_process"]) != set(EXPECTED_ARMS)
        ):
            raise AuditFailure("formal900 adoption PASS qualification mismatch")
        expected_process = {
        ...
        _strict_equal(
            qualification["per_arm_process"],
            {arm_id: expected_process for arm_id in EXPECTED_ARMS},
            label="formal900 adoption one-shot process qualification",
        )
        _finish_held_tree_snapshot(snapshot)
        return {
            "identity": dict(expected_identity),
            "authorized_namespace_root": str(ARTIFACT_NAMESPACE_ROOT),
            "locked_inputs": copy.deepcopy(freeze["locked_inputs"]),
            "static_projection": projection,
            "normalized_manifest_static_contracts": {
                arm_id: _normalized_manifest_static_contract(
                    freeze["arms"][arm_id]["manifest_template"], arm_id=arm_id
                )
                for arm_id in (XFEAT_ARM, GFTT_ARM)
            },
            "normalized_producer_commands": {
                arm_id: _normalized_producer_command(
                    r3_contracts[arm_id], arm_id=arm_id, source_probe=True
                )
                for arm_id in (XFEAT_ARM, GFTT_ARM)
            },
            "raw_outcome_marker_schemas": {
                arm_id: _raw_outcome_marker_schema(
                    freeze["arms"][arm_id]["manifest_template"], arm_id=arm_id
                )
                for arm_id in (XFEAT_ARM, GFTT_ARM)
            },
            "probe_common_schedule_prefix": (
                _outcome_blind_common_schedule_prefix(
                    freeze, expected_rows=2 * PROBE_PUBLISHED_FRAMES
                )
            ),
            ...
            "source_probe_identities": {
                "freeze": copy.deepcopy(role_bindings["freeze"]),
                "start_receipt": copy.deepcopy(
                    role_bindings["pre_run_start_receipt"]
                ),
                "pass_seal": copy.deepcopy(role_bindings["post_run_pair_seal"]),
                "tree_inventory_sha256": core._canonical_sha256(entries),
            },
            "r3_pass_gate_validated": True,
            "r3_outcomes_exposed_to_formal_builder": False,
        }
    finally:
        if snapshot is not None:
            _close_held_tree_snapshot(snapshot)


def _validate_formal900_adoption(
    *, expected_identity: Mapping[str, object] | None = None
) -> dict[str, object]:
    """Hold the permit and human record across the complete r3 semantic gate."""

    if expected_identity is None:
        if (
            FORMAL900_ADOPTION_SIZE <= 0
            or not _SHA256_RE.fullmatch(FORMAL900_ADOPTION_SHA256)
        ):
            raise AuditFailure("formal900 adoption identity is not hard-pinned")
        expected_identity = {
            "path": str(FORMAL900_ADOPTION_PATH),
            "size_bytes": FORMAL900_ADOPTION_SIZE,
            "sha256": FORMAL900_ADOPTION_SHA256,
        }

    held: list[tuple[Path, int, tuple[int, int], bytes, str]] = []

    def open_held(path: Path, *, label: str) -> tuple[bytes, dict[str, object]]:
        lexical = path.expanduser().absolute()
        if lexical.resolve(strict=True) != lexical:
            raise AuditFailure(f"{label} path is not canonical/symlink-free")
        visible = os.lstat(lexical)
        descriptor = os.open(
            str(lexical),
            os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0)
            | getattr(os, "O_NONBLOCK", 0) | getattr(os, "O_CLOEXEC", 0),
        ...
        try:
            ...
            if (
                identity != (int(visible.st_dev), int(visible.st_ino))
                or not stat.S_ISREG(observed.st_mode)
                or int(observed.st_nlink) != 1
                or int(observed.st_uid) != int(os.getuid())
                or stat.S_IMODE(observed.st_mode) != 0o444
            ):
                raise AuditFailure(f"{label} held identity/mode/owner drift")
            encoded, final_stat = _read_regular_descriptor_bytes(
                descriptor, expected_identity=identity, label=label
            )
            claim = {
            ...
            held.append((lexical, descriptor, identity, encoded, label))
            descriptor = -1
    ...
    try:
        adoption_encoded, live_identity = open_held(
            FORMAL900_ADOPTION_PATH, label="formal900 adoption permit"
        )
        _strict_equal(
            live_identity, dict(expected_identity),
            label="formal900 adoption pinned held identity",
        )
        try:
            adoption = json.loads(adoption_encoded.decode("utf-8"))
        except (UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise AuditFailure("formal900 adoption is not UTF-8 JSON") from exc
        if type(adoption) is not dict or adoption_encoded != _canonical_bytes(adoption):
            raise AuditFailure("formal900 adoption is not canonical object JSON")
        human_encoded, human_identity = open_held(
            FORMAL900_ADOPTION_HUMAN_RECORD_PATH,
            label="formal900 human adoption record",
        )
        if not human_encoded:
            raise AuditFailure("formal900 human adoption record is empty")
        result = _validate_formal900_adoption_semantic(
            adoption,
            expected_identity=live_identity,
            held_human_identity=human_identity,
        )
        for path, descriptor, identity, initial, label in held:
            os.lseek(descriptor, 0, os.SEEK_SET)
            final, final_stat = _read_regular_descriptor_bytes(
                descriptor, expected_identity=identity, label=f"final held {label}"
            )
            visible = os.lstat(path)
            if (
                final != initial
                ...
                raise AuditFailure(f"{label} drifted before held close")
        return result
    finally:
        for _path, descriptor, _identity, _encoded, _label in reversed(held):
            os.close(descriptor)
...
def _load_pretty_object(path: Path, *, label: str) -> dict[str, object]:
    ...
    if held is not None:
        ...
        if type(payload) is not dict or held != expected:
...
def _validate_governance_contract(
    ...
    *,
    ...
    expected_builder_path = (
        WORKSPACE_ROOT
        / "scripts/build_matched_birth_rawlk_pair_formal_freeze_v2.py"
    ).resolve(strict=True)
...
def _validate_freeze(
    ...
    *,
    ...
    if _ACTIVE_FORMAL_SNAPSHOT is None and in_memory_command_contracts is None:
        raise AuditFailure(
            "live formal freeze validation requires the main-owned held snapshot"
        )
    ...
    _strict_equal(
        {
            "freeze": str(freeze_json),
            "pre_run_start_receipt": str(audit_paths["pre_run_start_receipt"]),
            "post_run_pair_seal": str(audit_paths["post_run_pair_seal"]),
        },
        {
            role: str(ARTIFACT_NAMESPACE_ROOT / name)
            for role, name in R4_ROOT_ARTIFACT_NAMES.items()
        },
        label="formal900 exact root artifact names",
    )
    ...
    adoption_gate = _validate_formal900_adoption()
    _strict_equal(
        freeze["formal900_adoption"],
        adoption_gate["identity"],
        label="freeze.formal900_adoption",
    )
    _strict_equal(
        freeze["infrastructure_incident"],
        adoption_gate["infrastructure_incident"],
    ...
    _validate_formal900_transition(
        freeze,
        adoption_gate=adoption_gate,
        allow_prefix=allow_prefix,
        expected_frames=expected_frames,
    ...
    if len(governed_paths) != 21 or len({str(path) for path in governed_paths}) != 21:
        raise AuditFailure("freeze governed r4 filesystem path set is not exact 21")
    ...
    if (
        ...
        != adoption_gate["authorized_namespace_root"]
    ):
        raise AuditFailure("freeze r4 filesystem root is not adoption-authorized")
    ...
    if _ACTIVE_FORMAL_SNAPSHOT is not None:
        _ACTIVE_FORMAL_SNAPSHOT["semantic_baseline"] = {
            "formal900_adoption_gate": copy.deepcopy(adoption_gate),
            "audit_code_closure": copy.deepcopy(live_closure),
            "audit_runtime": copy.deepcopy(runtime),
        }
    return {
...
def _pre_run_start_receipt_payload(
    *,
    ...
    return {
        ...
        "formal900_adoption": gate["formal900_adoption"],
...
def check_pre_run_start(
    *,
    ...
    pre_run_start_receipt: Path,
    post_run_pair_seal: Path,
    held_snapshot: Mapping[str, object] | None = None,
) -> dict[str, object]:
    _require_borrowed_active_snapshot(held_snapshot, action="check-start")
    runtime = _audit_runtime_observation()
...
def _validate_manifest_exact(
    ...
    *,
    ...
    if _held_active_mode(attempt_path) != 0o444:
...
def _validate_legacy_manifest(
    *,
    ...
    held_work = _held_active_directory_record_or_none(work_dir)
    if held_work is None:
        ...
        work_entries = list(work_dir.iterdir())
    else:
        work_stat = os.fstat(int(held_work["descriptor"]))
        work_entries = os.listdir(int(held_work["descriptor"]))
    ...
    return {
        ...
        "private_work_directory": {
            "path": str(_active_lexical_path(work_dir)),
            "mode_octal": "0700",
            "entry_count": 0,
        },
    }
...
def _read_diagnostics(path: Path) -> list[dict[str, object]]:
    ...
    if held is None:
        ...
        try:
            ...
            raise AuditFailure("diagnostics is not valid UTF-8") from exc
...
def _reconstruct_common_diagnostics(
    *,
    ...
    source_read_path = _held_active_proc_path(source_bag)
    raw_read_path = _held_active_proc_path(raw_bag)
...
def _serialized_nonfeature(
    ...
    path = _held_active_proc_path(path)
...
def _feature_contract(
    ...
    *,
    ...
    source_read_path = _held_active_proc_path(source_bag)
    candidate_read_path = _held_active_proc_path(candidate_bag)
    camera_read_path = _held_active_proc_path(camera_yaml)
    reference = core._call_trusted_rosbag_scoped(
        audit_base.load_feature_frames, source_read_path, feature_topic
    )
    candidate = core._call_trusted_rosbag_scoped(
        audit_base.load_feature_frames, candidate_read_path, feature_topic
    )
    if allow_prefix:
        reference = reference[:expected_frames]
    camera = audit_base.load_camera_model(camera_read_path)
    violations: list[str] = []
...
def audit_pair(
    *,
    ...
    _require_borrowed_active_snapshot(held_snapshot, action="audit")
    runtime = _audit_runtime_observation()
    freeze = _load_canonical_object(freeze_json, label="pre-run freeze")
    arm_paths = {
        XFEAT_ARM: {
    ...
    return {
        ...
        "inputs": {
            **freeze_gate["locked_inputs"],
            ...
            "formal900_adoption": freeze_gate["formal900_adoption"],
...
def _open_reserved_output(
    output: Path,
) -> tuple[int, int, tuple[int, int], tuple[int, int], tuple[str, ...]]:
    """Reserve one output relative to a held canonical parent directory."""
    ...
    try:
        ...
        parent_identity = (int(parent_held.st_dev), int(parent_held.st_ino))
        if (
            ...
            raise AuditFailure("audit output parent is not the held canonical directory")
        initial_entries = tuple(sorted(os.listdir(parent_descriptor)))
        if output.name in initial_entries:
        ...
        output_identity = (int(output_stat.st_dev), int(output_stat.st_ino))
        if (
            not stat.S_ISREG(output_stat.st_mode)
            or stat.S_IMODE(output_stat.st_mode) != 0o444
            or int(output_stat.st_uid) != int(os.getuid())
            or int(output_stat.st_nlink) != 1
            or int(output_stat.st_dev) != int(parent_held.st_dev)
        ):
            raise AuditFailure("audit output reservation descriptor contract failed")
        os.fsync(descriptor)
        visible = os.lstat(output)
        if (
            stat.S_ISLNK(visible.st_mode)
            or not stat.S_ISREG(visible.st_mode)
            or (int(visible.st_dev), int(visible.st_ino)) != output_identity
            or tuple(sorted(os.listdir(parent_descriptor)))
            != tuple(sorted((*initial_entries, output.name)))
        ):
            raise AuditFailure("audit output reservation is not parent-visible")
        os.fsync(parent_descriptor)
        _verify_reserved_output(
            output=output,
            descriptor=descriptor,
...
def _verify_reserved_output(
    *,
    ...
    initial_parent_entries: Sequence[str],
    expected_bytes: bytes,
) -> None:
    """Cross-bind held bytes, inode, visible name, and durable parent entry."""
    expected_entries = tuple(sorted((*initial_parent_entries, output.name)))

    def crossbind_once(*, phase: str) -> None:
        os.lseek(descriptor, 0, os.SEEK_SET)
        held_bytes, held_stat = _read_regular_descriptor_bytes(
            descriptor,
            expected_identity=output_identity,
            label=(
                "final held audit output"
                if phase == "pre-parent-fsync" and bool(expected_bytes)
                else f"{phase} held audit output"
            ),
        )
        if (
            held_bytes != expected_bytes
            or int(held_stat.st_size) != len(expected_bytes)
            or stat.S_IMODE(held_stat.st_mode) != 0o444
            or int(held_stat.st_uid) != int(os.getuid())
            or int(held_stat.st_nlink) != 1
        ):
            raise AuditFailure(f"{phase} held audit output bytes/stat drift")
        parent_held = os.fstat(parent_descriptor)
        parent_visible = os.lstat(output.parent)
        if (
            not stat.S_ISDIR(parent_held.st_mode)
            or (int(parent_held.st_dev), int(parent_held.st_ino)) != parent_identity
            or stat.S_ISLNK(parent_visible.st_mode)
            or (int(parent_visible.st_dev), int(parent_visible.st_ino))
            != parent_identity
        ):
            raise AuditFailure(f"{phase} audit output parent identity drift")
        visible = os.lstat(output)
        if (
            stat.S_ISLNK(visible.st_mode)
            or not stat.S_ISREG(visible.st_mode)
            ...
            or int(visible.st_uid) != int(os.getuid())
            or int(visible.st_size) != len(expected_bytes)
        ):
            raise AuditFailure(f"{phase} visible audit output cross-binding drift")
        reopened = os.open(
            output.name,
            os.O_RDONLY
            | getattr(os, "O_NOFOLLOW", 0)
        ...
        try:
            ...
            if (
                (int(reopened_stat.st_dev), int(reopened_stat.st_ino))
                != output_identity
                or stat.S_IMODE(reopened_stat.st_mode) != 0o444
                or int(reopened_stat.st_uid) != int(os.getuid())
                or int(reopened_stat.st_nlink) != 1
                or int(reopened_stat.st_size) != len(expected_bytes)
            ):
                raise AuditFailure(f"{phase} audit output openat stat drift")
            os.lseek(reopened, 0, os.SEEK_SET)
            reopened_bytes, _ = _read_regular_descriptor_bytes(
                reopened,
                expected_identity=output_identity,
                label=f"{phase} parent-relative audit output",
            )
            if reopened_bytes != expected_bytes:
                raise AuditFailure(f"{phase} audit output openat bytes drift")
        finally:
            os.close(reopened)
        if tuple(sorted(os.listdir(parent_descriptor))) != expected_entries:
            raise AuditFailure(f"{phase} audit output parent inventory drift")

    crossbind_once(phase="pre-parent-fsync")
    os.fsync(parent_descriptor)
    crossbind_once(phase="post-parent-fsync")
...
def main(argv: Sequence[str] | None = None) -> int:
    global _ACTIVE_FORMAL_SNAPSHOT
    ...
    try:
        ...
        expected_output = ARTIFACT_NAMESPACE_ROOT / R4_ROOT_ARTIFACT_NAMES[
            "pre_run_start_receipt"
            if args.action == "check-start"
            else "post_run_pair_seal"
        ]
        if output != expected_output:
            raise AuditFailure("audit output is not the exact formal900 r4 path")
    ...
    held_snapshot: dict[str, object] | None = None
    ...
    try:
        ...
        if _ACTIVE_FORMAL_SNAPSHOT is not None:
            raise AuditFailure("active formal snapshot lifecycle is already occupied")
        held_snapshot = _held_active_formal_snapshot(
            action=args.action,
            output=output,
            ...
            **{
                key: value
                for key, value in common_arguments.items()
                if key
                not in {
                    "feature_topic",
                    "image_topic",
        ...
        _ACTIVE_FORMAL_SNAPSHOT = held_snapshot
        result = (
            check_pre_run_start(**common_arguments, held_snapshot=held_snapshot)
            if args.action == "check-start"
            else audit_pair(**common_arguments, held_snapshot=held_snapshot)
        )
        _finish_held_active_formal_snapshot(held_snapshot)
        _late_validate_active_formal_snapshot(held_snapshot)
        _finish_held_active_formal_snapshot(held_snapshot)
        encoded = _publish_reserved_output(
            output=output,
            descriptor=descriptor,
            parent_descriptor=parent_descriptor,
            output_identity=output_identity,
        ...
        completion_line = _canonical_bytes(
        ...
        _ACTIVE_FORMAL_SNAPSHOT = None
        close_errors: list[BaseException] = []
        if held_snapshot is not None:
            try:
                _close_held_active_formal_snapshot(held_snapshot)
            except BaseException as exc:
