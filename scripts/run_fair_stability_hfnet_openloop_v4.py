#!/usr/bin/python3
"""Prepare and run the open-loop HFNet arms of runtime-exclusive v4.
...
from fair_stability_ordinal_common_v4 import (
    EXPERIMENT_ID,
...
from fair_stability_runtime_resource_monitor_v4 import (
...
RUNTIME_EXCLUSIVITY_ADDENDUM = (
    ROOT / "papers/fair_stability_runtime_exclusivity_addendum_v4.md"
)
...
CONTROL_SUPERSESSION = ROOT / "papers/fair_stability_control_supersession_v4.md"
...
VINS_ROOT = EXPERIMENT_ROOT / "vins_dev_nativeq_schedfix_runtimeexcl_v4"
BACKEND_FREEZE = VINS_ROOT / "backend_freeze.json"
ORDINAL_COMMON = ROOT / "scripts/fair_stability_ordinal_common_v4.py"
RUNTIME_RESOURCE_MONITOR = (
    ROOT / "scripts/fair_stability_runtime_resource_monitor_v4.py"
)
SYSTEMD_SUPERVISOR = ROOT / "scripts/run_fair_stability_systemd_supervisor_v4.py"
...
EXPECTED_CONTROL_DOCUMENTS = {
    "runtime_exclusivity_addendum": (
        RUNTIME_EXCLUSIVITY_ADDENDUM,
        13_573,
        "17e3fd789495f55727ba7dcb1c981e97cc4cb224b633b267962c6d5cc2c7d96a",
    ),
    "v1_runtime_contamination_report": (
        V1_RUNTIME_CONTAMINATION_REPORT,
        3_436,
    ...
    "control_supersession": (
        CONTROL_SUPERSESSION,
        7_169,
        "b47e65a905dbffafb5a92b2f3ea684c2900f2b2c63bfc1fc640e69886915d805",
    ),
...
def load_experiment() -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != "aqua-fe-fair-stability-hfnet-input-freeze-v4":
...
def prepare_attempt(
    ...
    *,
    ...
    try:
        ...
        attempt = {
            "schema_version": "aqua-fe-fair-stability-hfnet-attempt-v4",
            "experiment_id": EXPERIMENT_ID,
...
def verify_attempt(
    ...
    if attempt.get("schema_version") != "aqua-fe-fair-stability-hfnet-attempt-v4":
...
def wait_with_zero_kf_watchdog(
    ...
    *,
    ...
    def record_observation_gap(observed: float) -> None:
        nonlocal maximum_gap, permanently_disabled
        if action is not None or last_sample is None:
            return
    ...
    def outcome() -> dict[str, object]:
        return {
            "schema_version": "aqua-fe-fair-stability-hfnet-zero-kf-watchdog-outcome-v4",
                ...
                if group.get("proven") is not True:
                    ...
                    if permanently_disabled:
                        ...
                        if permanently_disabled:
                            ...
                            if action.get("signal_sent") is not True:
                                monitoring_errors.extend(
...
def run_attempt(case_id: str, budget: int, repeat: int, attempt_index: int = 1) -> dict[str, object]:
    ...
    with open_regular_lock(lock_path) as lock_stream:
        ...
        claim = {
            "schema_version": "aqua-fe-fair-stability-hfnet-start-claim-v4",
            "experiment_id": EXPERIMENT_ID,
        ...
        result = {
            "schema_version": "aqua-fe-fair-stability-hfnet-result-v4",
            "experiment_id": EXPERIMENT_ID,
            "status": status,
...
def summary() -> dict[str, object]:
    ...
    from summarize_fair_stability_v4 import summarize as summarize_receipts
