...
RUNTIME_ROOT="/media/ma/Data/AQUA-FE_WS_storage_offload/frontend_admission_continuation_v1"
...
mkdir -p "$RUN_DIR"
# Preserve every previous scratch artifact. The canonical YAML is unchanged.
if [[ -e "$DECLARED_SCRATCH_DIR" ]]; then
  [[ ! -L "$DECLARED_SCRATCH_DIR" ]] || exit 64
  SCRATCH_ARCHIVE="$RUN_DIR/prior_canonical_scratch"
  [[ ! -e "$SCRATCH_ARCHIVE" ]] || exit 65
  mv -- "$DECLARED_SCRATCH_DIR" "$SCRATCH_ARCHIVE"
fi
...
{
  printf 'schema_version=aqua-fe-admission-continuation-v1-backend-cell-v1\n'
  printf 'window_id=%s\n' "$WINDOW_ID"
  printf 'run_slug=%s\n' "$RUN_SLUG"
  printf 'cell_id=%s\n' "$CELL_ID"
  printf 'repeat=%s\n' "$REPEAT"
