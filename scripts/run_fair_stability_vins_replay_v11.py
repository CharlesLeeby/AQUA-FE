#!/usr/bin/python3
"""Run paired July Learned+KLT/KLT bags under the runtime-exclusive v11 protocol.
...
DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V11 identity so stability remains paired.
"""
...
from fair_stability_ordinal_common_v11 import (
    EXPERIMENT_ID,
...
from fair_stability_runtime_resource_monitor_v11 import (
...
RUNTIME_ADDENDUM = ROOT / "papers/fair_stability_runtime_exclusivity_addendum_v11.md"
...
REPLAY_WRAPPER = ROOT / "scripts/run_fair_stability_vins_replay_child_v11.sh"
RECORD_ENV = ROOT / "scripts/record_vins_env.sh"
WAIT_SUBSCRIBERS = ROOT / "scripts/wait_for_ros_subscribers.py"
RUNTIME_MONITOR = ROOT / "scripts/fair_stability_runtime_resource_monitor_v11.py"
ORDINAL_COMMON = ROOT / "scripts/fair_stability_ordinal_common_v11.py"
ORDINAL_CONTROLLER = ROOT / "scripts/run_fair_stability_next_v11.py"
HFNET_RUNNER = ROOT / "scripts/run_fair_stability_hfnet_openloop_v11.py"
CONTROL_TESTS = ROOT / "scripts/test_fair_stability_controls_v11.py"
UNIFIED_SUMMARY = ROOT / "scripts/summarize_fair_stability_v11.py"
SYSTEMD_SUPERVISOR = ROOT / "scripts/run_fair_stability_systemd_supervisor_v11.py"
CONTROL_SUPERSESSION = ROOT / "papers/fair_stability_control_supersession_v11.md"
...
def systemd_start_authority(ordinal_state: Mapping[str, Any]) -> dict[str, object]:
    ...
    if (
        ...
        != "aqua-fe-fair-stability-systemd-submission-v11"
...
def verify_common_inputs() -> tuple[dict[str, dict[str, str]], dict[str, dict[str, str]], dict[str, Any]]:
    ...
    if (
        ...
        != "aqua-fe-fair-stability-hfnet-input-freeze-v11"
...
def freeze_backend() -> dict[str, object]:
    ...
    freeze = {
        "schema_version": "aqua-fe-dev-nativeq-schedfix-runtimeexcl-backend-freeze-v11",
        "experiment_id": EXPERIMENT_ID,
        "status": "FROZEN_BEFORE_NEW_VINS_REPLAY",
        "frozen_at_utc": now_utc(),
        "backend_id": "DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V11",
        "identity_boundary": {
...
def load_freeze() -> dict[str, Any]:
    ...
    if (
        ...
        != "aqua-fe-dev-nativeq-schedfix-runtimeexcl-backend-freeze-v11"
        or freeze.get("experiment_id") != EXPERIMENT_ID
        or freeze.get("backend_id") != "DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V11"
...
def prepare_attempt(
    ...
    *,
    ...
    tag = (
        f"fair_stability_runtimeexcl_v11_{case_id}_{arm}_"
        f"repeat_{repeat:03d}_attempt_{attempt_index:03d}"
    )
    attempt = {
        "schema_version": "aqua-fe-fair-stability-vins-attempt-v11",
        "experiment_id": EXPERIMENT_ID,
...
def verify_attempt(
    ...
    if (
        attempt.get("schema_version") != "aqua-fe-fair-stability-vins-attempt-v11"
        or attempt.get("experiment_id") != EXPERIMENT_ID
        or attempt.get("backend_id") != "DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V11"
...
def run_attempt(case_id: str, arm: str, repeat: int, attempt_index: int = 1) -> dict[str, object]:
    ...
    with open_regular_lock(lock_path) as lock_stream:
        ...
        claim = {
            "schema_version": "aqua-fe-fair-stability-vins-start-claim-v11",
            "experiment_id": EXPERIMENT_ID,
...
def summary() -> dict[str, object]:
    ...
    from summarize_fair_stability_v11 import summarize as summarize_receipts
