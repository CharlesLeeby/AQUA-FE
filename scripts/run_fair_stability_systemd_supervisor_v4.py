#!/usr/bin/python3
"""Durable systemd-user supervision for one fair-stability v4 ordinal.
...
from fair_stability_ordinal_common_v4 import (  # noqa: E402
    EXPERIMENT_ID,
    canonical_json,
    identity,
    next_state,
...
CONTROLLER = ROOT / "scripts/run_fair_stability_next_v4.py"
...
MATRIX_FREEZE = EXPERIMENT_ROOT / "attempt_matrix_freeze_v4.json"
...
UNIT_PREFIX = "aqua-fe-fair-stability-v4"

SUBMISSION_SCHEMA = "aqua-fe-fair-stability-systemd-submission-v4"
SUBMIT_RECEIPT_SCHEMA = "aqua-fe-fair-stability-systemd-submit-receipt-v4"
START_RECEIPT_SCHEMA = "aqua-fe-fair-stability-systemd-start-receipt-v4"
EXECUTION_SCHEMA = "aqua-fe-fair-stability-systemd-execution-v4"
TERMINAL_SCHEMA = "aqua-fe-fair-stability-systemd-terminal-v4"
ADOPTION_CLAIM_SCHEMA = "aqua-fe-fair-stability-systemd-adoption-claim-v4"
ADOPTION_SCHEMA = "aqua-fe-fair-stability-systemd-adoption-v4"
...
def host_preflight() -> dict[str, object]:
    ...
    return {
        "schema_version": "aqua-fe-fair-stability-systemd-host-preflight-v4",
        ...
        "login": {
            ...
            "limitation": (
                "LAST_LOGIN_SESSION_EXIT_MAY_STOP_USER_MANAGER_AND_INVALIDATE_V4"
...
def systemd_run_argv(unit: str, directory: Path) -> list[str]:
    ...
    return [
        ...
        f"--description=AQUA-FE fair-stability v4 one ordinal {unit}",
...
def poll(directory: Path | None = None) -> dict[str, object]:
    ...
    return {
        "schema_version": "aqua-fe-fair-stability-systemd-poll-v4",
        "experiment_id": EXPERIMENT_ID,
...
def adopt(directory: Path | None = None) -> dict[str, object]:
    ...
    if service_result == "timeout" or exit_code in {"killed", "dumped"}:
        raise SystemdSupervisorError("SYSTEMD_TIMEOUT_OR_SIGNAL_FAIL_STOP_ABANDON_V4")
    ...
    if state.get("state") == "STARTED_WITHOUT_RESULT":
        raise SystemdSupervisorError("STARTED_WITHOUT_RESULT_FAIL_STOP_ABANDON_V4")
    ...
    if state.get("state") == "TERMINAL_UNADOPTED":
        from run_fair_stability_next_v4 import adopt_terminal
