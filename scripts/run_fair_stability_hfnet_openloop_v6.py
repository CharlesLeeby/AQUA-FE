#!/usr/bin/python3
"""Prepare and run the open-loop HFNet arms of runtime-exclusive v6.
...
from fair_stability_ordinal_common_v6 import (
    EXPERIMENT_ID,
...
from fair_stability_runtime_resource_monitor_v6 import (
...
RUNTIME_EXCLUSIVITY_ADDENDUM = (
    ROOT / "papers/fair_stability_runtime_exclusivity_addendum_v6.md"
)
...
CONTROL_SUPERSESSION = ROOT / "papers/fair_stability_control_supersession_v6.md"
...
VINS_ROOT = EXPERIMENT_ROOT / "vins_dev_nativeq_schedfix_runtimeexcl_v6"
BACKEND_FREEZE = VINS_ROOT / "backend_freeze.json"
ORDINAL_COMMON = ROOT / "scripts/fair_stability_ordinal_common_v6.py"
RUNTIME_RESOURCE_MONITOR = (
    ROOT / "scripts/fair_stability_runtime_resource_monitor_v6.py"
)
SYSTEMD_SUPERVISOR = ROOT / "scripts/run_fair_stability_systemd_supervisor_v6.py"
...
EXPECTED_CONTROL_DOCUMENTS = {
    "runtime_exclusivity_addendum": (
        RUNTIME_EXCLUSIVITY_ADDENDUM,
        20_431,
        "350141a6f9741cea22e03b8737393c142d8f3dbcbd1afcf83b93d10244626a8f",
    ),
    "v1_runtime_contamination_report": (
        V1_RUNTIME_CONTAMINATION_REPORT,
        3_436,
    ...
    "control_supersession": (
        CONTROL_SUPERSESSION,
        12_669,
        "83cda7dcbf6109a35c48ff17cf82f85f7e026af59f9f9acde1b8544e4cd43c83",
    ),
...
def load_experiment() -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != "aqua-fe-fair-stability-hfnet-input-freeze-v6":
...
def prepare_attempt(
    ...
    *,
    ...
    try:
        ...
        attempt = {
            "schema_version": "aqua-fe-fair-stability-hfnet-attempt-v6",
            "experiment_id": EXPERIMENT_ID,
...
def verify_attempt(
    ...
    if attempt.get("schema_version") != "aqua-fe-fair-stability-hfnet-attempt-v6":
...
def wait_with_zero_kf_watchdog(
    ...
    *,
    ...
    def record_passive_exit(
        ...
        record_observation_gap(float(monotonic()), force=True)
    ...
    def record_observation_gap(observed: float, *, force: bool = False) -> None:
        nonlocal maximum_gap, permanently_disabled
        if (action is not None and not force) or last_sample is None:
            return
        gap = max(0.0, float(observed) - last_sample)
        maximum_gap = max(maximum_gap, gap)
        if (
            gap > ZERO_KF_WATCHDOG_POLL_SECONDS
            and not permanently_disabled
        ):
            monitoring_errors.append(f"WATCHDOG_POLL_GAP_EXCEEDED:{gap:.9f}")
            permanently_disabled = True

    def outcome() -> dict[str, object]:
        return {
            "schema_version": "aqua-fe-fair-stability-hfnet-zero-kf-watchdog-outcome-v6",
    ...
    while True:
        ...
        if returncode is not None:
            record_observation_gap(float(monotonic()))
            try:
                ...
                monitoring_errors.append(
                    f"LEADER_REAP_FAILED:{type(error).__name__}:{error}"
                )
            ...
            if signature.get("parser_proven") is not True:
                ...
                if group.get("proven") is not True:
                    ...
                    if group_returncode is not None:
                        try:
                            group_returncode = process.wait(timeout=0)
                    ...
                    confirmation_completed = float(monotonic())
                    record_observation_gap(confirmation_completed)
                    ...
                    if permanently_disabled:
                        ...
                        record_observation_gap(final_signature_completed)
                        ...
                        if permanently_disabled:
                            ...
                            monitoring_errors.append(
                                "FINAL_SIGNATURE_REVALIDATION_FAILED"
                            )
                            permanently_disabled = True
                        else:
                            action = runtime_monitor.watchdog_sigterm_exact_group(
                                expected_executable,
                                expected_argv,
                                deadline_monotonic=deadline,
                                sample_started_monotonic=last_sample,
                                maximum_observation_gap_seconds=(
                                    ZERO_KF_WATCHDOG_POLL_SECONDS
                                ),
                                monotonic=monotonic,
                            )
                            last_signature = final_signature
                            if action.get("signal_sent") is True:
                                pass
                            elif (
                                action.get("classification")
                                == "LEADER_REAPED_BEFORE_SIGNAL"
                                and type(action.get("leader_returncode")) is int
                            ):
                                final_returncode = process.wait(timeout=0)
                                record_passive_exit(
                                    f"FINAL_SIGNAL_GATE:{action.get('phase')}",
                                    action,
                                    final_returncode,
                                )
                                action = None
                                ...
                                pending_terminal_transition = {
                                    ...
                                    "errors": [
                                        ...
                                        for value in action.get("errors", [])
                                    ],
                                }
                                action = None
                                signature_since = None
...
def write_unproven_monitor_receipt(
    ...
    receipt = {
        "schema_version": "aqua-fe-fair-stability-runtime-resource-monitor-v6",
        "experiment_id": EXPERIMENT_ID,
        "finalized_at_utc": now_utc(),
...
def run_attempt(case_id: str, budget: int, repeat: int, attempt_index: int = 1) -> dict[str, object]:
    ...
    with open_regular_lock(lock_path) as lock_stream:
        ...
        claim = {
            "schema_version": "aqua-fe-fair-stability-hfnet-start-claim-v6",
            "experiment_id": EXPERIMENT_ID,
        ...
        result = {
            "schema_version": "aqua-fe-fair-stability-hfnet-result-v6",
            "experiment_id": EXPERIMENT_ID,
            "status": status,
...
def summary() -> dict[str, object]:
    ...
    from summarize_fair_stability_v6 import summarize as summarize_receipts
