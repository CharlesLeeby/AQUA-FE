#!/usr/bin/env python3
"""Freeze all-and-only active pre-refill-slot lineages into a matched plan."""
...
def main() -> int:
    ...
    payload = {
        "schema_version": "aqua-fe-protected-prefill-slot-v1-matched-plan-v1",
        ...
        "selection_basis": "all and only action-positive arm-windows in the frozen 6x3 pre-refill-slot frontend matrix",
    ...
    lock = {
        "schema_version": (
            "aqua-fe-protected-prefill-slot-v1-matched-plan-lock-v1"
        ),
        "frozen_at": datetime.now(timezone.utc).astimezone().isoformat(),
