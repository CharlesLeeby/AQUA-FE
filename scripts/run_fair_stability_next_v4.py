#!/usr/bin/python3
"""Single ordinal dispatcher for the systemd-supervised v4 experiment."""
...
VINS_RUNNER = ROOT / "scripts/run_fair_stability_vins_replay_v4.py"
...
def verify_systemd_authority() -> dict[str, object]:
    ...
    if (
        claim.get("schema_version") != "aqua-fe-fair-stability-systemd-submission-v4"
...
def verify_control_freeze() -> dict[str, Any]:
    ...
    for path in (
        ...
        HFNET_RUNNER,
        SYSTEMD_SUPERVISOR,
        SCRIPT_DIR / "fair_stability_ordinal_common_v4.py",
    ):
...
def adopt_terminal(
    ...
    payload = {
        "schema_version": "aqua-fe-fair-stability-ordinal-terminal-v4",
...
def adjudicate_pipeline_invalid(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        "schema_version": "aqua-fe-fair-stability-pipeline-invalid-adjudication-v4",
        "experiment_id": EXPERIMENT_ID,
        "adjudicated_at_utc": now_utc(),
        "accepted_as_external_pipeline_fault": True,
        "automatic_retry_policy": "EXPLICIT_EXTERNAL_TRANSIENT_ONLY_V4",
...
def dispatch_claim(state: Mapping[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    cell = state["cell"]
    claim = {
        "schema_version": "aqua-fe-fair-stability-ordinal-dispatch-v4",
        "experiment_id": EXPERIMENT_ID,
...
def dispatch(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    execution = {
        "schema_version": "aqua-fe-fair-stability-controller-dispatch-execution-v4",
        "experiment_id": EXPERIMENT_ID,
