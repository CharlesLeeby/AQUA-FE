#!/usr/bin/env python3
"""Rewrite quality/sigma for feature ids above a threshold.

The learned LoFTR sidecars exported by the underwater frontend use a high
feature-id range (default: 10,000,000). This helper changes only those
observations and leaves all KLT/classical qualities untouched, which is useful
for isolating backend q_i/sigma_i effects.
"""
...
def main() -> int:
    ...
    print(
        "feature_frames={feature_frames} rewritten_observations={rewritten_observations} "
        "untouched_observations={untouched_observations}".format(**stats)
    )
    return 0
