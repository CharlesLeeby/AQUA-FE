#!/usr/bin/python3
"""Continuous resource-exclusivity evidence for fair-stability v2 runs.
...
def _is_forbidden_process(record: Mapping[str, object]) -> bool:
    ...
    return bool(names & FORBIDDEN_PROCESS_NAMES) or any(
...
def _classify_gpu_applications(
    ...
    for source in applications:
        ...
        # Deliberately no ToDesk or desktop-process exemption in v2.
        external.append(row)
    return {
        "external": external,
...
def resource_gate(
    ...
    """Run the strict v2 pre-start exclusivity gate.
    ...
    return {
        "schema_version": "aqua-fe-fair-stability-prestart-resource-gate-v2",
