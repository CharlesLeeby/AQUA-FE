# Fair-stability runtime-exclusivity addendum v10
...
3. `papers/fair_stability_control_supersession_v10.md`; and
4. `papers/fair_stability_runtime_exclusivity_addendum_v10.md` (this file).
...
Every v10 input manifest and backend/control freeze must bind the complete
normative and provenance bundles by absolute path, byte size, and SHA-256.
The frozen v9 control documents remain historical development provenance but
provide no v10 execution authority.

## V10 scope and non-import contract

The output namespace is
`/mnt/data/AQUA-FE_WS/experiments/fair_stability_positive_roster_openloop_v10`.
The experiment id is
`fair-stability-positive-roster-openloop-runtimeexcl-v10`; the backend id is
`DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V10`; all new schemas and control identities
are versioned v10.

V10 is a fresh full-120 experiment beginning at ordinal 1.  It does not resume
v9 after its aggregate 27-terminal fail-stop boundary.  No v9 scientific
outcome or generated artifact is eligible for reuse.  This includes every v9
...
No file under the v9 formal root may be imported by copy, link, reflink,
...
sources after fresh validation.  V10 regenerates and freezes its own runtime
inputs and cache.  Each input manifest, backend freeze, attempt matrix, and
summary must contain all nine Boolean result-import assignments
`v1_results_imported=false` through `v9_results_imported=false` and, beside the
v9 result declaration, these three exact runtime-reuse declarations:
...
`v9_runtime_namespace_read=false` prohibits every v10 freeze, preparation,
execution, adoption, and summary process from reading below the formal v9
runtime root; only the frozen `papers/` provenance reports may carry historical
facts.  `v9_attempts_reused=false` excludes every v9 attempt manifest, tree,
state, submission, and result.  `v9_cache_reused=false` excludes every v9 cache
...
and the atomic monitor handshake below, all v9 controls are retained.  The
...
Every formal ordinal is submitted by the frozen v10 supervisor as one unique
...
an immutable `no_start_provenance` object with v10 schema inside its
...
v10 supervisor's `authorize-resubmit` for that exact prior submission
...
the manager; if it does, v10 is abandoned and no retry follows.
...
after the single supervised `Popen` until the v10 close handshake completes,
...
### Atomic reservation and coverage closure

The v9 race arose because a GPU iteration could leave its timed wait without
reserving the upcoming probe, while another thread committed coverage end.
V10 makes those two authorities mutually exclusive under the monitor's shared
lock.
...
CLOSE_PENDING --all prior reservations settled + empty proof rechecked--> CLOSED
```
...
consumed, both loops are unable to reserve again, and the exact reap/empty
fact is revalidated, the end time is recorded and state becomes `CLOSED`.
...
finish.  Because `CLOSE_PENDING` blocks new internal reservations and coverage
end is deferred until every active token has published/consumed and the empty
ownership domain is revalidated, those receipt invariants plus the frozen
producer identity are sufficient; the validator need not reconstruct or log
the internal token map.
...
This handshake does not add a sample or a grace interval.  It merely settles
a probe already reserved while monitoring was open before committing the
control-evidence end.  Sampling limits, intrusion definitions, ownership,
cleanup, and classifications are unchanged.  A clean receipt still supports
only sampled non-observation at the declared cadence, not a kernel exec-event
trace, and runtime measurements remain ineligible for performance claims.
...
starts no probe, cases 4--5 fail closed, case 6 writes one end, and no
admissible trace contains a real probe start after coverage end.
...
halts v10, and authorizes no replacement.  The same applies to every
...
attempt or a control change after any v10 estimator starts requires abandoning
v10 and defining a newly versioned prospective experiment.

## Retained cross-arm time/support controls

The HFNet runtime IMU stream must be regenerated inside v10 from the paired
...
the authoritative raw source, never from the v9 root.
...
## Fresh-v10 freeze gate

Before the first estimator start, pure tests and a temporary-root no-estimator
shadow chain must pass, the formal v10 root must be proven absent, and fresh
v10 `freeze-inputs`, `freeze-backend`, both `prepare-all` passes, and
`freeze-matrix` must run in that order.  The freeze must bind these documents
and the entire nine-report provenance bundle.
...
prepared attempt/cache tree must match its exact v10 allowlist and originate
from freshly validated upstream sources, with no v9 cache or prepared-tree
identity.  All nine predecessor-import declarations must be exactly false.
...
`PIPELINE_INVALID` result instead invalidates completion of v10 and is neither
an algorithm zero nor replaceable.

Until all 120 fresh v10 coordinates have validated terminal receipts and all
...
Holm gate, and claim limitations remain unchanged.  No v1--v9 outcome enters
