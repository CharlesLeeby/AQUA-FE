# Fair-stability runtime-exclusivity addendum v4
...
3. `fair_stability_control_supersession_v4.md`; and
...
Any future v4 input manifest and backend/control freeze must bind all seven files
...
boundaries remain unchanged.  No v1, v2, or v3 result is imported; v4, if later
authorized and frozen, begins at planned ordinal 1.

## Submit-only systemd user-service supervision

Every formal ordinal is claimed and submitted by the frozen v4 systemd
...
last session may stop the manager.  If that occurs, v4 is permanently
...
The HFNet runtime input must be regenerated inside the v4 frozen namespace
...
in v4 because it is a window-specific recovery setting.  The v4 VINS runtime
