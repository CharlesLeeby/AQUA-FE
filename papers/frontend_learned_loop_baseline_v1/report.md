# 水下学习回环基线 v1：Bus 有条件改善，Cemetery 小幅退化

## 最新：2026-09-23T23:40:00+08:00 — 保存结果的条件评价完成

**CONDITIONAL_PROXY_EVALUATION_COMPLETE_FORMAL_CLAIM_UNRESOLVED。** 已对原六组保存输出完成18行条件数值评价，并逐对查看全部8对不同的通过图像。本轮新增 VIO、位姿图优化、模型推理、训练均为0。原始局部轨迹、检索、几何、门值及失败记录不变；原7次局部启动/6次有效完成的执行偏差仍保留。

主要结果：在作者源码支持的 `world_T_cam0` 解释下，直接使用已发布 proxy 的数值，Bus 的学习闭环 L 相对传统 C 三次 APE 都下降；Cemetery 一次持平、两次小幅上升。**这些是条件 proxy 数值诊断，不是经过认证的米制误差、独立 GT 精度或正式 WIN/TIE/LOSS。** 原正式结果表仍为 Not evaluated，新增数字单独列在[18行条件诊断表](proxy_diagnostic_accuracy.csv)，不会用诊断绕过原参考有效性门。

| 序列/方法 | 固定尺度 APE 中位 [最小,最大] | 固定1秒平移RPE 中位 [最小,最大] | 显式Sim(3) APE 中位 [最小,最大] | Sim(3)拟合尺度 中位 [最小,最大] |
|---|---:|---:|---:|---:|
| Bus C（与B数值一致） | 1.040787 [0.992910,1.119885] | 0.042158 [0.040986,0.045931] | 0.689820 [0.649533,0.824334] | 0.822977 [0.820154,0.826586] |
| Bus L | 0.910719 [0.830001,0.941825] | 0.042044 [0.040988,0.045816] | 0.484339 [0.387069,0.497573] | 0.824553 [0.816238,0.831277] |
| Cemetery C（与B数值一致） | 0.854371 [0.850744,0.875645] | 0.058709 [0.058647,0.059340] | 0.810968 [0.807695,0.827098] | 1.076373 [1.075852,1.082190] |
| Cemetery L | 0.880116 [0.854371,0.896982] | 0.058709 [0.058701,0.059382] | 0.846645 [0.810968,0.856902] | 1.075454 [1.067895,1.076373] |

表中平移量只按发布 proxy 数字计量，**不标 m**；固定尺度列保持尺度1，隐含双方数值单位相容的条件假设，未用任何 VIO 结果校准参考。Sim(3)仅作显式形状诊断，不能替代主口径。中位数之比的变化：Bus固定APE −12.497%、Sim(3) APE −29.788%；Cemetery固定APE +3.013%、Sim(3) APE +4.399%。逐次配对固定APE变化分别为 Bus −9.508/−16.407/−18.677%，Cemetery 0/+3.452/+2.437%。这些百分比定义不同，不能混用。

1秒RPE三次配对变化：Bus +0.004/−0.270/−0.251%；Cemetery 0/+0.091/+0.072%。Cemetery中位RPE相同不等于每次都没有变化。六组共同支撑仍为497/501/496和406/406/406姿态，覆盖91.85–92.78%和96.90%，全部过原数值门。18行×2种对齐的独立计算/evo交叉核验均PASS；固定尺度proper SE(3)、相同1秒网格和1秒最大插值间隔均未改变。每次的初始化、覆盖、旋转RPE、拟合尺度和全序列对齐下的回环前后分段误差已在CSV与[新评价回执](evaluation_completion_receipts.json)完整保留。分段是最终优化轨迹的时间子集，不是在线因果收敛时间。

### 参考来源查清了什么，仍缺什么

Confirmed fact：两条本地参考与作者仓库在固定commit的 `colmap_aligned.txt` **逐字节一致**。作者[评测代码](https://github.com/joshi-bharat/slamutils-python/blob/5cecd1f7544a3e0c074ee6f47c997d292ad9ef34/vi_comparison/compute_results.py)先读取旧COLMAP轨迹，再用 `align_origin` 做全局刚体变换后输出。只用首姿态推导该变换，全部3388/2519行的位置最大残差分别为5.62e-15/2.88e-15，旋转残差约1e-15 rad，**尺度固定1即可复现全部数据**。因此HF文件相对旧公开文件没有额外尺度改动；这并不证明更早的重建一定未做尺度处理，但足以说明当前尺度依据存在冲突。

作者[COLMAP读写工具](https://github.com/joshi-bharat/slamutils-python/blob/5cecd1f7544a3e0c074ee6f47c997d292ad9ef34/slam_utils/colmap_utils.py)把COLMAP外参求逆、相机1归入左相机、以xyzw输出；同一写法把去掉小数点的时间戳末9位当纳秒，精确复现了Cemetery的异常小数点位置。原冻结 `afrl_digits` 修复可从该来源解释，无须拟合时间偏移。既有原始IMU检查也支持相机朝向解释。**完整独立认证仍未齐**：缺少对应原始COLMAP重建、相机ID映射和尺度校准回执，不能把通用导出源码冒充这两个文件的原始运行凭证。[来源核验JSON](reference_provenance_completion.json)保留固定URL、文件哈希、逐行数值核验和未认证状态。

新版[数据页](https://huggingface.co/datasets/afrl-uw/stereo-vi-underwater-dataset/blob/main/README.md)声称用双目基线修正尺度；旧[作者说明](https://github.com/AutonomousFieldRoboticsLab/SVIn/blob/main/colmap_groundtruth/README.md)称轨迹仅在尺度意义下成立。不能靠选择更有利的说明消除这一冲突。因此本轮依据[运行前补充说明](evaluation_completion_addendum.md)单独做带假设的诊断，原正式有效性门未放行。

### 全部通过对的复核与最终边界

[8对图像复核表](accepted_pair_review.csv)覆盖全部12次几何通过，包含原图哈希、原时间戳、所有重复中的索引及具体可见结构。7对Bus图像可见对应轮胎/轮圈、车窗立柱、腐蚀纹理，或车头圆孔、斜板和缆绳；Cemetery一对可见相容的石圈和竖立三角石，但重复石块与视角变化使唯一场景认证较弱。时间间隔为Bus220.7–235.3秒、Cemetery114.8秒，均非被排除的近时邻居。条件proxy的相机间距为Bus0.085–0.429、Cemetery0.567；这只是辅助一致性，不是独立真值标签。

审阅者已看过方法结果，不能追溯称盲审。故正式独立正确/错误标签仍Unknown，真Recall@4也Unknown；检查通过对不能构成完整召回分母。没有确认错误回环不等于证明没有误回环。

结论：**Bus出现可重复的条件proxy改善，Cemetery同一回环带来小幅退化，学习闭环不是自动无害。** 当前不能给PROMISING、RETRIEVAL_GAIN_ONLY或正式NO_LEARNED_LOOP_INCREMENT标签，也不能宣称AQUA-FE整体优于传统检索。传统分数门的影响、两条开发序列和非独立技术重复边界全部保留。可从现有资产完成的核验和条件计算已结束；正式系统评价卡在提供方参考认证与独立回环标签，继续跑VINS无法补齐，不自动换阈值、模型或序列。

本次测试29/29通过。评价源码基于 `c23a483` 加独立诊断开关，实际脚本SHA与新回执一起保存；它不是历史VIO的源码身份。所有原始未评价回执均保留；以下为历史检查点。

## 当前检查点：2026-09-22T03:37:00+08:00

**FULL_MATRIX_EXECUTED_OUTCOME_UNRESOLVED。** 原定完整 Bus Outside 与 Cemetery 各三次共享 KLT 局部复放已完成：2条物理序列、**6次有效局部 VIO／7次实际启动**、12次 C/L 原生位姿图处理、18/18 个 B/C/L 输出行。额外一次是外部 VINS 插队时中断的 Cemetery r1，超出了原“最多6次启动”的执行上限；这是明确的协议偏差，不计为有效重复或可选择的结果，也没有因中途表现重跑。三次是技术重复，不是六条独立序列。所有有效输入仍为原冻结 KLT、同一外部 VINS 二进制；每个重复的 B/C/L 共享一份局部轨迹、关键帧和地图点。没有换序列、模型、检索阈值或后端。

| 第一屏问题 | 完整矩阵所得证据 |
|---|---|
| 是否有真实重访？ | 两条完整序列均有提供方重访说明；逐查询独立标签仍 Unknown。 |
| L 比传统 C 多带来什么？ | C 六次几何通过均为0；L 在 Bus 为3/2/5，在 Cemetery 为0/1/1，合计12次通过，但只有8对不同图像，Cemetery后两次是同一物理图像对。 |
| 几何成立吗？ | 12次均经过共享的原生 BRIEF 对应、3D–2D/PnP 和原图规则；几何通过不等于独立确认真实回环。 |
| 全局轨迹改善吗？ | 6次 C 均与 B 字节一致；L 在 Bus 3次和 Cemetery后2次改变全局输出。固定尺度 APE、1秒 RPE、显式 Sim(3) 均 **Not evaluated**：精确 COLMAP proxy 文件的相机位姿约定未独立证实；无有效 WIN/TIE/LOSS 精度判定。 |
| 成本？ | Cemetery首轮3149张新图编码+VLAD计674.78秒，后两轮3148/3149次有效缓存命中、0新图推理；C/L 原生处理约24–50/28–52秒，含I/O与验证，不是独立优化时间。 |
| 是否继续这个方向？ | 完成了固定成熟组件基线，但不能给 `PROMISING_GLOBAL_ASSOCIATION_BASELINE`、`RETRIEVAL_GAIN_ONLY` 或已评价的 `NO_LEARNED_LOOP_INCREMENT`。不自动调门、换模型或新序列。 |

| 序列 / 技术重复 | 关键帧 | C 通过/拒绝 | L 通过/拒绝 | 共同位姿 / 跨度 / 参考可评覆盖 |
|---|---:|---:|---:|---:|
| Bus r1 | 2430 | 0/443 | 3/2195 | 497 / 579秒 / 92.04% |
| Bus r2 | 2441 | 0/443 | 2/2207 | 501 / 579秒 / 92.78% |
| Bus r3 | 2439 | 0/445 | 5/2201 | 496 / 579秒 / 91.85% |
| Cemetery r1 | 3149 | 0/514 | 0/3098 | 406 / 420秒 / 96.90% |
| Cemetery r2 | 3148 | 0/514 | 1/3096 | 406 / 420秒 / 96.90% |
| Cemetery r3 | 3149 | 0/514 | 1/3097 | 406 / 420秒 / 96.90% |

上述六块均过预注册的**数值**共同支撑门；参考语义未核实，不能说精度门已完全通过。[18行系统结果](system_results.csv)、[115425条候选与拒绝记录](loop_candidates.csv)、[输入/配置/图处理哈希回执](runtime_receipts.json)保留全分母。Cemetery三次局部各3806条 VIO 位姿、首次 NON_LINEAR 输出11.155363228秒；录制器收到3891条特征和43400条IMU，逐主题序列化哈希均与同一 feature bag 一致。Cemetery L 后两次通过对是相同原图时间戳及图像哈希（r2 `1447→615`、r3 `1448→616`），不能算两种重访；该对在 C 的前四候选中均不存在，且 C 最高分0.01872低于原0.05门。它是固定检索**配置**的增量，不能单独归功于学习表征。图像显示相似石堆，但不是严格盲审独立正确性标签，正式正确/错误仍 Unknown。真 Recall@4 也 Unknown。

执行中只修复了被动档案接口的时间戳歧义：Cemetery 7781张原图中有5组相同 header 时间戳且内容不同，3组落在r1关键帧。初次档案尝试如实失败并保留；新档案器只依据已冻结的 `every_n=2, frame_offset=0` 找到唯一**实际发布**图像，无法唯一识别仍报错。12项档案测试、总26项 loop 相关测试通过；KLT exporter、feature bag、VINS 后端与原生回环几何/图优化均未改。r1首次局部尝试遇另一工作区 VINS 插队而中断，保留在 Elements 盘；重新运行的有效r1、r2、r3回执均保留，无中途结果择优。

新运行尺度产物位于 `/media/ma/Elements/AQUA-FE_learned_loop_baseline_v1_runtime/`，r1有效目录是 `cemetery_r1_retry1`，首次中断的 `cemetery_r1` 和首次歧义档案未删除。85MB模型资产是从已锁定原件复制，官方权重SHA仍为 `b938bf1b...`，没有新下载或训练。报告中的历史检查点如下，保留当时资源阻塞事实，但**不代表当前进度**。数据提供方只称 COLMAP 为近似参考并称当前包做过双目尺度修正；这仍不证明精确文件的世界/相机姿态约定，故没有静默进行尺度或姿态选择。[提供方数据说明](https://huggingface.co/datasets/afrl-uw/stereo-vi-underwater-dataset/blob/main/README.md)；[旧 SVIn 仓库另有尺度限制说明](https://github.com/AutonomousFieldRoboticsLab/SVIn)。

唯一后续决策：停止本轮方法/序列扩展；若要正式比较全局精度，先独立确认这两个**精确哈希** proxy 文件的相机位姿约定并取得可审计的通过对正确性标签，然后只对已保存的同一六块轨迹评估，不重跑局部 VIO。
