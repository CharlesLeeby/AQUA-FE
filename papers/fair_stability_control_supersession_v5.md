# Fair-stability prospective control supersession v5

Status: **PROSPECTIVE; MUST BE FROZEN BEFORE THE FIRST V5 ESTIMATOR START**  
Parent protocol: `fair_stability_positive_roster_protocol_v1.md`  
Runtime addendum: `fair_stability_runtime_exclusivity_addendum_v5.md`
...
`fair-stability-positive-roster-openloop-runtimeexcl-v5`, and the backend id is
`DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V5`.  V5 schemas end in `-v5`; the backend
freeze is under
`vins_dev_nativeq_schedfix_runtimeexcl_v5/backend_freeze.json`.

V5 changes only execution supervision, lifecycle evidence, and versioned
...
No v1, v2, v3, or v4 result, manifest, generated runtime input, attempt cache,
claim, log, receipt, or summary is imported into v5.  All four
`v1_results_imported`, `v2_results_imported`, `v3_results_imported`, and
`v4_results_imported` claims
...
`run_fair_stability_systemd_supervisor_v5.py`.  Direct controller execution is
...
abandonment of the entire v5 experiment.
...
its service and possibly `ExecStopPost`; if that occurs, v5 is fail-stop and
...
no-estimator chain; verify the formal v5 root is absent; then run fresh
...
after the `ExecStopPost` terminal receipt.  V5 begins at ordinal 1.  No outer
