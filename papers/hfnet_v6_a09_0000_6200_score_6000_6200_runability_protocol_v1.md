# HFNet-v6 A09 natural-history runability protocol v1

Status: frozen before A09 input materialization and before any governed HFNet child process starts.

## Question

Can the unchanged external HFNet-SLAM system consume AQUALOC archaeology A09 continuously through the previously selected learned-plus-KLT positive score window `6000..6200` and produce usable score-window trajectory support?

This is a development-only whole-system runability experiment. The A09 window was selected because an older XFeat-plus-KLT frontend beat pure KLT in one fresh short-crop replay, with APE/RPE `0.111514/0.078392` versus KLT `4.702022/3.393186`. It is therefore outcome-selected, not held out. The historical hybrid/KLT runs cold-started at source frame 6000, whereas this HFNet run receives natural history. Those old accuracy values are provenance only and must not be placed in a head-to-head accuracy table with the new HFNet trajectory.

## Frozen feed

- Dataset: AQUALOC archaeology sequence A09.
- Canonical raw archive: `/mnt/data/AQUA-FE_WS/datasets/full_downloads/aqualoc/Archaeological_site_sequences/archaeo_sequence_9_raw_data.tar.gz`.
- Continuous HFNet feed: source camera frames `0..6200`, inclusive, 6201 cameras.
- History-only interval: `0..5999`.
- Sole score interval: `6000..6200`, inclusive, 201 cameras.
...
The selection identity and old-positive provenance are frozen in `papers/hfnet_v6_a09_0000_6200_score_6000_6200_selector_freeze_v1.json`.
...
2. Independently validate source archive identity, selected camera/IMU endpoints, every camera's reader brackets, generated CSVs, all 6201 PNG byte identities, exact input-tree closure, and the materialization manifest.
...
6. Run A09 alone. Ambient ToDesk and unrelated work are allowed by the user's development waiver; runtime, throughput, latency, real-time, and exclusive-GPU claims are forbidden.
...
- all 201 score cameras have one contiguous trajectory pose;
- at least one keyframe lies in the score interval;
- the active map was initialized before source frame 6000;
- there is no initialization, active-map reset, or unresolved reset boundary in `6000..6200`.
