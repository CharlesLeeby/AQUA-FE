# Fair-stability runtime-exclusivity addendum v7
...
3. `papers/fair_stability_control_supersession_v7.md`; and
4. `papers/fair_stability_runtime_exclusivity_addendum_v7.md` (this file).
...
Any future v7 input manifest and backend/control freeze must bind all ten files
...
thresholds, and reporting boundaries remain unchanged.  V7, if later
authorized and frozen, begins at planned ordinal 1.  No v1, v2, v3, v4, v5,
or v6 result, manifest, generated runtime input, attempt cache, claim, log,
receipt, or summary is imported.  Every v7 input manifest, backend
...
Every formal ordinal is claimed and submitted by the frozen v7 systemd
...
the immutable execution/adoption evidence chain, but v7 then enters
...
last session may stop the manager.  If that occurs, v7 is permanently
abandoned; no automatic continuation or retry is authorized.

## HFNet zero-keyframe post-Shutdown watchdog

All HFNet budgets, windows, and repeats use one frozen watchdog contract; v7
...
V7 retains the v6 correction for the v5 exit-observation race without relaxing
live identity checks, and adds the phase-scoped final-gate timing correction
required by the v6 abandonment report.  Exact-group snapshots are purely observational and never
...
and cannot authorize a substitute attempt.  From immediately after the
...
No automatic invalid-attempt receipt or replacement is created for any v7
pipeline fault.
...
The HFNet runtime input must be regenerated inside the v7 frozen namespace
...
in v7 because it is a window-specific recovery setting.  The v7 VINS runtime
...
## Fresh-v7 freeze gate

V7 is a new 120-cell experiment, not a continuation of any predecessor.  The
...
upstream sources only after fresh v7 validation.  V7 must regenerate its own
runtime inputs, input manifest, backend/control freeze, 120 attempt manifests,
attempt matrix, receipts, and summary.  No generated or outcome-bearing v1--v6
artifact may be copied into the v7 formal root.

Before the first estimator starts, pure tests and a temporary-root
no-estimator shadow chain must pass, the formal v7 root must be proven absent,
...
all six predecessor-import fields must be exactly false.  Each attempt root
...
result halts and invalidates completion of the v7 namespace; it is neither an
algorithm zero nor replaceable.  No final inference is authorized until all
120 fresh v7 planned cells have terminal receipts.  No partial v1--v6 outcome may enter any
