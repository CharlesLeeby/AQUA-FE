# Fair-stability runtime-exclusivity addendum v12
...
3. `papers/fair_stability_control_supersession_v12.md`; and
4. `papers/fair_stability_runtime_exclusivity_addendum_v12.md` (this file).
...
Every v12 input manifest and backend/control freeze must bind the complete
normative and provenance bundles by absolute path, byte size, and SHA-256.
The frozen v11 control documents remain historical development provenance but
provide no v12 execution authority.

## V12 scope and non-import contract

The output namespace is
`/mnt/data/AQUA-FE_WS/experiments/fair_stability_positive_roster_openloop_v12`.
The experiment id is
`fair-stability-positive-roster-openloop-runtimeexcl-v12`; the backend id is
`DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V12`; all new schemas and control identities
are versioned v12.

V12 is a fresh full-120 experiment beginning at ordinal 1.  It does not resume
v11 after its aggregate eight-terminal fail-stop boundary.  No v11 scientific
outcome or generated artifact is eligible for reuse.  This includes every v11
...
No file under the v11 formal root may be imported by copy, link, reflink,
...
sources after fresh validation.  V12 regenerates and freezes its own runtime
inputs and cache.  Each input manifest, backend freeze, attempt matrix, and
summary must contain all eleven Boolean result-import assignments
`v1_results_imported=false` through `v11_results_imported=false`, the retained
v9 and v10 runtime non-reuse declarations, and these exact v11 runtime-reuse
declarations:
...
`v11_runtime_namespace_read=false` prohibits every v12 freeze, preparation,
execution, adoption, and summary process from reading below the formal v11
runtime root; only the frozen `papers/` provenance reports may carry historical
facts.  `v11_attempts_reused=false` excludes every v11 attempt manifest, tree,
state, submission, and result.  `v11_cache_reused=false` excludes every v11 cache
...
and the watchdog pending-reap correction below, all v11 controls are retained.  The
...
Every formal ordinal is submitted by the frozen v12 supervisor as one unique
...
an immutable `no_start_provenance` object with v12 schema inside its
...
v12 supervisor's `authorize-resubmit` for that exact prior submission
...
the manager; if it does, v12 is abandoned and no retry follows.
...
The sole v12 change applies when either periodic or final-gate observation
proves the unchanged legal teardown predicate while the same `Popen.poll()`
is still `None`. The watchdog records candidate-proof completion as an
immutable monotonic anchor and enters `PENDING_REAP`. Its deadline is
`min(global_estimator_deadline, anchor + 0.25 s)`, with the `0.25 s` endpoint
inclusive. The `0.20 s` target cadence does not shorten that deadline. No
signal is permitted while this state is active.

Only exit and reap of the exact same `Popen` at or before the deadline,
followed by the retained final empty-ownership proof, is a proven passive
exit. The watchdog receipt binds entry path, anchor, deadline, actual passive
wait observations, poll/reap identity, return code, and adjudication. Still
live beyond the deadline, a wrong nonempty group, PID/start-tick reuse,
identity or argv evidence outside the frozen teardown predicate, probe error,
deadline ambiguity, or different-child reap fails closed. The boundary tests
must cover approximately `0.227 s`, exactly `0.25 s`, and greater than
`0.25 s`, plus wrong-group, PID-reuse, periodic-entry, and final-gate-entry
cases.
...
monitor. A passive nonzero exit retains that nonzero code for unchanged
algorithm classification. A proven signal remains the algorithm code
...
after the single supervised `Popen` until the v12 close handshake completes,
...
V12 retains v11's corrected atomic reservation handshake and evidence-bearing
closing ownership scan without change. The mandatory final ownership scan is
published as evidence rather than being invisible between the last ordinary
proc sample and coverage end.
...
The retained v11 monitor includes exactly one closing proc evidence row, not a
new grace interval, retry, cleanup target, exemption, or estimator-lifetime extension.
...
halts v12, and authorizes no replacement.  The same applies to every
...
attempt or a control change after any v12 estimator starts requires abandoning
v12 and defining a newly versioned prospective experiment.

## Retained cross-arm time/support controls

The HFNet runtime IMU stream must be regenerated inside v12 from the paired
...
the authoritative raw source, never from the v9, v10, or v11 runtime root.
...
## Fresh-v12 freeze gate

Before the first estimator start, pure tests and a temporary-root no-estimator
shadow chain must pass, the formal v12 root must be proven absent, and fresh
v12 `freeze-inputs`, `freeze-backend`, both `prepare-all` passes, and
`freeze-matrix` must run in that order.  The freeze must bind these documents
and the entire eleven-report provenance bundle.
...
prepared attempt/cache tree must match its exact v12 allowlist and originate
from freshly validated upstream sources, with no v9, v10, or v11 cache or
prepared-tree identity. All eleven predecessor-import declarations and all
v9/v10/v11 runtime non-reuse declarations must be exactly false.
...
`PIPELINE_INVALID` result instead invalidates completion of v12 and is neither
an algorithm zero nor replaceable.

Until all 120 fresh v12 coordinates have validated terminal receipts and all
...
Holm gate, and claim limitations remain unchanged. No v1--v11 outcome enters
