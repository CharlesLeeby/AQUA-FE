# A10 same-history final-online system comparison v2
...
## Infrastructure-only v2 correction

The v1 A10 KLT attempt is permanently retained as a failed formal attempt. Its
one process-launch allowance was consumed on 2026-08-24, but the child stopped
before reading an image or producing a feature artifact: the frozen minimal
environment omitted `ROS_MASTER_URI`, while Noetic's
`10.roslaunch.sh` dereferences that variable under the wrapper's `set -u`.
The v1 terminal receipt records raw return code 1, execution-integrity PASS,
artifact-contract FAIL, and `TERMINAL_PROCESS_FAILED`; it must never be deleted,
overwritten, or reclassified as an algorithmic result.

This v2 experiment changes launch plumbing only. It adds the fixed initial
environment value `ROS_MASTER_URI=http://localhost:11311`, which is required
while sourcing catkin. Each backend still replaces it with its already frozen,
item-specific loopback master URI before starting `roscore`. All data ranges,
methods, checkpoints, feature budgets, parameters, ports, score gates, output
rates, dependency order, and analysis rules remain unchanged. v2 has new output
directories, workspace links, claims, receipts, scripts, protocol, and execution
lock. The v1 claim, receipt, process log, and lock are pinned as v2 provenance.
Because the system partition had only about 148 MB free before the long run,
v2 also fixes `ROS_HOME` and `ROS_LOG_DIR` to experiment-specific directories
under `/mnt/data/AQUA-FE_WS/experiments/a10_samehistory_system_v2`. This routes
only ROS bookkeeping and diagnostics away from the nearly full system disk;
the directories are outside every governed numerical output tree and do not
change estimator inputs, parameters, executable, or score processing.
An exact `env -i` source-only preflight with `set -euo pipefail` passed after
this addition; that preflight launched no ROS process and wrote no result.
A second non-formal launch-plumbing probe used a distinct
`systemfair_a10_finalonline_v2_plumbing_preflight_0000_0004` tag. Because the
full materialized `RAW_BAG` override is authoritative, the runner's nominal
`0..4` arguments do not crop that bag; the probe therefore began scanning the
full input. It was manually terminated after it had passed both setup scripts,
entered `export_vins_features`, processed images, and created partial probe
artifacts. No ROS master or VINS process was started. The incomplete probe
directory is retained, is outside every formal v2 output path, and is forbidden
as numerical or method evidence.
...
- Large artifacts live under
  `/mnt/data/AQUA-FE_WS/experiments/a10_samehistory_system_v2`.
- The already audited immutable raw bag remains at
  ...
  v2 reuses it by exact path, size, and SHA-256 and does not rematerialize it.
