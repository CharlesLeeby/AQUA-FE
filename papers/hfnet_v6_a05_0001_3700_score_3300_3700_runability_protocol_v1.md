# HFNet-v6 A05 natural-history runability protocol v1

Status: frozen before input materialization and before any governed HFNet child process starts.

## Question

Can the unchanged external HFNet-SLAM system consume AQUALOC archaeology A05 continuously through the previously selected learned-plus-KLT positive score window `3300..3700` and produce usable score-window trajectory support?

This is a development-only whole-system runability experiment. The A05 window was selected because an older XFeat-plus-KLT frontend beat pure KLT on the short crop. It is therefore outcome-selected, not held out. The historical hybrid/KLT runs cold-started at source frame 3300, whereas this HFNet run receives natural history. Those old accuracy values are provenance only and may not be placed in a head-to-head accuracy table with the new HFNet trajectory.

## Frozen feed

- Dataset: AQUALOC archaeology sequence A05.
- Canonical raw archive: `/mnt/data/AQUA-FE_WS/datasets/full_downloads/aqualoc/Archaeological_site_sequences/archaeo_sequence_5_raw_data.tar.gz`.
- Continuous HFNet feed: source camera frames `1..3700`, inclusive.
- History-only interval: `1..3299`.
- Sole score interval: `3300..3700`, inclusive, 401 cameras.
- Frame 0 is excluded because, after the frozen `+53,694,112 ns` IMU clock shift, it has no shifted IMU predecessor. Frame 1 is the first legal reader-bracketed camera. This exclusion is determined from input timestamps, not trajectory behavior.
- No synthetic or extrapolated IMU sample, estimator state, pose, map, or image is permitted.

The selection identity and old-positive provenance are frozen in `papers/hfnet_v6_a05_0001_3700_score_3300_3700_selector_freeze_v1.json`.
...
- Dataset adaptation is limited to the existing AQUALOC calibration, EuRoC/ASL input layout, the frozen camera/IMU time conversion, a headless entry point, run-local model/cache paths, and output inspection.
...
2. Independently validate source archive identity, selected camera/IMU endpoints, reader brackets, generated CSVs, image inventory, input tree closure, and materialization manifest.
...
6. Run A05 alone. Ambient ToDesk and unrelated work are allowed by the user's development waiver; runtime, throughput, latency, real-time, and exclusive-GPU claims are forbidden.
7. Preserve A08 and A09 as unstarted independent later windows regardless of the A05 sign.
...
- all 401 score cameras have one contiguous trajectory pose;
- at least one keyframe lies in the score interval;
- the active map was initialized before source frame 3300;
- there is no initialization, active-map reset, or unresolved reset boundary in `3300..3700`.
