# Fair-stability runtime-exclusivity addendum v8
...
3. `papers/fair_stability_control_supersession_v8.md`; and
4. `papers/fair_stability_runtime_exclusivity_addendum_v8.md` (this file).
...
Any future v8 input manifest and backend/control freeze must bind the complete
...
not change the estimand.  V8, if later authorized and frozen, begins at planned
ordinal 1.  No v1, v2, v3, v4, v5,
v6, or v7 result, manifest, generated runtime input, attempt cache, claim, log,
receipt, or summary is imported.  Every v8 input manifest, backend
...
`v5_results_imported=false`, `v6_results_imported=false`, and
`v7_results_imported=false`.

## Submit-only systemd user-service supervision

Every formal ordinal is claimed and submitted by the frozen v8 systemd
...
the immutable execution/adoption evidence chain, but v8 then enters
...
last session may stop the manager.  If that occurs, v8 is permanently
abandoned; no automatic continuation or retry is authorized.

## HFNet zero-keyframe post-Shutdown watchdog

All HFNet budgets, windows, and repeats use one frozen watchdog contract; v8
...
V8 retains the v6 correction for the v5 exit-observation race and the
...
No automatic invalid-attempt receipt or replacement is created for any v8
pipeline fault.
...
The HFNet runtime input must be regenerated inside the v8 frozen namespace
...
in v8 because it is a window-specific recovery setting.  The v8 VINS runtime
...
## Fresh-v8 freeze gate

V8 is a new 120-cell experiment, not a continuation of any predecessor.  The
...
upstream sources only after fresh v8 validation.  V8 must regenerate its own
runtime inputs, input manifest, backend/control freeze, 120 attempt manifests,
attempt matrix, receipts, and summary.  No generated or outcome-bearing v1--v7
artifact may be copied into the v8 formal root.

Before the first estimator starts, pure tests and a temporary-root
no-estimator shadow chain must pass, the formal v8 root must be proven absent,
...
all seven predecessor-import fields must be exactly false.  Each attempt root
...
result halts and invalidates completion of the v8 namespace; it is neither an
algorithm zero nor replaceable.  No final inference is authorized until all
120 fresh v8 planned cells have terminal receipts.  No partial v1--v7 outcome may enter any
