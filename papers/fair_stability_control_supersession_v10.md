# Fair-stability prospective control supersession v10

Status: **PROSPECTIVE; MUST BE FROZEN BEFORE THE FIRST V10 ESTIMATOR START**  
Parent protocol: `papers/fair_stability_positive_roster_protocol_v1.md`  
Runtime addendum: `papers/fair_stability_runtime_exclusivity_addendum_v10.md`
...
`fair-stability-positive-roster-openloop-runtimeexcl-v10`, and the backend id
is `DEV_NATIVEQ_SCHEDFIX_RUNTIMEEXCL_V10`.  V10 schemas end in `-v10`; its
backend freeze is under
`vins_dev_nativeq_schedfix_runtimeexcl_v10/backend_freeze.json`.

V10 is a new experiment containing all 120 planned coordinates.  It starts at
planned ordinal 1 and is not a continuation, repair, completion, or retry of
v9.  No terminal count, attempt index, submission index, cache state, or
completion state carries forward.

Relative to the frozen v9 scientific design, v10 changes only the control
layer's probe-start reservation/coverage-close handshake, versioned identity,
fresh-root provenance, and predecessor-exclusion declarations.  The atomic
reservation correction is defined below and in the v10 runtime addendum.  It
does not alter an estimator process, estimator input, resource definition,
sampling target or maximum, algorithm classification, or scientific
estimand.
...
The frozen v9 control documents
`papers/fair_stability_control_supersession_v9.md` and
`papers/fair_stability_runtime_exclusivity_addendum_v9.md` remain historical
control provenance.  They provide no execution authority in v10; every path,
schema, executable, receipt, and identity used by v10 must be bound by the
fresh v10 control/backend freeze.

No v1--v9 result or generated experiment artifact is imported into v10.  In
particular, v9's 27 terminal observations and ordinal-028 diagnostic evidence
are excluded together with every v9 manifest, generated runtime input,
...
and derived value.  Nothing below the v9 formal root may be copied, hard-linked,
reflinked, renamed, or treated as a v10 prepared/cache hit.
...
seed may be reused only as upstream sources after fresh v10 validation.  V10
...
Every v10 input manifest, backend freeze, attempt matrix, and progress/final
summary must contain all nine exact result-import declarations plus the three
exact v9 runtime-reuse declarations:
...
`v9_runtime_namespace_read=false` means no v10 freeze, preparation, execution,
adoption, or summary process reads any path below the formal v9 runtime root;
historical provenance is taken only from the frozen `papers/` reports.
`v9_attempts_reused=false` forbids reuse of a v9 attempt manifest, attempt
tree, attempt/submission state, or result.  `v9_cache_reused=false` forbids a
v9 cache file, directory, identity, link, or cache-hit decision.  Any missing,
true, duplicated, or differently typed declaration fails the freeze or
validation closed.

## Sole control correction: atomic probe reservation

V9 was correctly stopped because one GPU probe recorded its start 67.392 us
after the frozen coverage-end timestamp.  There was no intrusion and no
algorithm failure.  V10 closes only the control race that admitted that
ordering.
...
reserved.  Only after all active reservations have been consumed, both
sampling loops are incapable of creating another reservation, and the
ownership-empty fact is revalidated may the closing path record the coverage
end and transition `CLOSE_PENDING -> CLOSED`.
...
internal state of the exactly frozen monitor producer.  V10 does not add a
...
This handshake changes neither sampling cadence nor observation content.  It
adds no final sample, grace period, retry, cleanup target, resource exemption,
or estimator lifetime.  A probe already reserved before close may finish;
after close becomes pending, no new probe may begin.  Coverage-end semantics
are therefore “the supervised ownership domain is empty and every probe
reserved before closure is settled,” which is a control-evidence boundary,
not an estimator timeout or performance measurement.

## Retained durable supervision and zero-retry policy

Every formal mutating `run-next` is launched through the frozen v10 systemd
...
`PRESTART_DISPATCH_BLOCKED_NO_ESTIMATOR_START`.  Their immutable v10
...
v10 rather than selecting a cleaner run.
...
is a v10 fail-stop abandonment event and never a retry.
...
## Fresh-v10 freeze and execution gate

Before the first formal v10 estimator start:

1. freeze the complete v10 normative/provenance document bundle by absolute
...
2. pass pure tests, including deterministic interleavings where a proc/GPU
   ...
   between reservation and real start;
3. pass a temporary-root, no-estimator shadow systemd chain;
4. prove the formal v10 root absent;
5. run fresh v10 `freeze-inputs`, `freeze-backend`, both `prepare-all` passes,
   and `freeze-matrix`, in that order; and
6. revalidate all nine predecessor-import declarations and the complete fresh
   tree before authorizing ordinal 1.
...
attempt/cache tree must be newly produced in v10 from its validated upstream
...
No partial v10 scientific result may be disclosed before all 120 fresh
...
conclusion or system ranking.  A pipeline-invalid v10 stops before completion
...
unchanged.  V10 cannot establish a learned-frontend causal effect, an
